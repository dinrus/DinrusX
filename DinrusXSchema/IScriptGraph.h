...
struct IScriptGraphNode // #SchematycTODO : Move to separate header?
{
	...
	virtual DrxGUID              GetInputTypeGUID(uint32 inputIdx) const = 0;
	...
	virtual DrxGUID              GetOutputTypeGUID(uint32 outputIdx) const = 0;
...
{
	inline SScriptGraphParams(const DrxGUID& _scopeGUID, const char* _szName, EScriptGraphType _type, const DrxGUID& _contextGUID)
...
struct IScriptGraph : public IScriptExtensionBase<EScriptExtensionType::Graph>
{
	...
	virtual uint32                               FindLink(const DrxGUID& srcNodeGUID, const CUniqueId& srcOutputId, const DrxGUID& dstNodeGUID, const CUniqueId& dstInputId) const = 0;
