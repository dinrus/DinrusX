...
public:
	...
	virtual bool operator()(int64& value, const char* szName = "", const char* szLabel = nullptr) override
	{
		...
		{
			char temp[32] = "";
			drx_sprintf(temp, "%llu", static_cast<unsigned long long>(value)); // #SchematycTODO : Make sure %llu works on Windows platforms!!!
			m_output.append(temp);
		}
		m_output.append(", ");
		return true;
	...
	virtual bool operator()(uint64& value, const char* szName = "", const char* szLabel = nullptr) override
	{
		...
		{
			char temp[32] = "";
			drx_sprintf(temp, "%lld", static_cast<unsigned long long>(value)); // #SchematycTODO : Make sure %lld works on Windows platforms!!!
			m_output.append(temp);
		}
		m_output.append(", ");
		return true;
