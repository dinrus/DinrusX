...
struct SScriptGraphNodePort
{
	...
	SScriptGraphNodePort(const CUniqueId& _id, const char* _szName, const DrxGUID& _typeGUID, const ScriptGraphPortFlags& _flags, const CAnyValuePtr& _pData);
...
public:
	...
	inline void                 AddInput(const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags)
	{
		AddInput(CUniqueId::FromIdx(m_inputs.size()), szName, typeGUID, flags, CAnyValuePtr());
	}

	inline void AddInput(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags)
	{
		AddInput(id, szName, typeGUID, flags, CAnyValuePtr());
	}

	inline void AddInputWithData(const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyConstRef& value)
	{
		AddInput(CUniqueId::FromIdx(m_inputs.size()), szName, typeGUID, flags, CAnyValue::CloneShared(value));
	}

	inline void AddInputWithData(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyConstRef& value)
	{
		AddInput(id, szName, typeGUID, flags, CAnyValue::CloneShared(value));
	}

	inline void AddOutput(const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags)
	{
		AddOutput(CUniqueId::FromIdx(m_outputs.size()), szName, typeGUID, flags, CAnyValuePtr());
	}

	inline void AddOutput(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags)
	{
		AddOutput(id, szName, typeGUID, flags, CAnyValuePtr());
	}

	inline void AddOutputWithData(const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyConstRef& value)
	{
		AddOutput(CUniqueId::FromIdx(m_outputs.size()), szName, typeGUID, flags, CAnyValue::CloneShared(value));
	}

	inline void AddOutputWithData(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyConstRef& value)
	{
...
private:
	...
	void AddInput(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyValuePtr& pData);
	void AddOutput(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyValuePtr& pData);
...
public:
	...
	virtual DrxGUID                GetInputTypeGUID(uint32 inputIdx) const override;
	...
	virtual DrxGUID                GetOutputTypeGUID(uint32 outputIdx) const override;
