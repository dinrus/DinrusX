...
#include <DinrusX/DinrusXSchema/PreprocessorUtils.h>
	...
	#define SCHEMATYC_ASSERT(streamId, expression)                    DRX_ASSERT((expression)); if (!((expression))) { SCHEMATYC_CRITICAL_ERROR(streamId, ("ASSERT" # expression)); }
	#define SCHEMATYC_ASSERT_MESSAGE(streamId, expression, ...)       DRX_ASSERT_MESSAGE((expression),__VA_ARGS__); if (!((expression))) { SCHEMATYC_CRITICAL_ERROR(streamId, __VA_ARGS__); }
	#define SCHEMATYC_ASSERT_FATAL(streamId, expression)              DRX_ASSERT((expression)); if (!((expression))) { SCHEMATYC_FATAL_ERROR(streamId, ("ASSERT" # expression)); }
	#define SCHEMATYC_ASSERT_FATAL_MESSAGE(streamId, expression, ...) DRX_ASSERT_MESSAGE((expression),__VA_ARGS__); if (!((expression))) { SCHEMATYC_FATAL_ERROR(streamId, __VA_ARGS__); }
