...
#include <DinrusX/CoreX/Platform/CryWindows.h>
...
#define DRX_LIVE_CREATE_MUTEX_ENABLE 0

#if DRX_LIVE_CREATE_MUTEX_ENABLE
...
int RunGame(const char *commandLine)
{
	...
	// set game project related system changes
	{
		...
		if (sProjectRoot)
		{
			...
			if (sProjectDllDir && sProjectDllDir[0])
			{
				...
				WideCharToMultiByte(CP_ACP, 0, sProjectDllDir, -1, startupParams.szProjectDllDir, DRX_ARRAY_COUNT(startupParams.szProjectDllDir), NULL, NULL);
			}
	...
	if (pGameStartup->Init(startupParams))
	{
		...
		if (isLevelRequested || isModRequested)
		{
			...
			GetModuleFileNameW(GetModuleHandle(NULL), szExecutablePath, DRX_ARRAY_COUNT(szExecutablePath));
