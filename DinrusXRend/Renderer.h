...
#include <DinrusX/DinrusXRend/TextMessages.h>                             // CTextMessages
#include <DinrusX/DinrusXRend/RenderAuxGeom.h>
#include <DinrusX/DinrusXRend/RenderPipeline.h>
#include <DinrusX/DinrusXRend/RenderThread.h>                             // SRenderThread
...
#include <DinrusX/DinrusXRend/RendererResources.h>
#include <DinrusX/DinrusXRend/RendererCVars.h>                    // Can only be included after the default values are defined.
...
public:
	...
	byte* Allocate(size_t nBytes)
	{
		...
		do
		{
			...
			if (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&m_nPoolUsed), nPoolUsed + nBytes, nPoolUsed) == nPoolUsed)
		...
		while (DrxInterlockedCompareExchangePointer(alias_cast<void* volatile*>(&m_pPages), pNewPage, (void*)pPages) != pPages);

		DrxInterlockedAdd((volatile int*)&m_nPageAllocated, nBytes);
...
public:
	...
	{
		void* pPtrRes = DrxModuleMemalign(Size, 16);
