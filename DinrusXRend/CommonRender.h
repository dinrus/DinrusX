...
#include <DinrusX/CoreX/Memory/MemoryAccess.h>
...
public:
	...
	{
		int32 nRef = DrxInterlockedIncrement(&m_nRefCount);
		return nRef;
	}
	virtual int32 Release()
	{
		...
		IF(m_nRefCount > 0, 1)
		{
			int32 nRef = DrxInterlockedDecrement(&m_nRefCount);
			if (nRef < 0)
			{
	...
	int32 TryAddRef()
	{
		...
		while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&m_nRefCount), nNewRef, nOldRef) != nOldRef);
		return nNewRef;
	}
	...
	static CBaseResource*      GetResource(const CDrxNameTSCRC& className, int nID, bool bAddRef);
