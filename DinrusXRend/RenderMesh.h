...
public:
	...
	inline void PrefetchVertexStreams() const { for (int i = 0; i < VSF_NUM; DrxPrefetch(m_VBStream[i++])); }
...
#   endif
  ...
  buffer_size_t Size(uint32 nFlags) const;
	void Size(uint32 nFlags, IDrxSizer *pSizer ) const;
  ...
  // Update system indices buffer
	...
	virtual int    GetTextureMemoryUsage(const IMaterial* pMaterial, IDrxSizer* pSizer = NULL, bool bStreamedIn = true) const final;
