...
public:
	...
	bool  UpdateBuffer(const void* src, buffer_size_t size, buffer_size_t offset = 0, int numDataBlocks = 1); // See CDeviceUpr::UploadContents for details on numDataBlocks
...
private:
	buffer_size_t                                      m_size;
	stl::aligned_vector<uint8, DRX_PLATFORM_ALIGNMENT> m_data;
...
class CConditonalDevManLock
{
	...
	int                   m_Active;
public:
	explicit CConditonalDevManLock(CDeviceBufferUpr* DevMan, int active)
		: m_pDevBufMan(DevMan)
		, m_Active(active)
	{
		if (m_Active)
