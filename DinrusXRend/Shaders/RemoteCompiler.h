...
protected:
	...
	bool         Send(DRXSOCKET Socket, const char* pBuffer, uint32 Size) const;
	bool         Send(DRXSOCKET Socket, std::vector<uint8>& rCompileData) const;
	EServerError Recv(DRXSOCKET Socket, std::vector<uint8>& rCompileData) const;
