...
public:
	...
	CDirEntry(CDrxNameTSCRC Name, uint32 size, uint32 flags = 0) : Name(Name), size(size), flags(flags) {}
	CDirEntry(CDrxNameTSCRC Name, uint32 size, uint32 offset, uint32 flags) : Name(Name), size(size), flags(flags), offset(offset) {}
...
//typedef std::map<CDrxNameTSCRC, int> ResFilesMap;
...
public:
	...
	int mfFileRead(CDrxNameTSCRC name);

	int mfFileWrite(CDrxNameTSCRC name, void* data);

	void mfFileRead2(CDirEntry* de, int size, void* buf);
	void mfFileRead2(CDrxNameTSCRC name, int size, void* buf);

	void* mfFileGetBuf(CDirEntry* de);
	void* mfFileGetBuf(CDrxNameTSCRC name);

	int mfFileSeek(CDirEntry* de, int offs, int type);
	int mfFileSeek(CDrxNameTSCRC name, int offs, int type);
	int mfFileSeek(char* name, int offs, int type);

	int mfFileLength(CDirEntry* de);
	int mfFileLength(CDrxNameTSCRC name);
	...
	//int mfFileDelete(CDrxNameTSCRC name);
	//int mfFileDelete(char* name);

	bool mfFileExist(CDrxNameTSCRC name);
	bool mfFileExist(const char* name);

	int            mfFileClose(const CDrxNameTSCRC &Name, uint32 flags);
	bool           mfCloseEntry(const CDrxNameTSCRC &Name, uint32 flags, bool bEraseOpenEntry = true);
