...
#include <DinrusX/DinrusXRend/D3DRender/Vulkan/VKCommandList.hpp>
...
public:
	...
	static bool                  FillBuffer(const void* pSrc, NDrxVulkan::CBufferResource* pDst, const SResourceMemoryMapping& mapping);                                       // CPU-fill
	NDrxVulkan::CBufferResource* UploadBuffer(const void* pSrc, NDrxVulkan::CBufferResource* pDst, const SResourceMemoryMapping& mapping, bool bAllowGpu);                     // CPU-fill (+ GPU-side copy if needed)
	void                         UploadImage(const void* pSrc, NDrxVulkan::CImageResource* pDst, const SResourceMemoryMapping& mapping, bool bExt = false);                    // CPU-fill (+ GPU-side copy if needed)
...
private: // All function below are implemented inline and forward to one of the functions above
	...
	NDrxVulkan::CBufferResource* UploadBuffer(const void* pSrc, NDrxVulkan::CBufferResource* pDst, const SResourceMemoryAlignment& alignment, bool bAllowGpu)
	{
	...
	void UploadImage(const void* pSrc, NDrxVulkan::CImageResource* pDst, const SResourceMemoryAlignment& alignment)
	{
		SResourceMemoryMapping mapping;
		mapping.MemoryLayout = alignment;
		mapping.ResourceOffset.Left = 0;
