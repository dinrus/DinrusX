...
#include <DinrusX/DrxPlugins/VR/DrxOculusVR/Interface/IHmdOculusRiftDevice.h>
...
private:
	...
	struct SMirrorTextureRenderData
	{
		...
		_smart_ptr<CTexture>    pMirrorTexture;  // DinrusX's texture used as render target for Oculus to write their mirror texture
		IUnknown*               pMirrorTextureNative;
	};

	bool InitializeTextureSwapSet(ID3D11Device* d3dDevice, EEyeType eye, STextureSwapChainRenderData& eyeRenderData, DrxVR::Oculus::TextureDesc desc, const std::string& name);
	bool InitializeTextureSwapSet(ID3D11Device* d3dDevice, EEyeType eye, DrxVR::Oculus::TextureDesc desc, const std::string& name);
	bool InitializeQuadTextureSwapSet(ID3D11Device* d3dDevice, RenderLayer::EQuadLayers id, DrxVR::Oculus::TextureDesc desc, const std::string& name);
	bool InitializeMirrorTexture(ID3D11Device* d3dDevice, DrxVR::Oculus::TextureDesc desc, const std::string& name);
