...
public:
	...
	void                     Serialize(Serialization::IArchive& ar);
	DRS::IConditionSharedPtr operator[](int index)         { DRX_ASSERT(index < m_conditions.size()); return m_conditions[index].m_pCondition; }
	bool                     IsConditionNegated(int index) { DRX_ASSERT(index < m_conditions.size());  return m_conditions[index].m_bNegated; }
