...
struct  IServerReport : public INetworkInterface
{
	...
	virtual void ProcessQuery(char* data, int len, DRXSOCKADDR_IN* addr) = 0;
	// </interfuscator:shuffle>
};
...
struct INatNegListener
{
	...
	virtual void OnCompleted(int cookie, bool success, DRXSOCKADDR_IN* addr) = 0;
	// </interfuscator:shuffle>
};
...
struct INatNeg : public INetworkInterface
{
	...
	virtual void OnPacket(char* data, int len, DRXSOCKADDR_IN* fromaddr) = 0;
	// </interfuscator:shuffle>
};
