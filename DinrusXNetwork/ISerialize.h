...
#include <DinrusX/DinrusXScript/IScriptSystem.h> // <> required for Interfuscator
#include <DinrusX/CoreX/CountedValue.h>
...
public:
	...
	template<typename T>
	void Value(const char* szName, DrxStringLocalT<T>& value, int policy)
	{
		if (IsWriting())
		{
			SSerializeString& serializeString = SetSharedSerializeString(value);
	...
	template<typename T>
	ILINE void Value(const char* szName, DrxStringLocalT<T>& value)
	{
		Value(szName, value, 0);
	}
	template<typename T>
	void Value(const char* szName, const DrxStringLocalT<T>& value, int policy)
	{
		if (IsWriting())
		{
			SSerializeString& serializeString = SetSharedSerializeString(value);
	...
	template<typename T>
	ILINE void Value(const char* szName, const DrxStringLocalT<T>& value)
	{
		Value(szName, value, 0);
	}
	template<size_t S>
	void Value(const char* szName, DrxFixedStringT<S>& value, int policy)
	{
		if (IsWriting())
		{
			SSerializeString& serializeString = SetSharedSerializeString(value);
	...
	template<size_t S>
	ILINE void Value(const char* szName, DrxFixedStringT<S>& value)
	{
		Value(szName, value, 0);
	}
