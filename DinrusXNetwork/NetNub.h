...
public:
	...
	virtual void         DisconnectChannel(EDisconnectionCause cause, DrxSessionHandle session, const char* reason);
...
private:
	...
	void CreateChannel(const TNetAddress& ip, const CExponentialKeyExchange::KEY_TYPE& key, const string& connectionString, uint32 remoteSocketCaps, uint32 proifle, DrxSessionHandle session, CNubConnectingLock conlk); // first half, network thread
...
private:
	...
	void AddDisconnectEntry(const TNetAddress& ip, DrxSessionHandle session, EDisconnectionCause cause, const char* reason);
