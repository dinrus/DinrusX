...
#include <DinrusX/DinrusXNetwork/ArithStream.h>
#if NEW_BANDWIDTH_MANAGEMENT
	#include <DinrusX/DinrusXNetwork/BandwidthCalculator.h>
#else
	#include <DinrusX/DinrusXNetwork/PacketRateCalculator.h>
#endif // NEW_BANDWIDTH_MANAGEMENT
#include <DinrusX/DinrusXNetwork/MessageMapper.h>
#include <DinrusX/DinrusXNetwork/Serialize.h>
#include <DinrusX/DinrusXNetwork/FrameTypes.h>
#include <DinrusX/DinrusXNetwork/NetworkInspector.h>
#include <DinrusX/DinrusXNetwork/MessageQueue.h>
#include <DinrusX/DinrusXNetwork/ICTPEndpointListener.h>
#include <DinrusX/DinrusXNetwork/AutoFreeHandle.h>

#include <DinrusX/DinrusXNetwork/ArithAlphabet.h>
#include <DinrusX/DinrusXNetwork/ArithModel.h>
...
#define ACK_TYPE_NUM_BITS              2
...
#if ENCRYPTION_RIJNDAEL
		void CreateState(CBigEndpointStateUpr* pBigMgr, CMessageMapper& msgMapper, Rijndael::Direction dir, const uint8* key, uint8* initVec)
		{
			NET_ASSERT(!m_pBigState);
			m_pBigState = pBigMgr->Create(ACK_TYPE_NUM_TYPES, msgMapper, dir, key, initVec);
		}
#elif ENCRYPTION_STREAMCIPHER
		void CreateState(CBigEndpointStateUpr* pBigMgr, CMessageMapper& msgMapper, const uint8* key, int keyLen)
		{
			NET_ASSERT(!m_pBigState);
	...
	{
	public:
		void   Clone(CBigEndpointStateUpr* pBigMgr, const CInputState&, uint32 nSeq);
