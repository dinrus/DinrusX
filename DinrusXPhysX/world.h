...
public:
    ...
    virtual void             SetHeightfieldMatMapping(int* pMatMapping, int nMats) { DRX_PHYSX_LOG_FUNCTION; }
    ...
    virtual void  SetiPhysicsTime(int itime) { DRX_PHYSX_LOG_FUNCTION; }

    virtual void  SetSnapshotTime(float time_snapshot, int iType = 0) { DRX_PHYSX_LOG_FUNCTION; }
    virtual void  SetiSnapshotTime(int itime_snapshot, int iType = 0) { DRX_PHYSX_LOG_FUNCTION; }
    ...
    virtual int                GetEntityCount(int iEntType) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; } //!< iEntType is of pe_type
    virtual int                ReserveEntityCount(int nExtraEnts) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; } //!< can prevent excessive internal lists re-allocations
    ...
    virtual void RasterizeEntities(const primitives::grid3d& grid, uchar* rbuf, int objtypes, float massThreshold, const Vec3& offsBBox, const Vec3& sizeBBox, int flags) { DRX_PHYSX_LOG_FUNCTION; }

    virtual int   DeformPhysicalEntity(IPhysicalEntity* pent, const Vec3& ptHit, const Vec3& dirHit, float r, int flags = 0) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
    ...
    virtual void RemoveExplosionShape(int id) { DRX_PHYSX_LOG_FUNCTION; }
    ...
    virtual int   GetEntityProfileInfo(phys_profile_info*& pList) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
    virtual int   GetFuncProfileInfo(phys_profile_info*& pList) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
    virtual int   GetGroupProfileInfo(phys_profile_info*& pList) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
    virtual int   GetJobProfileInfo(phys_job_info*& pList) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
    ...
    virtual uint32           GetPumpLoggedEventsTicks() { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
    virtual void             ClearLoggedEvents();

    virtual IPhysicalEntity* AddGlobalArea();
    virtual IPhysicalEntity* AddArea(Vec3* pt, int npt, float zmin, float zmax, const Vec3& pos = Vec3(0, 0, 0), const quaternionf& q = quaternionf(IDENTITY), float scale = 1.0f, const Vec3& normal = Vec3(ZERO), int* pTessIdx = 0, int nTessTris = 0, Vec3* pFlows = 0) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }
    virtual IPhysicalEntity* AddArea(IGeometry* pGeom, const Vec3& pos, const quaternionf& q, float scale);
    virtual IPhysicalEntity* AddArea(Vec3* pt, int npt, float r, const Vec3& pos = Vec3(0, 0, 0), const quaternionf& q = quaternionf(IDENTITY), float scale = 1) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }
    ...
    virtual int              SetWaterUprParams(pe_params* params) { return 0; }
    virtual int              GetWaterUprParams(pe_params* params) { return 0; }
    virtual int              GetWatermanStatus(pe_status* status) { return 0; }
    virtual void             DestroyWaterUpr() {}

    virtual volatile int*    GetInternalLock(int idx) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; } //!< returns one of phys_lock locks
    ...
    virtual void             SerializeGarbageTypedSnapshot(TSerialize ser, int iSnapshotType, int flags) { DRX_PHYSX_LOG_FUNCTION; }
