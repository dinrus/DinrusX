...
#include "vectorn.h"
#include "quotient.h"
#include <DinrusX/DinrusXPhys/polynomial.h>
...
int BakeScaleIntoGeometry(struct phys_geometry *&pgeom,struct IGeomUpr *pGeoman, const Vec3& s, int bReleaseOld=0);
...
#else
	ILINE int AtomicCAS(volatile int *pLock, int setVal, int checkVal) { return DrxInterlockedCompareExchange((volatile LONG*) pLock, setVal, checkVal ); }
#endif
...
template<typename T, size_t HASH_BITS = 10> class CPhysHashMap { 
  ...
  entry* AllocateEntry(const uint32 key) { 
    if (m_entry_pool == NULL || m_entry_pool->counter == refill_size) { 
      ...
      char* buffer=(char*) DrxModuleMemalign(128, entrySize+blockSize);
