...
#include <DinrusX/CoreX/stridedptr.h>
...
public:
	...
	unused_marker&                     operator,(int& x) DRX_GCC48_AVOID_OPTIMIZE;
	unused_marker&                     operator,(unsigned int& x) DRX_GCC48_AVOID_OPTIMIZE;
...
struct IPhysicalEntity
{
	...
	virtual int          GetStateSnapshot(class CStream& stm, float time_back = 0, int flags = 0) = 0; //!< obsolete, was used in Far Drx
...
struct IPhysicalWorld
{
	...
	virtual int              SetWaterUprParams(pe_params* params) = 0; //!< pe_params_waterman; creates water manager if none is present and nExtraTiles is set
	virtual int              GetWaterUprParams(pe_params* params) = 0;
