...
struct SEntityGrid : public CPhysicalPlaceholder, grid {
	...
	virtual int AddRef() { return DrxInterlockedIncrement(&m_refCount); }
	virtual int Release();
...
public:
  ...
  virtual IPhysicalEntityIt* GetEntitiesIterator();
	...
	virtual int GetGroupProfileInfo(phys_profile_info *&pList) { pList=m_grpProfileData; return DRX_ARRAY_COUNT(m_grpProfileData); }
	virtual int GetJobProfileInfo(phys_job_info *&pList) { pList = m_JobProfileInfo; return DRX_ARRAY_COUNT(m_JobProfileInfo); }
	...
	virtual int SetWaterUprParams(pe_params *params);
	virtual int GetWaterUprParams(pe_params *params);
...
public:
	...
	virtual int AddRef() { return DrxInterlockedIncrement(&m_lockRef); }
	virtual int Release() { return DrxInterlockedDecrement(&m_lockRef); }
...
inline int IsPODThread(CPhysicalWorld *pWorld) { return iszero((int32)(DrxGetCurrentThreadId()-(pWorld->m_idPODThread))); }
...
struct CPhysEntityProfiler : CPhysEntityProfilerBase {
	...
	~CPhysEntityProfiler() {
		if (m_pEntity->m_pWorld->m_vars.bProfileEntities)
			m_pEntity->m_pWorld->AddEntityProfileInfo(m_pEntity,(int)((DrxGetTicks()-m_iStartTime)));
	}
};
struct CPhysAreaProfiler : CPhysEntityProfilerBase {
	...
	~CPhysAreaProfiler() {
		if (m_pArea && m_pArea->m_pWorld->m_vars.bProfileEntities)
			m_pArea->m_pWorld->AddEntityProfileInfo((CPhysicalEntity*)m_pArea,(int)((DrxGetTicks()-m_iStartTime)));
	}
};
...
struct CPhysFuncProfiler {
	...
	~CPhysFuncProfiler() {
		if (m_pWorld->m_vars.bProfileFunx)
			m_pWorld->AddFuncProfileInfo(m_name, (int)(DrxGetTicks()-m_iStartTime));
	}
};
