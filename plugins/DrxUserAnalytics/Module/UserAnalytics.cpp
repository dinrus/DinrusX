...
#include "UserAnalytics.h"
	...
	<DinrusX/CoreX/Thread/IThreadUpr.h>
	<DinrusX/CoreX/Extension/DrxGUID.h>
	<DinrusX/DinrusXSys/DrxVersion.h>
	<DinrusX/CoreX/Serialization/IArchiveHost.h>
	<DinrusX/CoreX/String/Path.h>

	<DinrusX/winsock2.h>
	<DinrusX/curl/curl.h>
	<DinrusX/shlobj.h>

	#define UA_FULL_SERVER_URL "https://www.drxengine.com/useranalytics/" // slash at the end to avoid redirect
...
public:
	...
	void ThreadEntry()
	{
		...
		while (bRun)
		{
			DrxAutoLock<DrxMutexFast> lock(m_shutdownMutex);
...
private:
	...
	DrxMutexFast         m_shutdownMutex;
	DrxConditionVariable m_shutdownCond;
...
{
	DrxLogAlways("[User Analytics DebugCurl] %s", data);
	return 0;
}
	...
	case ESYSTEM_EVENT_DRXSYSTEM_INIT_DONE:
		{
			...
			if (!gEnv->pThreadUpr->SpawnThread(m_pUserAnalyticsSendThread, "UserAnalytics"))
			{
				DRX_ASSERT_MESSAGE(false, "Error spawning \"UserAnalytics\" thread.");
...
void CUserAnalytics::Shutdown()
{
	...
	if (m_pUserAnalyticsSendThread)
	{
		...
		gEnv->pThreadUpr->JoinThread(m_pUserAnalyticsSendThread, eJM_Join);
		delete m_pUserAnalyticsSendThread;
	}
...
{
	static DrxGUID sessionId = DrxGUID::Create();
...
void CUserAnalytics::ReadWriteAnonymousToken()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		drx_strcpy(szLocalDirectoryPath, MAX_PATH, DrxStringUtils::WStrToUTF8(pLocalDirectoryPath));

		string sFilePath = szLocalDirectoryPath;
		sFilePath.append("\\Dinrus\\DRXENGINE\\");

		DrxCreateDirectory(sFilePath.c_str());
	...
	{
		DrxLogAlways("[User Analytics] Could not read or write anonymous user token");
	}
}
...
void CUserAnalytics::ReadUserIdFromDisk()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		string sFilePath = PathUtil::Make(szLocalDirectoryPath, ".drxengine/common.json");
...
void CUserAnalytics::PrepareAndSendEvents()
{
	...
	if (InitializeCURL())
	{
		...
		if (res == CURLE_OK)
		{
			...
			{
				DrxLogAlways("[User Analytics] Successfully pushed %d event(s) to %s", eventsCount, m_userAnalyticsServerAddress ? m_userAnalyticsServerAddress->GetString() : UA_FULL_SERVER_URL);

				DrxLogAlways("[User Analytics] Queuing Event: SessionAlive");
			}
		...
		else
		{
			if (cv_logging && cv_logging->GetIVal() > 0)
			{
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[User Analytics] Failed to send user events to server: %s, stopping to collect new events.", curl_easy_strerror(res));
			}
			...
			if (cv_collect)
			{
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[User Analytics] Setting CVar sys_UserAnalyticsCollect back to zero.");
				cv_collect->Set(0);
			}
		}
	}
