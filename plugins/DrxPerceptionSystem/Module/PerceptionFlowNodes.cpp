...
void CFlowNode_PerceptionState::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	case eFE_Activate:
	{
		...
		if (pAIActor)
		{
			...
			{
				CPerceptionUpr::GetInstance()->SetActorState(pAIObject, true);
			}
			else if (IsPortActive(pActInfo, IN_DISABLE))
			{
				CPerceptionUpr::GetInstance()->SetActorState(pAIObject, false);
			}
			else if (IsPortActive(pActInfo, IN_RESET))
			{
				CPerceptionUpr::GetInstance()->ResetActor(pAIObject);
...
{
	if (IPerceptionUpr::GetInstance())
	{
		IPerceptionUpr::GetInstance()->UnregisterAIStimulusEventListener(functor(*this, &CFlowNode_AIEventListener::OnAIEvent));
	}
}
...
void CFlowNode_AIEventListener::ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
{
	...
	if (event == eFE_Initialize)
	{
		...
		if (IPerceptionUpr::GetInstance())
		{
			...
			IPerceptionUpr::GetInstance()->RegisterAIStimulusEventListener(functor(*this, &CFlowNode_AIEventListener::OnAIEvent), params);
		}
...
void CFlowNode_AIEventListener::GetMemoryUsage(IDrxSizer * sizer) const
{
	sizer->Add(*this);
}
...
{
	DRX_ASSERT(m_pGraph->IsEnabled() && !m_pGraph->IsSuspended() && m_pGraph->IsActive());
