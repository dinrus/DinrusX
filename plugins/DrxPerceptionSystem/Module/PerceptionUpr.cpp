...
<DinrusX/DinrusXAI/IAgent.h>
<DinrusX/DinrusXAI/IActorLookUp.h>
<DinrusX/DinrusXAI/IAIObjectUpr.h>
<DinrusX/DinrusXAI/IAIActorProxy.h>

<DinrusX/DinrusXNetwork/ISerialize.h>

<DinrusX/DinrusXPhys/RayCastQueue.h>
...
CPerceptionUpr* CPerceptionUpr::s_pInstance = nullptr;

//-----------------------------------------------------------------------------------------------------------
CPerceptionUpr::CPerceptionUpr() :
	m_pScriptBind(nullptr),
	m_bRegistered(false)
{
	GetISystem()->GetISystemEventDispatcher()->RegisterListener(this, "CPerceptionUpr");
	...
	DRX_ASSERT(s_pInstance == nullptr);
	s_pInstance = this;
}
...
CPerceptionUpr::~CPerceptionUpr()
{
	...
	if (gEnv->pAISystem)
	{
		gEnv->pAISystem->Callbacks().ObjectCreated().Remove(functor(*this, &CPerceptionUpr::OnAIObjectCreated));
		gEnv->pAISystem->Callbacks().ObjectRemoved().Remove(functor(*this, &CPerceptionUpr::OnAIObjectRemoved));
		gEnv->pAISystem->UnregisterSystemComponent(this);
	}
	delete m_pScriptBind;
}

void CPerceptionUpr::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
{
	switch (event)
	{
	case ESYSTEM_EVENT_GAME_POST_INIT:
	{
		DRX_ASSERT(gEnv->pAISystem);
		if (!m_bRegistered && gEnv->pAISystem)
		{
			gEnv->pAISystem->RegisterSystemComponent(this);
			gEnv->pAISystem->Callbacks().ObjectCreated().Add(functor(*this, &CPerceptionUpr::OnAIObjectCreated));
			gEnv->pAISystem->Callbacks().ObjectRemoved().Add(functor(*this, &CPerceptionUpr::OnAIObjectRemoved));

			m_bRegistered = true;
		}
		if (!m_pScriptBind)
		{
			m_pScriptBind = new CScriptBind_PerceptionUpr(gEnv->pSystem);
		}
	}
	default:
		break;
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::InitCommonTypeDescs()
{
...
//-----------------------------------------------------------------------------------------------------------
bool CPerceptionUpr::RegisterStimulusDesc(EAIStimulusType type, const SAIStimulusTypeDesc& desc)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::Reset(IAISystem::EResetReason reason)
{
	m_visBroadPhaseDt = 0;
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::SetPriorityObjectType(uint16 type)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::OnAIObjectCreated(IAIObject* pAIObject)
{
	DRX_ASSERT(pAIObject->GetAIObjectID() != INVALID_AIOBJECTID);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::OnAIObjectRemoved(IAIObject* pAIObject)
{
	DRX_ASSERT(pAIObject->GetAIObjectID() != INVALID_AIOBJECTID);
...
//-----------------------------------------------------------------------------------------------------------
CPerceptionActor* CPerceptionUpr::GetPerceptionActor(IAIObject* pAIObject)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::SetActorState(IAIObject* pAIObject, bool enabled)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::ResetActor(IAIObject* pAIObject)
{
	CPerceptionActor* perceptionActor = GetPerceptionActor(pAIObject);
	if (perceptionActor)
	{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::ActorUpdate(IAIObject* pAIObject, IAIObject::EUpdateType type, float frameDelta)
{
	...
	{
		DRX_PROFILE_SECTION(PROFILE_AI, "AIPlayerVisibilityCheck");
	...
	{
		DRX_PROFILE_SECTION(PROFILE_AI, "ProbableTargets");
		// Probable targets.
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::GatherProbableTargetsForActor(IAIActor* pAIActor)
{
	m_probableTargets.clear();
	
	DRX_ASSERT(gEnv->pAISystem->GetActorLookup());
...
//-----------------------------------------------------------------------------------------------------------
bool CPerceptionUpr::FilterStimulus(SAIStimulus* stim)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::Update(float deltaTime)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::UpdateIncomingStimuli()
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::UpdateStimuli(float deltaTime)
{
	// Update stimuli
	// Merge and filter with current active stimuli.
	for (unsigned int i = 0; i < AI_MAX_STIMULI; ++i)
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::VisCheckBroadPhase(float deltaTime)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
	...
	for (auto it = gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_VEHICLE); it->GetObject(); it->Next())
	{
	...
	for (auto it = gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_TARGET); it->GetObject(); it->Next())
	{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::UpdatePriorityTargets()
{
	...
	if (gEnv->pAISystem->GetActorLookup()->GetActiveCount() > 0)
	{
		for (unsigned i = 0, ni = m_priorityObjectTypes.size(); i < ni; ++i)
		{
			...
			for (auto it = gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_TYPE, type); it->GetObject(); it->Next())
			{
				IAIObject* pTarget = it->GetObject();
				if (!pTarget->IsEnabled())
					continue;
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::RegisterStimulus(const SAIStimulus& stim)
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::IgnoreStimulusFrom(EntityId sourceId, EAIStimulusType type, float time)
{
	// Do not handle events while serializing.
	if (gEnv->pSystem->IsSerializingFile())
		return;
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::HandleSound(const SStimulusRecord& stim)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
	...
	auto playerIt = gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_PLAYER);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::HandleCollision(const SStimulusRecord& stim)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::HandleExplosion(const SStimulusRecord& stim)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::HandleBulletHit(const SStimulusRecord& stim)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::HandleBulletWhizz(const SStimulusRecord& stim)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::HandleGrenade(const SStimulusRecord& stim)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::SetLastExplosionPosition(const Vec3& position, IAIObject* pAIObject) const
{
	DRX_ASSERT(pAIObject);
	if (pAIObject)
	{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::RegisterAIStimulusEventListener(const EventCallback& eventCallback, const SAIStimulusEventListenerParams& params)
{
	m_eventListeners[eventCallback] = params;
}

//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::UnregisterAIStimulusEventListener(const EventCallback& eventCallback)
{ 
	m_eventListeners.erase(eventCallback);
}

//-----------------------------------------------------------------------------------------------------------
bool CPerceptionUpr::IsPointInRadiusOfStimulus(EAIStimulusType type, const Vec3& pos) const
{
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::NotifyAIEventListeners(const SStimulusRecord& stim, float threat)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
float CPerceptionUpr::SupressSound(const Vec3& pos, float radius)
{
	float minScale = 1.0f;

	for (auto it = gEnv->pAISystem->GetAIObjectUpr()->GetFirstAIObject(OBJFILTER_TYPE, AIOBJECT_SNDSUPRESSOR); it->GetObject(); it->Next())
	{
...
//-----------------------------------------------------------------------------------------------------------
bool CPerceptionUpr::IsSoundOccluded(IAIActor* pAIActor, const Vec3& vSoundPos)
{
	DRX_PROFILE_FUNCTION( PROFILE_AI);
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::Serialize(TSerialize ser)
{
	if (ser.IsReading())
	{
		m_probableTargets.clear();
	...
	ser.BeginGroup("PerceptionUpr");
	for (unsigned i = 0; i < AI_MAX_STIMULI; ++i)
	{
		char name[64];
		drx_sprintf(name, "m_stimuli%02d", i);
		ser.Value(name, m_stimuli[i]);
		drx_sprintf(name, "m_ignoreStimuliFrom%02d", i);
...

	{
		...
		if (ser.IsReading())
		{
			...
			for (uint32 i = 0; i < actorsCount; ++i)
			{
				...
				IAIObject* pAIObject = gEnv->pAISystem->GetAIObjectUpr()->GetAIObject(objID);
				if (pAIObject)
				{
					IAIActor* pAIActor = pAIObject->CastToIAIActor();
					if (pAIActor)
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::RecordStimulusEvent(const SStimulusRecord& stim, float fRadius, IAIObject& receiver) const
{
...
//-----------------------------------------------------------------------------------------------------------
bool CPerceptionUpr::IsStimulusVisible(const SStimulusRecord& stim, const IAIObject* pAIObject)
{
...
void CPerceptionUpr::CVars::Init()
{
	DefineConstIntCVarName("ai_DebugPerceptionUpr", DebugPerceptionUpr, 0, VF_CHEAT | VF_CHEAT_NOCHECK,
...
<DinrusX/DinrusXAI/IAIDebugRenderer.h>
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::DebugDraw(IAIDebugRenderer* pDebugRenderer)
{
	if (m_cVars.DebugPerceptionUpr > 0)
	{
		DRX_PROFILE_FUNCTION(PROFILE_AI);

		DebugDrawPerception(pDebugRenderer, m_cVars.DebugPerceptionUpr);
		DebugDrawPerformance(pDebugRenderer, m_cVars.DebugPerceptionUpr);
	}
}

//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::DebugDrawPerception(IAIDebugRenderer* pDebugRenderer, int mode)
{
	...
	for (unsigned i = 0; i < AI_MAX_STIMULI; ++i)
	{
		...
		for (unsigned j = 0, nj = stimuli.size(); j < nj; ++j)
		{
			...
			DRX_ASSERT(s.subType < SAIStimulusTypeDesc::AI_MAX_SUBTYPES);
			...
			case AISTIM_SOUND:
				...
				{
				case AISOUND_GENERIC:
					drx_sprintf(subTypeTxt, "GENERIC  R=%.1f", s.radius);
					break;
				case AISOUND_COLLISION:
					drx_sprintf(subTypeTxt, "COLLISION  R=%.1f", s.radius);
					break;
				case AISOUND_COLLISION_LOUD:
					drx_sprintf(subTypeTxt, "COLLISION LOUD  R=%.1f", s.radius);
					break;
				case AISOUND_MOVEMENT:
					drx_sprintf(subTypeTxt, "MOVEMENT  R=%.1f", s.radius);
					break;
				case AISOUND_MOVEMENT_LOUD:
					drx_sprintf(subTypeTxt, "MOVEMENT LOUD  R=%.1f", s.radius);
					break;
				case AISOUND_WEAPON:
					drx_sprintf(subTypeTxt, "WEAPON\nR=%.1f", s.radius);
					break;
				case AISOUND_EXPLOSION:
					drx_sprintf(subTypeTxt, "EXPLOSION  R=%.1f", s.radius);
					break;
				}
				break;
			case AISTIM_COLLISION:
				...
				{
				case AICOL_SMALL:
					drx_sprintf(subTypeTxt, "SMALL  R=%.1f", s.radius);
					break;
				case AICOL_MEDIUM:
					drx_sprintf(subTypeTxt, "MEDIUM  R=%.1f", s.radius);
					break;
				case AICOL_LARGE:
					drx_sprintf(subTypeTxt, "LARGE  R=%.1f", s.radius);
					break;
				}
				;
				break;
			...
			case AISTIM_BULLET_HIT:
			case AISTIM_GRENADE:
				drx_sprintf(subTypeTxt, "R=%.1f", s.radius);
				break;
			default:
				break;
			}
...
//-----------------------------------------------------------------------------------------------------------
void CPerceptionUpr::DebugDrawPerformance(IAIDebugRenderer* pDebugRenderer, int mode)
{
