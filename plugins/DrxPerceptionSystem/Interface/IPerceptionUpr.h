...
#include "IDrxPerceptionSystemPlugin.h"
...
struct IPerceptionUpr
{
	...
	static IPerceptionUpr* GetInstance()
	{
...
private:
	struct SCachedInstancePtr
	{
		...
		IPerceptionUpr* Get()
		{
			if (!m_pInstancePtr)
			{
				IDrxPerceptionSystemPlugin* perceptionPlugin = gEnv->pSystem->GetIPluginUpr()->QueryPlugin<IDrxPerceptionSystemPlugin>();
				if (perceptionPlugin)
				{
					m_pInstancePtr = &perceptionPlugin->GetPerceptionUpr();
				}
			}
			return m_pInstancePtr;
		}
		IPerceptionUpr* m_pInstancePtr;
	};
};
