...
namespace Drx
{
	namespace DefaultComponents
	{
		void CAdvancedAnimationComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
		{
			// Functions
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CAdvancedAnimationComponent::ActivateContext, "{2DA18E2A-75F6-4EEE-9C7C-60ABF275555E}"_drx_guid, "ActivateContext");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CAdvancedAnimationComponent::QueueFragment, "{6F5AA73B-B8F9-4392-9651-468DEB342D91}"_drx_guid, "QueueFragment");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CAdvancedAnimationComponent::SetTag, "{04768686-96DF-4F82-8CC8-109522A9F2E0}"_drx_guid, "SetTag");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CAdvancedAnimationComponent::SetMotionParameter, "{67B8D89B-D56B-443E-B88B-BFEBEE52AFB7}"_drx_guid, "SetMotionParameter");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CAdvancedAnimationComponent::SetMeshType, "{A4D2B249-64BE-48DA-8700-E3B479BB6F65}"_drx_guid, "SetType");
		...
		inline bool Serialize(Serialization::IArchive& archive, CAdvancedAnimationComponent::SDefaultScopeSettings& defaultSettings, const char* szName, const char* szLabel)
		{
			...
			if (archive.isEdit())
			{
				...
				{
					pParams->pControllerDef = animationDatabaseUpr.LoadControllerDef(defaultSettings.m_controllerDefinitionPath);
				}
			}
				...
				if (m_pCachedCharacter != nullptr)
				{
					if (IPhysicalEntity* pPhysicalEntity = m_pEntity->GetPhysicalEntity())
					{
						...
						if (pPhysicalEntity->GetStatus(&dynStatus))
						{
							...
							// Update the turn speed in DinrusXAnimation, note that the maximum motion parameter (10) applies here too.
