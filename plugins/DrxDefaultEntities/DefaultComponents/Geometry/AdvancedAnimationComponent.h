...
public:
	struct SDefaultScopeSettings
	{
		...
		{
			desc.SetGUID("{1ADE6AA3-79F6-48F2-9D61-913BF88573E6}"_drx_guid);
	...
	{
		desc.SetGUID("{3CD5DDC5-EE15-437F-A997-79C2391537FE}"_drx_guid);
	...
	virtual void ActivateContext(const Schematyc::CSharedString& contextName)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to find scope context %s in controller definition.", contextName.c_str());
			return;
		}
	...
	virtual void QueueFragment(const Schematyc::CSharedString& fragmentName)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to find Mannequin fragment %s in controller definition %s", fragmentName.c_str(), m_pAnimationContext->controllerDef.m_filename.c_str());
			return;
		}
	...
	{
		DRX_ASSERT(m_pCachedCharacter != nullptr);
		if (m_pCachedCharacter != nullptr)
		{
	...
	virtual void LoadFromDisk()
	{
		...
		{
			m_pCachedCharacter = gEnv->pCharacterUpr->CreateInstance(m_characterFile.value);
			if (m_pCachedCharacter == nullptr)
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load character %s!", m_characterFile.value.c_str());
				return;
			}
		...
		if (m_defaultScopeSettings.m_controllerDefinitionPath.size() > 0 && m_databasePath.value.size() > 0)
		{
			...
			IAnimationDatabaseUpr& animationDatabaseUpr = mannequinInterface.GetAnimationDatabaseUpr();

			// Load the Mannequin controller definition.
			// This is owned by the animation database manager
			m_pControllerDefinition = animationDatabaseUpr.LoadControllerDef(m_defaultScopeSettings.m_controllerDefinitionPath);
			if (m_pControllerDefinition == nullptr)
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load controller definition %s!", m_defaultScopeSettings.m_controllerDefinitionPath.c_str());
				return;
			}

			// Load the animation database
			m_pDatabase = animationDatabaseUpr.Load(m_databasePath.value);
			if (m_pDatabase == nullptr)
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load animation database %s!", m_databasePath.value.c_str());
				return;
			}
		}
	}
	...
	virtual void ResetCharacter()
	{
		...
		if (m_pControllerDefinition != nullptr)
		{
			...
			DRX_ASSERT(m_pActionController != nullptr);
