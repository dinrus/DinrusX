...
void CStaticMeshComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
{
	// Functions
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CStaticMeshComponent::LoadFromDisk, "{E900651C-169C-4FF2-B987-CFC5613D51EB}"_drx_guid, "LoadFromDisk");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CStaticMeshComponent::ResetObject, "{86AA9C32-3D54-4622-8032-C99C194FF24A}"_drx_guid, "ResetObject");
		pFunction->SetDescription("Resets the object");
		pFunction->SetFlags({ Schematyc::EEnvFunctionFlags::Member, Schematyc::EEnvFunctionFlags::Construction });
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CStaticMeshComponent::SetMeshType, "{6C4F6C80-2F84-4C9B-ADBF-5C131EFDD98A}"_drx_guid, "SetType");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CStaticMeshComponent::SetObject, "{7E31601A-2766-4F3D-AF97-65F34556C664}"_drx_guid, "SetObject");
...
void CStaticMeshComponent::ResetObject()
{
	if (m_pCachedStatObj != nullptr)
	{
		...
		{
			if (IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(m_materialPath.value, false))
			{
...
}} // namespace Drx::DefaultComponents
