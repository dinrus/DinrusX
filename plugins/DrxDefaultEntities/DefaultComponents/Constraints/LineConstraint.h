...
namespace Drx
{
	namespace DefaultComponents
	{
		...
		public:
			...
			{
				desc.SetGUID("{D25359AC-E566-47F9-B056-AC52C4361348}"_drx_guid);
			...
			virtual void ConstrainTo(IPhysicalEntity* pOtherEntity, bool bDisableCollisionsWith = false, bool bAllowRotation = true)
			{
				...
				if (IPhysicalEntity* pConstraintOwner = m_pEntity->GetPhysicalEntity())
				{
					...
					if (pConstraintOwner->GetType() != PE_RIGID && pConstraintOwner->GetType() != PE_WHEELEDVEHICLE)
					{
						...
						{
							DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried to add line constraint to non-rigid or vehicle entity!");
							return;
						}
						...
						if (pConstraintOwner->GetType() != PE_RIGID && pConstraintOwner->GetType() != PE_WHEELEDVEHICLE)
						{
							DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried to add line constraint to non-rigid or vehicle entities!");
							return;
						}
#endif
					}
