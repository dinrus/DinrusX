...
namespace Drx
{
	namespace DefaultComponents
	{
		...
		{
			desc.SetGUID("{F5C08E7D-6C16-422e-AF2F-4BD19A55E81F}"_drx_guid);
		...
		{
			desc.SetGUID("{BA874580-D9CF-4db0-B4BB-8B4E95CF867D}"_drx_guid);
		...
		{
			desc.SetGUID("{BC763AAA-C1CB-4f83-8F7E-4AF3F3CC156B}"_drx_guid);
		...
		void CBreakableJointComponent::Physicalize()
		{
			if (IPhysicalEntity *pent = m_pEntity->GetPhysics())
			{
				...
				static IGeometry *pSph = gEnv->pPhysicalWorld->GetGeomUpr()->CreatePrimitive(primitives::sphere::type, &sph);
