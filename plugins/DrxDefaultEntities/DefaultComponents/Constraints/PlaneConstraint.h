...
namespace Drx
{
	namespace DefaultComponents
	{
		...
		public:
			...
			{
				desc.SetGUID("{F9C3A204-809B-4DA8-965E-CA9417FEBD95}"_drx_guid);
			...
			virtual void ConstrainTo(IPhysicalEntity* pOtherEntity, bool bDisableCollisionsWith = false)
			{
				...
				if (IPhysicalEntity* pConstraintOwner = m_pEntity->GetPhysicalEntity())
				{
					...
					if (pConstraintOwner->GetType() != PE_RIGID && pConstraintOwner->GetType() != PE_WHEELEDVEHICLE)
					{
						...
						{
							DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried to add plane constraint to non-rigid or vehicle entity!");
							return;
						}
						...
						if (pConstraintOwner->GetType() != PE_RIGID && pConstraintOwner->GetType() != PE_WHEELEDVEHICLE)
						{
							DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried to add plane constraint to non-rigid or vehicle entities!");
							return;
						}
#endif
					}
			...
			virtual void SetLimitsX(DrxTransform::CAngle minLimit, DrxTransform::CAngle maxLimit) { m_limitMin = minLimit; m_limitMax = maxLimit; }
			DrxTransform::CAngle GetMinimumLimitX() const { return m_limitMin; }
			DrxTransform::CAngle GetMaximumLimitX() const { return m_limitMax; }

			virtual void SetLimitsY(DrxTransform::CAngle maxLimit) { m_limitMaxY = maxLimit; }
			DrxTransform::CAngle GetMaximumLimitY() const { return m_limitMaxY; }
		...
		protected:
			...
			DrxTransform::CClampedAngle<-360, 360>  m_limitMin = -360.0_degrees;
			DrxTransform::CClampedAngle<-360, 360>  m_limitMax = 360.0_degrees;
			DrxTransform::CClampedAngle<0, 180>  m_limitMaxY = 0.0_degrees;
