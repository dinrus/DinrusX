...
namespace Drx
{
	namespace DefaultComponents
	{
		...
		public:
			...
			{
				desc.SetGUID("{4A010113-38A2-4CB0-8C98-BC1956675F40}"_drx_guid);
			...
			virtual void ConstrainTo(IPhysicalEntity* pOtherEntity, bool bDisableCollisionsWith = false)
			{
				...
				if (IPhysicalEntity* pConstraintOwner = m_pEntity->GetPhysicalEntity())
				{
					...
					if (pConstraintOwner->GetType() != PE_RIGID && pConstraintOwner->GetType() != PE_WHEELEDVEHICLE)
					{
						...
						{
							DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried to add point constraint to non-rigid or vehicle entity!");
							return;
						}
						...
						if (pConstraintOwner->GetType() != PE_RIGID && pConstraintOwner->GetType() != PE_WHEELEDVEHICLE)
						{
							DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried to add point constraint to non-rigid or vehicle entities!");
							return;
						}
#endif
					}
			...
			virtual void SetRotationLimitsX(DrxTransform::CAngle minValue, DrxTransform::CAngle maxValue) { m_rotationLimitsX0 = minValue; m_rotationLimitsX1 = maxValue; }
			DrxTransform::CAngle GetRotationLimitXMin() const { return m_rotationLimitsX0; }
			DrxTransform::CAngle GetRotationLimitXMax() const { return m_rotationLimitsX1; }

			virtual void SetRotationLimitsyz(DrxTransform::CAngle maxValue) { m_rotationLimitsYZ1 = maxValue; }
			DrxTransform::CAngle GetRotationLimitYZMax() const { return m_rotationLimitsYZ1; }
		...
		protected:
			...
			DrxTransform::CClampedAngle<-360, 360> m_rotationLimitsX0 = 0.0_degrees;
			DrxTransform::CClampedAngle<-360, 360> m_rotationLimitsX1 = 0.0_degrees;
			DrxTransform::CClampedAngle<0, 180> m_rotationLimitsYZ1 = 0.0_degrees;
