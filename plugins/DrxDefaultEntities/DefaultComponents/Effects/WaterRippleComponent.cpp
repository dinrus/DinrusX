...
namespace Drx
{
	namespace DefaultComponents
	{
		...
		{
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CWaterRippleComponent::Enable, "{BB44DB39-BD03-402F-B2E4-152B6D2E295A}"_drx_guid, "Enable");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CWaterRippleComponent::SpawnRippleAtPosition, "{DF99F8E1-56B5-4E59-86C0-D0A00020839F}"_drx_guid, "SpawnRippleAtPosition");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CWaterRippleComponent::IsEnabled, "{EC45ADC0-3CDE-4739-958B-2D8662A9D703}"_drx_guid, "IsEnabled");
		...
		void CWaterRippleComponent::ProcessEvent(const SEntityEvent& event)
		{
			...
			{
				float fFrequency = m_frequency + drx_random(-1.0f, 1.0f)*m_randFrequency;
		...
		void CWaterRippleComponent::ProcessHit(bool bIsMoving)
		{
			if (m_bEnabled)
			{
				...
				{
					vWorldPos += Vec3(m_randomOffset).CompMul(drx_random_componentwise(Vec3(-1, -1, 0), Vec3(1, 1, 0)));
				}

				float fScale = m_scale + drx_random(-1.0f, 1.0f) * m_randScale;
				float fStrength = m_strength + drx_random(-1.0f, 1.0f) * m_randStrength;
