...
namespace Drx
{
	...
	{
#if !defined(RELEASE) && defined(WIN32) && !defined(DRX_PLATFORM_CONSOLE)
#define SUPPORT_ENVIRONMENT_PROBE_GENERATION
		...
		public:
			...
			{
				desc.SetGUID("63297E89-EA00-4F56-AC23-2E93C68FE71D"_drx_guid);
			...
			struct SOptions
			{
				...
				{
					desc.SetGUID("{DB10AB64-7A5B-4B91-BC90-6D692D1D1222}"_drx_guid);
			...
			struct SColor
			{
				...
				{
					desc.SetGUID("{B71C3414-F85A-4EAA-9CE0-5110A2E040AD}"_drx_guid);
			...
			struct SGeneration
			{
				...
				{
					desc.SetGUID("{1CF55472-FAD0-42E7-B06A-20B96F954914}"_drx_guid);
			...
			virtual void Generate(const Schematyc::TextureFileName& cubemapFileName)
			{
				...
				if (vecData.size() >= srcSideSize * sides)
				{
					...
					for (int side = 0; side < sides; ++side)
					{
						for (uint32 y = 0; y < (uint32)m_generation.m_resolution; ++y)
						{
							...
							DrxHalf4* pDst = (DrxHalf4*)&downsampledImageData[side * dstPitch + y * width];
							for (uint32 x = 0; x < (uint32)m_generation.m_resolution; ++x)
							{
								...
								for (uint32 yres = 0; yres < 4; ++yres)
								{
									...
									{
										const DrxHalf4& pSrc = pSrcSide[(y * 4 + yres) * renderResolution + (x * 4 + xres)];
								...
								*pDst++ = DrxHalf4(cResampledColor.x, cResampledColor.y, cResampledColor.z, cResampledColor.w);
							}
						}
					}

					outputPath = PathUtil::Make(gEnv->pSystem->GetIProjectUpr()->GetCurrentAssetDirectoryAbsolute(), outputPath);

					string outputDirectory = PathUtil::GetPathWithoutFilename(outputPath);
					gEnv->pDrxPak->MakeDir(outputDirectory);
				...
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to generate cube map!");
				}
			...
			virtual void LoadFromDisk(const Schematyc::TextureFileName& cubemapFileName)
			{
				...
				if (GetCubemapTextures(cubemapFileName.value, &pSpecularTexture, &pDiffuseTexture))
				{
					...
					if (gEnv->IsEditor())
					{
						...
						if (IMaterial* pSourceMaterial = pMaterialUpr->LoadMaterial("%EDITOR%/Objects/envcube", false, true))
						{
							IMaterial* pMaterial = pMaterialUpr->CreateMaterial(cubemapFileName.value, pSourceMaterial->GetFlags() | MTL_FLAG_NON_REMOVABLE);
							if (pMaterial)
							{
			...
			virtual void SetCubemap(const Schematyc::ExplicitTextureId specularTextureId, const Schematyc::ExplicitTextureId diffuseTextureId)
			{
				...
				else
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load set cube map for environment probe component attached to entity %s, diffuse texture with id %i did not exist!!", m_pEntity->GetName(), (int)diffuseTextureId);
					}
					if (pSpecularTexture == nullptr)
					{
						DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load set cube map for environment probe component attached to entity %s, specular texture with id %i did not exist!!", m_pEntity->GetName(), (int)specularTextureId);
					}
				}
			}
			...
			virtual bool GetCubemapTextures(const char* path, ITexture** pSpecular, ITexture** pDiffuse, bool bLoadFromDisk = true) const
			{
				...
				if (bLoadFromDisk)
				{
					if (!gEnv->pDrxPak->IsFileExist(specularCubemapUnix) || !gEnv->pDrxPak->IsFileExist(diffuseCubemapUnix))
					{
						DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load cube map for environment probe component attached to entity %s!", m_pEntity->GetName());
						return false;
					}
				...
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load specular cube map for environment probe component attached to entity %s!", m_pEntity->GetName());
					return false;
				}

				if (*pDiffuse == nullptr || !(*pDiffuse)->IsTextureLoaded())
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed to load diffuse cube map for environment probe component attached to entity %s!", m_pEntity->GetName());
					return false;
				}
		...
		{
			desc.SetGUID("204041D6-A198-4DAA-B8B0-4D034DF849BD"_drx_guid);
