...
void CProjectorLightComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
{
	// Functions
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CProjectorLightComponent::Enable, "{54F96D7F-3B98-47F2-B256-4AD856CFE5BD}"_drx_guid, "Enable");
...
void CProjectorLightComponent::Initialize()
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Light projector texture %s not found, disabling projector component for entity %s", szProjectorTexturePath, m_pEntity->GetName());
		FreeEntitySlot();
		return;
	}
	...
	if (m_flare.HasTexturePath())
	{
		...
		if (gEnv->pOpticsUpr->Load(m_flare.GetTexturePath(), nLensOpticsId))
		{
			IOpticsElementBase* pOptics = gEnv->pOpticsUpr->GetOptics(nLensOpticsId);
			DRX_ASSERT(pOptics != nullptr);
		...
		{
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Flare lens optics %s for projector component in entity %s doesn't exist!", m_flare.GetTexturePath(), GetEntity()->GetName());
	...
	{
		int32 opticsIndex = 0;
		if (gEnv->pOpticsUpr->Load(m_optics.m_lensFlareName.c_str(), opticsIndex))
		{
			IOpticsElementBase* pOpticsElement = gEnv->pOpticsUpr->GetOptics(opticsIndex);
	...
	if (m_projectorOptions.HasMaterialPath())
	{
		...
		if (IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(m_projectorOptions.GetMaterialPath(), false))
		{
	...
	{
		IMaterial* pMaterial = gEnv->p3DEngine->GetMaterialUpr()->LoadMaterial(g_szDefaultLensFlareMaterialName);
		if (pMaterial)
			m_pEntity->SetSlotMaterial(GetEntitySlotId(), pMaterial);
	}

	DrxTransform::CTransformPtr pTransform = m_pTransform;
