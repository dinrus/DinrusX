...
{
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CListenerComponent::SetActive, "1ECF05D6-7E0B-4954-AC2B-087488E42F2B"_drx_guid, "SetActive");
...
void CListenerComponent::Initialize()
{
	...
	{
		SetName("Listener");
		DrxFixedStringT<DrxAudio::MaxObjectNameLength> name;
		...
		if (m_pIListener != nullptr)
		{
			...
			DRX_ASSERT_MESSAGE(tm.IsValid(), "Invalid Matrix34 during CListenerComponent::Initialize");
...
void CListenerComponent::OnTransformChanged()
{
	...
	if (!m_previousTransformation.IsEquivalent(tm, 0.01f))
	{
		...
		// Add entity to the AreaUpr for raising audio relevant events.
		gEnv->pEntitySystem->GetAreaUpr()->MarkEntityForUpdate(m_pEntity->GetId());
	}
}
} // namespace DefaultComponents
} // namespace Audio
