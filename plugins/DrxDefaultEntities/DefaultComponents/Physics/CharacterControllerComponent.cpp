...
void CCharacterControllerComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
{
	// Functions
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::IsOnGround, "{C41A9117-0292-4FF6-978A-F433345841B3}"_drx_guid, "IsOnGround");
		pFunction->SetDescription("Checks whether or not the character is on ground");
		pFunction->BindOutput(0, 'ongr', "OnGround");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::GetGroundNormal, "{72858886-D2D8-4FDD-A8C3-30A6AA85E7FD}"_drx_guid, "GetGroundNormal");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::IsWalking, "{80796905-AC9E-4C0F-A1F1-4CBE0B0807C3}"_drx_guid, "IsWalking");
		pFunction->SetDescription("Checks whether or not the player is moving on ground");
		pFunction->BindOutput(0, 'walk', "Walking");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::AddVelocity, "{B9D4C79B-1BDF-4A5C-969D-3456036C9CD4}"_drx_guid, "AddVelocity");
		pFunction->SetDescription("Adds velocity to that of the character");
		pFunction->BindInput(1, 'vel', "Velocity");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::SetVelocity, "{32A91404-8E63-4D59-8330-ADF5F2AB71DE}"_drx_guid, "SetVelocity");
		pFunction->SetDescription("Overrides the character's velocity");
		pFunction->BindInput(1, 'vel', "Velocity");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::ChangeVelocity, "{B2D208A7-6F80-4580-8AD5-BDD6859872E7}"_drx_guid, "ChangeVelocity");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::GetMoveDirection, "{7D21BDFC-FE14-4020-A685-6F7917AC759C}"_drx_guid, "GetMovementDirection");
		pFunction->SetDescription("Gets the direction the character is moving in");
		pFunction->BindOutput(0, 'dir', "Direction");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CCharacterControllerComponent::GetVelocity, "{BF80D631-05D3-4571-A51D-E0AF6A252F91}"_drx_guid, "GetVelocity");
...
{
	desc.SetGUID("{24C421C5-3E60-42C2-B0D0-6BA3DFC7CF7C}"_drx_guid);
...
void CCharacterControllerComponent::ProcessEvent(const SEntityEvent& event)
{
	if (event.event == ENTITY_EVENT_UPDATE)
	{
		...
		DRX_ASSERT_MESSAGE(pPhysicalEntity != nullptr, "Physical entity removed without call to IEntity::UpdateComponentEventMask!");
