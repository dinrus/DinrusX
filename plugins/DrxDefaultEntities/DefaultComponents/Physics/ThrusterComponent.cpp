...
void CThrusterComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
{
	// Functions
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CThrusterComponent::ApplySingleThrust, "{BAFA4C36-CFDC-4309-B5A1-C0DD7F7F6D15}"_drx_guid, "ApplySingleThrust");
		pFunction->SetDescription("Applies one impulse through the thruster");
		pFunction->BindInput(1, 'impl', "Impulse");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CThrusterComponent::EnableConstantThrust, "{987CF5A0-72C9-4501-859A-7EE85B1499E0}"_drx_guid, "EnableConstantThrust");
		pFunction->SetDescription("Enables automatically thrusting every frame");
		pFunction->BindInput(1, 'enab', "Enable");
		componentScope.Register(pFunction);
	}
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CThrusterComponent::IsConstantThrustEnabled, "{5FEB48B3-0AFF-44CD-B408-127887D412F2}"_drx_guid, "IsConstantThrustEnabled");
