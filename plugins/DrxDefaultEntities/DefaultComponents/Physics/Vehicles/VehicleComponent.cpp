...
<DinrusX/DinrusX3dEng/IRenderNode.h>
...
namespace Drx
{
	namespace DefaultComponents
	{
		void CVehiclePhysicsComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
		{
			// Functions
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::UseHandbrake, "{12279EAD-5B52-49B0-B875-16195B04C103}"_drx_guid, "UseHandbrake");
				pFunction->SetDescription("Activates the handbrake");
				pFunction->BindInput(1, 'use', "Use", "Whether or not to use the handbrake");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::IsUsingHandbrake, "{E9F98A9B-54C8-4D8B-BBBD-B7ADF231D233}"_drx_guid, "IsUsingHandbrake");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::SetThrottle, "{4F73C7FC-2D48-491C-AD5D-E86A0EB5262C}"_drx_guid, "SetThrottleRatio");
				pFunction->SetDescription("Sets the pedal ratio, 0 - 1 (0 being no pedal, 1 fully held down)");
				pFunction->BindInput(1, 'rati', "Ratio");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetThrottle, "{DEBFA0B8-DB6F-4648-AF2A-2A4295A02338}"_drx_guid, "GetThrottleRatio");
				pFunction->SetDescription("Gets the current pedal ratio");
				pFunction->BindOutput(0, 'rati', "Ratio");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::SetBrake, "{06D60C2A-48C2-4591-8F5E-EC33EBAB7CBA}"_drx_guid, "SetBrakeRatio");
				pFunction->SetDescription("Sets the brake ratio, 0 - 1 (0 being no brake, 1 fully held down)");
				pFunction->BindInput(1, 'rati', "Ratio");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetBrake, "{2CE6F62B-EFD7-415E-8D2A-6EBE54B3E38A}"_drx_guid, "GetBrakeRatio");
				pFunction->SetDescription("Gets the current brke ratio");
				pFunction->BindOutput(0, 'rati', "Ratio");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::SetCurrentGear, "{3031C561-42DC-4405-90A7-9DF6B965C3A2}"_drx_guid, "SetCurrentGear");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetCurrentGear, "{5878EC1E-0B95-4765-83ED-3336BF1EF4CB}"_drx_guid, "GetCurrentGear");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GearUp, "{CE4608AB-1B1A-4525-98D7-FF29301CBD7F}"_drx_guid, "GearUp");
				pFunction->SetDescription("Switched to the next gear");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GearDown, "{DE79FA1F-A7AB-4119-B2EF-A691A27EB1E3}"_drx_guid, "GearDown");
				pFunction->SetDescription("Switched to the previous gear");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::SetSteeringAngle, "{105A1735-86D9-4B54-B7DA-D8D22DC52189}"_drx_guid, "SetSteeringAngle");
				pFunction->SetDescription("Sets the current steering angle");
				pFunction->BindInput(1, 'stee', "SteerAngle");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetSteeringAngle, "{0CC76A47-93EA-4890-AAA4-05CEB1DC5CE5}"_drx_guid, "GetSteeringAngle");
				pFunction->SetDescription("Gets the current steering angle");
				pFunction->BindOutput(0, 'stee', "SteerAngle");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetEngineRPM, "{1FCC083E-4EE7-4F19-BED1-B71B74DE96DE}"_drx_guid, "GetEngineRPM");
				pFunction->SetDescription("Gets the engine's current rotation speed");
				pFunction->BindOutput(0, 'rots', "RotationSpeed");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetTorque, "{FBAA748C-FD06-4E29-9585-3B5DEC4826DC}"_drx_guid, "GetTorque");
				pFunction->SetDescription("Gets the driving torque");
				pFunction->BindOutput(0, 'torq', "Torque");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::HasWheelContact, "{C3DC40DD-6D25-4A17-8182-DC68E0697C40}"_drx_guid, "HasWheelContact");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::SetVelocity, "{09CAC615-3D68-44D2-A527-2530C40B7FF0}}"_drx_guid, "SetVelocity");
				pFunction->SetDescription("Set Entity Velocity");
				pFunction->BindInput(1, 'vel', "velocity");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetVelocity, "{31463178-71C0-48AC-8A8B-3DEF8DF36A68}"_drx_guid, "GetVelocity");
				pFunction->SetDescription("Get Entity Velocity");
				pFunction->BindOutput(0, 'vel', "velocity");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::SetAngularVelocity, "{F999C8FB-366F-4F0B-9BF0-CACB744DBF8C}"_drx_guid, "SetAngularVelocity");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::GetAngularVelocity, "{1652D5A3-23D0-47E0-B224-112123272680}"_drx_guid, "GetAngularVelocity");
				pFunction->SetDescription("Get Entity Angular Velocity");
				pFunction->BindOutput(0, 'vel', "angular velocity");
				componentScope.Register(pFunction);
			}
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::ApplyImpulse, "{EEAA0C2C-1B26-4DC4-A649-1B70EC4FBD9A}"_drx_guid, "ApplyImpulse");
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CVehiclePhysicsComponent::ApplyAngularImpulse, "{D869D9B5-0FDC-4942-B485-0622D1CA7156}"_drx_guid, "ApplyAngularImpulse");
		...
		{
			desc.SetGUID("{5E5E88FF-C466-4579-9F74-4E59DB3B2A47}"_drx_guid);
		...
		void CVehiclePhysicsComponent::ProcessEvent(const SEntityEvent& event)
		{
			if (event.event == ENTITY_EVENT_COLLISION)
			{
				...
				if (ISurfaceType* pSurfaceType = pSurfaceTypeUpr->GetSurfaceType(physCollision->idmat[1]))
				{
