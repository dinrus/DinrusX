		...
		namespace VirtualReality
		{
			...
			void CInteractionComponent::Initialize()
			{
				if (IHmdDevice* pDevice = gEnv->pSystem->GetHmdUpr()->GetHmdDevice())
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Unsupported HMD device encountered in interaction component!");
						return;
					}
			...
			void CInteractionComponent::LoadHandModel(const char* szModelPath, EHand handType)
			{
				...
				if (IStatObj* pHandObject = m_pEntity->GetStatObj(hand.slotId))
				{
					...
					if (IIndexedMesh* pIndexedMesh = pHandObject->GetIndexedMesh(true))
					{
						...
						hand.pPhysicsGeometry = gEnv->pPhysicalWorld->GetGeomUpr()->CreateMesh(
			...
			void CInteractionComponent::ProcessEvent(const SEntityEvent& event)
			{
				...
				{
					IHmdDevice* pDevice = gEnv->pSystem->GetHmdUpr()->GetHmdDevice();
					if (pDevice == nullptr)
						return;
