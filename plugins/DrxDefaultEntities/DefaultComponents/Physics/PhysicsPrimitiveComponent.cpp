	...
	namespace DefaultComponents
	{
		...
		void CPhysicsPrimitiveComponent::AddPrimitive()
		{
			if (IPhysicalEntity* pPhysicalEntity = m_pEntity->GetPhysicalEntity())
			{
				...
				phys_geometry* pPhysGeom = gEnv->pPhysicalWorld->GetGeomUpr()->RegisterGeometry(pPrimGeom, 0);
				...
				if (m_surfaceTypeName.value.size() > 0)
				{
					...
					if (ISurfaceType* pSurfaceType = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeByName(surfaceType))
					{
						pGeomParams->surface_idx = pSurfaceType->GetId();
					}
					else
					{
						DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Invalid surface type %s specified on primitive component in entity %s!", m_surfaceTypeName.value.c_str(), m_pEntity->GetName());
					}
				}
