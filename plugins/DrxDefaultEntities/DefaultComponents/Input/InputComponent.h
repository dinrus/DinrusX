	...
	namespace DefaultComponents
	{
		...
		public:
			...
			virtual void RegisterAction(const char* szGroupName, const char* szName, TActionCallback callback)
			{
				...
				IActionMap* pActionMap = pActionMapUpr->GetActionMap(szGroupName);
				if (pActionMap == nullptr)
				{
					pActionMap = pActionMapUpr->CreateActionMap(szGroupName);
			...
			{
				desc.SetGUID("{183E3C84-2E08-4506-95FA-B362F7E735E9}"_drx_guid);
		...
		protected:
			struct SGroup final : public IActionListener
			{
					...
					, m_actions(other.m_actions)
				{
					if (m_actions.size() > 0)
					{
						gEnv->pGameFramework->GetIActionMapUpr()->AddExtraActionListener(this, m_szName);
					}
				}
				...
				virtual ~SGroup()
				{
					if (m_actions.size() > 0)
					{
						gEnv->pGameFramework->GetIActionMapUpr()->RemoveExtraActionListener(this, m_szName);
					}
				}
