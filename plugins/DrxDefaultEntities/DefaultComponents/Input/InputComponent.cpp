...
{
	desc.SetGUID("{91C5E76B-8DB1-409B-8326-DA99E2CFF6E9}"_drx_guid);
...
{
	desc.SetGUID("{C16D3CD2-F2D0-4BB8-BE16-FBDF09A308E7}"_drx_guid);
...
{
	desc.SetGUID("{2FAB7843-AAF8-4573-ABC0-6190C5C5900A}"_drx_guid);
...
{
	desc.SetGUID("{B09F6A7D-6AFD-48F6-B589-A25AD326CD0E}"_drx_guid);
	desc.SetLabel("Key Id");
	desc.SetDescription("Input Key Identifier");
	desc.SetFlags(Schematyc::ETypeFlags::Switchable);
...
{
	desc.SetGUID("{8C26C065-8A61-4B89-93FA-9C102C225BBA}"_drx_guid);
	desc.SetLabel("Key Id");
	desc.SetDescription("Input Key Identifier");
	desc.SetFlags(Schematyc::ETypeFlags::Switchable);
...
{
	desc.SetGUID("{9AC5093B-737C-493E-8866-D116996EA6E4}"_drx_guid);
	desc.SetLabel("Key Id");
	desc.SetDescription("Input Key Identifier");
	desc.SetFlags(Schematyc::ETypeFlags::Switchable);
...
void CInputComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
{
	// Functions
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CInputComponent::BindKeyboardAction, "{0F22767B-6AA6-4EDF-92F0-7BCD10CAB00E}"_drx_guid, "RegisterKeyboardAction");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CInputComponent::BindMouseAction, "{64199BB3-88A5-4404-BAEF-212BC0D0ACCC}"_drx_guid, "RegisterMouseAction");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CInputComponent::BindXboxAction, "{5D8DDAD6-AEFD-4A1A-8BA0-F27F7836AC05}"_drx_guid, "RegisterXboxAction");
	...
	{
		auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CInputComponent::BindPS4Action, "{D7CE1BF1-F192-4E0D-B3D6-305B31E8C693}"_drx_guid, "RegisterPS4Action");
...
{
	desc.SetGUID("{603771B5-6321-4113-82FB-B839F1BE7F64}"_drx_guid);
...
{
	desc.SetGUID("{CB22A659-40AD-4CF0-AD07-42DCCEA24F44}"_drx_guid);
...
{
	desc.SetGUID("{ECDF0D8F-7E4B-4FA1-AEFA-51DAFD382CE9}"_drx_guid);
...
{
	IActionMapUpr *pActionMapUpr = gEnv->pGameFramework->GetIActionMapUpr();

	pActionMapUpr->ClearInputDevicesMappings();

	pActionMapUpr->AddInputDeviceMapping(eAID_KeyboardMouse, "keyboard");
	pActionMapUpr->AddInputDeviceMapping(eAID_XboxPad, "xboxpad");
	pActionMapUpr->AddInputDeviceMapping(eAID_PS4Pad, "ps4pad");
	pActionMapUpr->AddInputDeviceMapping(eAID_OculusTouch, "oculustouch");

	pActionMapUpr->SetVersion(0);
...
{
	DRX_ASSERT(m_pEntity->GetSchematycObject() != nullptr);
...
{
	IActionMapUpr *pActionMapUpr = gEnv->pGameFramework->GetIActionMapUpr();
	IActionMap* pActionMap = pActionMapUpr->GetActionMap(groupName.c_str());
	if (pActionMap == nullptr)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Tried to bind input component action %s in group %s without registering it first!", name.c_str(), groupName.c_str());
		return;
	}
	...
	pActionMapUpr->EnableActionMap(groupName.c_str(), true);
}
