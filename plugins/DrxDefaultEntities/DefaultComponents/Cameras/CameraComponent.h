...
namespace Drx
{
	namespace DefaultComponents
	{
		...
		{
			desc.SetGUID("{E4DC6A37-2FAE-4DFF-AB21-3C4308A266D6}"_drx_guid);
		...
		protected:
			...
			{
				m_pCameraUpr->AddCamera(this);

				m_pAudioListener = m_pEntity->GetOrCreateComponent<Drx::Audio::DefaultComponents::CListenerComponent>();
				DRX_ASSERT(m_pAudioListener != nullptr);
		...
		virtual void ProcessEvent(const SEntityEvent& event) override
		{
			...
			if (event.event == ENTITY_EVENT_UPDATE && !gEnv->IsEditing())
			{
				...
				if (IHmdDevice* pDevice = gEnv->pSystem->GetHmdUpr()->GetHmdDevice())
				{
			...
			{
				m_pCameraUpr->RemoveCamera(this);
			}
			// ~IEntityComponent
		...
		public:
			CCameraComponent()
			{
				m_pCameraUpr = gEnv->pSystem->GetIPluginUpr()->QueryPlugin<IPlugin_DrxDefaultEntities>()->GetICameraUpr();
			}
			...
			{
				desc.SetGUID("{A4CB0508-5F07-46B4-B6D4-AB76BFD550F4}"_drx_guid);
			...
			virtual void Activate()
			{
				...
				m_pCameraUpr->SwitchCameraToActive(this);
			}

			bool IsActive() const
			{
				return m_pCameraUpr->IsThisCameraActive(this);
			}
			...
			virtual void SetFieldOfView(DrxTransform::CAngle angle) { m_fieldOfView = angle; }
			DrxTransform::CAngle GetFieldOfView() const { return m_fieldOfView; }
		...
		protected:
			...
			DrxTransform::CClampedAngle<20, 360> m_fieldOfView = 70.0_degrees;

			ICameraUpr* m_pCameraUpr = nullptr;

			CCamera m_camera;
			Drx::Audio::DefaultComponents::CListenerComponent* m_pAudioListener = nullptr;
		};
	}
}