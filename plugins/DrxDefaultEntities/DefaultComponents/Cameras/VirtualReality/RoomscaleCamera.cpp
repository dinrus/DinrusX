	...
	namespace DefaultComponents
	{
		namespace VirtualReality
		{
			void CRoomscaleCameraComponent::Register(Schematyc::CEnvRegistrationScope& componentScope)
			{
				// Functions
				{
					auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CRoomscaleCameraComponent::Activate, "{4A595550-254B-4A5F-BFF6-7A08AAC317CE}"_drx_guid, "Activate");
					pFunction->SetDescription("Makes this camera active");
					pFunction->SetFlags(Schematyc::EEnvFunctionFlags::Construction);
					componentScope.Register(pFunction);
				}
				{
					auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CRoomscaleCameraComponent::IsActive, "{4C1647E1-49C0-4D48-A7E0-B71B7EB1DE60}"_drx_guid, "IsActive");
			...
			void CRoomscaleCameraComponent::ProcessEvent(const SEntityEvent& event)
			{
				if (event.event == ENTITY_EVENT_UPDATE)
				{
					...
					{
						if (IHmdDevice* pDevice = pHmdUpr->GetHmdDevice())
						{
							float arf_notUsed;
