...
<DinrusX/DinrusX3dEng/IIndexedMesh.h>
<DinrusX/CoreX/Renderer/IRenderAuxGeom.h>
<DinrusX/CoreX/Math/Drx_Color.h>
	...
	, m_predictedRenderPoseTimestamp(0)
{
	...
	if (GetISystem()->GetISystemEventDispatcher())
		GetISystem()->GetISystemEventDispatcher()->RegisterListener(this, "DrxVR::OpenVR::Device");
...
void Device::CreateDevice()
{
	...
	// the following is (hopefully just) a (temporary) hack to shift focus back to the DinrusX window, after (potentially) spawning the SteamVR Compositor
	if (!gEnv->IsEditor())
	{
...
void Device::SubmitOverlay(int id, const RenderLayer::CProperties* pOverlayProperties)
{
	...
	DRX_ASSERT(pOverlayProperties->IsActive());
...
void Device::OnSetupEyeTargets(ERenderAPI api, ERenderColorSpace colorSpace, void* leftEyeHandle, void* rightEyeHandle)
{
	...
	{
	default:
	case DrxVR::OpenVR::ERenderColorSpace::eRenderColorSpace_Auto:
		...
		break;
	case DrxVR::OpenVR::ERenderColorSpace::eRenderColorSpace_Gamma:
		...
		break;
	case DrxVR::OpenVR::ERenderColorSpace::eRenderColorSpace_Linear:
...
void Device::OnSetupOverlay(int id, ERenderAPI api, ERenderColorSpace colorSpace, void* overlayTextureHandle, const QuatTS &pose, bool absolute, bool highestQuality)
{
	...
	char keyName[64];
	drx_sprintf(keyName, 64, "drx.overlay_%d", id);

	char overlayName[64];
	drx_sprintf(overlayName, 64, "drxOverlay_%d", id);
