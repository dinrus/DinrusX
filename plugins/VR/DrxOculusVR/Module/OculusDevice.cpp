...
<DinrusX/CoreX/Renderer/IStereoRenderer.h>
#include "Core/Renderer/IRenderAuxGeom.h"

<DinrusX/d3d11.h>
<DinrusX/d3d12.h>

<DinrusX/Extras/OVR_StereoProjection.h>

namespace
{
inline DrxVR::Oculus::OculusStatus CheckOvrResult(ovrResult result) 
{
	if (OVR_SUCCESS(result))
		return DrxVR::Oculus::OculusStatus::Success;
	...
	if (result == ovrError_DisplayLost)
		return DrxVR::Oculus::OculusStatus::DeviceLost;
	return DrxVR::Oculus::OculusStatus::UnknownError;
}
...
ovrFovPort ComputeSymmetricalFov(ovrHmdType hmdType, const ovrFovPort& fovLeftEye, const ovrFovPort& fovRightEye)
{
	...
	DrxLog("[OculusDevice] Aspect Ratio selected: %f", stereoDeviceAR);
	ovrFovPort fovMax;
	...
	DrxLog("[OculusDevice] Fov: Up/Down tans [%f] Left/Right tans [%f]", fovSym.UpTan, fovSym.LeftTan);
	return fovSym;
}
...
void LogHmdCharacteristics(const ovrHmdDesc& hmdDesc, const HmdDeviceInfo& devInfo, const DrxVR::Oculus::COculusController& controller)
{
	DrxLogAlways("[HMD][Oculus] - Device: %s - Type:%u", hmdDesc.ProductName ? hmdDesc.ProductName : "unknown", hmdDesc.Type);
	DrxLogAlways("[HMD][Oculus] --- Manufacturer: %s", hmdDesc.Manufacturer ? hmdDesc.Manufacturer : "unknown");
	DrxLogAlways("[HMD][Oculus] --- VendorId: %d", hmdDesc.VendorId);
	DrxLogAlways("[HMD][Oculus] --- ProductId: %d", hmdDesc.ProductId);
	DrxLogAlways("[HMD][Oculus] --- Firmware: %d.%d", hmdDesc.FirmwareMajor, hmdDesc.FirmwareMinor);
	DrxLogAlways("[HMD][Oculus] --- SerialNumber: %s", hmdDesc.SerialNumber);
	DrxLogAlways("[HMD][Oculus] - Resolution: %dx%d", hmdDesc.Resolution.w, hmdDesc.Resolution.h);
	DrxLogAlways("[HMD][Oculus] - Horizontal FOV: %.2f degrees", devInfo.fovH * 180.0f / gf_PI);
	DrxLogAlways("[HMD][Oculus] - Vertical FOV: %.2f degrees", devInfo.fovV * 180.0f / gf_PI);

	DrxLogAlways("-[HMD][Oculus] Low persistence mode: Enabled by default");
	DrxLogAlways("-[HMD][Oculus] Dynamic prediction: Enabled by default (for DK2 and later)");

	DrxLogAlways("-[HMD][Oculus] Sensor orientation tracking: %s", (hmdDesc.AvailableTrackingCaps & ovrTrackingCap_Orientation) ? "supported" : "unsupported");
	DrxLogAlways("-[HMD][Oculus] Sensor mag yaw correction: %s", (hmdDesc.AvailableTrackingCaps & ovrTrackingCap_MagYawCorrection) ? "supported" : "unsupported");
	DrxLogAlways("-[HMD][Oculus] Sensor position tracking: %s", (hmdDesc.AvailableTrackingCaps & ovrTrackingCap_Position) ? "supported" : "unsupported");

	DrxLogAlways("-[HMD][Oculus] Controller - Left Hand : %s", controller.IsConnected(eHmdController_OculusLeftHand) ? "Connected" : "Disconnected");
	DrxLogAlways("-[HMD][Oculus] Controller - Right Hand: %s", controller.IsConnected(eHmdController_OculusRightHand) ? "Connected" : "Disconnected");
}
...
{
	CreateDevice();
	gEnv->pSystem->GetHmdUpr()->AddEventListener(this);
}

// -------------------------------------------------------------------------
Device::~Device()
{
	gEnv->pSystem->GetHmdUpr()->RemoveEventListener(this);
...
void Device::CreateDevice()
{
	...
	if (m_pSession && OVR_SUCCESS(result))
	{
		...
		if (m_hmdDesc.Type >= ovrHmd_ES06 || m_hmdDesc.Type == ovrHmd_Other)
		{
			...
			DrxLogAlways("[HMD][Oculus] Touch Controllers initialized.");
		}
		else
		{
			DrxLogAlways("[HMD][Oculus] Touch Controllers were not initialized because the detected HMD does not support them. Device %d", m_hmdDesc.Type);
		}
	...
	else
	{
		ovrErrorInfo errorInfo;
		ovr_GetLastErrorInfo(&errorInfo);
		DrxLogAlways("[HMD][Oculus] Failed to initialize HMD device! [%s]", *errorInfo.ErrorString ? errorInfo.ErrorString : "unspecified error");
	}

	if (m_pSession && GetISystem()->GetISystemEventDispatcher())
		GetISystem()->GetISystemEventDispatcher()->RegisterListener(this, "DrxVR::Oculus::Device");
}
...
void Device::GetCameraSetupInfo(float& fov, float& aspectRatioFactor) const
{
	...
	{
		DrxLog("[HMD][Oculus] GetCameraSetupInfo returns invalid information since the HMD was not created");
	...
	if (m_pSession)
	{
		...
		if (m_perf_hud_info != CPlugin_OculusVR::s_hmd_perf_hud)
		{
			if (CPlugin_OculusVR::s_hmd_perf_hud >= 0 && CPlugin_OculusVR::s_hmd_perf_hud < ovrPerfHud_Count)
			{
				...
				{
					DrxLog("[HMD][Oculus] OVR_PERF_HUD_MODE changed to %d", m_perf_hud_info);
				}
				else
				{
					DrxLog("[HMD][Oculus] OVR_PERF_HUD_MODE failed to change to %d", m_perf_hud_info);
				}
			}
			else
			{
				m_perf_hud_info = CPlugin_OculusVR::s_hmd_perf_hud = ovrPerfHud_Off;
				DrxLog("[HMD][Oculus] OVR_PERF_HUD_MODE passed a wrong value (it should be be between 0 and %d, see help). Setting the value to %d (off)", ovrPerfHud_Count, ovrPerfHud_Off);
			}
		}
		...
		if (m_queueAhead != cvarQueueAhead)
		{
			...
			{
				DrxLog("[HMD][Oculus] QueueAheadEnabled changed to %s", m_queueAhead ? "TRUE" : "FALSE");
			}
			else
			{
				DrxLog("[HMD][Oculus] QueueAheadEnabled changed failed trying to set the value to %s", m_queueAhead ? "TRUE" : "FALSE");
			}
		}
		...
		{
			if (((type & eVRComponent_Hmd) != 0) && (DrxGetCurrentThreadId() != gEnv->mMainThreadId) && (m_bLoadingScreenActive == false))
			{
...
{
	DRX_PROFILE_REGION(PROFILE_SYSTEM, "OculusDevice::BeginFrame()");
	...
	return waitResult == DrxVR::Oculus::OculusStatus::Success ? beginResult : waitResult;
}
...
OculusStatus Device::SubmitFrame(const SHmdSubmitFrameData &data)
{
	...
	{
		DRX_PROFILE_REGION(PROFILE_SYSTEM, "OculusDevice::ovr_EndFrame");
