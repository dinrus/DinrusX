...
<DinrusX/Osvr/ClientKit/InterfaceC.h>
<DinrusX/Osvr/ClientKit/InterfaceCallbackC.h>
<DinrusX/Osvr/ClientKit/ParametersC.h>
...
<DinrusX/CoreX/Math/Drx_Camera.h>
...
namespace Osvr
{
static const char* s_szClientName = "com.drxtek.drxengine";
...
void OsvrQuatToDrxQuat(const OSVR_Quaternion& quat, Quat& out)
{
...
void FixRenderUprOrientation(Quat& orientation)
{
	orientation.v = -orientation.v;
}

void OsvrVec3ToDrxVec3(const OSVR_Vec3& vec, Vec3& out)
{
...
{
	Vec3 l, r;
	OsvrVec3ToDrxVec3(left.translation, l);
	OsvrVec3ToDrxVec3(right.translation, r);
	...
	m_loadingScreenActive(false)
{
	...
	{
		GetISystem()->GetISystemEventDispatcher()->RegisterListener(this, "DrxVR::Osvr::Device");
	}
...
void Device::UpdateTrackingState(EVRComponent e, uint64_t frameId)
{
	if ((e & eVRComponent_Hmd) != 0)
	{
		...
		if (!gEnv->IsEditor())
		{
			if (((e & eVRComponent_Hmd) != 0) && (DrxGetCurrentThreadId() != gEnv->mMainThreadId) && (m_loadingScreenActive == false))
			{
...
void Device::UpdatePoseFromRenderInfo()
{
	if (m_latestRenderInfo.size() > 0)
	{
		...
		if (m_latestRenderInfo.size() == 1)
		{
			...
			OsvrQuatToDrxQuat(m_latestRenderInfo[0].pose.rotation, orientation);
			OsvrVec3ToDrxVec3(m_latestRenderInfo[0].pose.translation, translation);
		}
		else
		{
			Vec3 l, r;
			OsvrQuatToDrxQuat(m_latestRenderInfo[0].pose.rotation, orientation);
			OsvrVec3ToDrxVec3(m_latestRenderInfo[0].pose.translation, l);
			OsvrVec3ToDrxVec3(m_latestRenderInfo[1].pose.translation, r);
			translation = Vec3::CreateLerp(l, r, 0.5f);
		}

		FixRenderUprOrientation(orientation);
...
void Device::UpdatePoseFromOsvrPose(const OSVR_PoseState& newPose)
{
	Quat o;
	Vec3 t;
	OsvrQuatToDrxQuat(newPose.rotation, o);
	OsvrVec3ToDrxVec3(newPose.translation, t);
	UpdatePose(o, t);
}
...
{
	if (m_renderUpr && m_renderUprD3D11)
	{
		float nearPlane = GetISystem()->GetViewCamera().GetNearPlane();
		float farPlane = GetISystem()->GetViewCamera().GetFarPlane();
		...
		osvrRenderUprGetNumRenderInfo(m_renderUpr, m_renderParams, &count);
		...
		for (OSVR_RenderInfoCount i = 0; i < count; ++i)
		{
			...
			osvrRenderUprGetRenderInfoD3D11(m_renderUpr, i, m_renderParams, &info);
...
bool Device::InitializeRenderer(void* d3dDevice, void* d3dContext)
{
	...
	bool bSuccess = osvrCreateRenderUprD3D11(m_context, "Direct3D11", library, &m_renderUpr, &m_renderUprD3D11) == OSVR_RETURN_SUCCESS;
	if (bSuccess && m_renderUpr && m_renderUprD3D11)
	{
		OSVR_OpenResultsD3D11 res;
		bSuccess = osvrRenderUprOpenDisplayD3D11(m_renderUprD3D11, &res) == OSVR_RETURN_SUCCESS;
	}
	if (bSuccess)
	{
		bSuccess = m_renderUpr && osvrRenderUprGetDoingOkay(m_renderUpr) == OSVR_RETURN_SUCCESS;
	}

	if (bSuccess)
	{
		osvrRenderUprGetDefaultRenderParams(&m_renderParams);
	...
	if (!bSuccess)
	{
		DrxLogAlways("[OsvrDevice] Error: Failed to create rendermanager! Are you using a whitelisted GPU?");
	}
...
bool Device::PresentTextureSet(int textureSetIndex)
{
	...
	{
		DrxLogAlways("[OsvrDevice] Error: render info not populated properly before present called!");
		return false;
	}

	OSVR_RenderUprPresentState presentBufState;
	osvrRenderUprStartPresentRenderBuffers(&presentBufState);
	...
	{
		osvrRenderUprPresentRenderBufferD3D11(presentBufState, renderBuffers[i], m_latestRenderInfo[i], viewPort);
	}

	return (OSVR_RETURN_SUCCESS == osvrRenderUprFinishPresentRenderBuffers(m_renderUpr, presentBufState, m_renderParams, OSVR_FALSE));
}

bool Device::RegisterTextureSwapSet(TextureSwapSet* swapSet)
{
	OSVR_RenderUprRegisterBufferState regBufState;
	osvrRenderUprStartRegisterRenderBuffers(&regBufState);
	...
	for (int i = 0; i < swapSet->numTextureSets; ++i)
	{
		...
		for (int texIndex = 0; texIndex < texCount; ++texIndex)
		{
			...
			bSuccess = (bSuccess && osvrRenderUprRegisterRenderBufferD3D11(regBufState, renderBuffer) == OSVR_RETURN_SUCCESS);
		}
	}
	...
	return (bSuccess && osvrRenderUprFinishRegisterRenderBuffers(m_renderUprD3D11, regBufState, OSVR_FALSE) == OSVR_RETURN_SUCCESS);
}
...
{
	osvrDestroyRenderUpr(m_renderUpr);
	m_renderUpr = 0;
	m_renderUprD3D11 = 0;
}
}
