	...
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			void CSharedRemoteFile::Download(int downloadPriority)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "Steam remote storage service not available");
					return;
				}
			...
			void CSharedRemoteFile::OnDownloaded(RemoteStorageDownloadUGCResult_t* pResult, bool bIOFailure)
			{
				...
				if (result == 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Failed to read shared file!");
					return;
				}
