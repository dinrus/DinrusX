		...
		namespace Steam
		{
			...
			void CUserGeneratedContent::OnContentUpdated(SubmitItemUpdateResult_t* pResult, bool bIOError)
			{
				switch (pResult->m_eResult)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] File not found!");
					}
					break;
					default:
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Failed to update UGC asset!");
					}
					break;
				}
