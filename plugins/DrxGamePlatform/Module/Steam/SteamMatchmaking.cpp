...
namespace Drx
{
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			{
				DrxFixedStringT<32> versionString = gEnv->pSystem->GetProductVersion().ToString();

				// Always query lobbies for this version only
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] [Lobby] Looking for lobby with version: %s", versionString.c_str());
#ifdef RELEASE
			...
			void CMatchmaking::OnCreateLobby(LobbyCreated_t* pResult, bool bIOFailure)
			{
				if (pResult->m_eResult == k_EResultOK)
				{
					...
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] [Lobby] Creating lobby with game version: %s", versionString.c_str());
