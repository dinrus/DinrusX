...
<DinrusX/DinrusXSys/ICmdLine.h>
<DinrusX/DinrusXSys/ILocalizationUpr.h>
	...
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			extern "C" void __cdecl SteamAPIDebugTextHook(int nSeverity, const char* pchDebugText)
			{
				...
				DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] %s", pchDebugText);
			}
			...
			bool CPlugin::Initialize(SSystemGlobalEnvironment& env, const SSystemInitParams& initParams)
			{
...
#if !defined(RELEASE)
				char buff[MAX_PATH];
				DrxGetExecutableFolder(DRX_ARRAY_COUNT(buff), buff);
				...
				if (!m_isInitialized)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[Steam] SteamApi_Init failed");
				...
				{
					DrxLogAlways("[Steam] Running Steam game beta %s", betaName);
				}

				DrxLogAlways("[Steam] Successfully initialized Steam API, running build id %i", pSteamApps->GetAppBuildId());

				// Check if user requested to join a lobby right away
				if (const ICmdLineArg* pCmdArg = pCmdLine->FindArg(eCLAT_Post, "connect_lobby"))
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Received pre-launch join request, loading lobby");
				...
				{
					DrxLogAlways("Attempting to automatically set user preferred language %s", szDesiredLanguage);

					for (int i = 0; i < ILocalizationUpr::ePILID_MAX_OR_INVALID; i++)
					{
						const char* languageName = pLocalizationUpr->LangNameFromPILID((ILocalizationUpr::EPlatformIndependentLanguageID)i);

						if (!stricmp(languageName, szDesiredLanguage))
						{
							if (pLocalizationUpr->IsLanguageSupported((ILocalizationUpr::EPlatformIndependentLanguageID)i))
							{
			...
			bool CPlugin::GetAuthToken(string &tokenOut, int &issuerId)
			{
				...
				if (!pSteamUser)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "Steam user service not available");
					return false;
				}
