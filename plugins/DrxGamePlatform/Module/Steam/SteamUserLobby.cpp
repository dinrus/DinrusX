...
<DinrusX/../DinrusXAct/IActorSystem.h>
		...
		namespace Steam
		{
			...
			void CUserLobby::OnSystemEvent(ESystemEvent event, UINT_PTR wparam, UINT_PTR lparam)
			{
				...
				if (gEnv->bMultiplayer)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[Steam] Failed to create server!");
						return;
					}
					...
					drx_sprintf(ipAddressFinal, 15, "%d.%d.%d.%d", octet[3], octet[2], octet[1], octet[0]);

					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Created server at %s:%i", ipAddressFinal, m_serverPort);
					...
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "Steam matchmaking service not available");
					}
			...
			bool CUserLobby::HostServer(const char* szLevel, bool isLocal)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "Failed to create server!");
					return false;
				}
			...
			void CUserLobby::ConnectToServer(uint32 ip, uint16 port, IServer::Identifier serverId)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Lobby game created, connecting to %s:%i", ipAddressFinal, m_serverPort);
