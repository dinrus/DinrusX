	...
	namespace GamePlatform
	{
		namespace Steam
		{
			...
			bool CRemoteFile::Write(const char* pData, int length)
			{
				if (m_name.length() == 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Attempted to write remote file with empty name");
					return false;
				}
				...
				if (ISteamRemoteStorage* pSteamRemoteStorage = SteamRemoteStorage())
				{
					...
					if (!bResult)
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Failed to write %i bytes to remote file %s", length, m_name.c_str());
			...
			bool CRemoteFile::Read(std::vector<char>& bufferOut)
			{
				if (m_name.length() == 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Attempted to read remote file with empty name");
					return false;
				}
