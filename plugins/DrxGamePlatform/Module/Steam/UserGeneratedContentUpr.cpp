...
namespace Drx
{
	namespace GamePlatform
	{
		namespace Steam
		{
			void CUserGeneratedContentUpr::Create(unsigned int appId, IUserGeneratedContent::EType type)
			{
				m_lastUsedId = appId;
			...
			void CUserGeneratedContentUpr::CreateDirect(unsigned int appId, IUserGeneratedContent::EType type,
				...
				const char* *pTags, int numTags, const char* contentFolderPath, const char* previewPath)
			{
				...
				if (ISteamUGC* pSteamUGC = SteamUGC())
				{
					...
					m_callResultContentCreated.Set(result, this, &CUserGeneratedContentUpr::OnContentCreated);

					CPlugin::GetInstance()->SetAwaitingCallback(1);
				}
			}

			void CUserGeneratedContentUpr::OnContentCreated(CreateItemResult_t* pResult, bool bIOError)
			{
				CPlugin::GetInstance()->SetAwaitingCallback(-1);
				...
				if (pResult->m_eResult == k_EResultOK)
				{
					...
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "[Steam] Content creation failed, result %i", pResult->m_eResult);
			}
		}
	}
}