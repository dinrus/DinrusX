		...
		namespace PSN
		{
			...
			CUser::~CUser()
			{
				...
				if (m_bUserLibraryOwner)
				{
					...
					if (returnCode < 0)
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to terminate user service");
					}
				}
			}
			...
			void CUser::SetSignedInState(ESignInStatus status)
			{
				...
				{
					m_bPSNInitialized = false;
					DrxMessageBox("User signed out!", "PSN Error", EMessageBox::eMB_Info);
				}
			...
			void CUser::InitPSN()
			{
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create asynchronous request");
					return;
				}
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to check Np availability");
					return;
				}
			...
			void CUser::Initialize()
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to initialize user service");
					return;
				}
				else
				{
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to set PSN state callback");
				}
			...
			void CUser::Update()
			{
				...
				if (m_authorizationCodeStatus == EAuthorizationCodeStatus::Invalid)
				{
					...
					if (returnCode < 0)
					{
						...
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to poll for authorization code!");
					}
				...
				if (m_npCheckAvailabilityRequestId != 0)
				{
					...
					if (returnCode == SCE_NP_AUTH_POLL_ASYNC_RET_FINISHED)
					{
						...
						case SCE_NP_ERROR_USER_NOT_FOUND:
							DrxMessageBox("User not found!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Unknown;
							bError = true;
							break;

						case SCE_NP_ERROR_AGE_RESTRICTION:
							DrxMessageBox("User does not meet the age limit!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Unknown;
							bError = true;
							break;

						case SCE_NP_ERROR_LOGOUT:
							DrxMessageBox("User was signed out!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Offline;
							bError = true;
							break;

						case SCE_NP_ERROR_LATEST_SYSTEM_SOFTWARE_EXIST:
							DrxMessageBox("New system update is available, please update!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Offline;
							bError = true;
							break;

						case SCE_NP_ERROR_LATEST_SYSTEM_SOFTWARE_EXIST_FOR_TITLE:
							DrxMessageBox("New system software is available for the application, please update!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Offline;
							bError = true;
							break;

						case SCE_NP_ERROR_LATEST_PATCH_PKG_EXIST:
							DrxMessageBox("New patch package is available, please update!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Offline;
							bError = true;
							break;

						case SCE_NP_ERROR_LATEST_PATCH_PKG_DOWNLOADED:
							DrxMessageBox("New patch package was downloaded, please update!", "PSN Error", EMessageBox::eMB_Info);
							m_PSNSignInStatus = ESignInStatus::Offline;
							bError = true;
							break;
						}
					...
					else if (returnCode < SCE_OK)
					{
						...
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get network availability");
					}
				}
			}
			...
			void CUser::GetIDsFromPSN()
			{
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get local user");
					return;
				}
				...
				if (returnCode < 0)
				{
					...
					case SCE_NP_ERROR_NOT_SIGNED_UP:
						...
						break;
					default:
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get Np Id for user");
						return;
					}
				}
				...
				if (returnCode < 0)
				{
					...
					case SCE_NP_ERROR_NOT_SIGNED_UP:
						...
						break;
					default:
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get online id for user");
						return;
					}
				}
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get account id for user");
					return;
				}
			...
			void CUser::RefreshAuthorizationCode()
			{
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get asynchronous auth token request");
					return;
				}
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Attempted to request authorization code without valid Np Client Id, please set the psn_npClientId CVar");
					return;
				}
				...
				drx_strcpy(clientId.id, m_pNpClientIdCVar->GetString(), SCE_NP_CLIENT_ID_MAX_LEN);
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get authorization code");
