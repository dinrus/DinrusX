	...
	namespace GamePlatform
	{
		namespace PSN
		{
			CStatistics::~CStatistics()
			{
				...
				if (retVal != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to destroy trophy context: %x\n", retVal);
				}
			}
			...
			void CStatistics::Initialize()
			{
				...
				if (retVal < SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to initialize Trophy module: %x\n", retVal);
					return;
				}
				...
				if (retVal != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create trophy context (Init): %x\n", retVal);
					return;
				}
				...
				if (retVal != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create trophy handle (Init): %x\n", retVal);
					return;
				}
				...
				if (retVal != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to register trophy context (Init): %x\n", retVal);
					return;
				}
				...
				if (retVal != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get trophy game info (Init): %x\n", retVal);
					return;
				}
				...
				if (retVal != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to destroy trophy handle (Init): %x\n", retVal);
					return;
				}
			...
			void CStatistics::ParseTrophies()
			{
				...
				if (retValue != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create trophy handle (ParseTrophies): %x\n", retValue);
				}
				...
				for (int i = 0; i < m_trophyGameDetails.numTrophies; i++)
				{
					...
					if (retValue != SCE_OK)
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get trophy info (ParseTrophies): %x\n", retValue);
					}
					else
					{
						...
						{
							DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Got a corrupt trophy with an unknown grade. (ParseTrophies)");
						}
					}
				}
				...
				if (retValue != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to destroy trophy handle (ParseTrophies): %x\n", retValue);
				}
			}
