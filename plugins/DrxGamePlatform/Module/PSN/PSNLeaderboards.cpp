	...
	namespace GamePlatform
	{
		namespace PSN
		{
			CLeaderboards::CLeaderboards()
			{
				...
				if (ret < SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to load ranking module");
					return;
				}
				...
				if (ret < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create score np title");
					return;
				}
			...
			void CLeaderboards::Update()
			{
				...
				for (auto it = m_leaderboardRequests.begin(); it != m_leaderboardRequests.end();)
				{
					...
					if (returnCode < 0 || numEntries < 0)
					{
						...
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to poll for leaderboard update!");
			...
			void CLeaderboards::DownloadEntries(Identifier leaderboardIdentifier, ERequest request, int minRange, int maxRange, bool bTimeBased)
			{
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create score request");
					return;
				}
				...
				if (ret < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to get ranged rankings for scoreboard");
			...
			void CLeaderboards::UpdateScore(Identifier leaderboardIdentifier, int score, EScoreType scoreType, bool bForce)
			{
				...
				if (ret < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to record score!");
