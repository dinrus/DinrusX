	...
	namespace GamePlatform
	{
		namespace PSN
		{
			...
			bool CAchievement::Achieve()
			{
				...
				if (retValue != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to create trophy handle: %x\n", retValue);
					return false;
				}
				...
				if (retValue != SCE_OK)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to unlock trophy: %x\n", retValue);
						return false;
					}
				}
				...
				if (retValue != SCE_OK)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[PSN] Failed to destroy trophy handle: %x\n", retValue);
				}
