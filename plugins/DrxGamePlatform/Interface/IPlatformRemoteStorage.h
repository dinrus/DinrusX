...
namespace Drx
{
	namespace GamePlatform
	{
		...
		struct IUserGeneratedContentUpr;
		...
		struct IRemoteFileBase
		{
			...
			bool ReadToFile(const char* szPath)
			{
				...
				FILE* f = gEnv->pDrxPak->FOpen(szPath, "wb");
				bool bSuccess = f != nullptr;

				if (bSuccess)
				{
					gEnv->pDrxPak->FWrite(data.data(), data.size(), f);
		...
		struct IRemoteFile : public IRemoteFileBase
		{
			...
			{
				FILE* f = gEnv->pDrxPak->FOpen(path, "rb");
				if (f == nullptr)
					return false;

				std::vector<char> data;
				data.resize(gEnv->pDrxPak->FGetSize(f));

				gEnv->pDrxPak->FRead(data.data(), data.size(), f);
				gEnv->pDrxPak->FClose(f);
		...
		struct IRemoteStorage
		{
			...
			virtual IUserGeneratedContentUpr* GetUserGeneratedContentUpr() const = 0;
