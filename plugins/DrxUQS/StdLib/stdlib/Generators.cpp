...
<DinrusX/DinrusXAI/INavigationSystem.h>
		...
		{
			{
				...
				ctorParams.guid = "498bce51-a2b9-4e77-b0f9-e127e8a5cc72"_drx_guid;
...

			{
				...
				ctorParams.guid = "70b13c46-e012-4fde-bba5-70ccd4f8b321"_drx_guid;
				ctorParams.szDescription =
...

			{
				...
				ctorParams.guid = "91486c28-ff91-4bf2-86e4-7b0c63e30e54"_drx_guid;
				ctorParams.szDescription =
		...
		Client::IGenerator::EUpdateStatus CGenerator_PointsOnNavMesh::DoUpdate(const SUpdateContext& updateContext, Client::CItemListProxy_Writable<Pos3>& itemListToPopulate)
		{
			...
			if (const NavigationMeshID meshID = pNavigationSystem->GetEnclosingMeshID(m_params.navigationAgentTypeID, m_params.pivot.value))
			{
				...
				if (numTrianglesFound > 0)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_UNKNOWN, VALIDATOR_WARNING, "UQS: CGenerator_PointsOnNavMesh::DoUpdate: reached the triangle limit of %i (there might be more triangles that we cannot use)", (int)maxTrianglesToLookFor);
					}
					...
					Core::IHubPlugin::GetHub().GetQueryUpr().AddItemMonitorToQuery(updateContext.queryID, std::move(pItemMonitorNavMeshChanges));
				}
			}
