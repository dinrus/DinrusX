	...
	namespace Client
	{
		...
		namespace Internal
		{
			...
			public:
				...
				virtual const DrxGUID&                     GetGUID() const override final;
			...
			protected:
				explicit                                   CItemFactoryBase(const char* szName, const DrxGUID& guid, const char* szDescription, bool bIsContainerForShuttledItems);
			...
			inline CItemFactoryBase::CItemFactoryBase(const char* szName, const DrxGUID& guid, const char* szDescription, bool bIsContainerForShuttledItems)
			...
			inline const DrxGUID& CItemFactoryBase::GetGUID() const
			{
			...
			public:
				...
				explicit                                  CItemFactoryInternal(const char* szName, const DrxGUID& guid, const char* szDescription, const SItemFactoryCallbacks<TItem>& callbacks, bool bAutoRegisterBuiltinElements);
													      ~CItemFactoryInternal();
			...
			template <class TItem>
			CItemFactoryInternal<TItem>::CItemFactoryInternal(const char* szName, const DrxGUID& guid, const char* szDescription, const SItemFactoryCallbacks<TItem>& callbacks, bool bAutoRegisterBuiltinElements)
			...
			void* CItemFactoryInternal<TItem>::CreateItems(size_t numItems, EItemInitMode itemInitMode)
			{
				...
				case IItemFactory::EItemInitMode::UseUserProvidedFunction:
					{
						DRX_ASSERT(m_callbacks.pCreateDefaultObject != nullptr);
		...
		public:
			...
			struct SCtorParams
			{
				...
				DrxGUID                       guid = DrxGUID::Null();                                          // GUID to uniquely identify the item factory; this should never change!
		...
		private:
			...
			static void RegisterShuttledItemsContainer(const char* szName, const DrxGUID& originalGUID)
			{
				//
				...
				const DrxGUID newGUID = DrxGUID::Construct(Internal::SPartialGUIDs::ItemType_ShuttledItemsContainer(), originalGUID.lopart);
