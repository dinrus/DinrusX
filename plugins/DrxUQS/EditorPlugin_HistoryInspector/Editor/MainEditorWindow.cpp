...
<DinrusX/QLayout>
<DinrusX/QVBoxLayout>
<DinrusX/QAbstractButton>
<DinrusX/QDialogButtonbox>
<DinrusX/QLineEdit>
<DinrusX/QAdvancedTreeView.h>
<DinrusX/QSplitter>
<DinrusX/QMessageBox>
<DinrusX/QFontMetrics>

<DinrusX/QtUtil.h>

<DinrusX/Serialization/QPropertyTree/QPropertyTree.h>
<DinrusX/Controls/QuestionDialog.h>
<DinrusX/EditorFramework/Events.h>

// UQS 3d rendering
<DinrusX/IViewportUpr.h>
<DinrusX/SandboxAPI.h>	// SANDBOX_API
...
struct SQuery
{
	...
	UQS::Core::IQueryHistoryUpr::SEvaluatorDrawMasks evaluatorDrawMasks;
	...
	SQuery() : pParent(nullptr), queryID(UQS::Core::CQueryID::CreateInvalid()), evaluatorDrawMasks(UQS::Core::IQueryHistoryUpr::SEvaluatorDrawMasks::CreateAllBitsSet()) {}
	SQuery(SQuery* _pParent, const UQS::Core::IQueryHistoryConsumer::SHistoricQueryOverview& overview)
		...
		, evaluatorDrawMasks(UQS::Core::IQueryHistoryUpr::SEvaluatorDrawMasks::CreateAllBitsSet())
	{
...
protected:
	virtual void currentChanged(const QModelIndex& current, const QModelIndex& previous) override
	{
		...
		UQS::Core::IQueryHistoryUpr* pHistoryQueryUpr = GetHistoryQueryUpr();
		...
		if (current.isValid())
		{
			...
			const UQS::Core::IQueryHistoryUpr::EHistoryOrigin whichHistory = pHistoryQueryUpr->GetCurrentQueryHistory();
			pHistoryQueryUpr->MakeHistoricQueryCurrentForInWorldRendering(whichHistory, pQueryToSwitchTo->queryID);
		}
	}
	...
	virtual void mouseDoubleClickEvent(QMouseEvent *event) override
	{
		...
		{
			if (UQS::Core::IQueryHistoryUpr* pQueryHistoryUpr = GetHistoryQueryUpr())
			{
				...
				cameraView = pQueryHistoryUpr->GetIdealDebugCameraView(
					pQueryHistoryUpr->GetCurrentQueryHistory(),
					pQueryHistoryUpr->GetCurrentHistoricQueryForInWorldRendering(pQueryHistoryUpr->GetCurrentQueryHistory()),
					cameraView);
...
private:
	UQS::Core::IQueryHistoryUpr* GetHistoryQueryUpr()
	{
		if (UQS::Core::IHub* pHub = UQS::Core::IHubPlugin::GetHubPtr())
		{
			return &pHub->GetQueryHistoryUpr();
		}
		else
		{
			return nullptr;
...
void CMainEditorWindow::CUQSHistoryPostRenderer::OnPostRender() const
{
	if (UQS::Core::IHub* pHub = UQS::Core::IHubPlugin::GetHubPtr())
	{
		if (const CViewport* pGameViewport = GetIEditor()->GetViewportUpr()->GetGameViewport())
		{
			...
			UQS::Core::IQueryHistoryUpr::SEvaluatorDrawMasks evaluatorDrawMasks = UQS::Core::IQueryHistoryUpr::SEvaluatorDrawMasks::CreateAllBitsSet();
			...
			pHub->GetQueryHistoryUpr().UpdateDebugRendering3D(&uqsCameraView, evaluatorDrawMasks);
		}
	}
}
	...
	, m_pHistoryPostRenderer(nullptr)
{
	...
	m_pComboBoxHistoryOrigin->addItem("Live history", QVariant(UQS::Core::IQueryHistoryUpr::EHistoryOrigin::Live));
	m_pComboBoxHistoryOrigin->addItem("Deserialized history", QVariant(UQS::Core::IQueryHistoryUpr::EHistoryOrigin::Deserialized));
	...
	GetIEditor()->GetViewportUpr()->GetGameViewport()->AddPostRenderer(m_pHistoryPostRenderer);	// this increments its refcount

	if (UQS::Core::IHub* pHub = UQS::Core::IHubPlugin::GetHubPtr())
	{
		m_pQueryHistoryUpr = &pHub->GetQueryHistoryUpr();
		m_pQueryHistoryUpr->RegisterQueryHistoryListener(this);
		m_pQueryHistoryUpr->EnumerateHistoricQueries(m_pQueryHistoryUpr->GetCurrentQueryHistory(), *this);
	}
	else
	{
		// disable everything
...
CMainEditorWindow::~CMainEditorWindow()
{
	if (m_pQueryHistoryUpr)
	{
		m_pQueryHistoryUpr->UnregisterQueryHistoryListener(this);
	}
	...
	if (IViewportUpr* pViewportUpr = GetIEditor()->GetViewportUpr())
	{
		if(CViewport* pGameViewport = pViewportUpr->GetGameViewport())
		{
...
void CMainEditorWindow::OnQueryHistoryEvent(const UQS::Core::IQueryHistoryListener::SEvent& ev)
{
	...
	case UQS::Core::IQueryHistoryListener::EEventType::HistoricQueryJustFinishedInLiveQueryHistory:
		if (m_pQueryHistoryUpr->GetCurrentQueryHistory() == UQS::Core::IQueryHistoryUpr::EHistoryOrigin::Live)
		{
			assert(ev.relatedQueryID.IsValid());
			m_pQueryHistoryUpr->EnumerateSingleHistoricQuery(m_pQueryHistoryUpr->GetCurrentQueryHistory(), ev.relatedQueryID, *this);
		}
		break;
	...
	case UQS::Core::IQueryHistoryListener::EEventType::DeserializedQueryHistoryCleared:
		if (m_pQueryHistoryUpr->GetCurrentQueryHistory() == UQS::Core::IQueryHistoryUpr::EHistoryOrigin::Deserialized)
		{
			m_pTreeModel->ClearAllHistoricQueries();
		}
		break;
	...
	case UQS::Core::IQueryHistoryListener::EEventType::DifferentHistoricQuerySelected:
		{
			const UQS::Core::IQueryHistoryUpr::EHistoryOrigin historyOrigin = m_pQueryHistoryUpr->GetCurrentQueryHistory();
			const UQS::Core::CQueryID queryID = m_pQueryHistoryUpr->GetCurrentHistoricQueryForInWorldRendering(historyOrigin);
			m_pTextQueryDetails->clear();
			m_pTextItemDetails->clear();   // get rid of some leftovers in case no item will be focused anymore
			m_pQueryHistoryUpr->GetDetailsOfHistoricQuery(historyOrigin, queryID, *this);
		}
		break;
	...
	case UQS::Core::IQueryHistoryListener::EEventType::FocusedItemChanged:
		{
			...
			m_pQueryHistoryUpr->GetDetailsOfFocusedItem(*this);
		}
		break;
...
void CMainEditorWindow::AddOrUpdateHistoricQuery(const SHistoricQueryOverview& overview)
{
	...
	m_pQueryHistoryUpr->EnumerateInstantEvaluatorNames(m_pQueryHistoryUpr->GetCurrentQueryHistory(), m_pFreshlyAddedOrUpdatedQuery->queryID, *this);
	m_pQueryHistoryUpr->EnumerateDeferredEvaluatorNames(m_pQueryHistoryUpr->GetCurrentQueryHistory(), m_pFreshlyAddedOrUpdatedQuery->queryID, *this);
...
void CMainEditorWindow::OnHistoryOriginComboBoxSelectionChanged(int index)
{
	if (m_pQueryHistoryUpr)
	{
		...
		const UQS::Core::IQueryHistoryUpr::EHistoryOrigin origin = (UQS::Core::IQueryHistoryUpr::EHistoryOrigin)data.toInt();
		m_pQueryHistoryUpr->MakeQueryHistoryCurrent(origin);
	}
}
...
void CMainEditorWindow::OnClearHistoryButtonClicked(bool checked)
{
	if (m_pQueryHistoryUpr)
	{
		m_pQueryHistoryUpr->ClearQueryHistory(m_pQueryHistoryUpr->GetCurrentQueryHistory());
	}
}
...
void CMainEditorWindow::OnSaveLiveHistoryToFile()
{
	...
	if (!sFilePath.empty())  // would be empty if pressing the "cancel" button
	{
		...
		if (!m_pQueryHistoryUpr->SerializeLiveQueryHistory(sFilePath.c_str(), uqsErrorMessage))
		{
			// show the error message
			stack_string error;
			error.Format("Serializing the live query history to '%s' failed: %s", sFilePath.c_str(), uqsErrorMessage.c_str());
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "UQS Query History Inspector: %s", error.c_str());
...
void CMainEditorWindow::OnLoadHistoryFromFile()
{
	...
	if (!qFilePath.isEmpty())
	{
		string sFilePath = QtUtil::ToString(qFilePath);
		UQS::Shared::CUqsString uqsErrorMessage;
		if (m_pQueryHistoryUpr->DeserializeQueryHistory(sFilePath.c_str(), uqsErrorMessage))
		{
