...
namespace UQS
{
	namespace DataSource_XML
	{
		...
		void CQueryBlueprintFileLibrary::EnumerateQueriesInFolder(const char* szFolderPath)
		{
			...
			const DrxPathString folder = PathUtil::AddSlash(szFolderPath);
			const DrxPathString search = PathUtil::Make(folder, DrxPathString("*.*"));
			DrxPathString subName;
			...
			do
			{
				...
				{
					DrxLog("[UQS] CQueryBlueprintFileLibrary: skipping file '%s%s': wrong extension",
		...
		void CQueryBlueprintFileLibrary::CreateRecordFromFile(const DrxPathString& fileName)
		{
			...
			{
				DrxLog("[UQS] CQueryBlueprintFileLibrary: failed to load file '%s' for query name '%s' - name validation failed: %s",
			...
			if (iter == m_queriesMap.end())
			{
				...
				DrxLog("[UQS] CQueryBlueprintFileLibrary: loaded file '%s' for query name '%s'",
					fileName.c_str(), queryRecordName.c_str());
			}
		}

		void CQueryBlueprintFileLibrary::GetQueryRecordNameFromFilePath(const DrxPathString& fileName, stack_string& outName) const
		{
			outName = fileName;
		...
		bool CQueryBlueprintFileLibrary::CreateNewTextualQueryBlueprint(const char* szQueryName, Core::ITextualQueryBlueprint& outBlueprint, Shared::IUqsString& error)
		{
			...
			DrxLog("[UQS] CQueryBlueprintFileLibrary: created record for query name '%s' (file '%s')",
		...
		bool CQueryBlueprintFileLibrary::RemoveQueryBlueprint(const char* szQueryName, Shared::IUqsString& error)
		{
			...
			// queryRecord.bLoaded is not enough, because it is set to false after reloading error.
			{
				...
				{
					DrxLog("[UQS] CQueryBlueprintFileLibrary: removed query blueprint '%s' from core blueprints library", szQueryName);
				}
				else
				{
					DrxLog("[UQS] CQueryBlueprintFileLibrary: unable to remove query blueprint '%s' from core blueprints library: %s",
			...
			if (IDrxPak* pPak = gEnv->pDrxPak)
			{
				if (pPak->RemoveFile(queryRecord.filePath.c_str()))
				{
					DrxLog("[UQS] CQueryBlueprintFileLibrary: removed query blueprint '%s' file at path '%s'",
						szQueryName, queryRecord.filePath.c_str());
				}
				else
				{
					DrxLog("[UQS] CQueryBlueprintFileLibrary: unable to remove query blueprint '%s' file at path '%s'",
						szQueryName, queryRecord.filePath.c_str());
				}
			}
