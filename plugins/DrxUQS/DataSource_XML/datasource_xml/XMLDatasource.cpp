...
namespace UQS
{
	namespace DataSource_XML
	{
		...
		void CXMLDatasource::EnumerateAndLoadAllQueryBlueprints()
		{
			...
			if (errors.empty())
			{
				DrxLog("UQS CXMLDatasource: Successfully loaded all query-blueprints.");
			}
			else
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "UQS CXMLDatasource: %" PRISIZE_T " errors while loading all query-blueprints:", errors.size());
				for (const string& err : errors)
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "UQS CXMLDatasource:   %s", err.c_str());
				}
			}
		}
		...
		void CXMLDatasource::CmdLoadQueryBlueprintLibrary(IConsoleCmdArgs* pArgs)
		{
			...
			{
				DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "UQS CXMLDatasource::CmdLoadQueryBlueprintLibrary: the XML datasource has not been set up and installed yet");
			}
