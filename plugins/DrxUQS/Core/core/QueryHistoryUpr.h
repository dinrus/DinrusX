...
namespace UQS
{
	namespace Core
	{
		...
		class CQueryHistoryUpr : public IQueryHistoryUpr
		{
		public:
			explicit                           CQueryHistoryUpr();
		...
		private:
			                                   UQS_NON_COPYABLE(CQueryHistoryUpr);
		...
		private:
			CQueryHistory                      m_queryHistories[2];                  // "live" and "deserialized" query history; the live one gets filled by ongoing queries from the CQueryUpr
