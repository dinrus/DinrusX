...
namespace UQS
{
	namespace Core
	{
		...
		{
			DRX_PROFILE_FUNCTION_ARG(UQS_PROFILED_SUBSYSTEM_TO_USE, m_pQueryBlueprint->GetName());	// mainly for keeping an eye on the copy operation of the items below
			...
			//
			// * but if this was the last query in the chain, then copy the result set away to have it picked up by the CQueryUpr (or the parent query, of course)
			...
			CQueryBase* pChildQuery = g_pHub->GetQueryUpr().FindQueryByQueryID(childQueryID);
			assert(pChildQuery);
