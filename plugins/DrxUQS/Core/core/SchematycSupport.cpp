...
namespace UQS
{
	namespace Core
	{
			...
			, m_itemConvertersFromSchematycToUqs(itemConverters, CItemConverterLookup::ELookupMethod::UseToTypeAsKey)
		{
			...
			SetGUID(DrxGUID::Construct(envDataType.GetGUID().hipart, partialGUID));
		...
		{
			desc.SetGUID("b0c4a5a2-8a6e-4a4f-b7c1-90bfb9d1898b"_drx_guid);
		...
		void CSchematycUqsComponent::SQueryIdWrapper::Register(Schematyc::CEnvRegistrationScope& scope)
		{
			...
			{
				auto pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&SchematycFunction_Equal, "ae7a5b0a-e3f4-4cd7-b01a-bbad7c7fe11e"_drx_guid, "Equal");
		...
		{
			desc.SetGUID("babd0a8b-2f84-4019-b6f5-0758ac62ec56"_drx_guid);
		...
		{
			desc.SetGUID("5d1ea803-63bb-436d-8044-252a150b916f"_drx_guid);
		...
		{
			desc.SetGUID("1d00010d-5b91-4b56-8cdd-52c8d966acb8"_drx_guid);
			desc.SetLabel("UQS");
			...
			Schematyc::CEnvRegistrationScope entityScope = registrar.Scope(IEntity::GetEntityScopeGUID());
			{
				Schematyc::CEnvRegistrationScope componentScope = entityScope.Register(SCHEMATYC_MAKE_ENV_COMPONENT(CSchematycUqsComponent));
				{
...

					{
						...
						{
							std::shared_ptr<Schematyc::CEnvFunction> pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CSchematycUqsComponent::SchematycFunction_BeginQuery, "5ebcdda0-31f5-4e2d-afef-63fc0368cf9c"_drx_guid, "BeginQuery");
						...
						{
							std::shared_ptr<Schematyc::CEnvFunction> pFunction = SCHEMATYC_MAKE_ENV_FUNCTION(&CSchematycUqsComponent::SchematycFunction_StartQuery, "f7205181-41cb-478d-a773-0d33a3805a99"_drx_guid, "StartQuery");
		...
		void CSchematycUqsComponent::SchematycFunction_StartQuery(SQueryIdWrapper& outQueryId)
		{
			...
			const CQueryID queryID = pHub->GetQueryUpr().StartQuery(request, errorMessage);
		...
		void CSchematycUqsComponent::CancelRunningQueriesAndClearFinishedOnes()
		{
			...
			{
				IQueryUpr& queryUpr = pHub->GetQueryUpr();

				for (const CQueryID& queryID : m_runningQueries)
				{
					queryUpr.CancelQuery(queryID);
				}
			}
