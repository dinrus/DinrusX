...
namespace UQS
{
	namespace Core
	{
		...
		//===================================================================================
		//
		// CQueryUpr::SRunningQueryInfo
		//
		//===================================================================================

		CQueryUpr::SRunningQueryInfo::SRunningQueryInfo()
			: pQuery()
			, pQueryBlueprint()
			, pCallback(0)
		{}
		...
		//===================================================================================
		//
		// CQueryUpr::SFinishedQueryInfo
		//
		//===================================================================================

		CQueryUpr::SFinishedQueryInfo::SFinishedQueryInfo(const std::shared_ptr<CQueryBase>& _pQuery, const std::shared_ptr<const CQueryBlueprint>& _pQueryBlueprint, const Functor1<const SQueryResult&>& _pCallback, const CQueryID& _queryID, bool _bQueryFinishedWithSuccess, const string& _errorIfAny)
			: pQuery(_pQuery)
		...
		//===================================================================================
		//
		// CQueryUpr::SHistoryQueryInfo2D
		//
		//===================================================================================

		CQueryUpr::SHistoryQueryInfo2D::SHistoryQueryInfo2D(const CQueryID &_queryID, const CQueryBase::SStatistics& _statistics, bool _bQueryFinishedWithSuccess, const CTimeValue& _timestamp)
			: queryID(_queryID)
		...
		const CTimeValue CQueryUpr::s_delayBeforeFadeOut(2.5f);
		const CTimeValue CQueryUpr::s_fadeOutDuration(0.5f);
		const CTimeValue CQueryUpr::s_totalDebugDrawDuration = CQueryUpr::s_delayBeforeFadeOut + CQueryUpr::s_fadeOutDuration;

		CQueryUpr::CQueryUpr(CQueryHistoryUpr& queryHistoryUpr)
		...
		CQueryID CQueryUpr::StartQuery(const Client::SQueryRequest& request, Shared::IUqsString& errorMessage)
		{
			if (!request.queryBlueprintID.IsOrHasBeenValid())
			{
				...
				errorMessage.Format("CQueryUpr::StartQuery: unknown query blueprint: '%s'", request.queryBlueprintID.GetQueryBlueprintName());
			...
			if (!qbp)
			{
				errorMessage.Format("CQueryUpr::StartQuery: the blueprint '%s' was once in the library, but has been removed and not been (successfully) reloaded since then", request.queryBlueprintID.GetQueryBlueprintName());
				return CQueryID::CreateInvalid();
			}
		...
		void CQueryUpr::CancelQuery(const CQueryID& idOfQueryToCancel)
		{
		...
		void CQueryUpr::AddItemMonitorToQuery(const CQueryID& queryID, Client::ItemMonitorUniquePtr&& pItemMonitorToInstall)
		{
		...
		CQueryID CQueryUpr::StartQueryInternal(const CQueryID& parentQueryID, std::shared_ptr<const CQueryBlueprint> pQueryBlueprint, const Shared::IVariantDict& runtimeParams, const char* szQuerierName, Functor1<const Core::SQueryResult&> pCallback, const std::shared_ptr<CItemList>& pPotentialResultingItemsFromPreviousQuery, Shared::IUqsString& errorMessage)
		{
			...
			{
				pOptionalHistoryEntry = m_queryHistoryUpr.AddNewLiveHistoricQuery(id, szQuerierName, parentQueryID);
			}
			...
			{
				errorMessage.Format("CQueryUpr::StartQueryInternal: %s", error.c_str());
				return CQueryID::CreateInvalid();
			}
		...
		CQueryBase* CQueryUpr::FindQueryByQueryID(const CQueryID& queryID)
		{
			auto it = m_queries.find(queryID);
			return (it == m_queries.end()) ? nullptr : it->second.pQuery.get();
		}

		void CQueryUpr::Update()
		{
		...
		void CQueryUpr::DebugDrawRunningQueriesStatistics2D() const
		{
		...
		void CQueryUpr::NotifyCallbackOfFinishedQuery(const SFinishedQueryInfo& finishedQueryInfo)
		{
			DRX_PROFILE_FUNCTION_ARG(UQS_PROFILED_SUBSYSTEM_TO_USE, finishedQueryInfo.pQueryBlueprint->GetName());
		...
		void CQueryUpr::ExpireDebugDrawStatisticHistory2D()
		{
			const CTimeValue now = gEnv->pTimer->GetAsyncTime();
		...
		void CQueryUpr::PrintRunningQueriesToConsole(CLogger& logger) const
		{
			logger.Printf("");
		...
		void CQueryUpr::CancelAllRunningQueriesDueToUpcomingTearDownOfHub()
		{
		...
		void CQueryUpr::DebugPrintQueryStatistics(CLogger& logger, const CQueryBase& query, const CQueryID& queryID)
		{
			CQueryBase::SStatistics stats;
		...
		int CQueryUpr::DebugDrawQueryStatistics(const CQueryBase::SStatistics& statisticsToDraw, const CQueryID& queryID, int row, const ColorF& color)
		{
			Shared::CUqsString queryIDAsString;
			queryID.ToString(queryIDAsString);
