...
namespace UQS
{
	namespace Core
	{
		...
		public:
			...
			virtual void                  RegisterFactory(TFactory* pFactoryToRegister, const char* szName, const DrxGUID& guid) override final;
			virtual TFactory*             FindFactoryByName(const char* szName) const override final;
			virtual TFactory*             FindFactoryByGUID(const DrxGUID& guid) const override final;
			...
			std::map<DrxGUID, int>        GetDuplicateFactoryGUIDs() const;
		...
		private:
			...
			std::map<DrxGUID, TFactory*>  m_mapByGUID;               // for fast lookup by GUID
			...
			std::map<DrxGUID, int>        m_guid2registrationCount;  // to detect duplicate factory GUIDs that occurred via RegisterFactory() (which is an error from the client side)
		...
		template <class TFactory>
		void CFactoryDatabase<TFactory>::RegisterFactory(TFactory* pFactoryToRegister, const char* szName, const DrxGUID& guid)
		{
		...
		template <class TFactory>
		TFactory* CFactoryDatabase<TFactory>::FindFactoryByGUID(const DrxGUID& guid) const
		{
		...
		template <class TFactory>
		std::map<DrxGUID, int> CFactoryDatabase<TFactory>::GetDuplicateFactoryGUIDs() const
		{
			std::map<DrxGUID, int> duplicates;
		...
		std::set<string> CFactoryDatabase<TFactory>::GetFactoryNamesWithEmptyGUIDs() const
		{
			...
			{
				if (pFactory->GetGUID() == DrxGUID::Null())
				{
		...
		void CFactoryDatabase<TFactory>::PrintToConsole(CLogger& logger, const char* szDatabaseNameToPrint) const
		{
			...
			for (size_t i = 0; i < numFactories; ++i)
			{
				...
				const DrxGUID& guid = factory.GetGUID();         // this compiles only as long as the TFactory class has such a GetGUID() method
