...
namespace UQS
{
	namespace Core
	{
		...
		void CStartupConsistencyChecker::CheckFactoryDatabaseConsistency(const TFactoryDB& factoryDB, const char* szFactoryDatabaseNameForErrorMessages)
		{
			...
			{
				const std::map<DrxGUID, int> duplicateFactoryGUIDs = factoryDB.GetDuplicateFactoryGUIDs();
...
#if UQS_SCHEMATYC_SUPPORT
		void CStartupConsistencyChecker::CheckItemConvertersConsistency(const Client::IItemConverterCollection& itemConverters, const char* szItemFactoryNameForErrorMessages, std::set<DrxGUID>& guidsInUse)
		{
			for (size_t i = 0, n = itemConverters.GetItemConverterCount(); i < n; ++i)
			{
				...
				const DrxGUID& guid = converter.GetGUID();
