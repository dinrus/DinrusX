...
namespace UQS
{
	namespace Core
	{
		...
		CHistoricQuery::CHistoricQuery(const CQueryID& queryID, const char* szQuerierName, const CQueryID& parentQueryID, CQueryHistoryUpr* pOwningHistoryUpr)
			: m_pOwningHistoryUpr(pOwningHistoryUpr)
			, m_queryID(queryID)
		...
		void CHistoricQuery::OnQueryCreated()
		{
			...
			m_pOwningHistoryUpr->UnderlyingQueryJustGotCreated(m_queryID);
		}
		...
		void CHistoricQuery::OnQueryDestroyed()
		{
			...
			m_pOwningHistoryUpr->UnderlyingQueryIsGettingDestroyed(m_queryID);
		}
		...
		CHistoricQuery::EItemAnalyzeStatus CHistoricQuery::AnalyzeItemStatus(const SHistoricItem& itemToAnalyze, const IQueryHistoryUpr::SEvaluatorDrawMasks& evaluatorDrawMasks, float& outAccumulatedAndWeightedScoreOfMaskedEvaluators, bool& outFoundScoreOutsideValidRange)
		{
		...
		void CHistoricQuery::DrawDebugPrimitivesInWorld(size_t indexOfItemCurrentlyBeingFocused, const IQueryHistoryUpr::SEvaluatorDrawMasks& evaluatorDrawMasks) const
		{
			...
			const float alphaValueForDiscardedItems = (float)drxmath::clamp(SCvars::debugDrawAlphaValueOfDiscardedItems, 0, 255) / 255.0f;
		...
		HistoricQuerySharedPtr CQueryHistory::AddNewHistoryEntry(const CQueryID& queryID, const char* szQuerierName, const CQueryID& parentQueryID, CQueryHistoryUpr* pOwningHistoryUpr)
		{
			//
			...
			HistoricQuerySharedPtr pNewEntry(new CHistoricQuery(queryID, szQuerierName, parentQueryID, pOwningHistoryUpr));
