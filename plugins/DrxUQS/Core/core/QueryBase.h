	...
	namespace Core
	{
		...
		public:
			...
			struct SStatistics
			{
				...
				// this is stuff that CQueryUpr::DebugDrawQueryStatistics() is partly interested in (but it's also interested in the number of items - hmmm, could we just display the elapsed frames instead?)
				string                                  querierName;
		...
		protected:
			...
			const CQueryID                              m_queryID;                        // the unique queryID that can be used to identify this instance from inside the CQueryUpr
