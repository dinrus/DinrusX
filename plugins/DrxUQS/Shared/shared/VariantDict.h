...
namespace UQS
{
	namespace Shared
	{
		...
		void CVariantDict::AddOrReplaceFromOriginalValue(const char* szKey, const TItem& originalValueToClone)
		{
			...
			if (!pHub)
			{
				DrxWarning(VALIDATOR_MODULE_UNKNOWN, VALIDATOR_ERROR, "UQS: CVariantDict::AddOrReplaceFromOriginalValue: UQS Plugin is not loaded. The dictionary you're trying to fill will be lacking the entry with key = '%s'.", szKey);
				return;
			}
			...
			// Ideally, we should trigger a DrxFatalError(), but that sounds a bit too harsh?! For now, we only issue a warning.
			...
			{
				DrxWarning(VALIDATOR_MODULE_UNKNOWN, VALIDATOR_ERROR, "UQS: CVariantDict::AddOrReplaceFromOriginalValue: tried to add a value of type [%s] under the name '%s' but there is no item-factory registered for that type.", typeOfOriginalValue.name(), szKey);
			}
		}
