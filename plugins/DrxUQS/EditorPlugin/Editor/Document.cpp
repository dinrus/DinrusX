...
<DinrusX/Serialization/PropertyTree/PropertyTree.h>
...
bool CUqsQueryDocument::CreateNew(const char* szQueryName, stack_string& outErrorMessage)
{
	...
	else
	{
		...
		DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_ERROR, "CUqsQueryDocument::CreateNew: %s", error.c_str());
...
void CUqsQueryDocument::Delete()
{
	...
	if (!m_queryName.empty())
	{
		...
		if (!bResult)
		{
			DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_ERROR, "CUqsQueryDocument::Delete: %s", error.c_str());
		}
...
bool CUqsQueryDocument::Load(const char* szQueryName)
{
	...
	{
		DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_ERROR, "CUqsQueryDocument::Load: %s", error.c_str());
...
bool CUqsQueryDocument::Save()
{
	...
	if (m_pQueryBlueprint)
	{
		...
		{
			DrxWarning(VALIDATOR_MODULE_AI, VALIDATOR_ERROR, "CUqsQueryDocument::Save: %s", error.c_str());
		}
