	...
	if (m_bPeriodicRenderingEnabled)
	{
		if (UQS::Core::IHub* pHub = UQS::Core::IHubPlugin::GetHubPtr())
		{
			if (const CViewport* pGameViewport = GetIEditor()->GetViewportUpr()->GetGameViewport())
			{
				...
				pHub->GetQueryHistoryUpr().UpdateDebugRendering3D(&uqsCameraView, UQS::Core::IQueryHistoryUpr::SEvaluatorDrawMasks::CreateAllBitsSet());
			}
		}
	}
}
...
{
	CCentralEventUpr::StartOrStopQuerySimulator.Connect(this, &CQuerySimulator::OnStartOrStopQuerySimulator);
	CCentralEventUpr::SingleStepQuerySimulatorOnce.Connect(this, &CQuerySimulator::OnSingleStepQuerySimulatorOnce);
...
CQuerySimulator::~CQuerySimulator()
{
	...
	CCentralEventUpr::StartOrStopQuerySimulator.DisconnectObject(this);
}
...
void CQuerySimulator::OnStartOrStopQuerySimulator(const CCentralEventUpr::SStartOrStopQuerySimulatorEventArgs& args)
{
	if (m_runState == ERunState::NotRunningAtAll)
	{
		...
		if (StartQuery())
		{
			...
			CCentralEventUpr::QuerySimulatorRunningStateChanged(CCentralEventUpr::SQuerySimulatorRunningStateChangedEventArgs(true));
		}
	}
	else
	{
...
void CQuerySimulator::OnSingleStepQuerySimulatorOnce(const CCentralEventUpr::SSingleStepQuerySimulatorOnceEventArgs& args)
{
	if (m_runState == ERunState::NotRunningAtAll)
	{
		// prepare a new query and start it
...
bool CQuerySimulator::StartQuery()
{
	...
	if (queryID.IsValid())
	{
		if (UQS::Core::IHub* pHub = UQS::Core::IHubPlugin::GetHubPtr())
		{
			pHub->GetQueryHistoryUpr().MakeHistoricQueryCurrentForInWorldRendering(UQS::Core::IQueryHistoryUpr::Live, queryID);
		}
	}
...
void CQuerySimulator::EnsureStopRunning()
{
	...
	m_runState = ERunState::NotRunningAtAll;
	m_singleStepState = ESingleStepState::NotSingleSteppingAtAll;
	CCentralEventUpr::QuerySimulatorRunningStateChanged(CCentralEventUpr::SQuerySimulatorRunningStateChangedEventArgs(false));
}
