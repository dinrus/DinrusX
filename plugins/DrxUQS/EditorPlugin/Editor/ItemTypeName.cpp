...
SItemTypeName::SItemTypeName(const DrxGUID& typeGUID)
	: m_typeGUID(typeGUID)
{}
...
void SItemTypeName::Serialize(Serialization::IArchive& archive)
{
	...
	{
		const DrxGUID oldTypeGUID = m_typeGUID;
		auto setTypeGUID = [this](const DrxGUID& newTypeGUID)
		{
		...
		{
			CKeyValueStringList<DrxGUID> itemTypeList;
...
const DrxGUID& SItemTypeName::GetTypeGUID() const
{
	return m_typeGUID;
}
