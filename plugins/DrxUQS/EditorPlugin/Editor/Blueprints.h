...
private:
	...
	public:
		...
		int              FindByGUIDAndParam(const DrxGUID& guid, const string& param) const;

		void             Clear();
	private:
		std::vector<SFunction>    m_functions;
		CKeyValueStringList<DrxGUID> m_functionsGuidList;
	};

public:

	CFunctionSerializationHelper();
	CFunctionSerializationHelper(const DrxGUID& functionGUID, const char* szParamOrReturnValue, bool bAddReturnValueToDebugRenderWorldUponExecution);
...
public:
	...
	const DrxGUID&         GetFuncGUID() const;
	const char*            GetFuncReturnValueLiteral() const;
	bool                   GetAddReturnValueToDebugRenderWorldUponExecution() const;
	CInputBlueprint&       AddChild(const char* szParamName, const UQS::Client::CInputParameterID& paramID, const DrxGUID& funcGUID, const char* szFuncReturnValueLiteral, bool bAddReturnValueToDebugRenderWorldUponExecution);
	...
	CInputBlueprint();
	CInputBlueprint(const char* szParamName, const UQS::Client::CInputParameterID& paramID, const char* szParamDescription, const DrxGUID& funcGUID, const char* szFuncReturnValueLiteral, bool bAddReturnValueToDebugRenderWorldUponExecution);
...
{
public:
	void   AddParameter(const char* szName, const DrxGUID& typeGUID, CItemLiteral&& value, bool bAddToDebugRenderWorld);
	size_t GetParameterCount() const;
	void   GetParameterInfo(size_t index, const char*& szName, const char*& szType, DrxGUID& typeGUID, string& szValue, bool& bAddToDebugRenderWorld, std::shared_ptr<CErrorCollector>& pErrorCollector) const;
...
private:
	...
	{
		SConstParam();
		SConstParam(const char* szName, const DrxGUID& typeGUID, CItemLiteral&& value, bool _bAddToDebugRenderWorld);
...
{
public:
	void   AddParameter(const char* szName, const DrxGUID& typeGUID, bool bAddToDebugRenderWorld);
	size_t GetParameterCount() const;
	void   GetParameterInfo(size_t index, const char*& szName, const char*& szType, DrxGUID& typeGUID, bool& bAddToDebugRenderWorld, std::shared_ptr<CErrorCollector>& pErrorCollector) const;
...
private:
	...
	{
		SRuntimeParam();
		SRuntimeParam(const char* szName, const DrxGUID& typeGUID, bool _bAddToDebugRenderWorld);
...
{
public:
	void                   SetEvaluatorGUID(const DrxGUID& evaluatorGUID);
	void                   SetWeight(float weight);
	void                   SetScoreTransformGUID(const DrxGUID& scoreTransformGUID);
	void                   SetNegateDiscard(bool bNegateDiscard);
	float                  GetWeight() const;
	const char*            GetScoreTransformName() const;
	const DrxGUID&         GetScoreTransformGUID() const;
	bool                   GetNegateDiscard() const;
	CInputBlueprint&       GetInputRoot();
	const CInputBlueprint& GetInputRoot() const;
	const char*            GetEvaluatorName() const;
	const DrxGUID&         GetEvaluatorGUID() const;
...
struct SQueryFactoryType
{
	...
	SQueryFactoryType(const DrxGUID& queryFactoryGUID);
	...
	const DrxGUID&                  GetFactoryGUID() const { return m_queryFactoryGUID; }
