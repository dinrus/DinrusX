...
<DinrusX/CoreX/Serialization/STL.h>
...
public:
	...
	CEvaluatorFactoryHelper(const CUqsDocSerializationContext& context, const DrxGUID& evaluatorGUID)
		: m_pInstantFactory(nullptr)
		, m_pDeferredFactory(nullptr)
	{
...
void CFunctionSerializationHelper::CFunctionList::Build(const SItemTypeName& typeName, const CUqsDocSerializationContext& context)
{
...

	{
		...
		for (size_t i = 0; i < functionFactoryDB.GetFactoryCount(); i++)
		{
			...
			if (pItemFactoryOfReturnType->GetGUID() == typeName.GetTypeGUID() || !context.GetSettings().bFilterAvailableInputsByType)
			{
				...
				{
					func.returnType = SItemTypeName(DrxGUID::Null());
				}
			}
		}
	}
	...
	// Build string list
	{
		...
		m_functionsGuidList.AddEntry("", DrxGUID::Null());
	...
	const int oldFunctionIdx)
{
	...
	const CKeyValueStringList<DrxGUID>::SSerializeResult serializeResult = m_functionsGuidList.SerializeByLabelForDisplayInDropDownList(archive, szName, szLabel, &validatorKey, oldPrettyName);
...
const CFunctionSerializationHelper::SFunction& CFunctionSerializationHelper::CFunctionList::GetByIdx(const int idx) const
{
	...
	{
		DrxFatalError("Wrong index");
	}

	return m_functions[idx];
}

int CFunctionSerializationHelper::CFunctionList::FindByGUIDAndParam(const DrxGUID& guid, const string& param) const
{
...
CFunctionSerializationHelper::CFunctionSerializationHelper(const DrxGUID& functionGUID, const char* szParamOrReturnValue, bool bAddReturnValueToDebugRenderWorldUponExecution)
	...
	const CUqsDocSerializationContext& context)
{
	...
	{
		const DrxGUID oldGUID = m_funcGUID;
		const auto setGUID = [this, &bGUIDChanged](const DrxGUID& newGUID)
		{
			m_funcGUID = newGUID;
			bGUIDChanged = true;
		};
...
CInputBlueprint::CInputBlueprint(const char* szParamName, const UQS::Client::CInputParameterID& paramID, const char* szParamDescription, const DrxGUID& funcGUID, const char* szFuncReturnValueLiteral, bool bAddReturnValueToDebugRenderWorldUponExecution)
...
void CInputBlueprint::SetChildrenFromFactoryInputRegistry(const TFactory& factory, const CUqsDocSerializationContext& context)
{
	...
	DRX_ASSERT(paramCount <= m_children.size());
...
const DrxGUID& CInputBlueprint::GetFuncGUID() const
{
	...
	{
		static const DrxGUID nullGUID = DrxGUID::Null();
		return nullGUID;
	}
}
...
CInputBlueprint& CInputBlueprint::AddChild(const char* szParamName, const UQS::Client::CInputParameterID& paramID, const DrxGUID& funcGUID, const char* szFuncReturnValueLiteral, bool bAddReturnValueToDebugRenderWorldUponExecution)
{
...
CConstParamBlueprint::SConstParam::SConstParam(const char* szName, const DrxGUID& typeGUID, CItemLiteral&& value, bool _bAddToDebugRenderWorld)
	: name(szName)
	, type(typeGUID)
...
void CConstParamBlueprint::AddParameter(const char* szName, const DrxGUID& typeGUID, CItemLiteral&& value, bool bAddToDebugRenderWorld)
{
...
void CConstParamBlueprint::GetParameterInfo(size_t index, const char*& szName, const char*& szType, DrxGUID& typeGUID, string& value, bool& bAddToDebugRenderWorld, std::shared_ptr<CErrorCollector>& pErrorCollector) const
{
...
CRuntimeParamBlueprint::SRuntimeParam::SRuntimeParam(const char* szName, const DrxGUID& typeGUID, bool _bAddToDebugRenderWorld)
	: name(szName)
	, type(typeGUID)
	, bAddToDebugRenderWorld(_bAddToDebugRenderWorld)
	, pErrorCollector(new CErrorCollector)
...
void CRuntimeParamBlueprint::AddParameter(const char* szName, const DrxGUID& typeGUID, bool bAddToDebugRenderWorld)
{
...
void CRuntimeParamBlueprint::GetParameterInfo(size_t index, const char*& szName, const char*& szType, DrxGUID& typeGUID, bool& bAddToDebugRenderWorld, std::shared_ptr<CErrorCollector>& pErrorCollector) const
{
	assert(index < m_params.size());
...
void CGeneratorBlueprint::SetGeneratorGUID(const DrxGUID& generatorGUID)
{
...
const DrxGUID& CGeneratorBlueprint::GetGeneratorGUID() const
{
	return m_guid;
}
...
bool CGeneratorBlueprint::SerializeGUID(Serialization::IArchive& archive, const char* szName, const char* szLabel, const CUqsDocSerializationContext& context)
{
	...
	const DrxGUID oldGeneratorGUID = m_guid;
	auto setGUID = [this, &bGUIDChanged](const DrxGUID& newGUID)
	{
		m_guid = newGUID;
		bGUIDChanged = true;
	};
	...
	if (context.GetSettings().bUseSelectionHelpers)
	{
		CKeyValueStringList<DrxGUID> generatorGuidList;
...
void CInstantEvaluatorBlueprint::SetEvaluatorGUID(const DrxGUID& evaluatorGUID)
{
...
void CInstantEvaluatorBlueprint::SetScoreTransformGUID(const DrxGUID& scoreTransformGUID)
{
...
const DrxGUID& CInstantEvaluatorBlueprint::GetScoreTransformGUID() const
{
...
const DrxGUID& CInstantEvaluatorBlueprint::GetEvaluatorGUID() const
{
...
void CDeferredEvaluatorBlueprint::SetEvaluatorGUID(const DrxGUID& evaluatorGUID)
{
	return Owner().SetEvaluatorGUID(evaluatorGUID);
}
...
void CDeferredEvaluatorBlueprint::SetScoreTransformGUID(const DrxGUID& scoreTransformGUID)
{
	Owner().SetScoreTransformGUID(scoreTransformGUID);
}
...
const DrxGUID& CDeferredEvaluatorBlueprint::GetScoreTransformGUID() const
{
	return Owner().GetScoreTransformGUID();
}
...
const DrxGUID& CDeferredEvaluatorBlueprint::GetEvaluatorGUID() const
{
	return Owner().GetEvaluatorGUID();
}
...
void CEvaluator::SetEvaluatorGUID(const DrxGUID& evaluatorGUID)
{
...
void CEvaluator::SetScoreTransformGUID(const DrxGUID& scoreTransformGUID)
{
...
const DrxGUID& CEvaluator::GetScoreTransformGUID() const
{
...
const DrxGUID& CEvaluator::GetEvaluatorGUID() const
{
...
CInstantEvaluatorBlueprint& CEvaluator::AsInstant()
{
	if (m_evaluatorType != EEvaluatorType::Instant)
	{
		DrxFatalError("Can't cast evaluator to instant evaluator");
	}
...
CDeferredEvaluatorBlueprint& CEvaluator::AsDeferred()
{
	...
	{
		DrxFatalError("Can't cast evaluator to deferred evaluator");
	}
...
void CEvaluator::SetType(EEvaluatorType type)
{
	...
	default:
		DrxFatalError("Attempt to create an evaluator of unknown type");
		break;
	}
}
...
bool CEvaluator::SerializeEvaluatorGUID(Serialization::IArchive& archive, const char* szName, const char* szLabel, const CUqsDocSerializationContext& context)
{
	...
	const DrxGUID oldGUID = m_evaluatorGUID;
	auto setGUID = [this, &bGUIDChanged](const DrxGUID& newGUID)
	{
	...
	if (context.GetSettings().bUseSelectionHelpers)
	{
		CKeyValueStringList<DrxGUID> evaluatorGuidList;
...
{
	const DrxGUID oldScoreTransformGUID = m_scoreTransformGUID;
	auto setScoreTransformGUID = [this](const DrxGUID& newGUID)
	{
	...
	if (context.GetSettings().bUseSelectionHelpers)
	{
		CKeyValueStringList<DrxGUID> scoreTransformsList;
...
CInstantEvaluatorBlueprint& CEvaluators::GetInstantEvaluator(size_t index)
{
	...
	DrxFatalError("Index out of bounds");
...
SQueryFactoryType::SQueryFactoryType(const DrxGUID& queryFactoryGUID)
...
bool SQueryFactoryType::Serialize(Serialization::IArchive& archive, const char* szName, const char* szLabel, const CUqsDocSerializationContext* pContext)
{
	...
	const DrxGUID oldGUID = m_queryFactoryGUID;
	auto setGUID = [this, &bGUIDChanged](const DrxGUID& newGUID)
	{
	...
	{
		CKeyValueStringList<DrxGUID> queryGuidList;
...
void CQueryBlueprint::HelpBuildCInputBlueprintHierarchyFromITextualInputBlueprint(CInputBlueprint& targetRoot, const UQS::Core::ITextualInputBlueprint& sourceRoot)
{
	for (size_t i = 0, n = sourceRoot.GetChildCount(); i < n; ++i)
	{
		...
		const DrxGUID& funcGUID = sourceChild.GetFuncGUID();
...
void CParametersListContext::SetParamsChanged()
{
	...
	CCentralEventUpr::QueryBlueprintRuntimeParamsChanged(CCentralEventUpr::SQueryBlueprintRuntimeParamsChangedEventArgs(*m_pOwner));
}
  ...
  std::vector<CFunctionSerializationHelper::SFunction>& outParamFunctions)
{
	...
	if (bApplyFilter)
	{
		...
		const DrxGUID& generatorGUID = m_pOwner->GetGenerator().GetGeneratorGUID();
