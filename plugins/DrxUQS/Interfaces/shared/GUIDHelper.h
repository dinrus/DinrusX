...
namespace UQS
{
	namespace Shared
	{
		namespace Internal
		{
			...
			// CGUIDHelper
			//
			// - intermediate helper class to extend DrxGUID's functionality until Timur submits his changes
			//
			//===================================================================================
			...
			public:
				...
				static bool     TryParseFromString(DrxGUID& out, const char* szGuidAsString);  // returns false and leaves 'out' untouched if given string is no valid GUID representation
				static void     ToString(const DrxGUID& guid, Shared::IUqsString& out);
			...
			inline bool CGUIDHelper::TryParseFromString(DrxGUID& out, const char* szGuidAsString)
			{
				if (IsValidStringRepresentation(szGuidAsString))
				{
					out = DrxGUID::FromString(szGuidAsString);
					return true;
				}
				else
				{
			...
			inline void CGUIDHelper::ToString(const DrxGUID& guid, Shared::IUqsString& out)
			{
			...
			inline bool CGUIDHelper::IsValidStringRepresentation(const char* szGuidAsStringToCheck)
			{
				...
				if (szGuidAsStringToCheck[0] == '{' && len == DRX_ARRAY_COUNT("{XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX}") - 1)
					szGuidAsStringToCheck += 1;
				else if (len != DRX_ARRAY_COUNT("XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX") - 1)
					return false;
