...
<DinrusX/CoreX/Extension/ClassWeaver.h>
<DinrusX/CoreX/Extension/DrxCreateClassInstance.h>

// yasli::TypeID is used by UQS::Shared::CTypeInfo
<DinrusX/CoreX/Serialization/TypeID.h>
<DinrusX/CoreX/Serialization/ClassFactory.h>  // weird: this header includes ClassFactory.cpp (yeah, .cpp), which has some methods marked as inline (!) that we make use of, so we need to include that header to prevent unresolved symbols at link time
