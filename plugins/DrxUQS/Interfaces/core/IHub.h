...
namespace UQS
{
	namespace Core
	{
		...
		// EHubOverrideFlags
		//
		// - specifies what parts of the IHub the game wants to customize, rather than having them run automatically through the DrxPlugin system
		...
		struct IHub
		{
			...
			virtual IQueryUpr&                                     GetQueryUpr() = 0;
			virtual IQueryHistoryUpr&                              GetQueryHistoryUpr() = 0;
			...
			virtual ISettingsUpr&                                  GetSettingsUpr() = 0;
			...
			// provider as game extension DRXINTERFACE and query it by GUID - same as Hub).
