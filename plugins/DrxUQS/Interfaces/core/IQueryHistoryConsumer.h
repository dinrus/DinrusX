...
namespace UQS
{
	namespace Core
	{
		...
		//
		// - reads back data from an IQueryHistoryUpr
		// - basically, it's the IQueryHistoryUpr that calls back into this class to provide it with the requested data
		...
		struct IQueryHistoryConsumer
		{
			...
			// - called when requesting to enumerate all historic queries via IQueryHistoryUpr::EnumerateHistoricQueries()
			// - also called when requesting information of a specific query via IQueryHistoryUpr::EnumerateSingleHistricQuery()
			virtual void                  AddOrUpdateHistoricQuery(const SHistoricQueryOverview& overview) = 0;

			// - called when requesting details about a specific historic query via IQueryHistoryUpr::GetDetailsOfHistoricQuery()
			...
			// - see IQueryHistoryUpr::EnumerateInstantEvaluatorNames() and EnumerateDeferredEvaluatorNames()
