...
bool FixedString<BUFFER_SIZE>::appendString(const char* const str)
{
	...
	m_bOverflow = !drx_strcpy(m_buffer + m_size - 1, BUFFER_SIZE - m_size + 1, str);
...
bool FixedString<BUFFER_SIZE>::appendVA(const char* const format, va_list args)
{
	...
	m_bOverflow = !drx_vsprintf(m_buffer + m_size - 1, BUFFER_SIZE - m_size + 1, format, args);
...
int CrashHandler::UpdateCallStack(EXCEPTION_POINTERS* const pex)
{
	...
	SettingsUprHelpers::Utils::strcpy_with_clamp(m_excModule, sizeof(m_excModule), "<Unknown>");
	...
	if (m_bSymbols)
	{
		...
		if (dwAddr) 
		{
			...
			if (GetModuleFileName((HMODULE)dwAddr, fullpath, sizeof(fullpath))) 
			{
				...
				SettingsUprHelpers::Utils::strcpy_with_clamp(m_excModule, sizeof(m_excModule), fullpath);
			}
		}
	}
...
void CrashHandler::LogExceptionInfo(EXCEPTION_POINTERS* const pex)
{
	...
	LogLine(eCrashLog, "Exception Описание: %s", descString.c_str());
