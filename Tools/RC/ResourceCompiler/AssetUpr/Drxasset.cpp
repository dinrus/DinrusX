...
bool CAsset::New()
{
	m_xml = m_pRC->CreateXml(AssetUpr::GetMetadataTag());
...
bool CAsset::Save(const string& filename)
{
	...
	{
		RCLogError("Can't rename '%s' to '%s'. Error code: %s.", tmpFilename.c_str(), filename.c_str(), Private_Drxasset::GetLastErrorString().c_str());
...
{
	return AssetUpr::WriteMetadata(m_xml, metadata);
}
...
{
	return AssetUpr::ReadMetadata(m_xml, metadata);
}

XmlNodeRef CAsset::GetMetadataRoot()
{
	return AssetUpr::GetMetadataNode(m_xml);
}
