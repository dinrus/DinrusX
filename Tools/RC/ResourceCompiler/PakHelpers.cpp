...
bool PakHelpers::PakEntry::MakeSortableStreamingSuffix(const string& suffix, string* sortable, int nDigits, int nIncrement)
{
	...
	case 2:
		// .dds.0a, .dds.1a, etc.
		drx_sprintf(verify, "%d%c", nMipmap, cAttached);
		break;
	case 1:
		// .dds.0, .dds.1, etc.
		drx_sprintf(verify, "%d", nMipmap);
		break;
	default:
		// .dds.a
		if (sscanf(suffix.data(), "%c", &cAttached) == 1)
		{
			drx_sprintf(verify, "%c", cAttached);
			nTokens = 2;
		}
		break;
	}
	...
	if (sortable)
	{
		...
		if (nDigits > 0)
		{
			drx_sprintf(digits, "%%0%dd", nDigits);
		}
		else
		{
			drx_strcpy(digits, "%d");
		...
		{
		case 2:
			drx_sprintf(verify, digits, nMipmap + nIncrement, cAttached);
			break;
		case 1:
			drx_sprintf(verify, digits, nMipmap + nIncrement);
			break;
		default:
			break;
		}
