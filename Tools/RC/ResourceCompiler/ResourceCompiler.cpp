...
ResourceCompiler::ResourceCompiler()
{
	...
	m_pAssetUpr.reset(new CAssetUpr(this));
...
void ResourceCompiler::ShowProgress(const char* pMessage, size_t progressValue, size_t maxProgressValue)
{
	...
	if ((percent <= 100.0f) && (percent != m_progressLastPercent))
	{
		...
		char str[1024];
		drx_sprintf(str, "Progress: %d.%d%% %s", int(percent), int(percent * 10.0f) % 10, pMessage);
...
{
	m_extensionUpr.RegisterConverter(name, conv, this);
}

//////////////////////////////////////////////////////////////////////////
IPakSystem* ResourceCompiler::GetPakSystem()
{
	return (m_pPakUpr ? m_pPakUpr->GetPakSystem() : 0);
}
...
bool ResourceCompiler::CompileFiles(const std::vector<RcFile>& files, const IConfig* config, CompileFilesFunction compileFiles)
{
	...
	const PakUpr::ECallResult eResult = 
		m_pPakUpr->CompileFilesIntoPaks(config, filesToConvert.m_allFiles);

	if (eResult != PakUpr::eCallResult_Skipped)
	{
		return eResult == PakUpr::eCallResult_Succeeded;
	}
	...
	for (size_t i = 0; i < filesToConvert.m_allFiles.size(); ++i)
	{
		...
		IConverter* const converter = m_extensionUpr.FindConverter(filenameForConverterSearch.c_str());
	...
	drx_sprintf(szTimeMsg, " in %.1f sec", secondsElapsed);
	...
	ICompiler* compiler)
{
...

	{
		...
		// in such cases. Also, some tools, including DrxTif exporter, call RC without
		// providing /refresh
...
void ResourceCompiler::LogLine(const IRCLog::EType eType, const char* szText)
{
	...
	if (pThreadData) 
	{
		drx_sprintf(threadString, "%d> ", pThreadData->threadId);
	}
	...
	if (m_bTimeLogging)
	{
		...
		drx_sprintf(timeString, "%2d:%02d ", minutes, seconds - (minutes * 60));
	}
...
void ResourceCompiler::InitPakUpr()
{
	delete m_pPakUpr;

	m_pPakUpr = new PakUpr(this);
	m_pPakUpr->RegisterKeys(this);
}
	...
	const char* rowSeparator) const
{
	...
	drx_strcpy(buffer, bufferSize, s.c_str());
}
...
static void GetCommandLineArguments(std::vector<string>& resArgs)
{
	...
	{
		resArgs.push_back(DrxStringUtils::WStrToUTF8(argv[i]));
	}
}
...
IAssetUpr* ResourceCompiler::GetAssetUpr() const
{
	assert(m_pAssetUpr);
	return m_pAssetUpr.get();
}
...
void ResourceCompiler::SetupMaxThreads()
{
	...
	m_pPakUpr->SetMaxThreads(m_maxThreads);
}
...
void ResourceCompiler::LogV(const IRCLog::EType eType, const char* szFormat, va_list args)
{
	...
	drx_vsprintf(str, szFormat, args);
...
void ResourceCompiler::CleanTargetFolder(bool bUseOnlyInputFiles)
{
	...
	m_pPakUpr->DeleteFilesFromPaks(config, deletedTargetFilesPak);
...
//////////////////////////////////////////////////////////////////////////
static IDrxXML* LoadIDrxXML()
{
	HMODULE hXMLLibrary = LoadLibrary("DrxXML.dll");
	if (NULL == hXMLLibrary)
	{
