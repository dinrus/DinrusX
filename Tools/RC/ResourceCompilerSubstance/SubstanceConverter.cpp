...
#undef ADD
		...
		SubstanceAir::OutputInstanceBase * outputInstance) override
	{
		...
		if (result.get() != NULL)
		{
			...
			string commandline;
			commandline.Format("/autooptimizefile=0 /preset=%s /reduce=0 /drxasset=source,%s", data->texturePreset, data->source);
...
void CSubstanceConverter::Init(const ConverterInitContext& context)
{
	...
	InitDrxSubstanceLib(fileReader); // Life-time of fileReader will be managed by substance lib.
	ISubstanceUpr::Instance()->RegisterInstanceRenderer(m_pRenderer);
...
bool CSubstanceConverter::ConvertToTiff(const string& path, const string& target)
{
	...
	SettingsUprHelpers::CFixedString<wchar_t, MAX_PATH * 3> wEnv;
