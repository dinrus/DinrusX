...
ColladaCompiler::ColladaCompiler(IDrxXML* pDrxXML, IPakSystem* pPakSystem)
:	pDrxXML(pDrxXML),
...
// -----------------------------------------------------------------------------------------------------------
// Compile collada file to Dinrus Geometry File (CGF) --------------------------------------------------------
// -----------------------------------------------------------------------------------------------------------
	...
	const string& exportFileName)
{
	...
	if (!this->pPhysicsInterface->GetGeomUpr())
	{
		return false;
	}
...
// -----------------------------------------------------------------------------------------------------------
// Compile collada file to Dinrus Character File (CHR) -------------------------------------------------------
...
// -----------------------------------------------------------------------------------------------------------
// Compile collada file to Dinrus Animation File (ANM) -------------------------------------------------------
// -----------------------------------------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------------------------------------
// Compile collada file to Dinrus Bone Animation File (CAF) --------------------------------------------------
// -----------------------------------------------------------------------------------------------------------
...
bool ColladaCompiler::Process()
{
	...
	if (!LoadColladaFile(exportFileList, materialLibraryList, m_CC.GetSourcePath(), this->pDrxXML, pPakSystem, &listener))
	{
		return false;
	}
	...
	for (int i=0; i<exportFileList.size(); i++)
	{
		...
		for (int j=0; j<numNodes; j++)
		{
			...
			if (pos == string::npos || pos > 0)
			{
				...
				drx_strcpy(pNode->name, newNodeName);
			}
		}
			...
			case EXPORT_CGF:
				{
					...
					if (!CompileToCGF(exportfile, exportFileNameExt)
						|| !m_CC.pRC->GetAssetUpr()->SaveDrxasset(m_CC.config, m_CC.GetSourcePath(), { exportFileNameExt }))
					{
						success = false;
					}
