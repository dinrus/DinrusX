...
CPhysicsInterface::EPhysicalizeResult CPhysicsInterface::Physicalize( CNodeCGF *pNodeCGF,CContentCGF *pCGF )
{
	...
	for (size_t i = 0; i < DRX_ARRAY_COUNT(pNodeCGF->physicalGeomData); ++i)
	{
...
int CPhysicsInterface::CheckNodeBreakable(CNodeCGF *pNode, IGeometry *pGeom)
{
	...
	if (!pGeom)
	{
		...
		pPhysGeom = GetGeomUpr()->LoadPhysGeometry(stmIn, pNode->pMesh->m_pPositions, pNode->pMesh->m_pIndices, 0);
	...
	if (pPhysGeom)
		GetGeomUpr()->UnregisterGeometry(pPhysGeom);
	return res;
}
...
void CPhysicsInterface::ProcessBreakablePhysics( CContentCGF *pCompiledCGF,CContentCGF *pSrcCGF )
{
	...
	char filepath[4096];
	drx_sprintf(filepath, "%sng.exe", path);

	char stlFile[1024];
	drx_sprintf(stlFile, "%stempin.stl", path);

	char volFile[1024];
	drx_sprintf(volFile, "%stempout.vol", path);

	{
		// Remove read only attribute of ng.ini
		char ngIni[1024];
		drx_sprintf(ngIni, "%sng.ini", path);
	...
	char cmd[4096];
	drx_sprintf(cmd, "%s -geofile=tempin.stl -meshfile=tempout.vol -batchmode %s", filepath, pGr);
...
bool CPhysicsInterface::DeletePhysicalProxySubsets(CNodeCGF* pNodeCGF, bool const bCga)
{
	...
	if (!bCga && !strstr(pNodeCGF->properties,"other_rendermesh") && !strstr(pNodeCGF->properties,"notaproxy"))
	{
		...
		if (bEmptyMesh)
		{
			...
			drx_strcpy(pNodeCGF->name, newName.c_str());
		}
	}
...
void CPhysicsInterface::RephysicalizeNode( CNodeCGF *pNode,CContentCGF *pCGF )
{
...

	{
		...
		for (int nPhysGeomType = 0; nPhysGeomType < DRX_ARRAY_COUNT(pNode->physicalGeomData); ++nPhysGeomType)
		{
	...
	for (int nPhysGeomType = 0; nPhysGeomType < DRX_ARRAY_COUNT(pNode->physicalGeomData); ++nPhysGeomType)
	{
...

		{
			...
			if (pNode->pMesh)
			{
				...
				pPhysGeom = GetGeomUpr()->LoadPhysGeometry(stmIn,mesh.m_pPositions,mesh.m_pIndices,pFaceMats);
			}
			else
			{
				// Load only using physics saved data.
				pPhysGeom = GetGeomUpr()->LoadPhysGeometry(stmIn,0,0,0);
			}
		}
		...
		if (pPhysGeom->pGeom->GetType() == GEOM_TRIMESH)
		{
			...
			CMemStream stm(false);
			GetGeomUpr()->SavePhysGeometry( stm,pPhysGeom );
