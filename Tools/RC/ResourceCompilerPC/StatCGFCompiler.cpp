...
static string getTextMatrix( const Matrix34& M )
{
	...
	drx_sprintf(buff, "axisX(%g %g %g)", M.m00,M.m10,M.m20);
	text += buff;
	drx_sprintf(buff, " axisY(%g %g %g)", M.m01,M.m11,M.m21);
	text += buff;
	drx_sprintf(buff, " axisZ(%g %g %g)", M.m02,M.m12,M.m22);
	text += buff;
	drx_sprintf(buff, " trans(%g %g %g)", M.m03,M.m13,M.m23);
	text += buff;
...
//	_CrtSetDbgFlag(_CRTDBG_CHECK_ALWAYS_DF | _CRTDBG_CHECK_CRT_DF | _CRTDBG_DELAY_FREE_MEM_DF | _CRTDBG_REPORT_FLAG);
	...
	try
	{
		...
		*/
		...
		if (!m_CC.pRC->GetAssetUpr()->SaveDrxasset(m_CC.config, /*m_CC.GetSourcePath()*/ string(), outputFiles))
		{
			return false;
		}
