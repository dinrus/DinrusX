...
std::vector<string> FindCgfAssetFiles(const string& filename)
{
	...
	DRX_ASSERT(MAX_STATOBJ_LODS_NUM < 10);
...
void CollectCgfDetails(XmlNodeRef& xmlnode, const CContentCGF& cgf)
{
	...
	AssetUpr::AddDetails(xmlnode, details);
	AssetUpr::AddDependencies(xmlnode, dependencies);
}
...
{
	XmlNodeRef metadata = AssetUpr::GetMetadataNode(xmlnode);
	if (!metadata)
	{
		return false;
	}
	...
	AssetUpr::AddDetails(metadata, details);
	AssetUpr::AddDependencies(metadata, dependencies);
	return true;
}
...
// Having the tag part of the drxasset means we do not have to touch the data XML file on disk.
	...
	: m_pResourceCompiler(pRc)
{
	...
	{
		pRc->GetAssetUpr()->RegisterDetailProvider(CollectCgfDetails, szExt);
	}

	pRc->GetAssetUpr()->RegisterDetailProvider(CollectMtlDetails, "mtl");
	pRc->GetAssetUpr()->RegisterDetailProvider(CollectXmlDetails, "xml");
	pRc->GetAssetUpr()->RegisterDetailProvider(CollectCdfDetails, "cdf");
}
...
bool CMetadataCompiler::Process()
{
	...
	const string metadataFilename = IAssetUpr::GetMetadataFilename(sourceFilename);
	...
	{
		// Do not create drxassets for lod files, but repair existent.
	...
	return m_pResourceCompiler->GetAssetUpr()->SaveDrxasset(m_CC.config, "", files, m_CC.GetOutputFolder());
}
