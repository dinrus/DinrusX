...
#include "../DinrusX/DinrusX3dEng/CGF/CGFSaver.h"
...
protected:
	int addMaterial(int submatIndex, const char* name, EPhysicsGeomType physicalization)
	{
		...
		if (!m_pCgfMaterial)
		{
			...
			m_bOwnMaterial = true;
			drx_strcpy(m_pCgfMaterial->name, m_materialName.c_str());
		...
		while (submatIndex >= m_pCgfMaterial->subMaterials.size())
		{
			...
			drx_strcpy(p->name, "-unused-");
			p->nPhysicalizeType = PHYS_GEOM_TYPE_NONE;
			m_pCgfMaterial->subMaterials.push_back(p);
		}

		drx_strcpy(m_pCgfMaterial->subMaterials[submatIndex]->name, name);
...
{
public:
	CSceneSaver(const ConvertContext* pContext, Scene::IScene* pScene, const Matrix34& sceneWorldToDrxWorldTM)
		: m_pContext(pContext)
		, m_pRc(pContext->pRC)
		, m_pScene(pScene)
		, m_sceneWorldToDrxWorldTM(sceneWorldToDrxWorldTM)
	{
		assert(pContext);
		assert(m_pRc);
		assert(m_pScene);
...
protected:
	...
	bool SaveToCgf(
		...
		bool bAsIsScene)
	{
		// _EM_INVALID is used to avoid Floating Point Exception inside DinrusXPhys
...
protected:
	...
	Matrix34 m_sceneWorldToDrxWorldTM;  // orthonormal
};
...
public:
	...
	CAsIsResultingScene(const ConvertContext* pContext, Scene::IScene* pScene, const Matrix34& sceneWorldToDrxWorldTM, const float scale)
		: CSceneSaver(pContext, pScene, sceneWorldToDrxWorldTM)
		, m_scale(scale)
	{
	}
	...
	bool Save(const string& outputCgfFilename, const CImportRequest& ir)
	{
...

		{
			...
			drx_strcpy(pExportInfo->rc_version_string, "fbx scene import");
		}
...
private:
		...
		ESceneType sceneType) const
	{
		...
		for (int i = 0; i < (int)m_nodes.size(); ++i)
		{
			...
			drx_strcpy(pCGFNode->name, ni.pName->c_str());
		...
		const std::vector<int>& mapSceneNodesToBoneId) const
	{
		...
		proxyNodeInfo.worldOriginal = m_sceneWorldToDrxWorldTM * pProxyNode->worldTransform;
		...
		ESceneType sceneType) const
	{
		...
		for (const auto& node : m_nodes)
		{
			...
			DrxBoneDescData& boneDesc = pSkinningInfo->m_arrBonesDesc[boneID];
			memset(&boneDesc, 0, sizeof(boneDesc));
			drx_strcpy(boneDesc.m_arrBoneName, node.pName->c_str());
			boneDesc.m_nControllerID = SkeletonHelpers::ComputeControllerId(node.pName->c_str());

			if (node.parentNode == -1)
			{
				// DinrusX expects that root nodes have identity orientation. 
		...
		const std::vector<int>& mapSceneNodesToBoneId)
	{
		...
		std::vector<DrxKeyPQS> frames(frameCount);
		...
		for (size_t i = 0; i < m_nodes.size(); ++i)
		{
			...
			for (int frame = 0; frame < frameCount; ++frame)
			{
				...
				if (saveNodeAnimation)
				{
					...
					nodeTransform = m_sceneWorldToDrxWorldTM * Matrix34::Create(trs.scale, trs.rotation, trs.translation);
					...
					if (transformToLocalSpace)
					{
						...
						parentTransform = m_sceneWorldToDrxWorldTM * Matrix34::Create(trs.scale, trs.rotation, trs.translation);
					}
				}
	...
	bool CollectNodes(ESceneType sceneType, const CImportRequest& ir)
	{
		...
		for (size_t i = 1, j = 0; i < m_pScene->GetNodeCount(); ++i)
		{
			...
			ni.worldOriginal = m_sceneWorldToDrxWorldTM * pNode->worldTransform;
		...
		for (const auto& node : m_nodes)
		{
			// Please see Code\DinrusX\DinrusX3dEng\StatObjLoad.cpp - CStatObj::LoadCGF_Int.
	...
	{
		// DRXENGINE has an optimization that ignores transform (assumes it's identity)
		...
		for (size_t i = 0; i < m_nodes.size(); ++i)
		{
			...
			// DinrusX expects that the following nodes have identity orientation:
			// - all root cgf nodes.
...
// Compute coordinate system transform to convert everything to DinrusX's CS.
bool ComputeSceneAxisTransform(
	Matrix34& axesTM,
	const Scene::IScene* const pScene,
	const CImportRequest& ir)
{
	const string drxEngineForwardUp = "-Y+Z";
	...
	RCLog("Converting scene's forward & up coordinate axes '%s' to DinrusX's '%s'", sceneForwardUp.c_str(), drxEngineForwardUp.c_str());

	const char* const err = TransformHelpers::ComputeForwardUpAxesTransform(axesTM, sceneForwardUp.c_str(), drxEngineForwardUp.c_str());
	if (err)
	{
	...
	const CImportRequest& ir)
{
	...
	double sourceUnitSizeInCm;
	{
		...
		for (int i = 0; i < DRX_ARRAY_COUNT(units); ++i)
		{
...
bool CFbxConverter::Process()
{
	...
	return m_CC.pRC->GetAssetUpr()->SaveDrxasset(m_CC.config, sourceSceneFilename, { outputFilename });
}
