...
public:
	DrxFbxMesh(bool bGenerateTextureCoordinates)
		: m_pFbxMesh(nullptr)
		, m_pFbxNode(nullptr)
		, m_bMeshReady(false)
		, m_pMesh(nullptr)
...
public:
	...
	bool LoadScene(const char* fbxFilename)
	{
		...
		FbxIOSettings* const pSettings = FbxIOSettings::Create(m_pFbxUpr, IOSROOT);
		...
		m_pFbxUpr->SetIOSettings(pSettings);

		FbxImporter* const pImporter = FbxImporter::Create(m_pFbxUpr, "");
		...
		if (!pImporter->Initialize(fbxFilename, -1, m_pFbxUpr->GetIOSettings()))
		{
		...
		m_pFbxScene = FbxScene::Create(m_pFbxUpr, "myScene");
...
struct DrxFbxSceneImpl
{
	...
	std::vector<DrxFbxNode> m_nodes;
	std::vector<DrxFbxMesh> m_meshes;

	std::vector<DrxFbxMaterial> m_materials;
	...
	bool PrepareBones()
	{
		bool bHasSkin = std::any_of(m_meshes.begin(), m_meshes.end(), [](const DrxFbxMesh& mesh)
		{
...
DrxFbxScene::DrxFbxScene()
	: m_pImpl(new DrxFbxSceneImpl())
{
	assert(m_pImpl);
}
...
void DrxFbxScene::Reset()
{
	m_pImpl->Reset();
}

bool DrxFbxScene::LoadScene(const char * fbxFilename, bool bVerboseLog, bool bCollectSkinningInfo, bool bGenerateTextureCoordinates)
{
...
const char* DrxFbxScene::GetForwardUpAxes() const
{
	return &m_pImpl->m_forwardUpAxes[0];
}


double DrxFbxScene::GetUnitSizeInCentimeters() const
{
	return m_pImpl->m_unitSizeInCentimeters;
}


int DrxFbxScene::GetNodeCount() const
{
	return (int)m_pImpl->m_nodes.size();
}

const Scene::SNode* DrxFbxScene::GetNode(int idx) const
{
...
Scene::STrs DrxFbxScene::EvaluateNodeLocalTransform(int nodeIdx, int frameIndex) const
{
	Scene::STrs trs;
...
Scene::STrs DrxFbxScene::EvaluateNodeGlobalTransform(int nodeIdx, int frameIndex) const
{
	Scene::STrs trs;
...
int DrxFbxScene::GetMeshCount() const
{
	return (int)m_pImpl->m_meshes.size();
}

const MeshUtils::Mesh* DrxFbxScene::GetMesh(int idx) const
{
...
const Scene::SLinksInfo* DrxFbxScene::GetMeshLinks(int idx) const
{
	if (!GetMesh(idx))
	{
		return nullptr;
...
int DrxFbxScene::GetMaterialCount() const
{
	return (int)m_pImpl->m_materials.size();
}

const Scene::SMaterial* DrxFbxScene::GetMaterial(int idx) const
{
...
int DrxFbxScene::GetAnimationStackCount() const
{
	return (int)m_pImpl->m_animations.size();
}
const Scene::SAnimation* DrxFbxScene::GetAnimationStack(int idx) const
{
...
bool DrxFbxScene::SetCurrentAnimationStack(int idx)
{
	if (idx < 0 || idx >= GetAnimationStackCount())
	{
		return false;
...
bool DrxFbxScene::LoadScene_StaticMesh(const char* fbxFilename, bool bVerboseLog, bool bGenerateTextureCoordinates)
{
	Reset();
		...
		for (int i = 0; i < m_pImpl->m_nodes[parent].m_pFbxNode->GetChildCount(); ++i)
		{
			...
			for (int j = 0; j < pFbxNode->GetNodeAttributeCount(); ++j)
			{
				...
				if (pAttribute->GetAttributeType() == FbxNodeAttribute::eMesh)
				{
					...
					DrxFbxMesh mesh(bGenerateTextureCoordinates);
