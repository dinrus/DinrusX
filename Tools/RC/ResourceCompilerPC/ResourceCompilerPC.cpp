...
void __stdcall RegisterConverters(IResourceCompiler* const pRC)
{
	...
	IDrxXML* const pDrxXML = LoadIDrxXML();
	...
	{
		pRC->RegisterConverter("CharacterCompiler", new CharacterCompiler(pDrxXML));
		pRC->RegisterConverter("AnimationConverter", new CAnimationConverter(pDrxXML, pRC->GetPakSystem(), pRC));
		pRC->RegisterConverter("ColladaCompiler", new ColladaCompiler(pDrxXML, pRC->GetPakSystem()));
	}
	...
	pRC->RegisterKey("stripMetadata", "[drxasset] Strip metadata chunks from the output files");
	pRC->RegisterKey("assetTypes", "[drxasset] Use it to specify a mapping between file extensions end asset typenames.\n"
		"syntax is /assettypes=<pair[; pair[; pair[...]]]> where\n"
		"<pair> is <ext>,<assetTypeName>.\n"
		"Example: /assettypes=cgf,Geometry;chr,Character");
	pRC->RegisterKey("drxasset", "Use it to overwrite drxasset metadata.\n"
		"syntax is /drxasset=<pair[; pair[; pair[...]]]> where\n"
		"<pair> is <key>,<value>.\n"
		"Allowed keys:\n"
