...
//
//	Dinrus Character Animation source code
//	
//	
//  Contains:
...
bool CAnimationCompressor::ProcessAimPoses(bool isLook, GlobalAnimationHeaderAIM& rAIM) 
{
	...
	for (uint32 i=0; i<numRot; i++)
	{
		...
		if (idx<0)
		{
			RCLogError("DinrusXAnimation: Aim/Look-pose '%s' disabled. Model doesn't have Jointname '%s'",rAIM.m_FilePath.c_str(),rRot[i].m_strJointName );
			return false;
		}
	}
	...
	{
		RCLogError("DinrusXAnimation: Aim/Look-pose '%s' disabled. Invalid number of poses: %i (expected 9, 15 or 21)",rAIM.m_FilePath.c_str(),int(numPoses));
		return false;
	}
	...
	for (uint32 r=0; r<numRotJoints; r++)
	{
		...
		{
			RCLogError("DinrusXAnimation: Aim/Look-pose '%s' disabled. ParameterJoint '%s' not marked as primary",rAIM.m_FilePath.c_str(),strDirectionJointName);
			return false;
		}
	}
	...
	if (found==0)
	{
		RCLogError("DinrusXAnimation: Aim/Look-pose '%s' disabled. ParameterJoint '%s' not defined in rotation list",rAIM.m_FilePath.c_str(),strDirectionJointName);
		return false;
	}
