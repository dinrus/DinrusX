...
#include <DinrusX/DinrusX3dEng/ChunkData.h>
...
//////////////////////////////////////////////////////////////////////////
CAnimationConverter::CAnimationConverter(IDrxXML* pXMLParser, IPakSystem* pPakSystem, IResourceCompiler* pRC)
...
void CAnimationConverter::InitSkeletonUpr(const std::set<string>& usedSkeletons)
{
	m_skeletonUpr.reset(new SkeletonUpr(m_pPakSystem, m_pXMLParser, m_rc));
	...
	m_skeletonUpr->LoadSkeletonList(skeletonListPath, m_sourceGameFolderPath, usedSkeletons);
}
...
static string GetDbaTableFilename(const string& sourceGameFolderPath, const string& configSubfolder)
{
	...
	{
		RCLogError(
			"DrxENGINE 3.8.5 and higher don't support the DBATable.xml file. Instead, DrxENGINE uses the\n"
			"DBATable.json file. Please refer to the online documentation for the conversion instructions:\n"
			"http://docs.drxengine.com/display/SDKDOC1/EaaS+3.7.0#EaaS3.7.0-DBATableConversion");
	}
	else
	{
...
bool CAnimationConverter::RebuildDatabases()
{
	...
	for (size_t dbaIndex = 0; dbaIndex < dbaCount; ++dbaIndex)
	{
		...
		for (size_t anim = 0; anim < dba.animationCount; ++anim)
		{
			...
			if (isAIM)
			{
				...
				{
					if (!animationUpr.AddAIMHeaderOnly(gaim))
					{
			...
			if (!animationUpr.AddCAFHeaderOnly(globalAnimationHeader))
			{
...

	{
		...
		RCLog("Looking for non dba animations for creating animation .img files.");
		{
			...
			for(size_t i = 0; i < cafFileList.size(); ++i)
			{
				...
				const CSkeletonInfo* pSkeleton = m_skeletonUpr->LoadSkeleton(animDesc.m_skeletonName);
				if (!pSkeleton)
				{
				...
				if (isLook || isAIM)
				{
					...
					{
						if (!animationUpr.HasAIMHeader(gaim))
						{
							if (!animationUpr.AddAIMHeaderOnly(gaim))
							{
								RCLogError("AIM header for '%s' already exists in CAnimationUpr.", 
														animationPath.c_str());
							}
						}
					}
				...
				else
				{
					...
					if (!animationUpr.HasCAFHeader(globalAnimationHeader))
					{
						if (!dbaPath.empty())
						{
						...
						{
							animationUpr.AddCAFHeaderOnly(globalAnimationHeader);
						}
					}
				}
			}
	...
	if (!animationUpr.SaveCAFImage( sAnimationsImgFilename, latest, bigEndianOutput))
	{
	...
	if (!animationUpr.CanBeSkipped())
	{
		...
		if (!animationUpr.SaveAIMImage(sDirectionalBlendsImgFilename, latest, bigEndianOutput))
		{
...
static bool GetFromAnimSettings(SAnimationDesc* desc, const CSkeletonInfo** skeleton, bool* pbErrorReported, const char* sourcePath, const char* animationName, IPakSystem* pakSystem, IDrxXML* xmlParser, SkeletonUpr* skeletonUpr)
{
	...
	*skeleton = skeletonUpr->FindSkeleton(desc->m_skeletonName);
	if (!*skeleton)
	{
...
static void ProcessAnimationJob(AnimationJob* job)
{
	...
	if (fileChanged || failedToLoadCompressed)
	{
		...
		if (writeDest)
		{
			...
			context.pRC->GetAssetUpr()->SaveDrxasset(context.config, sourcePath, { destPath });
		}
	...
	if (context.pRC->GetVerbosityLevel() >= 1)
	{
		...
		if (fileChanged || failedToLoadCompressed )
		{
			drx_sprintf(processTime, "in %.1f sec", float(durationMS) * 0.001f);
		}
		else
		{
			drx_strcpy(processTime, "is up to date");
		}
