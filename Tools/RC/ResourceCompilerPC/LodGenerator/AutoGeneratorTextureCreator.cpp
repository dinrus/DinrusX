...
#include "IEditor.h"
#include "Material/MaterialUpr.h"
#include <DinrusX/CoreX/Renderer/IMeshBaking.h>
...
namespace LODGenerator 
{
	...
	bool CAutoGeneratorTextureCreator::GenerateTexture(int level)
	{
		...
		if(bUseAutoTextureSize)
		{
			...
			if(mesh)
			{
				...
				DrxLog("AutoTextureSize Object Radius: %f, Texture Size: %d",objectSize,textureSize);
			}
			nTWidth = textureSize;
		...
		for ( int idx = 0; idx < submatCount && idx < MAX_MAT_SLOTS; ++idx)
		{
			...
			{
				if( DrxMessageBox("Detected overlapping UVs in target mesh. This could cause artifacts. Continue anyway?", "UV Layout",MB_YESNO|MB_ICONQUESTION ) == IDNO)
					return false;
			}
		}
	...
	void CAutoGeneratorTextureCreator::AssignToMaterial(const int nLod, const bool bAlpha, const bool bSaveSpec, const CString& exportPath, const CString& fileNameTemplate)
	{
		...
		if(pMtl)
		{
			...
			if(nSubMatId == -1)
			{
				DrxLog("Error in AssignToMaterial: nSubMatId is -1");
				return;
			}

			CMaterial * pTemplateMaterial = GetIEditor()->GetMaterialUpr()->LoadMaterial("Editor/Materials/lodgen_template.mtl");
			...
			if(!pMtl->Save(false))
			{
				DrxMessageBox("The material file cannot be saved. The file is located in a PAK archive or access is denied", "Material File Save Failed", MB_OK | MB_APPLMODAL | MB_ICONWARNING);
			}
