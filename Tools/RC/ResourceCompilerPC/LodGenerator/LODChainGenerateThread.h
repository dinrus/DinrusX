...
namespace LODGenerator 
{
	...
	public:
		...
		LODChainGenerateThread()
		{
			...
			if (!gEnv->pThreadUpr->SpawnThread(this, "LODChainGenerateThread"))
			{
				DrxFatalError("Error spawning \"LODChainGenerateThread\" thread.");
			}
		}
		...
		void CreateLodders(const CLODGeneratorLib::SLODSequenceGenerationInputParams *pInputParams, CLODGeneratorLib::SLODSequenceGenerationOutputList *pReturnValues)
		{
			...
			for(int i = 0; i<nodeCout; ++i)
			{
				...
				if (!pSubReturnValues)
				{
					DrxLog("LODGen: Failed to allocate SubObjects Generation Output, possible out of memory problem");
					return;
				}
				...
				if ( !pLodder )
				{
					DrxLog("LODGen: Failed to allocate visual change calculator, possible out of memory problem");
					return;
				}
			...
			while (true)
			{
				...
				case TASK_GENERATELODCHAIN:
					...
					for(std::vector<VisualChangeCalculator *>::iterator lodder = m_Lodders.begin(), end = m_Lodders.end(); lodder != end; ++ lodder)
					{
						...
						{
							DrxLog("LODGen: Failed to allocate numPositions '%d' while generating lod chain, possible out of memory problem", pLodder->m_pReturnValues->numPositions);
						...
						{
							DrxLog("LODGen: Failed to allocate numIndices '%d' while generating lod chain, possible out of memory problem", pLodder->m_pReturnValues->numIndices);
						...
						{
							DrxLog("LODGen: Failed to allocate numMoves '%d' while generating lod chain, possible out of memory problem", pLodder->m_pReturnValues->numMoves);
