...
// 		}
	...
	void AutoUV::Prepare(Vec3 *pVertices, vtx_idx *indices, int faces)
	{
		...
		{
			DrxLog("AutoUV: Input mesh has an %d edges shared by more than two faces. They were split.\n",nSplitEdgesCount);
		}
		DrxLog("AutoUV: Total faces:%d vertices:%d edges:%d\n", faces, m_vertices.size(), m_edges.size());
	}
		...
		while (true)
		{
			...
			while (!stack.empty())
			{
				...
				else
				{
					if (lastDone==done)
					{
						DrxLogAlways("ERROR: AutoUV couldn't deal with unwrap at all for some weird reason %p\n", startingEdge);
