...
namespace LODGenerator 
{
	...
	{
		CMaterial * templateMaterial = GetIEditor()->GetMaterialUpr()->LoadMaterial("Editor/Materials/lodgen_template.mtl");
		if (templateMaterial)
			templateMaterial->AddRef();

		CMaterial * pNewMaterial = (CMaterial*)GetIEditor()->GetMaterialUpr()->FindItemByName(matName);
		if (!pNewMaterial)
			pNewMaterial = GetIEditor()->GetMaterialUpr()->DuplicateMaterial(matName,templateMaterial);
