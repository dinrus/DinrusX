	...
	public:
		...
		void Update(int currentMoveId, float expectedErrorChange)
		{
			...
			for (int phase=0; phase<2; phase++)
			{
				...
				DrxLog("%d: Rasterisation Time:%fms\n", phase, dt.GetMilliSeconds());
#endif
				if (phase==0)
				{
					// Patch moves
			...
			if (pReturnValues->node != NULL)
			{
				...
				if (pMesh)
				{
					...
					if (m_bCheckTopology)
					{
						...
						{
							DrxWarning(VALIDATOR_MODULE_RENDERER, VALIDATOR_WARNING, "LOD generator input mesh doesn't meet topology rules. Found %d violations.\n",nTopologyErrors);
						}
					}
						...
						for (std::vector<Move>::const_iterator it=m_moves.begin(), end=m_moves.end(); it!=end; ++it)
						{
							if (it->from!=it->to)
							{
								...
								if (it->from==pit->v[i] && it->to==pit->v[(i==2)?0:(i+1)])
								{
									if (found)
									{
										DrxLogAlways("Duplicate move %d->%d\n", it->from, it->to);
										//it->from=it->to;
									}
									found=true;
								}
