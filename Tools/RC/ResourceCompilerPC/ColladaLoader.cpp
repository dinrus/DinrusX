...
#include "../../DrxXML/Xml/xml.h"
#include "../../DinrusX/DinrusX3dEng/MeshCompiler/MeshCompiler.h"
#include "../../DinrusX/DinrusX3dEng/MeshCompiler/TransformHelpers.h"
...
bool ColladaLoaderImpl::Load(std::vector<ExportFile>& exportFiles, std::vector<sMaterialLibrary>& materialLibraryList, const char* szFileName, IDrxXML* pDrxXML, IPakSystem* pPakSystem, IColladaLoaderListener* pListener)
{
	XmlNodeRef root = this->LoadXML(szFileName, pPakSystem, pDrxXML, pListener);
	if (root == 0)
		return false;
...
XmlNodeRef ColladaLoaderImpl::LoadXML(const char* szFileName, IPakSystem* pPakSystem, IDrxXML* pDrxXML, IColladaLoaderListener* pListener)
{
	ReportInfo(pListener, "Loading XML...");

	// Get the xml serializer.
	IXMLSerializer* pSerializer = pDrxXML->GetXMLSerializer();
...
string ColladaLoaderImpl::ReadXmlNodeUserProperties(XmlNodeRef node, IColladaLoaderListener* pListener)
{
	...
	if (node_extra)
	{
		...
		if (node_technique && node_technique->haveAttr("profile") && (str_profile = node_technique->getAttr("profile")) && (strcmp(str_profile,"DinrusX")==0))
		{
...
static bool ExtractDrxExportNodeParams(const string& nodeID, const string& properties, DrxExportNodeParams& nodeParams)
{
	if (StringHelpers::MakeLowerCase(nodeID.substr(0,DRXEXPORTNODE_LENGTH)) != DRXEXPORTNODE_LOWERCASE)
	{
		return false; 
	}
	
	nodeParams.fullName = nodeID;
	nodeParams.nodeName = nodeID.substr(DRXEXPORTNODE_LENGTH, string::npos);
...
void ColladaLoaderImpl::ParseSceneDefinition(SceneMap& scenes, XmlNodeRef sceneNode, const GeometryMap& geometries, const ControllerMap& skinControllers, const MaterialMap& materials, const SceneTransform& sceneTransform, IColladaLoaderListener* pListener)
{
	...
	std::vector<ColladaScene*> drxScenes;			// collection of DrxExportNode scenes
	...
	bool drxExportNodeProcess = false;				// true if DrxExportNode processing
	...
	while (!nodeStack.empty())
	{
		...
		{
			// If this scene in DrxExportNode hierarchy then store it
			if (nodeStack.back().drxExportNode)
			{
				drxExportNodeProcess = false;

				if (drxScenes.back())
				{
					scenes.insert(std::make_pair(parentNode->getAttr("id"), drxScenes.back()));
					ColladaScene * pScene = drxScenes.back();
					if ( pScene )
					{
		...
		else
		{
			...
			if (nodeTag == "node")
			{
				...
				if (safeBoneNameLower.substr(safeBoneNameLower.size() - DRXPHYSBONESUFFIX_LENGTH, DRXPHYSBONESUFFIX_LENGTH) == DRXPHYSBONESUFFIX)
				{
					string boneName = safeBoneNameLower.substr(0, safeBoneNameLower.size() - DRXPHYSBONESUFFIX_LENGTH - 1); // Subtract an extra 1 to skip the character between the name and 'phys'.
				...
				if (safeBoneNameLower.substr(safeBoneNameLower.size() - DRXPHYSPARENTFRAMESUFFIX_LENGTH, DRXPHYSPARENTFRAMESUFFIX_LENGTH) == DRXPHYSPARENTFRAMESUFFIX)
				{
					string boneName = safeBoneNameLower.substr(0, safeBoneNameLower.size() - DRXPHYSPARENTFRAMESUFFIX_LENGTH - 1);
				...
				// DrxExportNode check
				if (id_attr_lower.substr(0,DRXEXPORTNODE_LENGTH) == DRXEXPORTNODE_LOWERCASE)
				{
					if (drxExportNodeProcess)
					{
						ReportError(pListener, LINE_LOG" Cannot parse '%s'. It is in another DrxExportNode. Scene skipped.", nodeNode->getLine(), id_attr.c_str());
						return;
					}

					DrxExportNodeParams nodeParams;
					if (!ExtractDrxExportNodeParams(id_attr_lower,properties,nodeParams))
					{
						ReportError(pListener, LINE_LOG" Cannot parse '%s'. Invalid DrxExportNode 'id' format. Scene skipped.", nodeNode->getLine(), id_attr.c_str());
						return;
					}
					
					drxExportNodeProcess = true;
					nodeStack.back().drxExportNode = true;

					ColladaScene* scene = new ColladaScene(nodeParams);

					drxScenes.push_back(scene);

					ReportInfo(pListener, "Processing '%s'", id_attr.c_str());

					// Don't export the DrxExportNode.
					continue;
				}
				...
				if (!drxScenes.empty() && drxExportNodeProcess)
				{
					drxScenes.back()->AddNode(colladaNode);

					NameNodeMap * nodeNameMap = drxScenes.back()->GetNodesNameMap();
...
static bool SplitMaterialName(const string& name, int& id, string& actualName, CCustomMaterialSettings& settings)
{
	...
	// Check whether there is an between the number and the name, and if so skip it.
	...
	while (*position != 0)
	{
		...
		// Add the string up until the next to the name.
...
static void debugWriteMesh(const MeshUtils::Mesh& mesh, const std::vector<int>& mapping, const char* meshFilename, int meshIdx)
{
	...
	drx_sprintf(filename, meshFilename, meshIdx);
...
ColladaNode::ColladaNode(const string& a_id, const string& a_name, ColladaNode* const a_parent, int const a_nodeIndex)
{
	...
	// Replace any possible special name prefix to DinrusX's internal "$" prefix
	{
...
static void RefineProperties(string& prop)
{
	...
	for (size_t i=0; i<parts.size(); ++i)
	{
		if (!parts[i].empty())
		{
			...
			else if (StringHelpers::StartsWithIgnoreCase(parts[i], "drx_"))
			{
	...
	int contentFlags)
{
	...
	{
		ReportError(pListener, "DrxExportNode's node name is missing.");
		return NULL;
	}
	...
	// Set transformation of DrxExportNode
	{
		int rootCount = 0;
		...
		// Fixing orientation of world transform matrices
		{
			...
			for (int i = 0; i < cgfNodes.size(); ++i)
			{
				...
				if (node.parentCGFNodeIndex < 0)
				{
					// DinrusX expects that all root cgf nodes have identity orientation
			...
			// DinrusX ignores transforms of LOD nodes (all LOD nodes are expected
...
void ColladaScene::AddBoneRecursive(ColladaNode *node, int parent_id, CSkinningInfo *pSkinningInfo, const ColladaController* controller, const NameNodeMap& nodes, const std::map<string, int>& boneNameIndexMap)
{
	...
	if (nodes.find(node->GetID()) != nodes.end())
	{
		...
		drx_strcpy(bone_ent.prop, node->GetProperty().c_str());
	...
	for (int nodeIndex = 0, nodeCount = int(this->nodes.size()); nodeIndex < nodeCount; ++nodeIndex)
	{
		...
		for (int i=0; i<numJoints; i++)
		{
			...
			if (jointNodePos != nameNodeMap.end())
			{
				...
				drx_strcpy(boneDesc.m_arrBoneName, boneSafeName.c_str());
...
bool ColladaScene::SetIKParams(CContentCGF* cgf, IColladaLoaderListener* pListener)
{
	...
	for (int entityIndex = 0, entityCount = int(pSkinningInfo->m_arrBoneEntities.size()); entityIndex < entityCount; ++entityIndex)
	{
		...
		DrxBoneDescData& desc = pSkinningInfo->m_arrBonesDesc[e.BoneID];
		...
		if (physBonePos != this->physBones.end())
		{
			...
			typedef std::map<PhysBone::Axis, float> PhysBone::*PhysBoneAxisMapPtr;
			static const PhysBoneAxisMapPtr maps[] = {&PhysBone::dampings, &PhysBone::springAngles, &PhysBone::springTensions};
			typedef float (DrxBonePhysics_Comp::*DOFArrayPtr)[3];
			static const DOFArrayPtr arrays[] = {&DrxBonePhysics_Comp::damping, &DrxBonePhysics_Comp::spring_angle, &DrxBonePhysics_Comp::spring_tension};
...
static inline float DrxQuatDotProd(const DrxQuat& q, const DrxQuat& p)
{
...
CInternalSkinningInfo* ColladaScene::CreateControllerTCBSkinningInfo(CContentCGF* pCGF, const ColladaAnimClip* pAnimClip, const SceneTransform& sceneTransform, IColladaLoaderListener* pListener)
{
	...
	for (int nodeIndex = 0; nodeIndex < nodes.size(); nodeIndex++)
	{
		...
		if (numPosKeys > 0)
		{
			DynArray<DrxTCB3Key>* pPositionTrack = new DynArray<DrxTCB3Key>;
		...
		if (numRotKeys > 0)
		{
			DynArray<DrxTCBQKey>* pRotationTrack = new DynArray<DrxTCBQKey>;
			...
			for (int i = 0; i < numRotKeys; i++)
			{
				...
				DrxQuat rotation(Ang3(angles.x,angles.y,angles.z));
				rotation = !rotation;

				DrxQuat relRotation;
				DrxQuat rotationAxisAngle;
		...
		if (numSclKeys > 0)
		{
			DynArray<DrxTCB3Key>* pScaleTrack = new DynArray<DrxTCB3Key>;
	...
	for (int nodeIndex = 0, nodeCount = int(this->nodes.size()); nodeIndex < nodeCount; ++nodeIndex)
	{
		...
		for (int jointIndex = -1, jointCount = (controller ? controller->GetNumJoints() : 0); jointIndex < jointCount; ++jointIndex)
		{
			...
			for (int keyIdx=0; keyIdx<numKeys; keyIdx++)
			{
				...
				DrxQuat quat(Ang3(angle.x, angle.y, angle.z));

				quat = !quat;

				if (DrxQuatDotProd(lastQuat, quat) >= 0)
				{
					lastQuat = quat;
				}
				else
...
void ColladaScene::CreateMaterial(CGFMaterialMap& cgfMaterials, const NameNodeMap& nameNodeMap, NodeMaterialMap& nodeMaterialMap, IColladaLoaderListener* pListener)
{
	...
	for (LibraryMap::const_iterator libraryPos = libraries.begin(), libraryEnd = libraries.end(); libraryPos != libraryEnd; ++libraryPos)
	{
		...
		drx_strcpy(material->name, libraryName);
		...
		for (MaterialCreationMap::iterator itMaterialCreationEntry = materialCreationMap.begin(); itMaterialCreationEntry != materialCreationMap.end(); ++itMaterialCreationEntry)
		{
			...
			drx_strcpy(subMaterial->name, entry.name.c_str());
...
bool LoadColladaFile(std::vector<ExportFile>& exportFiles, std::vector<sMaterialLibrary>& materialLibraryList, const char* szFileName, IDrxXML* pDrxXML, IPakSystem* pPakSystem, IColladaLoaderListener* pListener)
{
	pDrxXML->AddRef();
	bool success = ColladaLoaderImpl().Load(exportFiles,materialLibraryList,szFileName, pDrxXML, pPakSystem, pListener);
	pDrxXML->Release();
	return success;
}
