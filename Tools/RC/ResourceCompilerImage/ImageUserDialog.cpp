...
#include <DinrusX/DinrusXSys/resource.h>               // IDD_ ...
...
void CImageUserDialog::UpdateProgressBar(float fProgress)
{
	...
	char inszTxt[8];
	drx_sprintf(inszTxt, "%3.0f%%", fProgress * 100.0f);
...
void CImageUserDialog::Draw(HDC inHdc)
{
	...
	{
		char str[32];
		drx_sprintf(str,"(%dx%d)", original->GetWidth(0), original->GetHeight(0));
	...
	case WM_INITDIALOG:
		{
			...
			for (int i = Reduce::kMinReduce; i <= Reduce::kMaxReduce; ++i)
			{
				...
				drx_sprintf(str, "%u x %u  (%i)", dwWidth, dwHeight, i);
