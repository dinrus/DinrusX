...
bool CImageCompiler::SaveOutput(const ImageObject* pImageObject, const char* szType, const char* lpszPathName)
{
	...
	return m_CC.pRC->GetAssetUpr()->SaveDrxasset(m_CC.config, m_CC.GetSourcePath(), { lpszPathName });
}
...
bool CImageCompiler::ProcessImplementation()
{
	...
	if (m_CC.config->HasKey("savesettings"))
	{
		...
		const bool isDrxTif2012 = m_CC.config->GetAsBool("DrxTIF2012", false, true);

		return ImageTIFF::UpdateAndSaveSettingsToTIF(sourceFile.c_str(), GetOutputPath().c_str(), settings, isDrxTif2012);
	}
