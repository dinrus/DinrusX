...
void __stdcall RegisterConverters(IResourceCompiler* pRC)
{
	...
	// image formats
	{
		...
		pRC->RegisterKey("cleansettings","[TIF] remove obsolete/unneeded items from DrxTIF settings stored in the TIF file(s)");
		pRC->RegisterKey("showsettings","[TIF] print DrxTIF settings stored in the TIF file(s)");
		pRC->RegisterKey("savesettings","[TIF] replace DrxTIF settings stored in the TIF file(s) by specified string");
	...
	{
		pRC->RegisterKey("DrxTIF2012", 
			"[TIF] Preventing interpretation of old DrxTif 16-bit float images as 16-bit uint images\n"
			"      0 - disabled (default)\n"
			"      1 - DrxTIF 2012 compatibility mode"
		);
	}
}
