...
public:
	...
	void ConvertReduceStringToMap(const string& src, ReduceMap& resReduce) const
	{
		...
		if (bExplicitPlatforms)
		{
			...
			for (int i = 0; i < (int)parts.size(); ++i)
			{
				...
				if (resReduce.find(nameAndValue[0]) == resReduce.end())
				{
					...
					drx_strcpy(ri.platformName, nameAndValue[0].c_str());
			...
			for (int i = 0; i < knownPlatformCount; ++i)
			{
				...
				{
					ReduceItem ri;
					drx_strcpy(ri.platformName, name.c_str());
					ri.platformIndex = i;
		...
		else
		{
			...
			// that the platforms x360, ps3, wiiu are not supported by the DinrusX anymore -
	...
	static const char* GetDefaultProperty(int index)
	{
		...
		// 1) The key/value pairs below will be removed from DrxTIF strings stored in files, so
