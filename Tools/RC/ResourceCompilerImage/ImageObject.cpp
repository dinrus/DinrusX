...
{
	fwrite("CExt", 4, 1, out);  // marker for the start of Dinrus Extended data
	...
	fwrite("CEnd", 4, 1, out);  // marker for the end of Dinrus Extended data
...
bool ImageObject::LoadExtendedData(FILE *in, bool bForceDX10)
{
	...
	fread(Marker, 4, 1, in); 
	if (!strcmp("CExt", Marker)) // marker for the start of Dinrus Extended data
	{
		fread(Marker, 4, 1, in);  
		...
		if (!strcmp("CEnd", Marker)) // marker for the end of Dinrus Extended data
		{
			fread(Marker, 4, 1, in); 
		}
	}
