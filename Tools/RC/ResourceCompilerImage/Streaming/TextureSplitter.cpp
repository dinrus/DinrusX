...
$DateTime$
Описание:  Routine for creation of streaming pak
...
#define DRX_MAKEFOURCC(ch0, ch1, ch2, ch3)                              \
...
bool CTextureSplitter::LoadTexture( const char* fileName, std::vector<uint8>& fileContent )
{
	...
	// check for valid DinrusX DDS file
	if (pFileDesc->dwMagic != DRX_MAKEFOURCC('D','D','S',' ') || 
	...
	const uint32 nChunkNumber, std::vector<uint8>* pOutChunk )
{
	...
	if(nChunkNumber == 0)
	{
		...
		if(!(resource.m_nFileFlags & eStreamableResourceFlags_DDSIsAttachedAlpha))
		{
			*reinterpret_cast<DWORD*>(&(*pOutChunk)[0]) = DRX_MAKEFOURCC('D','D','S',' ');
			offset = sizeof(DWORD);
		}
...
bool CTextureSplitter::AddResourceToAdditionalList( const char* fileName, const std::vector<uint8>& fileContent )
{
	...
	if (!m_CC.pRC->GetAssetUpr()->SaveDrxasset(m_CC.config, fileName, { sFullDestFileName }))
	{
		return false;
	}
...
bool CTextureSplitter::Process()
{
	...
	if (!resources[0].m_chunks.empty())
	{
		...
		if (!m_CC.pRC->GetAssetUpr()->SaveDrxasset(m_CC.config, sInputFile, savedFiles))
		{
			return false;
		}
	}
...
void CTextureSplitter::ProcessPlatformSpecificConversions(std::vector<STexture>& resourcesOut, byte* fileContent, const size_t fileSize)
{
	...
	if (header.dwTextureStage != DRX_MAKEFOURCC('F','Y','R','C'))
	{
		header.dwReserved1 = 0;
		header.dwTextureStage = DRX_MAKEFOURCC('F','Y','R','C');
	}
