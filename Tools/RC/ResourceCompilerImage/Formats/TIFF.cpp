...
// Note that a similar function exists in DrxTIFPlugin.cpp
// of the new DrxTif plugin (Jan 2013).
static bool IsFloatTif(TIFF* const tif, const bool isDrxTif2012)
{
	...
	if (dwBitsPerChannel == 16)
	{
		...
		// Preventing interpretation of old DrxTif/RC/Sandbox 16-bit
		// float images as 16-bit uint images.
		if (isDrxTif2012)
		{
			// TIFFTAG_SUBFILETYPE exists in files written by the
			// generic Photoshop TIFF writer, but not in DrxTif files.
...
ImageObject* LoadByUsingTIFFLoader(const char* filenameRead, CImageProperties* pProps, string& res_specialInstructions)
{
	...
	else
	{
		...
		const bool isDrxTif2012 = pProps && pProps->GetConfigAsBool("DrxTIF2012", false, true);
		...
		else if (dwBitsPerChannel == 16)
		{
			...
			if (IsFloatTif(tif, isDrxTif2012))
			{
		...
		else if (dwBitsPerChannel == 32 && IsFloatTif(tif, isDrxTif2012))
		{
...
static bool UpdateAndSaveSettingsToTIF(const char *filenameRead, const char *filenameWrite, const std::vector<char>& photoshopData, bool isDrxTif2012)
{
	...
	if (IsFloatTif(tif_in, isDrxTif2012))
	{
	...
	}
	{
		...
		if (pFilename)
		{
			drx_sprintf(filename, "%s", pFilename);
		}
	}
...
{
	DrxTifConfigSink sink;
	pProps->GetMultiConfig().CopyToConfig(eCP_PriorityFile, &sink);

	// Photoshop silently truncates DrxTIF settings to 255 characters, so
	...
	{
		RCLogError("DrxTIF settings are too long: %d characters (max is %d characters)", (int)sink.GetAsString().length(), maxLen);
	}
...
bool UpdateAndSaveSettingsToTIF(const char *filenameRead, const char *filenameWrite, const string& settings, bool isDrxTif2012)
{
	...
	if (!UpdateAndSaveSettingsToTIF(filenameRead, filenameWrite, data, isDrxTif2012))
	{
...
bool UpdateAndSaveSettingsToTIF(const char *filenameRead, const char *filenameWrite, const CImageProperties* pProps, const string* pOriginalSettings, bool bLogSettings)
{
	...
	return UpdateAndSaveSettingsToTIF(filenameRead, filenameWrite, settings, isDrxTIF2012);
}
...
#ifdef DRXTIF_DISABLE_COMPRESSION
...
#endif // DRXTIF_DISABLE_COMPRESSION

		// DinrusX expects a "source DCC filename" stored TIFFTAG_IMAGEDESCRIPTION,
