...
XMLConverter::XMLConverter(IDrxXML* pDrxXML)
	: m_pDrxXML(pDrxXML)
	, m_refCount(1)
{
	m_pDrxXML->AddRef();
...
static XmlNodeRef ConvertFromExcelXmlToDinrusXTableXml(XmlNodeRef root, IXMLSerializer* pSerializer, const string& sInputFile)
{
...
bool XMLCompiler::Process()
{
	...
	// Get XML serializer.
	IXMLSerializer* pSerializer = m_pDrxXML->GetXMLSerializer();
	...
	// Convert Excel's XML format to DinrusX's table XML format, if requested.
	{
		bool bConvert = false;
		...
		if (bConvert)
		{
			XmlNodeRef rootConverted = ConvertFromExcelXmlToDinrusXTableXml(root, pSerializer, sInputFile);
			if (!rootConverted)
			{
				return false;
			}
...
{
	return new XMLCompiler(m_pDrxXML, m_filter, m_tableFilemasks, m_nameConverter);
}
