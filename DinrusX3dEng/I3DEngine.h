...
#include <DinrusX/DinrusX3dEng/IMaterial.h>
#include <DinrusX/DinrusX3dEng/ISurfaceType.h>
...
//! Warning: deprecated Segmented World implementation is not supported by DinrusX anymore
struct ISegmentsUpr
{
...
struct I3DEngine : public IProcess
{
	...
	virtual ChunkFile::IChunkFileWriter* CreateChunkFileWriter(EChunkFileFormat eFormat, IDrxPak* pPak, const char* filename) const = 0;
	...
	virtual void GetLayerMemoryUsage(uint16 nLayerId, IDrxSizer* pSizer, int* pNumBrushes, int* pNumDecals) const = 0;
	...
	virtual int  GetSvoCompiledData(IDrxArchive* pArchive) = 0;
...
{
	void Set(uint16 t, uint16 v)   { drx_strcpy(signature, "DRX"); file_type = (uint8)t; version = v; }
	bool Check(uint16 t, uint16 v) { return strcmp(signature, "DRX") == 0 && t == file_type && v == version; }
