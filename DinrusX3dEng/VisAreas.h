...
struct CVisAreaUpr : public IVisAreaUpr, DinrusX3dEngBase
{
	...
	template<class T>
	bool                 Load_T(T*& f, int& nDataSize, struct SVisAreaManChunkHeader* pVisAreaUprChunkHeader, std::vector<struct IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, bool bHotUpdate, SHotUpdateInfo* pExportInfo);
	virtual bool         Load(FILE*& f, int& nDataSize, struct SVisAreaManChunkHeader* pVisAreaUprChunkHeader, std::vector<struct IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable);
