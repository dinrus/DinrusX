...
public:
	...
	//CONST_VAR_FUNCTION
	inline void* front()  { assert(!empty()); return getNode(m_pHead); }
	
	inline const void* front() const  { assert(!empty()); return getNode(m_pHead); }
	
	//CONST_VAR_FUNCTION
	inline void* back() { assert(!empty()); return getNode(m_pTail); }
	
	inline const void* back() const { assert(!empty());  return getNode(m_pTail); }
	//
	// Обход.
	//
	...
	// Добавить элементы.
	//
	void* push_back_new() DRX_FUNCTION_CONTAINS_UNDEFINED_BEHAVIOR
	{
		void* pNode = allocate();
		if (pNode)
		{
	...
	void* push_front_new() DRX_FUNCTION_CONTAINS_UNDEFINED_BEHAVIOR
	{
		void* pNode = allocate();
		if (pNode)
		{
	...
	void* insert_new(const T* pNext) DRX_FUNCTION_CONTAINS_UNDEFINED_BEHAVIOR
	{
		void* pNode = allocate();
		if (pNode)
		{
...
protected:
	...
	void* getNode(Node* nod){return reinterpret_cast<void*> (nod);}
	const void* getNode(Node* nod) const{return reinterpret_cast<const void*> (nod);}
