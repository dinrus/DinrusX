...
struct SSaveEnvironment
{
	...
	SSaveEnvironment(CDrxAction* pDrxAction, const char* checkPointName, const char* method, Checkpoint& checkpoint, const std::map<string, SaveGameFactory>& saveGameFactories) :
...
struct SLoadEnvironment
{
	...
	SLoadEnvironment(CDrxAction* pDrxAction, Checkpoint& checkpoint, const char* method, const std::map<string, LoadGameFactory>& loadGameFactories) :
		m_pDrxAction(pDrxAction),
...
public:
	...
	bool            SaveGame(CDrxAction* pDrxAction, const char* method, const char* name, ESaveGameReason reason = eSGR_QuickSave, const char* checkPointName = NULL);
	ELoadGameResult LoadGame(CDrxAction* pDrxAction, const char* method, const char* path, SGameStartParams& params, bool requireQuickLoad);
