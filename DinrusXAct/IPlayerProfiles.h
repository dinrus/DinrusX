...
struct IPlayerProfileUpr
{
	...
	virtual bool GetUserInfo(int index, IPlayerProfileUpr::SUserInfo& outInfo) = 0;
	...
	virtual bool GetProfileInfo(const char* userId, int index, IPlayerProfileUpr::SProfileDescription& outInfo) = 0;
	...
	virtual void SetOnlineAttributes(IPlayerProfile* pProfile, const SDrxLobbyUserData* pData, const int32 onlineDataCount) = 0;
	...
	virtual uint32 GetOnlineAttributes(SDrxLobbyUserData* pData, uint32 numData) = 0;

	// retrieve the default online attributes, returns the number of attributes copied
	// numData is number of elements available to be filled in
	virtual uint32 GetDefaultOnlineAttributes(SDrxLobbyUserData* pData, uint32 numData) = 0;
	...
	virtual void GetOnlineAttributesDataFormat(SDrxLobbyUserData* pData, uint32 numData) = 0;
	...
	virtual void ApplyChecksums(SDrxLobbyUserData* pData, uint32 numData) = 0;

	// check checksums are valid
	virtual bool ValidChecksums(const SDrxLobbyUserData* pData, uint32 numData) = 0;
