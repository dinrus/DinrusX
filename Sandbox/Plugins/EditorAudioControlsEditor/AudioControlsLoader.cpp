...
namespace ACE
{
const string CAudioControlsLoader::ms_controlsPath = AUDIO_SYSTEM_DATA_ROOT DRX_NATIVE_PATH_SEPSTR "ace" DRX_NATIVE_PATH_SEPSTR;
const string CAudioControlsLoader::ms_levelsFolder = "levels" DRX_NATIVE_PATH_SEPSTR;
const string CAudioControlsLoader::ms_controlsLevelsFolder = "levels" DRX_NATIVE_PATH_SEPSTR;
const string CAudioControlsLoader::ms_configFilePath = AUDIO_SYSTEM_DATA_ROOT DRX_NATIVE_PATH_SEPSTR "ace" DRX_NATIVE_PATH_SEPSTR "config.xml";
...
void CAudioControlsLoader::LoadAllLibrariesInFolder(const string& folderPath, const string& level)
{
	...
	if (!level.empty())
	{
		path = path + ms_controlsLevelsFolder + level + DRX_NATIVE_PATH_SEPSTR;
	}
...
void CAudioControlsLoader::LoadScopesImpl(const string& sLevelsFolder)
{
	_finddata_t fd;
	ICryPak* pCryPak = gEnv->pCryPak;
	intptr_t handle = pCryPak->FindFirst(sLevelsFolder + DRX_NATIVE_PATH_SEPSTR "*.*", &fd);
	if (handle != -1)
	{
		do
		{
			...
			if (name != "." && name != ".." && !name.empty())
			{
				if (fd.attrib & _A_SUBDIR)
				{
					LoadScopesImpl(sLevelsFolder + DRX_NATIVE_PATH_SEPSTR + name);
				}
				else
				{
