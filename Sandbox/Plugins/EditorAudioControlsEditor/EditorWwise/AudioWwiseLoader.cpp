...
namespace ACE
{
const string g_gameParametersFolder = DRX_NATIVE_PATH_SEPSTR "Game Parameters";
const string g_gameStatesPath = DRX_NATIVE_PATH_SEPSTR "States";
const string g_switchesFolder = DRX_NATIVE_PATH_SEPSTR "Switches";
const string g_eventsFolder = DRX_NATIVE_PATH_SEPSTR  "Events";
const string g_environmentsFolder = DRX_NATIVE_PATH_SEPSTR "Master-Mixer Hierarchy";
const string g_soundBanksPath = AUDIO_SYSTEM_DATA_ROOT DRX_NATIVE_PATH_SEPSTR "wwise";
...
void CAudioWwiseLoader::Load(CAudioSystemEditor_wwise* pAudioSystemImpl)
{
	...
	LoadSoundBanks(PathUtil::GetLocalizationFolder() + DRX_NATIVE_PATH_SEPSTR + szLanguage + DRX_NATIVE_PATH_SEPSTR + g_soundBanksPath, true);
}
...
void CAudioWwiseLoader::LoadSoundBanks(const string& rootFolder, const bool bLocalized)
{
	...
	intptr_t handle = pCryPak->FindFirst(rootFolder + DRX_NATIVE_PATH_SEPSTR + "*.*", &fd);
	if (handle != -1)
	{
...
void CAudioWwiseLoader::LoadControlsInFolder(const string& folderPath)
{
	_finddata_t fd;
	ICryPak* pCryPak = gEnv->pCryPak;
	intptr_t handle = pCryPak->FindFirst(folderPath + DRX_NATIVE_PATH_SEPSTR "*.*", &fd);
	if (handle != -1)
	{
		do
		{
			...
			if (name != "." && name != ".." && !name.empty())
			{
				...
				{
					LoadControlsInFolder(folderPath + DRX_NATIVE_PATH_SEPSTR + name);
				}
				else
				{
					string filename = folderPath + DRX_NATIVE_PATH_SEPSTR + name;
