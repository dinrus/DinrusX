...
const string CAudioControlsWriter::ms_sControlsPath = AUDIO_SYSTEM_DATA_ROOT DRX_NATIVE_PATH_SEPSTR "ace" DRX_NATIVE_PATH_SEPSTR;
const string CAudioControlsWriter::ms_sLevelsFolder = "levels" DRX_NATIVE_PATH_SEPSTR;
	...
	, m_pAudioSystemImpl(pAudioSystemImpl)
{
	if (pATLModel && pLayoutModel && pAudioSystemImpl)
	{
		...
		if (root.isValid())
		{
			...
			{
				string sFullFilePath = PathUtil::GetGameFolder() + DRX_NATIVE_PATH_SEPSTR + *it;
	...
	if (root.isValid())
	{
		...
		for (; it != end; ++it)
		{
			...
			else
			{
				...
				sLibraryPath = ms_sControlsPath + ms_sLevelsFolder + sScope + DRX_NATIVE_PATH_SEPSTR + sLibraryName;
			}
			...
			if (libScope.bDirty)
			{
				...
				const string sFullFilePath = PathUtil::GetGameFolder() + DRX_NATIVE_PATH_SEPSTR + sLibraryPath + ".xml";
