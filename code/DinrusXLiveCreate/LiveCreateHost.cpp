...
bool CPlatformService::Bind()
{
	...
	else
	{
		...
		const int ret = DrxSock::bind(s, (struct DRXSOCKADDR*)&servaddr, sizeof(servaddr));
		if (ret < 0)
		{
...
void CPlatformService::ThreadEntry()
{
	while (!m_bQuiet)
	{
		...
		int length = DrxSock::recvfrom(m_socket, buffer, sizeof(buffer), 0, (DRXSOCKADDR*)&addr, &addrLen);
		if (length < 0)
		{
			...
			if (writer)
			{
				if (Process(reader, writer))
				{
					...
					if (writer->GetSize() < sizeof(responseBuffer))
					{
						...
						int ret = DrxSock::sendto(m_socket, responseBuffer, writer->GetSize(), 0, (DRXSOCKADDR*)&addr, addrLen);
						if (ret < 0)
						{
...
bool CUtilityService::FileTransferInProgress::Update()
{
	...
	while (NULL != pMessage)
	{
		...
		// write data to file
		const uint32 writtenSize = gEnv->pDrxPak->FWrite(pData, 1, dataSize, m_destFile);
