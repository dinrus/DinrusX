...
void CParticleUpr::PrintParticleMemory()
{
	...
	DrxLogAlways("Non heap: %d KB", int(pSizer->GetTotalSize() - memParticle.nUsed) >> 10);
...
const ParticleParams& CParticleUpr::GetDefaultParams(ParticleParams::EInheritance eInheritance, int nVersion) const
{
...
int CParticleUpr::AddEventTiming(cstr sEvent, const CParticleContainer* pCont)
{
...
CParticleUpr::SEffectsKey::SEffectsKey(const cstr& sName)
{
	...
	DrxStringUtils::toLowerInplace((char*)lowerName.c_str());
	...
	pZLib->MD5Update(&context, (const unsigned char*)lowerName.c_str(), lowerName.size());
	pZLib->MD5Final(&context, (unsigned char*)c16);
}
