...
void C3DEngine::PrintDebugInfo(const SRenderingPassInfo& passInfo)
{
	...
	if (m_pObjUpr && GetCVars()->e_StreamCgf && GetCVars()->e_StreamCgfDebug >= 3)
	{
		...
		for (int nObjId = 0; nObjId < m_pObjUpr->m_arrStreamableObjects.Count(); nObjId++)
		{
	...
	*szLevelName = 0;
	...
	if (bCGFStreaming)
	{
		...
		if (!(GetRenderer()->GetFrameID(false) & 15) || !szCGFStreaming[0] || GetCVars()->e_StreamCgfDebug)
		{
			...
			drx_sprintf(szCGFStreaming, "CgfStrm: Loaded:%d InProg:%d All:%d Act:%d PcP:%d MemUsed:%2.2f MemReq:%2.2f Pool:%d",
			            ...
			            (int)m_pObjUpr->m_vStreamPreCachePointDefs.size(),
	...
	#define MAX_PLE_TIME  4.f
		...
		IF (pCharUpr != NULL, 1)
		{
			...
			const float maxVal = 50.f;
	#endif
			uint32 nNumCharacters = pCharUpr->NumCharacters();
	...
	if (GetCVars()->e_MergedMeshesDebug)
	{
		...
		DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
		                     ...
		                     (int)m_pMergedMeshesUpr->ActiveNodes(),
		                     (int)m_pMergedMeshesUpr->StreamedOutNodes(),
		                     (int)m_pMergedMeshesUpr->InstanceCount(),
		                     (int)m_pMergedMeshesUpr->VisibleInstances());
		DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
		...
		DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
		                     "dynamic merged rendermeshes vram size : %3.3f kb", (int)m_pMergedMeshesUpr->CurrentSizeInVramDynamic() / 1024.f);
		DrawTextRightAligned(fTextPosX, fTextPosY += fTextStepY,
		                     "static merged rendermeshes vram size : %3.3f kb", (int)m_pMergedMeshesUpr->CurrentSizeInVramInstanced() / 1024.f);
	}
...
void C3DEngine::PrepareShadowPasses(const SRenderingPassInfo& passInfo, uint32& nTimeSlicedShadowsUpdatedThisFrame, std::vector<std::pair<ShadowMapFrustum*, const CLightEntity*>>& shadowFrustums, std::vector<SRenderingPassInfo>& shadowPassInfo)
{
	...
	for (int i = 0; i < m_pVisAreaUpr->m_lstVisibleAreas.Count(); ++i)
	{
