...
#include <DinrusX/DinrusX3dEng/ClipVolume.h>
...
bool CClipVolumeUpr::UpdateClipVolume(IClipVolume* pClipVolume, _smart_ptr<IRenderMesh> pRenderMesh, IBSPTree3D* pBspTree, const Matrix34& worldTM, uint8 viewDistRatio, bool bActive, uint32 flags, const char* szName)
{
	auto it = std::find(m_clipVolumes.begin(), m_clipVolumes.end(), static_cast<CClipVolume*>(pClipVolume));
...
void CClipVolumeUpr::TrimDeletedClipVolumes(int trimFrameId)
{
