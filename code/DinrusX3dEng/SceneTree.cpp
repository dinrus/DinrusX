...
#include <DinrusX/DinrusX3dEng/StdAfx.h>
	...
	#include <DinrusX/DinrusX3dEng/VoxelSegment.h>
	#include <DinrusX/DinrusX3dEng/SceneTree.h>

	#include <DinrusX/DinrusX3dEng/BlockPacker.h>
...
int CSvoEnv::ExportSvo(IDrxArchive* pArchive)
{
...
bool CSvoEnv::Render()
{
	...
	int maxLoadedNodes = DinrusX3dEngBase::GetCVars()->e_svoMaxBricksOnCPU;
		...
		for (int treeLevel = 0; (treeLevel < SVO_STREAM_QUEUE_MAX_SIZE); treeLevel++)
		{
			for (int distId = 0; (distId < SVO_STREAM_QUEUE_MAX_SIZE); distId++)
			{
				if (!m_bFirst_StartStreaming)
					for (int i = 0; (i < m_arrForStreaming[treeLevel][distId].Count()) && (CVoxelSegment::m_streamingTasksInProgress < DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests); i++)
					{
	...
	if (CVoxelSegment::m_arrLoadedSegments.Count() > (maxLoadedNodes - DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests))
	{
...

		{
			...
			int numNodesToDelete = 4 + DinrusX3dEngBase::GetCVars()->e_svoMaxStreamRequests;//CVoxelSegment::m_arrLoadedSegments.Count()/1000;
	...
	CVoxelSegment::m_maxBrickUpdates = max(1, bMultiUserMode ? DinrusX3dEngBase::GetCVars()->e_svoMaxBrickUpdates * 16 : (DinrusX3dEngBase::GetCVars()->e_svoMaxBrickUpdates / max(1, DinrusX3dEngBase::GetCVars()->e_svoTI_LowSpecMode / (int)2 + 1)));
...
uint32 CSvoNode::SaveNode(PodArray<byte>& rS, uint32& nodesCounterRec, IDrxArchive* pArchive, uint32& totalSizeCounter)
{
	assert(m_pSeg);
			...
			if (m_ppChilds && m_ppChilds[childId])
			{
				...
				if (pArchive->FindFile(szFileName) == nullptr)
				{
					...
					if (nodesCounter)
					{
						...
						int areaGridDim = int(DinrusX3dEngBase::Get3DEngine()->GetTerrainSize() / (SVO_ROOTLESS_PARENT_SIZE / 2));
						DinrusX3dEngBase::PrintMessage("Exported segment %s, Progress = %d %%", szFileName, int(100.f * float(CVoxelSegment::m_exportVisitedAreasCounter) / float(areaGridDim * areaGridDim * areaGridDim)));
...
void CSvoNode::MakeNodeFilePath(char* szFileName)
{
	...
	const char* pLevelPath = DinrusX3dEngBase::Get3DEngine()->GetLevelFilePath(COMPILED_SVO_FOLDER_NAME);
...
int C3DEngine::GetSvoCompiledData(IDrxArchive* pArchive)
{
...
{
	#define CVAR_CPP
	#include <DinrusX/DinrusX3dEng/SceneTreeCVars.inl>
