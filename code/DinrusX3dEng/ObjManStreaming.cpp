...
#include <DinrusX/DinrusX3dEng/CCullThread.h>
...
int CObjUpr::m_nUpdateStreamingPrioriryRoundId = 1;
int CObjUpr::m_nUpdateStreamingPrioriryRoundIdFast = 1;
int CObjUpr::s_nLastStreamingMemoryUsage = 0;
...
void CObjUpr::ProcessObjectsStreaming_Sort(bool bSyncLoad, const SRenderingPassInfo& passInfo)
{
	...
	if (nNumStreamableObjects && ((fTime > fLastTime + 0.1f) || bSyncLoad))
	{
		...
		else
		{
			...
			int nRoundId = CObjUpr::m_nUpdateStreamingPrioriryRoundId - 1;
...
void CObjUpr::PrecacheStatObj(CStatObj* pStatObj, int nLod, const Matrix34A& statObjMatrix, IMaterial* pMaterial, float fImportance, float fEntDistance, bool bFullUpdate, bool bHighPriority)
{
	if (!pStatObj)
		return;
...
void CObjUpr::UpdateRenderNodeStreamingPriority(IRenderNode* pObj, float fEntDistanceReal, float fImportanceFactor, bool bFullUpdate, const SRenderingPassInfo& passInfo, bool bHighPriority)
{
	...
	int nLod = CObjUpr::GetObjectLOD(pObj, fEntDistanceReal);
