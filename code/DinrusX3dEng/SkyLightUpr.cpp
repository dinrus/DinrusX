...
void CSkyLightUpr::UpdateInternal(int32 newFrameID, int32 numUpdates, int callerIsFullUpdate)
{
	FUNCTION_PROFILER_3DENGINE;
...
void CSkyLightUpr::SetQuality(int32 quality)
{
...
int CSkyLightUpr::GetFrontBuffer() const
{
	assert(m_curBackBuffer >= 0 && m_curBackBuffer <= 1);
	return((m_curBackBuffer + 1) & 1);
}

int CSkyLightUpr::GetBackBuffer() const
{
	assert(m_curBackBuffer >= 0 && m_curBackBuffer <= 1);
