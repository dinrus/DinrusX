...
#include <DinrusX/DinrusX3dEng/VMath.hpp>
...
#if MMRM_USE_BOUNDS_CHECK
	mmrm_assert(DrxInterlockedIncrement(const_cast<volatile int*>(&header->debugLock)) == 1);
#endif
...
#if MMRM_USE_BOUNDS_CHECK
	mmrm_assert(DrxInterlockedDecrement(const_cast<volatile int*>(&header->debugLock)) == 0);
#endif
}
