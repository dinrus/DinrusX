...
#include <DinrusX/DinrusX3dEng/CCullRenderer.h>
...
void CCullThread::PrepareOcclusion_ReprojectZBufferLine(int nStartLine, int nNumLines)
{
	...
	uint32 nRemainingJobs = DrxInterlockedDecrement((volatile int*)&m_nRunningReprojJobs);
	if (nRemainingJobs == 0)
	{
...
void CCullThread::PrepareOcclusion_ReprojectZBufferLineAfterMerge(int nStartLine, int nNumLines)
{
	...
	uint32 nRemainingJobs = DrxInterlockedDecrement((volatile int*)&m_nRunningReprojJobsAfterMerge);
	if (nRemainingJobs == 0)
	{
		TOcclusionPrepareRasterizeJob job;
		job.SetClassInstance(this);
