...
bool CVisAreaUpr::GetCompiledData(byte* pData, int nDataSize, std::vector<struct IStatObj*>** ppStatObjTable, std::vector<IMaterial*>** ppMatTable, std::vector<struct IStatInstGroup*>** ppStatInstGroupTable, EEndian eEndian, SHotUpdateInfo* pExportInfo)
{
...
int CVisAreaUpr::GetCompiledDataSize(SHotUpdateInfo* pExportInfo)
{
#if !ENGINE_ENABLE_COMPILATION
	DrxFatalError("serialization code removed, please enable 3DENGINE_ENABLE_COMPILATION in DinrusX3dEng/StdAfx.h");
	return 0;
...
bool CVisAreaUpr::Load(FILE*& f, int& nDataSize, struct SVisAreaManChunkHeader* pVisAreaUprChunkHeader, std::vector<struct IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable)
{
	bool bRes;
...
bool CVisAreaUpr::SetCompiledData(byte* pData, int nDataSize, std::vector<struct IStatObj*>** ppStatObjTable, std::vector<IMaterial*>** ppMatTable, bool bHotUpdate, SHotUpdateInfo* pExportInfo)
{
...
template<class T>
bool CVisAreaUpr::Load_T(T*& f, int& nDataSize, SVisAreaManChunkHeader* pVisAreaUprChunkHeader, std::vector<IStatObj*>* pStatObjTable, std::vector<IMaterial*>* pMatTable, bool bHotUpdate, SHotUpdateInfo* pExportInfo)
{
	if (pVisAreaUprChunkHeader->nVersion != VISAREAMANAGER_CHUNK_VERSION)
	{ Error("CVisAreaUpr::SetCompiledData: version of file is %d, expected version is %d", pVisAreaUprChunkHeader->nVersion, (int)VISAREAMANAGER_CHUNK_VERSION); return 0; }
