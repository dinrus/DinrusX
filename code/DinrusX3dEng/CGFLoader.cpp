...
#include <DinrusX/CoreX/Common_TypeInfo2.h>
#include <DinrusX/CoreX/String/StringUtils.h>
#include <DinrusX/DinrusXSys/InplaceFactory.h>
...
bool CLoaderCGF::ReadCompiledBones(IChunkFile::ChunkDesc* pChunkDesc)
{
	...
	if (pChunkDesc->chunkVersion == pBIPChunk->VERSION)
	{
		...
		const uint32 numBones = nDataSize / sizeof(DrxBoneDescData_Comp);
		CSkinningInfo* const pSkinningInfo = m_pCGF->GetSkinningInfo();
...
bool CLoaderCGF::ProcessSkinning()
{
	...
	if (m_CompiledBones == 0)
	{
		...
		static const int limbNamesCount = DRX_ARRAY_COUNT(arrLimbNames);
	...
	for (uint32 nVert = 0; nVert < numIntVertices; ++nVert)
	{
		...
		const int n = std::min(numVertexLinks, (int)DRX_ARRAY_COUNT(v.weights));
		for (int j = 0; j < n; ++j)
		{
			v.boneIDs[j] = rLinks_base.links[j].boneId;
			v.weights[j] = rLinks_base.links[j].weight;
		}
		for (int j = n; j < DRX_ARRAY_COUNT(v.weights); ++j)
		{
			v.boneIDs[j] = 0;
			v.weights[j] = 0;
		}
