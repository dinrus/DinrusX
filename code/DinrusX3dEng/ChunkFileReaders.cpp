...
#include <DinrusX/DinrusX3dEng/ChunkFileReaders.h>
...
bool DrxFileReader::Open(const char* filename)
{
	Close();
...
int32 DrxFileReader::GetSize()
{
	return m_f.GetLength();
}

bool DrxFileReader::SetPos(int32 pos)
{
	if (pos < 0)
	{
		return false;
...
bool DrxFileReader::Read(void* buffer, size_t size)
{
