...
#include <DinrusX/DinrusX3dEng/StdAfx.h>
	...
	#include <DinrusX/DinrusX3dEng/GeomCacheDecoder.h>
...
bool CGeomCache::LoadGeomCache()
{
	...
	SHeader header;
	bytesRead = gEnv->pDrxPak->FReadRaw((void*)&header, 1, sizeof(SHeader), geomCacheFileHandle);
	...
	if (!m_bUseStreaming)
	{
		...
		if (m_bPlaybackFromMemory && m_frameInfos.size() > 0)
		{
			...
			bytesRead = gEnv->pDrxPak->FReadRaw((void*)&animationData[0], 1, static_cast<uint>(m_compressedAnimationDataSize), geomCacheFileHandle);
	...
	{
		bytesRead = gEnv->pDrxPak->FReadRaw((void*)&m_staticDataHeader, 1, sizeof(GeomCacheFile::SCompressedBlockHeader), geomCacheFileHandle);
...
bool CGeomCache::ReadStaticBlock(FILE* pFile, GeomCacheFile::EBlockCompressionFormat compressionFormat, std::vector<char>& compressedData)
{
	...
	size_t bytesRead = 0;
	bytesRead = gEnv->pDrxPak->FReadRaw((void*)&compressedData[0], 1, sizeof(GeomCacheFile::SCompressedBlockHeader), pFile);
	if (bytesRead != sizeof(GeomCacheFile::SCompressedBlockHeader))
	{
		return false;
	}
...
bool CGeomCache::ReadFrameInfos(FILE* pFile, const uint32 numFrames)
{
	...
	bytesRead = gEnv->pDrxPak->FReadRaw((void*)&fileFrameInfos[0], 1, numFrames * sizeof(GeomCacheFile::SFrameInfo), pFile);
...
void CGeomCache::UpdateStreamableComponents(float importance, const Matrix34A& objMatrix, IRenderNode* pRenderNode, bool bFullUpdate)
{
	...
	const int nRoundId = GetObjUpr()->m_nUpdateStreamingPrioriryRoundId;
