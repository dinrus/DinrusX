...
CGeomCache* CGeomCacheUpr::FindGeomCacheByFilename(const char* filename)
{
	return stl::find_in_map(m_nameToGeomCacheMap, CONST_TEMP_STRING(filename), NULL);
}

CGeomCache* CGeomCacheUpr::LoadGeomCache(const char* szFileName)
{
                                                 ...
                                                 SGeomCacheBufferHandle* pDecompressHandle, SGeomCacheReadRequestHandle* pReadRequestHandle)
{
	...
	const int newDependencyCounter = DrxInterlockedDecrement(GetDependencyCounter(pStreamInfo, frameIndex));
...
void CGeomCacheUpr::DecodeIFrame_JobEntry(SDecodeFrameJobData jobData)
{
	...
	if (prevIFrame < jobData.m_frameIndex)
	{
		...
		{
			const int newDependencyCounter = DrxInterlockedDecrement(GetDependencyCounter(jobData.m_pStreamInfo, bFrameIndex));
...
template<class TBufferHandleType> TBufferHandleType* CGeomCacheUpr::NewBufferHandle(const uint32 size, SGeomCacheStreamInfo& streamInfo)
{
...
SGeomCacheReadRequestHandle* CGeomCacheUpr::NewReadRequestHandle(const uint32 size, SGeomCacheStreamInfo& streamInfo)
{
	FUNCTION_PROFILER_3DENGINE;
...
char* CGeomCacheUpr::GetFrameDecompressData(SGeomCacheStreamInfo* pStreamInfo, const uint frameIndex)
{
	SGeomCacheFrameHeader* pHeader = GetFrameDecompressHeader(pStreamInfo, frameIndex);
	SGeomCacheBufferHandle* pHandle = GetFrameDecompressHandle(pStreamInfo, frameIndex);
...
int* CGeomCacheUpr::GetDependencyCounter(SGeomCacheStreamInfo* pStreamInfo, const uint frameIndex)
{
	const uint frameDataSize = pStreamInfo->m_frameData.size();
