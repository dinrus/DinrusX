...
bool DrxPakFileWriter::Create(IDrxPak* pPak, const char* filename)
{
	Erase();
...
int32 DrxPakFileWriter::GetPos() const
{
	return m_offset;
}

bool DrxPakFileWriter::Write(const void* buffer, size_t size)
{
	if (!m_f)
	{
		return false;
