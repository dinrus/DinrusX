...
#include <DinrusX/DinrusXPhys/geometries.h>
...
IGeometry *CGeomUpr::CreatePrimitive(int type, const primitive *pprim)
{
	IGeometry *pRes = 0;
	switch (type) {
...
int CGeomUpr::AddRefGeometry(phys_geometry *pgeom)
{
...
ITetrLattice *CGeomUpr::CreateTetrLattice(const Vec3 *pt,int npt, const int *pTets,int nTets)
{
	return (new CTetrLattice(GetIWorld()))->CreateLattice(pt,npt,pTets,nTets);
}


int CGeomUpr::RegisterCrack(IGeometry *pGeom, Vec3 *pVtx, int idmat)
{
	int i0;
	float edgelen[3];
	Vec3 edge,pt0,n;
...
void CGeomUpr::UnregisterCrack(int id)
{
	int i;
...
IGeometry *CGeomUpr::GetCrackGeom(const Vec3 *pt,int idmat, geom_world_data *pgwd)
{
...
IBreakableGrid2d *CGeomUpr::GenerateBreakableGrid(Vec2 *ptsrc,int npt, const Vec2i &nCells, int bStaticBorder, int seed)
{	
