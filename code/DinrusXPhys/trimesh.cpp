...
#include <DinrusX/DinrusXPhys/overlapchecks.h>
#include <DinrusX/DinrusXPhys/intersectionchecks.h>
#include <DinrusX/DinrusXPhys/unprojectionchecks.h>
#include <DinrusX/DinrusXPhys/bvtree.h>
#include <DinrusX/DinrusXPhys/aabbtree.h>
#include <DinrusX/DinrusXPhys/obbtree.h>
#include <DinrusX/DinrusXPhys/singleboxtree.h>
#include <DinrusX/DinrusXPhys/geometry.h>
#include <DinrusX/DinrusXPhys/trimesh.h>
#include <DinrusX/DinrusXPhys/raybv.h>
#include <DinrusX/DinrusXPhys/raygeom.h>
	...
	, m_voxgrid(0)
{
	...
	for(int i=0;i<DRX_ARRAY_COUNT(m_bConvex);i++)	{
		m_bConvex[i]=0;
...
int CTriMesh::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl, bool bKeepPrevContacts)
{
	...
	IF (nNodeTris<=(int)(DRX_ARRAY_COUNT(g_IdxTriBuf))-g_IdxTriBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(int)(DRX_ARRAY_COUNT(g_IdxTriBuf))-g_IdxTriBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(int)(DRX_ARRAY_COUNT(g_iFeatureBuf))-g_iFeatureBufPos, 1) {
	...
	IF (szbuf<=(int)(DRX_ARRAY_COUNT(g_IdBuf))-g_IdBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(int)(DRX_ARRAY_COUNT(g_SurfaceDescBuf))-g_SurfaceDescBufPos, 1) {
	...
	IF (m_nMaxVertexValency<=(int)(DRX_ARRAY_COUNT(g_EdgeDescBuf))-g_EdgeDescBufPos, 1) {
...
void CTriMesh::CleanupAfterIntersectionTest(geometry_under_test *pGTest)
{
	...
	const int iCaller = pGTest->iCaller;
	if ((unsigned int)((indexed_triangle*)pGTest->primbuf-g_IdxTriBuf) > (unsigned int)(DRX_ARRAY_COUNT(g_IdxTriBuf)))
		delete[] pGTest->primbuf;
	if ((unsigned int)((indexed_triangle*)pGTest->primbuf1-g_IdxTriBuf) > (unsigned int)(DRX_ARRAY_COUNT(g_IdxTriBuf)))
		delete[] pGTest->primbuf1;
	if ((unsigned int)(pGTest->iFeature_buf-g_iFeatureBuf) > (unsigned int)(DRX_ARRAY_COUNT(g_iFeatureBuf)))
		delete[] pGTest->iFeature_buf;
	if ((unsigned int)(pGTest->idbuf-g_IdBuf) > (unsigned int)(DRX_ARRAY_COUNT(g_IdBuf)))
		delete[] pGTest->idbuf;
	if ((unsigned int)(pGTest->surfaces-g_SurfaceDescBuf) > (unsigned int)(DRX_ARRAY_COUNT(g_SurfaceDescBuf)))
		delete[] pGTest->surfaces;
	if ((unsigned int)(pGTest->edges-g_EdgeDescBuf) > (unsigned int)(DRX_ARRAY_COUNT(g_EdgeDescBuf)))
		delete[] pGTest->edges;
}
																	 ...
																	 prim_inters *pinters)
{
  ...
  piFeature[0] = piFeature[1] = &iFeature_dummy;
	...
	border.szbuf = min(100+pGTest1->pParams->bExactBorder*2900, (int)(DRX_ARRAY_COUNT(g_BrdPtBuf)) - g_BrdPtBufPos-16);
	if (border.szbuf<0)
		return 0;
														 ...
														 int *&pVtxIdBuf,int *&pEdgeIdBuf)
{
	...
	if (itri_new>=0 && !(pUsedTriMap[itri_new>>5] & 1<<(itri_new&31)) && m_pNormals[itri_new]*n0>1.0f-2E-4f) {	\
		ihead = ihead+1&(int)(DRX_ARRAY_COUNT(g_TriQueue))-1; \
		ntri = min(ntri+1,(int)(DRX_ARRAY_COUNT(g_TriQueue))); \
...
#include <DinrusX/DinrusXPhys/boxgeom.h>
	...
	vox_iterate(voxmain,ix,iy,iz) if ((voxmain(ix,iy,iz)&7)==1) {
		...
		do { // locate islands of 'unused' voxel; build semi-optimal boxes around them
			...
			for(d=Vec3(1); d.z<4; i=++d.x>>2,d.x-=3&-i,i=(d.y+=i)>>2,d.y-=3&-i,d.z+=i) {
				...
				if ((buddy&7)==1)
					buddy|=4, queueVox[ihead=min(ihead+1,(int)(DRX_ARRAY_COUNT(queueVox))-1)]=ivox1, ci+=ivox1;
			}
		} while(ihead!=itail);
...
inline bool MergeLine(SVoxLine& ldst, const SVoxLine& lsrc)
{
	...
	int len[]={ (int) dir[0].len2(), (int) dir[1].len2() }, i=isneg(len[0]-len[1]), closed[]={ ldst.closed,lsrc.closed };
...
#include <DinrusX/DinrusXPhys/cylindergeom.h>
#include <DinrusX/DinrusXPhys/capsulegeom.h>
