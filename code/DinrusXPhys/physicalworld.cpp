...
#include <DinrusX/DinrusXPhys/geoman.h>
...
#include <DinrusX/DinrusXPhys/spheregeom.h>
#include <DinrusX/DinrusXPhys/trimesh.h>
#include <DinrusX/DinrusXPhys/rigidbody.h>
#include <DinrusX/DinrusXPhys/physicalplaceholder.h>
#include <DinrusX/DinrusXPhys/physicalentity.h>
#include <DinrusX/DinrusXPhys/rigidentity.h>
#include <DinrusX/DinrusXPhys/walkingrigidentity.h>
#include <DinrusX/DinrusXPhys/particleentity.h>
#include <DinrusX/DinrusXPhys/livingentity.h>
#include <DinrusX/DinrusXPhys/wheeledvehicleentity.h>
#include <DinrusX/DinrusXPhys/articulatedentity.h>
#include <DinrusX/DinrusXPhys/ropeentity.h>
#include <DinrusX/DinrusXPhys/softentity.h>
#include <DinrusX/DinrusXPhys/tetrlattice.h>
#include <DinrusX/DinrusXPhys/physicalworld.h>
#include <DinrusX/DinrusXPhys/waterman.h>
...
CPhysicalWorld::CPhysicalWorld(ILog *pLog) : m_nWorkerThreads(0)
{
	...
	g_nPhysWorlds = min(g_nPhysWorlds+1,(int)(DRX_ARRAY_COUNT(g_pPhysWorlds)));
	m_pEventClient = 0;
	if (g_nPhysWorlds==1)
...
{ 
	int res = DrxInterlockedDecrement(&m_refCount); 
	if (res<=0 && m_pWorld)
...
void CPhysicalWorld::TimeStep(float time_interval, int flags)
{
    ...
    WriteLock lock1(m_lockCaller[MAX_PHYS_THREADS]),lock2(m_lockStep);
		char **pQueueSlots;
		int nQueueSlots;
		volatile int64 timer = DrxGetTicks();
...
NO_INLINE void TrackThunkUsageAlloc(int thunk)
{
	...
	if (DrxGetIMemReplay()->MemStatEnterAlloc())
		DrxGetIMemReplay()->MemStatLeaveAlloc((void*)(thunk + 0xf0000000), sizeof(pe_gridthunk));
}
...
NO_INLINE void TrackThunkUsageFree(int thunk)
{
	...
	if (DrxGetIMemReplay()->MemStatEnterFree())
		DrxGetIMemReplay()->MemStatLeaveFree((void*)(thunk + 0xf0000000));
}
...
void CPhysicalWorld::AddEntityProfileInfo(CPhysicalEntity *pent,int nTicks)
{
	...
	m_nProfiledEnts = min(m_nProfiledEnts+1, (int)(DRX_ARRAY_COUNT(m_pEntProfileData)));
...
int CPhysicalWorld::SetWaterUprParams(pe_params *params)
{
...
int CPhysicalWorld::GetWaterUprParams(pe_params *params)
{
...
int CPhysicalEntity::SetStateFromTypedSnapshot(TSerialize ser, int iSnapshotType, int flags)
{
	...
	return g_pTypedEnts[min((int)(DRX_ARRAY_COUNT(g_pTypedEnts)),max(0,iSnapshotType))]->
...
void CPhysicalWorld::SerializeGarbageTypedSnapshot( TSerialize ser, int iSnapshotType, int flags )
{
	...
	g_pTypedEnts[min((int)(DRX_ARRAY_COUNT(g_pTypedEnts)),max(0,iSnapshotType))]->
