...
int CAABBTree::PrepareForIntersectionTest(geometry_under_test *pGTest, CGeometry *pCollider,geometry_under_test *pGTestColl)
{
	if (m_maxSkipDim<=0 || m_pMesh->m_nTris<=16) {
		...
		if (mapsz<=(int)(DRX_ARRAY_COUNT(g_UsedNodesMap))-g_UsedNodesMapPos) {
		...
		pGTest->nMaxUsedNodes = min(32,(int)(DRX_ARRAY_COUNT(g_UsedNodesIdx)-g_UsedNodesIdxPos));
...
void CAABBTree::CleanupAfterIntersectionTest(geometry_under_test *pGTest)
{
	if (!pGTest->pUsedNodesMap)
		return;
	if ((unsigned int)(pGTest->pUsedNodesMap-g_UsedNodesMap) > (unsigned int)DRX_ARRAY_COUNT(g_UsedNodesMap)) {
...
int CAABBTree::SanityCheck()
{
	...
	const int maxDepth1 = (DRX_ARRAY_COUNT(g_BBoxBuf)-1)/4;
	const int maxDepth2 = (DRX_ARRAY_COUNT(g_BBoxExtBuf)-1)/2;
