...
#include <DinrusX/DinrusXAI/TileGenerator.h>
#include <DinrusX/DinrusXAI/NavMesh.h>
...
void CAISystem::UpdateExpensiveAccessoryQuota()
{
	...
	for (unsigned i = 0, ni = puppets.size(); i < ni && i < maxExpensiveAccessories; ++i)
	{
		...
		if (!puppets[i].first->IsAllowedToUseExpensiveAccessory())
		{
			...
			int timeMs = (int)(fUpdateTimeMs * 0.5f + drx_random(0.0f, fUpdateTimeMs) * 0.4f);
	...
	{
		int timeMs = (int)(drx_random(0.0f, fUpdateTimeMs) * 0.4f);
