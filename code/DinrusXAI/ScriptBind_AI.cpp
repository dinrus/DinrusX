...
#include <DinrusX/DinrusXAI/GoalPipe.h>
#include <DinrusX/DinrusXAI/GoalOp.h>
...
#include <DinrusX/DinrusXAI/IInterestSystem.h>
...
#include <DinrusX/DinrusXAI/GoalPipeXMLReader.h>
#include <DinrusX/DinrusXAI/IMovementSystem.h>
#include <DinrusX/DinrusXAI/MovementRequest.h>
...
int ParsePostureInfo(CPuppet* pPuppet, const SmartScriptTable& posture, const PostureUpr::PostureInfo& base, int parentID, IScriptSystem* pSS)
{
	...
	int id = pPuppet->GetPostureUpr().AddPosture(postureInfo);
...
int CScriptBind_AI::GetPotentialTargetCountFromFaction(IFunctionHandler* pH, ScriptHandle entityID, const char* factionName)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* entityObject = entity->GetAI())
		{
			...
			{
				int count = gAIEnv.pTargetTrackUpr->GetPotentialTargetCountFromFaction(entityObject->GetAIObjectID(), factionName);
...
int CScriptBind_AI::GetPotentialTargetCount(IFunctionHandler* pH, ScriptHandle entityID)
{
	if (IEntity* entity = gEnv->pEntitySystem->GetEntity((EntityId)entityID.n))
	{
		if (IAIObject* entityObject = entity->GetAI())
		{
			int count = gAIEnv.pTargetTrackUpr->GetPotentialTargetCount(entityObject->GetAIObjectID());
			return pH->EndFunction(count);
		}
	}
...
int CScriptBind_AI::SetPosturePriority(IFunctionHandler* pH, ScriptHandle entityId, const char* postureName, float priority)
{
	...
	int postureID = pPuppet->GetPostureUpr().GetPostureID(postureName);
	if (postureID > -1)
...
{
	gAIEnv.pCommunicationUpr->StopCommunication(CommPlayID(static_cast<uint32>(playID.n)));
	return pH->EndFunction();
}
