...
uint32 NameHash(const char* name)
{
	...
	{
		hash += (uint8)(DrxStringUtils::toLowerAscii(name[i]));
		hash += (hash << 10);
		hash ^= (hash >> 6);
	}
...
{ 
	return (uint32)m_updatesUpr.GetRequestQueueSize();
}
                                ...
                                uint16 x, uint16 y, uint16 z, bool bMt, bool bMarkupUpdate)
{
	...
	else
	{
		...
		for (; eit != eend; ++eit)
		{
			...
			{
				DrxLogAlways("NavigationSystem::SpawnJob(): Detected non-valid exclusion volume (%d) for mesh '%s', skipping", (uint32)exclusionVolumeID, mesh.name.c_str());
			}
		}
...
// Only needed for the NavigationSystemDebugDraw
	#include <DinrusX/DinrusXAI/PathHolder.h>
	#include <DinrusX/DinrusXAI/OffGridLinks.h>
