...
#include <DinrusX/DinrusXAI/GoalOp_Drxsis2.h>
#include <DinrusX/DinrusXAI/GoalOpTrace.h>
...
#include <DinrusX/DinrusXAI/GenericAStarSolver.h>
...
//
//-------------------------------------------------------------------------------------------------------------
IGoalOp* CGoalOpFactoryDrxsis2::GetGoalOp(const char* sGoalOpName, IFunctionHandler* pH, int firstParam, GoalParameters& params) const
{
...
IGoalOp* CGoalOpFactoryDrxsis2::GetGoalOp(EGoalOperations op, GoalParameters& params) const
{
	...
	case eGO_COMMUNICATE:
		{
			pResult = new COPDrxsis2Communicate(CommID(params.nValue), CommChannelID(params.nValueAux), params.fValue, static_cast<EAIRegister>(int(params.fValueAux)),
	...
	case eGO_FIREWEAPONS:
		{
			pResult = new COPDrxsis2FlightFireWeapons(static_cast<EAIRegister>(params.nValue), params.fValue, params.fValueAux, params.bValue, static_cast<uint32>(params.nValueAux));
		}
		break;
	default:
		return NULL;
...
//
//-------------------------------------------------------------------------------------------------------------
int COPDrxsis2AdjustAim::RandomizeTimeInterval() const
{
...
EGoalOpResult COPDrxsis2AdjustAim::Execute(CPipeUser* pPipeUser)
{
	...
	if (!m_startTime.GetValue())
	{
		...
		if (m_timeoutRandomness > 0.0001f)
			m_runningTimeoutMs += (int)drx_random(0.0f, m_timeoutRandomness * 1000.0f);
	...
	else if (elapsedMs >= m_nextUpdateMs)
	{
		...
		if (!m_queryID)
		{
			uint32 checks = PostureUpr::DefaultChecks;
...
//
//-------------------------------------------------------------------------------------------------------------
int COPDrxsis2Peek::RandomizeTimeInterval() const
{
	return drx_random(250, 500);
}
...
EGoalOpResult COPDrxsis2Peek::Execute(CPipeUser* pPipeUser)
{
	...
	if (elapsedMs >= m_nextUpdateMs)
	{
		...
		if (!m_queryID)
		{
			uint32 checks = PostureUpr::CheckVisibility | PostureUpr::CheckLeanability;
...
void COPDrxsis2Fly::ParseParam(const char* param, const GoalParams& value)
{
...
void COPDrxsis2Fly::SendEvent(CPipeUser* pPipeUser, const char* eventName)
{
...
void COPDrxsis2ChaseTarget::ParseParam(const char* param, const GoalParams& value)
{
	if (!stricmp("desiredSpeed", param))
	{
		value.GetValue(m_desiredSpeed);
...
COPDrxsis2FlightFireWeapons::COPDrxsis2FlightFireWeapons(EAIRegister reg, float minTime, float maxTime, bool primary, uint32 secondary) : m_State(eFP_START), m_NextState(eFP_STOP), m_InitCoolDown(0.0f), m_WaitTime(-1.0f), m_PausedTime(0.0f), m_PauseOverrideTime(0.0f), m_Rotation(0.0f), m_target(reg), m_minTime(minTime), m_maxTime(maxTime), m_FirePrimary(primary), m_SecondaryWeapons(secondary), m_OnlyLookAt(false), m_LastTarget(ZERO)
{}
...
void COPDrxsis2FlightFireWeapons::ParseParam(const char* param, const GoalParams& value)
{
