...
#include <DinrusX/DinrusXAI/CommunicationUpr.h>
...
const char* CCommunicationUpr::m_kTargetIsAbove_VariableName = "TargetIsAbove";
const char* CCommunicationUpr::m_kTargetIsBelow_VariableName = "TargetIsBelow";

CCommunicationUpr::CCommunicationUpr(const char* configurationFileName /* = NULL */)
	: m_rootFolder("")
...
uint32 CCommunicationUpr::GetConfigCount() const
{
	return m_configs.size();
}

const char* CCommunicationUpr::GetConfigName(uint32 index) const
{
...
CommConfigID CCommunicationUpr::GetConfigIDByIndex(uint32 index) const
{
	assert(index < m_configs.size());
...
void CCommunicationUpr::LoadConfigurationAndScanRootFolder(const char* rootFolder)
{
	Reset();
...
void CCommunicationUpr::ScanFolder(const char* folderName)
{
...
void CCommunicationUpr::LoadVariables(const XmlNodeRef& rootNode, const char* fileName)
{
...
bool CCommunicationUpr::LoadCommunicationSettingsXML(const char* fileName)
{
...
CommChannelID CCommunicationUpr::GetChannelID(const char* name) const
{
...
CommConfigID CCommunicationUpr::GetConfigID(const char* name) const
{
	return CommConfigID(DrxStringUtils::CalculateHashLowerCase(name));
}

CommID CCommunicationUpr::GetCommunicationID(const char* name) const
{
	return CommID(CCrc32::ComputeLowercase(name));
}

const char* CCommunicationUpr::GetCommunicationName(const CommID& communicationID) const
{
...
const char* CCommunicationUpr::GetConfigCommunicationNameByIndex(const CommConfigID& configID, uint32 index) const
{
...
uint32 CCommunicationUpr::GetConfigCommunicationCount(const CommConfigID& configID) const
{
	Configs::const_iterator configIt = m_configs.find(configID);
...
void CCommunicationUpr::OnCommunicationFinished(const CommPlayID& playID, uint8 stateFlags)
{
	PlayingCommunications::iterator it = m_playing.find(playID);
	if (it != m_playing.end())
	{
...
bool CCommunicationUpr::CanVariationBePlayed(const SCommunication& comm, const uint32 variationIndex) const
{
...
void CCommunicationUpr::UpdateHistory(SCommunication& comm, uint32 variation)
{
...
void CCommunicationUpr::SetVariableValue(const char* variableName, const bool newVariableValue)
{
	Variables::VariableID variableId = Variables::GetVariableID(variableName);
	m_variables.SetVariable(variableId, newVariableValue);
}

void CCommunicationUpr::GetVariablesNames(const char** variableNames, const size_t maxSize, size_t& actualSize) const
{
