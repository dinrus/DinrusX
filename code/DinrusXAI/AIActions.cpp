...
#include <DinrusX/DinrusXAI/AIActor.h>
...
int CAIActionUpr::SuspendActionsOnEntity(IEntity* pEntity, int goalPipeId, const IAIAction* pAction, bool bHighPriority, int& numHigherPriority)
{
...
void CAIActionUpr::ResumeActionsOnEntity(IEntity* pEntity, int goalPipeId)
{
	if (pEntity)
	{
		if (goalPipeId)
...
void CAIActionUpr::AbortAIAction(IEntity* pEntity, int goalPipeId /*=0*/)
{
	AIAssert(pEntity);
	if (!pEntity)
		return;
...
void CAIActionUpr::FinishAIAction(IEntity* pEntity, int goalPipeId)
{
	AIAssert(pEntity);
	if (!pEntity)
		return;
...
void CAIActionUpr::OnGoalPipeEvent(IPipeUser* pPipeUser, EGoalPipeEvent event, int goalPipeId, bool& unregisterListenerAfterEvent)
{
	TActiveActions::iterator it, itEnd = m_ActiveActions.end();
	for (it = m_ActiveActions.begin(); it != itEnd; ++it)
	{
...
IAIAction* CAIActionUpr::GetAIAction(const char* sName)
{
	if (!sName || !*sName)
		return NULL;
...
void CAIActionUpr::ExecuteAIAction(const IAIAction* pAction, IEntity* pUser, IEntity* pObject, int maxAlertness, int goalPipeId,
...
void CAIActionUpr::ExecuteAIAction(const char* sActionName, IEntity* pUser, IEntity* pObject, int maxAlertness, int goalPipeId, IAIAction::IAIActionListener* pListener)
{
...
void CAIActionUpr::LoadLibrary(const char* sPath)
{
	m_ActiveActions.clear();
