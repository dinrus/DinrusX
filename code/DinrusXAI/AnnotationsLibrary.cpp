...
NavigationAreaTypeID CAnnotationsLibrary::CreateAreaType(const uint32 id, const char* szName, const uint32 defaultFlags, const ColorB* pColor)
{
	...
	{
		AIWarning("Trying to create NavigationSystem AreaType with invalid id '%u'. Id must be between 0 and %d!", id, (int) AreaAnnotation::MaxAreasCount());
...
NavigationAreaFlagID CAnnotationsLibrary::CreateAreaFlag(const uint32 id, const char* szName, const ColorB* pColor)
{
	...
	{
		AIWarning("Trying to create NavigationSystem AreaFlag with invalid id '%u'. Id must be between 0 and %d!", id, (int) AreaAnnotation::MaxFlagsCount());
