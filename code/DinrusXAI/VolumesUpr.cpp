...
#include <DinrusX/DinrusXAI/VolumesUpr.h>
...
bool CVolumesUpr::RegisterArea(const char* volumeName, NavigationVolumeID& outVolumeId)
{
	...
	{
		{
			...
			{
				volumeId = it->second;
				AILogComment("CVolumesUpr::RegisterArea: area '%s' is already loaded with the id = %u", volumeName, (uint32)volumeId);
		...
		AILogComment("CVolumesUpr::RegisterArea: registering new area '%s' with the id = %u", volumeName, (uint32)volumeId);
		bResult = true;
	}
	else
	{
...
void CVolumesUpr::RegisterAreaFromLoadedData(const char* szVolumeName, NavigationVolumeID id)
{
	...
	AILogComment("CVolumesUpr::RegisterAreaFromLoadedData %s = %u", szVolumeName, (uint32)id);
...
bool CVolumesUpr::SetAreaID(const char* volumeName, NavigationVolumeID id)
{
...
void CVolumesUpr::UnRegisterArea(const char* volumeName)
{
...
bool CVolumesUpr::IsAreaPresent(const char* volumeName) const
{
	VolumesMap::const_iterator it = m_volumeAreas.find(volumeName);
	return it != m_volumeAreas.end();
}

NavigationVolumeID CVolumesUpr::GetAreaID(const char* volumeName) const
{
...
void CVolumesUpr::UpdateNameForAreaID(const NavigationVolumeID id, const char* newName)
{
	VolumesMap::const_iterator it = m_volumeAreas.begin();
	VolumesMap::const_iterator end = m_volumeAreas.end();
	for (; it != end; ++it)
...
bool CVolumesUpr::IsLoadedAreaPresent(const char* volumeName) const
{
	VolumesMap::const_iterator it = m_loadedVolumeAreas.find(volumeName);
	return it != m_loadedVolumeAreas.end();
}

NavigationVolumeID CVolumesUpr::GetLoadedAreaID(const char* volumeName) const
{
	NavigationVolumeID areaID;
	VolumesMap::const_iterator it = m_loadedVolumeAreas.find(volumeName);
	if (it != m_loadedVolumeAreas.end())
...
bool CVolumesUpr::RegisterEntityMarkups(INavigationSystem& navigationSystem, const IEntity& entity, const char** pShapeNamesArray, const size_t count, NavigationVolumeID* pOutIds)
{
	if (count == 0)
		return false;
...
void CVolumesUpr::SaveData(CDrxFile& file, const uint16 version) const
{
...
void CVolumesUpr::LoadData(CDrxFile& file, const uint16 version)
{
	if (version >= NavigationSystem::eBAINavigationFileVersion::ENTITY_MARKUP_GUIDS)
	{
