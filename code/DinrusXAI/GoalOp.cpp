...
EGoalOpResult COPTimeout::Execute(CPipeUser* pPipeUser)
{
	...
	{
		m_actualIntervalMs = (int)(1000.0f * (m_fIntervalMin + ((m_fIntervalMax - m_fIntervalMin) * drx_random(0.0f, 1.0f))));
		m_startTime = time;
	}
...
EGoalOpResult COPFireCmd::Execute(CPipeUser* pPipeUser)
{
	...
	if (m_startTime.GetMilliSecondsAsInt64() <= 0)
	{
		...
		if (m_fIntervalMin < 0.0f)
			...
			m_actualIntervalMs = (int)(drx_random(m_fIntervalMin, m_fIntervalMax) * 1000.0f);
		else
