...
#include <DinrusX/DinrusXAI/CoverSampler.h>
#include <DinrusX/DinrusXAI/CoverSurface.h>
#include <DinrusX/DinrusXAI/CoverUser.h> // TODO: remove
...
void CCoverSystem::NotifyCoverUsers(const CoverSurfaceID& surfaceID)
{
	...
	for (OccupiedCover::const_iterator it = occupiedTemp.begin(); it != occupiedTemp.end(); ++it)
	{
		...
		if (GetSurfaceID(coverID) == surfaceID)
		{
			...
			{
				DRX_ASSERT_MESSAGE(false, "Cover id=%u is occupied by unregistered entity id=%u", static_cast<uint32>(coverID), static_cast<uint32>(occupantInfo.entityId));
			}
		}
	}
}
