...
#include <DinrusX/DinrusXDynRespSys/SpeakerUpr.h>
...
#include <DinrusX/DinrusXDynRespSys/DialogLineDatabase.h>
		...
		if (activateSpeaker.pActor == pActor)
		{
			    ...
			    || (m_samePrioCancelsLinesCVar != 0 && priority >= activateSpeaker.priority && lineID != activateSpeaker.lineID)) //if the new line is not less important, stop the old one
			{
				...
				if (activateSpeaker.stopTriggerID != DrxAudio::InvalidControlId && m_playAudioCVar != 0)
				{
					...
					DrxAudio::SRequestUserData const userData(DrxAudio::ERequestFlags::DoneCallbackOnExternalThread | DrxAudio::ERequestFlags::CallbackOnExternalOrCallingThread, this, (void* const)(pLine), (void* const)(activateSpeaker.pActor));
...
//--------------------------------------------------------------------------------------------------
bool CSpeakerUpr::CancelSpeaking(const DRS::IResponseActor* pActor, int maxPrioToCancel /* = -1 */, const CHashedString& lineID /* = CHashedString::GetEmpty() */, bool bCancelQueuedLines /* = true */)
{
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::SetNumActiveSpeaker(int newAmountOfSpeaker)
{
...
void CSpeakerUpr::ExecuteStartSpeaking(SSpeakInfo* pSpeakerInfoToUse)
{
	...
	if (bHasAudioAsset)
	{
		...
		DrxAudio::SRequestUserData const userData(DrxAudio::ERequestFlags::DoneCallbackOnExternalThread | DrxAudio::ERequestFlags::CallbackOnExternalOrCallingThread, this, (void* const)(pSpeakerInfoToUse->pPickedLine), (void* const)(pSpeakerInfoToUse->pActor));
...
//--------------------------------------------------------------------------------------------------
void CSpeakerUpr::QueueLine(CResponseActor* pActor, const CHashedString& lineID, float maxQueueDuration, const int priority)
{
