...
#include <DinrusX/DinrusXPhysX/../../DinrusXPhys/geoman.h>
#include <DinrusX/DinrusXPhysX/geometries.h>
...
IGeometry* CGeomUpr::CreateMesh(strided_pointer<const Vec3> pVertices, strided_pointer<unsigned short> pIndices, char *pIds, int *pForeignIdx, int nTris,
...
ITetrLattice *CGeomUpr::CreateTetrLattice(const Vec3 *pt, int npt, const int *pTets, int nTets) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }
int CGeomUpr::RegisterCrack(IGeometry *pGeom, Vec3 *pVtx, int idmat) { DRX_PHYSX_LOG_FUNCTION; _RETURN_INT_DUMMY_; }
void CGeomUpr::UnregisterCrack(int i) {}
IGeometry *CGeomUpr::GetCrackGeom(const Vec3 *pt, int idmat, geom_world_data *pgwd) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }

IBreakableGrid2d *CGeomUpr::GenerateBreakableGrid(Vec2 *ptsrc, int npt, const Vec2i &nCells, int bStaticBorder, int seed) { DRX_PHYSX_LOG_FUNCTION; _RETURN_PTR_DUMMY_; }

//////////////////////////////////////////////////////////////////////////

IGeometry *CGeomUpr::CreatePrimitive(int type, const primitives::primitive *pprim)
{
	PhysXGeom	*pGeom = new PhysXGeom;	
...
phys_geometry *CGeomUpr::RegisterGeometry(IGeometry *pGeom,int defSurfaceIdx, int *pMatMapping,int nMats)
{
...
int CGeomUpr::AddRefGeometry(phys_geometry *pgeom) { return DrxInterlockedIncrement(&pgeom->nRefCount); }

int CGeomUpr::UnregisterGeometry(phys_geometry *pgeom)
{
	int res = DrxInterlockedDecrement(&pgeom->nRefCount);
	if (res <= 0)	{
...
void CGeomUpr::SetGeomMatMapping(phys_geometry *pgeom, int *pMatMapping, int nMats)
{
