...
#include <DinrusX/DinrusXAudio/ATL.h>
...
#include <DinrusX/DinrusXAudio/PoolObject_impl.h>
...
void CAudioTranslationLayer::ProcessRequest(CAudioRequest& request)
{
	...
	if (request.GetData())
	{
		...
		default:
			{
				Drx::Audio::Log(ELogType::Error, "Unknown audio request type: %d", static_cast<int>(request.GetData()->type));
				DRX_ASSERT(false);
