...
#include<DinrusX/DinrusXAnimation/FacialInstance.h>
...
void CCharInstance::StartAnimationProcessing(const SAnimationProcessParams& params)
{
	...
	int numberOfChildren = m_AttachmentUpr.GenerateAttachedInstanceContexts();
...
SMeshLodInfo CCharInstance::ComputeGeometricMean() const
{
	...
	const int attachmentCount = m_AttachmentUpr.GetAttachmentCount();
...
void CCharInstance::ReloadCHRPARAMS()
{
	...
	//--------------------------------------------------------------------------
	uint32 numAttachments = m_AttachmentUpr.m_arrAttachments.size();
...
void CCharInstance::EnableFacialAnimation(bool bEnable)
{
	...
	for (int attachmentIndex = 0, end = int(m_AttachmentUpr.m_arrAttachments.size()); attachmentIndex < end; ++attachmentIndex)
	{
...
void CCharInstance::SkinningTransformationsComputation(SSkinningData* pSkinningData, CDefaultSkeleton* pDefaultSkeleton, int nRenderLod, const Skeleton::CPoseData* pPoseData, f32* pSkinningTransformationsMovement)
{
	...
	const uint32 extraBonesCount = pAttachmentUpr->GetExtraBonesCount();
	...
	while (DrxInterlockedCompareExchangePointer(alias_cast<void* volatile*>(pSkinningData->pMasterSkinningDataList), NULL, pSkinningJobList) != pSkinningJobList);
...
void CCharInstance::SetupThroughParams(const SAnimationProcessParams* pParams)
{
	...
	int nCurrentFrameID = g_pCharacterUpr->m_nUpdateCounter; // g_pIRenderer->GetFrameID(false);
