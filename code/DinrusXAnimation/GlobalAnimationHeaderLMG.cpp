...
bool GlobalAnimationHeaderLMG::LoadFromXML(CAnimationSet* pAnimationSet, XmlNodeRef root)
{
	...
	if (root == 0)
	{
		int32 bPakPriority = gEnv->pDrxPak->GetPakPriority();
				...
				for (uint32 i = 0; i < num; i++)
				{
					...
					if (strcmp(ExampleTag, "BlendSpace") == 0)
					{
						...
						for (uint32 id = 0; id < numLMG; id++)
						{
							...
							{
								uint32 isBSValid = g_AnimationUpr.m_arrGlobalLMG[id].IsAssetLMGValid();
								if (isBSValid == 0)
								{
									const char* strBSFilePath = g_AnimationUpr.m_arrGlobalLMG[id].GetFilePath();
