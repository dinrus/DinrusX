...
#include <DinrusX/DinrusXAnimation/AttachmentFace.h>
...
uint32 CAttachmentFACE::ProjectAttachment(const Skeleton::CPoseData* pPoseData)
{
	...
	uint32 numAttachments = m_pAttachmentUpr->GetAttachmentCount();
	for (uint32 att = 0; att < numAttachments; att++)
	{
		uint32 localtype = m_pAttachmentUpr->m_arrAttachments[att]->GetType();
		if (localtype != CA_SKIN)
			continue;
		uint32 isHidden = m_pAttachmentUpr->m_arrAttachments[att]->IsAttachmentHidden();
		if (isHidden)
			continue;
