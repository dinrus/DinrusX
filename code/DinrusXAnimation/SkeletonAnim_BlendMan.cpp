...
/////////////////////////////////////////////////////////////////////////////////
uint32 CSkeletonAnim::BlendUpr(f32 fDeltaTime, DynArray<CAnimation>& arrAFIFO, uint32 nLayer)
{
...
void CSkeletonAnim::UpdateParameters(CAnimation* arrAnimFiFo, uint32 nMaxActiveInQueue, uint32 nLayer, f32 fFrameDeltaTime)
{
	...
	for (uint32 a = 0; a < nMaxActiveInQueue; a++)
	{
		...
		if (pParametric)
		{
			...
			{
				uint32 nMaxDebugInstances = g_pCharacterUpr->m_arrCharacterBase.size();
	...
	if (numLoops > 0)
	{
		...
		if (LoopAnimation == 0)
		{
			...
			if (NumAnimsInQueue == 1 && nLayer)
			{
				...
				uint32 fo = flags & CA_FADEOUT;  //this is something that DrxMannequin should do
				if (rlk && fo)
...
uint32 CSkeletonAnim::BlendUprDebug(DynArray<CAnimation>& arrAFIFO, uint32 nVLayer)
{
#ifndef _RELEASE
	f32 fColor[4] = { 1, 0, 0, 1 };
...
/////////////////////////////////////////////////////////////////////////////////
void CSkeletonAnim::LayerBlendUpr(f32 fDeltaTime, uint32 nLayer)
{
	if (nLayer == 0)
		return;
