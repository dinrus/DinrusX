...
int CAnimationSet::LoadFileCAF(const char* szFilePath, const char* szAnimName)
{
	...
	int nGlobalAnimID = g_AnimationUpr.CreateGAH_CAF(szFilePath); //create the global header. the data will be streamed in later.
...
int CAnimationSet::LoadFileAIM(const char* szFilePath, const char* szAnimName, const IDefaultSkeleton* pIDefaultSkeleton)
{
	...
	int nGlobalAnimID = g_AnimationUpr.CreateGAH_AIM(szFilePath);
...
//  THis function does not put up a warning in the case the animation couldn't be loaded.
//  It returns an error (false) and the caller must process it.
int CAnimationSet::LoadFileANM(const char* szFileName, const char* szAnimName, DynArray<CControllerTCB>& m_LoadCurrAnimation, DrxCGALoader* pCGA, const IDefaultSkeleton* pIDefaultSkeleton)
{
	int nModelAnimId = GetAnimIDByName(szAnimName);
	if (nModelAnimId == -1)
	{
		int nGlobalAnimID = g_AnimationUpr.CreateGAH_CAF(szFileName);
		...
		uint32 loaded = g_AnimationUpr.m_arrGlobalCAF[nGlobalAnimID].IsAssetLoaded();
		if (loaded)
		{
			// No asserts on data
...
int CAnimationSet::LoadFileLMG(const char* szFilePath, const char* szAnimName)
{
	...
	if (nModelAnimId == -1)
	{
		int nGlobalAnimID = g_AnimationUpr.CreateGAH_LMG(szFilePath);
		int nLocalAnimId = m_arrAnimations.size();
...
static bool AnimTokenMatchesOneOfDirectionalBlends(const CAnimationSet* pAnimationSet, int nAnimationId, const DynArray<DirectionalBlends>& blends)
{
	...
	{
		int32 nGlobalID = pAnim->m_nGlobalAnimId;
		uint32 animTokenCRC32 = g_AnimationUpr.m_arrGlobalAIM[nGlobalID].m_AnimTokenCRC32;
...
void CAnimationSet::VerifyAliases_Debug()
{
...

	{
		...
		uint32 numLMG = g_AnimationUpr.m_arrGlobalLMG.size();
...

	{
		...
		uint32 numCAF = g_AnimationUpr.m_arrGlobalCAF.size();
