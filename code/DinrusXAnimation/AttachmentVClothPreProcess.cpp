...
#include <DinrusX/DinrusXPhys/quotient.h>
#include <DinrusX/DinrusXAnimation/AttachmentVClothPreProcess.h>
#include <DinrusX/DinrusXAnimation/AttachmentVClothPreProcessDijkstra.h>
...
bool AttachmentVClothPreProcess::BendByTriangleAngle(std::vector<Vec3> const& vtx, std::vector<int> const& idx, std::vector<bool> const& attached)
{
	...
	//DrxWarning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_ERROR, "[Cloth] BendByTriangleAngleInit() BendEdges: Found %i edges with neighbors, out of %i edges total. Edge Done: %i", nEdgesWithNeighbor, nEdges,(int)edgeDone.size());
...
bool AttachmentVClothPreProcess::PruneWeldedVertices(std::vector<Vec3>& vtx, std::vector<int>& idx, std::vector<bool>& attached)
{
	...
	if (nWelded)
	{
		//DrxLog("[Character Cloth] Found %i welded vertices out of %i ", nWelded, (int)vtx.size());
