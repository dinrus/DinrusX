...
uint32 CAttachmentPROW::SetJointName(const char* szJointName)
{
	...
	int nJointID = m_pAttachmentUpr->m_pSkelInstance->m_pDefaultSkeleton->GetJointIDByName(szJointName);
	if (nJointID < 0)
	{
...
void CPendulaRow::PostUpdate(const CAttachmentUpr* pAttachmentUpr, const char* pJointName)
{
...
void CPendulaRow::UpdatePendulumRow(const CAttachmentUpr* pAttachmentUpr, Skeleton::CPoseData& rPoseData, const char* pAttName)
{
	const CCharInstance* pSkelInstance = pAttachmentUpr->m_pSkelInstance;
	const CDefaultSkeleton& rDefaultSkeleton = *pSkelInstance->m_pDefaultSkeleton;
	QuatTS rPhysLocation = pSkelInstance->m_location;
