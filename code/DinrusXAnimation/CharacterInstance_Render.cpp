...
void CCharInstance::Render(const struct SRendParams& RendParams, const SRenderingPassInfo& passInfo)
{
	...
	uint32 nFrameID = g_pCharacterUpr->m_nUpdateCounter;
...
{
	int nList = (int)CharacterUpr::GetRendererMainThreadId();
...
SSkinningData* CCharInstance::GetSkinningData(const SRenderingPassInfo& passInfo)
{
	...
	uint32 numSkinningBones = GetSkinningTransformationCount() + pAttachmentUpr->GetExtraBonesCount();
