...
#include <DinrusX/DinrusXAnimation/AttachmentBone.h>
...
{
	m_nJointID = -1;
	int32 JointId = m_pAttachmentUpr->m_pSkelInstance->m_pDefaultSkeleton->GetJointIDByName(m_strJointName.c_str());
	if (JointId < 0)
		return 0;
	CCharInstance* pSkelInstance = m_pAttachmentUpr->m_pSkelInstance;
...
uint32 CAttachmentBONE::Immediate_AddBinding(IAttachmentObject* pIAttachmentObject, ISkin* pISkin, uint32 nLoadingFlags)
{
	...
	{
		uint32 IsFastUpdateType = m_pAttachmentUpr->IsFastUpdateType(m_pIAttachmentObject->GetAttachmentType());
		if (IsFastUpdateType)
	...
	{
		uint32 IsFastUpdateType = m_pAttachmentUpr->IsFastUpdateType(pIAttachmentObject->GetAttachmentType());
		if (IsFastUpdateType)
