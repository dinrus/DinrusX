...
#include <DinrusX/DinrusXAnimation/ParamLoader.h>
...
bool CParamLoader::ExpandWildcards(uint32 listID)
{
	...
	uint32 numDBALoaders = g_AnimationUpr.m_arrGlobalHeaderDBA.size();
	...
	for (uint32 w = 0; w < numWildcard; ++w)
	{
		...
		uint32 img = g_pCharacterUpr->IsInitializedByIMG();
		if (img & 2 && IsLMG == 0 && IsFSQ == 0)
		{
			...
			const uint32 numAIM = g_AnimationUpr.m_arrGlobalAIM.size();
				...
				if (rGlobalHeaderDBA.m_pDatabaseInfo == 0)
				{
					...
					for (uint32 c = 0; c < numCAF; c++)
					{
						if (g_AnimationUpr.m_arrGlobalCAF[c].m_FilePathDBACRC32 == nDBACRC32)
						{
							...
							const char* currentFile = g_AnimationUpr.m_arrGlobalCAF[c].GetFilePath();
