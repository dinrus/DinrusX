...
#include <DinrusX/DinrusXAnimation/LoaderCHR.h>
#include <DinrusX/DinrusXAnimation/LoaderCGA.h>
#include <DinrusX/DinrusXAnimation/AnimationUpr.h>
#include<DinrusX/DinrusXAnimation/FaceAnimation.h>
#include <DinrusX/CoreX/Game/IGameFramework.h>
#include <DinrusX/DinrusXSys/IResourceUpr.h>
#include <DinrusX/DinrusXAnimation/ParametricSampler.h>
#include <DinrusX/DinrusXAnimation/AttachmentVCloth.h>
#include <DinrusX/DinrusXAnimation/AttachmentMerger.h>
#include <DinrusX/DinrusXAnimation/SerializationCommon.h>
...
uint32 CharacterUpr::s_renderFrameIdLocal = 0;
...
void CharacterUpr::PreloadLevelModels()
{
	...
	if (Console::GetInst().ca_PreloadAllCAFs)
	{
		...
		const int numGlobalCAF = GetAnimationUpr().m_arrGlobalCAF.size();
...
void CharacterUpr::PreloadModelsCHR()
{
	...
	while (sFilenameInResource)
	{
		...
		uint32 IsSKIN = stricmp(fileExt, DRX_SKIN_FILE_EXT) == 0;
		uint32 IsSKEL = stricmp(fileExt, DRX_SKEL_FILE_EXT) == 0;
		...
		if (IsSKEL)
		{
			...
			uint32 nFileOnDisk = gEnv->pDrxPak->IsFileExist(filename);
			if (nFileOnDisk)
			{
...
ICharacterInstance* CharacterUpr::CreateInstance(const char* szFilePath, uint32 nLoadingFlags)
{
...
ICharacterInstance* CharacterUpr::CreateSKELInstance(const char* strFilePath, uint32 nLoadingFlags)
{
...
ISkin* CharacterUpr::LoadModelSKIN(const char* szFilePath, uint32 nLoadingFlags)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_Other, 0, "Characters");
	if (szFilePath == 0)
...
IDefaultSkeleton* CharacterUpr::LoadModelSKEL(const char* szFilePath, uint32 nLoadingFlags)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_Other, 0, "Characters");
	if (szFilePath == 0)
		return 0;                                                                           //to prevent a crash in the frequent case the designers will mess bad filenames
...
//////////////////////////////////////////////////////////////////////////
CDefaultSkeleton* CharacterUpr::CheckIfModelSKELLoaded(const string& strFilePath, uint32 nLoadingFlags)
{
...
CDefaultSkeleton* CharacterUpr::CheckIfModelExtSKELCreated(const uint64 nCRC64, uint32 nLoadingFlags)
{
	if ((nLoadingFlags & CA_CharEditModel) == 0)
	{
		uint32 numModels = m_arrModelCacheSKEL.size();
...
//////////////////////////////////////////////////////////////////////////
CSkin* CharacterUpr::CheckIfModelSKINLoaded(const string& strFilePath, uint32 nLoadingFlags)
{
	if ((nLoadingFlags & CA_CharEditModel) == 0)
	{
...
CDefaultSkeleton* CharacterUpr::FetchModelSKEL(const char* szFilePath, uint32 nLoadingFlags)
{
...
CDefaultSkeleton* CharacterUpr::FetchModelSKELForGCA(const char* szFilePath, uint32 nLoadingFlags)
{
	CDefaultSkeleton* pModelSKEL = CheckIfModelSKELLoaded(szFilePath, nLoadingFlags);
	if (pModelSKEL == 0)
	{
...
CSkin* CharacterUpr::FetchModelSKIN(const char* szFilePath, uint32 nLoadingFlags)
{
...
bool CharacterUpr::LoadAndLockResources(const char* szFilePath, uint32 nLoadingFlags)
{
	MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_Other, 0, "Characters");
	if (szFilePath == 0)
		return false;
...
void CharacterUpr::TryLoadModelSkin(const char* szFilePath, uint32 nLoadingFlags, bool bKeep)
{
	const char* strAttFileExt = PathUtil::GetExt(szFilePath);
...
void CharacterUpr::StreamKeepCharacterResourcesResident(const char* szFilePath, int nLod, bool bKeep, bool bUrgent)
{
...
bool CharacterUpr::StreamHasCharacterResources(const char* szFilePath, int nLod)
{
	const char* fileExt = PathUtil::GetExt(szFilePath);
...
//////////////////////////////////////////////////////////////////////////
ICharacterInstance* CharacterUpr::CreateCGAInstance(const char* strPath, uint32 nLoadingFlags)
{
	LOADING_TIME_PROFILE_SECTION(g_pISystem);
...
void CharacterUpr::RegisterModelSKEL(CDefaultSkeleton* pDefaultSkeleton, uint32 nLoadingFlags)
{
	LOADING_TIME_PROFILE_SECTION(g_pISystem);
...
uint32 CharacterUpr::GetNumInstancesPerModel(const IDefaultSkeleton& rIDefaultSkeleton) const
{
...
ICharacterInstance* CharacterUpr::GetICharInstanceFromModel(const IDefaultSkeleton& rIDefaultSkeleton, uint32 num) const
{
	uint32 numDefaultSkeletons = m_arrModelCacheSKEL.size();
	for (uint32 m = 0; m < numDefaultSkeletons; m++)
	{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
//---------------------------------------------------------------------------------------------------
void CharacterUpr::RegisterModelSKIN(CSkin* pDefaultSkinning, uint32 nLoadingFlags)
{
	LOADING_TIME_PROFILE_SECTION(g_pISystem);
...
uint32 CharacterUpr::GetRendererThreadId()
{
...
uint32 CharacterUpr::GetRendererMainThreadId()
{
	bool bIsMultithreadedRenderer = false;
	g_pIRenderer->EF_Query(EFQ_RenderMultithreaded, bIsMultithreadedRenderer);
	if (bIsMultithreadedRenderer)
...
void CharacterUpr::Update(bool bPaused)
{
	...
	if (Console::GetInst().ca_DebugAnimMemTracking)
	{
		...
		if (g_AnimationUpr.m_AnimMemoryTracker.m_nAnimsCounter)
		{
			...
			g_pAuxGeom->Draw2dLabel(1, g_YLine, 2.0f, fBlue, false, "nAnimsPerFrame: %4d  nAnimsMax: %4d nAnimsAvrg: %4d", g_AnimationUpr.m_AnimMemoryTracker.m_nAnimsCurrent / 1024, g_AnimationUpr.m_AnimMemoryTracker.m_nAnimsMax / 1024, (uint32)(average / 1024));
			g_YLine += 16.0f;
		}
	}
...
void CharacterUpr::UpdateStreaming(int nFullUpdateRoundId, int nFastUpdateRoundId)
{
...
void CharacterUpr::UpdateStreaming_SKEL(std::vector<CDefaultSkeletonReferences>& skels, uint32 nRenderFrameId, const uint32* nRoundIds)
{
...
void CharacterUpr::UpdateStreaming_SKIN(std::vector<CDefaultSkinningReferences>& skins, uint32 nRenderFrameId, const uint32* nRoundIds)
{
...
//----------------------------------------------------------------------
CDefaultSkeleton* CharacterUpr::GetModelByAimPoseID(uint32 nGlobalIDAimPose)
{
	uint32 numModels = m_arrModelCacheSKEL.size();
	for (uint32 m = 0; m < numModels; m++)
	{
...
void CharacterUpr::DebugModelCache(uint32 printtxt, std::vector<CDefaultSkeletonReferences>& parrModelCacheSKEL, std::vector<CDefaultSkinningReferences>& parrModelCacheSKIN)
{
	float fWhite[4] = { 1, 1, 1, 1 };
...
void CharacterUpr::InternalStopAnimationsOnAllInstances(const IAnimationSet* pAnimationSet)
{
	...
	uint32 numLMGs = g_AnimationUpr.m_arrGlobalLMG.size();
...
void CharacterUpr::DatabaseUnloading()
{
	...
	uint32 numHeadersCAF = g_AnimationUpr.m_arrGlobalCAF.size();
	if (numHeadersCAF == 0)
		return;
	uint32 numHeadersDBA = g_AnimationUpr.m_arrGlobalHeaderDBA.size();
	if (numHeadersDBA == 0)
		return;
...
public:
	...
	void SaveCAF()
	{
		...
		for (ResourceSet::const_iterator it = m_set.begin(); it != m_set.end(); ++it)
		{
			...
			uint32 dbacrc32 = g_AnimationUpr.GetDBACRC32fromFilePath(pFilePath);
			uint32 cafcrc32 = CCrc32::ComputeLowercase(pFilePath);

			uint32 pos = 0;
			uint32 rot = 0;
			uint32 use = 0;
			uint32 nNumHeadersCAF = g_AnimationUpr.m_arrGlobalCAF.size();
	...
	void SaveLMG()
	{
		...
		for (ResourceSet::const_iterator it = m_set.begin(); it != m_set.end(); ++it)
		{
			...
			uint32 nNumHeadersLMG = g_AnimationUpr.m_arrGlobalLMG.size();
...
#ifdef EDITOR_PCDEBUGCODE
bool CharacterUpr::InjectCDF(const char* pathname, const char* fileContent, size_t fileLength)
{
...
void CharacterUpr::InjectCHRPARAMS(const char* pathname, const char* content, size_t contentLength)
{
...
void CharacterUpr::ClearCHRPARAMSCache()
{
 ...
 void CharacterUpr::InjectBSPACE(const char* pathname, const char* content, size_t contentLength)
{
...
int32 CharacterUpr::LoadCDF(const char* pathname)
{
...
int32 CharacterUpr::LoadCDFFromXML(XmlNodeRef root, const char* pathname)
{
	LOADING_TIME_PROFILE_SECTION(g_pISystem);
	...
	for (uint32 xmlnode = 0; xmlnode < numChilds; xmlnode++)
	{
		...
		if (strcmp(tag, "AttachmentList") == 0)
		{
			...
			if (numChildren)
			{
				...
				uint32 numValidAttachments = CAttachmentUpr::ParseXMLAttachmentList(&def.m_arrAttachments[0], numChildren, node);
...
ICharacterInstance* CharacterUpr::LoadCharacterDefinition(const string pathname, uint32 nLoadingFlags)
{
...
void CharacterUpr::SkelExtension(CCharInstance* pCharInstance, const char* pFilepathSKEL, const uint32 cdfId, const uint32 nLoadingFlags)
{
...
void CharacterUpr::ExtendDefaultSkeletonWithSkinAttachments(ICharacterInstance* pCharInst, const char* szFilepathSKEL, const char** szSkinAttachments, const uint32 skinsCount, const uint32 nLoadingFlags)
{
...
uint32 CharacterUpr::CompatibilityTest(CDefaultSkeleton* pCDefaultSkeleton, CSkin* pCSkinModel)
{
...
CDefaultSkeleton* CharacterUpr::CreateExtendedSkel(CCharInstance* pCharInstance, CDefaultSkeleton* pDefaultSkeleton, uint64 nExtendedCRC64, const std::vector<const char*>& mismatchingSkins, const uint32 nLoadingFlags)
{
...
void CharacterUpr::ReleaseCDF(const char* pathname)
{
...
uint32 CharacterUpr::GetCDFId(const string& pathname)
{
...
uint32 CharacterUpr::GetOrLoadCDFId(const string& pathname)
{
...
//////////////////////////////////////////////////////////////////////////
void CharacterUpr::GetLoadedModels(IDefaultSkeleton** pIDefaultSkeletons, uint32& nCount) const
{
...
//////////////////////////////////////////////////////////////////////////
void CharacterUpr::LoadAnimationImageFile(const char* filenameCAF, const char* filenameAIM)
{
	LOADING_TIME_PROFILE_SECTION;
...
//////////////////////////////////////////////////////////////////////////
bool CharacterUpr::LoadAnimationImageFileCAF(const char* filenameCAF)
{
	LOADING_TIME_PROFILE_SECTION;
	...
	const uint32 numHeaders = g_AnimationUpr.m_arrGlobalCAF.size();
	if (numHeaders > 0)
	{
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////////////////////
bool CharacterUpr::LoadAnimationImageFileAIM(const char* filenameAIM)
{
...
bool CharacterUpr::DBA_PreLoad(const char* pFilePathDBA, ICharacterUpr::EStreamingDBAPriority priority)
{
...
bool CharacterUpr::DBA_LockStatus(const char* pFilePathDBA, uint32 status, ICharacterUpr::EStreamingDBAPriority priority)
{
	const bool highPriority = (priority == ICharacterUpr::eStreamingDBAPriority_Urgent);
...
bool CharacterUpr::DBA_Unload(const char* pFilePathDBA)
{
	m_AllowStartOfAnimation = 0;
...
//////////////////////////////////////////////////////////////////////////
bool CharacterUpr::CAF_AddRef(uint32 filePathCRC)
{
	int globalID = g_AnimationUpr.m_AnimationMapCAF.GetValueCRC(filePathCRC);
...
//////////////////////////////////////////////////////////////////////////
bool CharacterUpr::CAF_IsLoaded(uint32 filePathCRC) const
{
	bool bRet = false;
...
//////////////////////////////////////////////////////////////////////////
bool CharacterUpr::CAF_Release(uint32 filePathCRC)
{
	int globalID = g_AnimationUpr.m_AnimationMapCAF.GetValueCRC(filePathCRC);
...
//////////////////////////////////////////////////////////////////////////
bool CharacterUpr::CAF_AddRefByGlobalId(int globalID)
{
	bool bRet = false;
...
//////////////////////////////////////////////////////////////////////////
bool CharacterUpr::CAF_ReleaseByGlobalId(int globalID)
{
	bool bRet = false;
...
bool CharacterUpr::StreamKeepCDFResident(const char* szFilePath, int nLod, int nRefAdj, bool bUrgent)
{
	bool bRet = true;
...
const char* CharacterUpr::GetDBAFilePathByGlobalID(int32 globalID) const
{
	if (globalID >= 0)
	{
		int32 num = int32(m_AnimationUpr.m_arrGlobalCAF.size());
		assert(globalID < num);
		if (globalID < num)
		{
			const GlobalAnimationHeaderCAF& rCAF = m_AnimationUpr.m_arrGlobalCAF[globalID];
			for (int i = 0; i < m_AnimationUpr.m_arrGlobalHeaderDBA.size(); i++)
			{
...
uint32 CharacterUpr::GetDatabaseUnloadTimeDelta() const
{
...
bool CharacterUpr::CAF_LoadSynchronously(uint32 filePathCRC)
{
	bool bRet = false;
...
bool CharacterUpr::LMG_LoadSynchronously(uint32 filePathCRC, const IAnimationSet* pAnimationSet)
{
	int globalID = GetAnimationUpr().GetGlobalIDbyFilePathCRC_LMG(filePathCRC);
...
void CharacterUpr::CreateDebugInstances(const char* szCharacterFileName)
{
...
bool CharacterUpr::HasDebugInstancesCreated(const char* szCharacterFileName) const
{
...
void CharacterUpr::RenderBlendSpace(const SRenderingPassInfo& passInfo, ICharacterInstance* pCharacterInstance, float fCharacterScale, unsigned int debugFlags)
{
