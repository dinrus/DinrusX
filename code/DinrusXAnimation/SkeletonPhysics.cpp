...
#include <DinrusX/DinrusXAnimation/SkeletonPhysics.h>
...
#include <DinrusX/DinrusXAnimation/CharacterInstanceProcessing.h>
#include <DinrusX/DinrusXAnimation/DrawHelper.h>
  ...
  const Matrix34& mtxloc)
{
		...
		if (pentype == PE_ARTICULATED)
		{
			...
			for (i = 0; i < nJointCount; i++)
			{
				if (GetModelJointPointer(i)->m_PhysInfo.pPhysGeom)
				{
					...
					{
						uint32 numJoints = rDefaultSkeleton.GetJointCount();
						for (j = 0; j < (int)numJoints && pj.nSelfCollidingParts < DRX_ARRAY_COUNT(iParts); j++)
						{
							const CDefaultSkeleton::SJoint& modelJoint = *GetModelJointPointer(j);
							if (modelJoint.m_PhysInfo.pPhysGeom &&
	...
	for (int i = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1; i >= 0; i--)
  ...
  const Matrix34& mtxloc)
{
	...
	/*IAttachmentObject* pAttachment;
	   for(int i=m_AttachmentUpr.GetAttachmentCount()-1; i>=0; i--)
...
void CSkeletonPhysics::SetJointPhysInfo(uint32 iJoint, const DrxBonePhysics& pi, int nLod)
{
	int i;
...
const DrxBonePhysics& CSkeletonPhysics::GetJointPhysInfo(uint32 iJoint, int nLod) const
{
	const CDefaultSkeleton& rDefaultSkeleton = *m_pInstance->m_pDefaultSkeleton;
...
void ParsePhysInfoProps(const DrxBonePhysics& physInfo, int nLod, pe_params_rope& pr, pe_simulation_params& simp, pe_params_flags& pf, const Quat& qrel)
{
...
void ParsePhysInfoPropsCloth(const DrxBonePhysics& physInfo, int nLod, pe_params_softbody& psb, pe_simulation_params& simp)
{
...
cloth_aux:
	...
	/* IZF temp
	   ...
	   for(int ic=m_pInstance->m_AttachmentUpr.GetAttachmentCount()-1; ic>=0; ic--)
	   {
...
void CSkeletonPhysics::SynchronizeWithPhysicsPost()
{
	...
	if (objectType == CGA || pCharPhys && pCharPhys->GetType() == PE_ARTICULATED && pCharPhys->GetParams(&pf1) && pf1.flags & aef_recorded_physics)
	{
		...
		if (m_bFullSkeletonUpdate)
		{
			...
			int32 numAttachments = m_pInstance->m_AttachmentUpr.GetAttachmentCount() - 1;
