...
void CSimulation::PostUpdate(const CAttachmentUpr* pAttachmentUpr, const char* pJointName)
{
	...
	uint32 numAllProxies = pAttachmentUpr->m_arrProxies.size();
...
void CSimulation::UpdateSimulation(const CAttachmentUpr* pAttachmentUpr, Skeleton::CPoseData& rPoseData, int32 nRedirectionID, const QuatT& rAttModelRelative, QuatT& raddTransformation, const char* pAttName)
{
...
void CSimulation::UpdatePendulumSimulation(const CAttachmentUpr* pAttachmentUpr, Skeleton::CPoseData& rPoseData, int32 nRedirectionID, const QuatT& rAttModelRelative, QuatT& raddTransformation, const char* pAttName)
{
	if (m_nClampType == 0)
		return;
...
#ifdef EDITOR_PCDEBUGCODE
	uint32 nDrawDynamicProxies = ((m_vCapsule.x + m_vCapsule.y) && (pAttachmentUpr->m_nDrawProxies & 1));
...
void CSimulation::UpdateSpringSimulation(const CAttachmentUpr* pAttachmentUpr, Skeleton::CPoseData& rPoseData, int32 nRedirectionID, const QuatT& rAttModelRelative, Vec3& raddTranslation, const char* pAttName)
{
	if (m_nClampType == 0)
		return;
	if (m_useRedirect && nRedirectionID < 0)
	...
	//-------------------------------------------------------------------------------------
#ifdef EDITOR_PCDEBUGCODE
	uint32 nDrawDynamicProxies = m_vCapsule.y && (pAttachmentUpr->m_nDrawProxies & 1);
	if (nDrawDynamicProxies || m_useDebugSetup || Console::GetInst().ca_DrawAllSimulatedSockets)
	{
		g_pAuxGeom->SetRenderFlags(SAuxGeomRenderFlags(e_Def3DPublicRenderflags));
...
void CSimulation::ProjectJointOutOfLozenge(const CAttachmentUpr* pAttachmentUpr, Skeleton::CPoseData& rPoseData, int32 nRedirectionID, const QuatT& rAttModelRelative, const char* pAttName)
{
	if (m_nClampType == 0)
		return;
	if (m_useRedirect && nRedirectionID < 0)
