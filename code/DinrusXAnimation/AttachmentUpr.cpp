...
#include <DinrusX/DinrusXAnimation/AttachmentUpr.h>

#include <DinrusX/CoreX/Renderer/IRenderAuxGeom.h>
#include <DinrusX/DinrusXAnimation/ModelMesh.h>
#include <DinrusX/DinrusXAnimation/SocketSimulation.h>
#include <DinrusX/DinrusXAnimation/AttachmentPRow.h>
...
#include <memory>
#include <DinrusX/DinrusXAnimation/Command_Commands.h>
#include <DinrusX/DinrusXAnimation/Command_Buffer.h>
...
uint32 CAttachmentUpr::LoadAttachmentList(const char* pathname)
{
...
//-----------------------------------------------------------
uint32 CAttachmentUpr::ParseXMLAttachmentList(CharacterAttachment* parrAttachments, uint32 numAttachments, XmlNodeRef nodeAttachements)
{
...
//-----------------------------------------------------------
void CAttachmentUpr::InitAttachmentList(const CharacterAttachment* parrAttachments, uint32 numAttachments, const string pathname, uint32 nLoadingFlags, int nKeepModelInMemory)
{
...
int32 CAttachmentUpr::FindExtraBone(IAttachment* pAttachment)
{
...
int32 CAttachmentUpr::AddExtraBone(IAttachment* pAttachment)
{
...
IAttachment* CAttachmentUpr::CreateAttachment(const char* szAttName, uint32 type, const char* szJointName, bool bCallProject)
{
...
void CAttachmentUpr::PhysicalizeAttachment(int idx, IPhysicalEntity* pent, int nLod)
{
	if (!pent)
...
void CAttachmentUpr::PhysicalizeAttachment(int idx, int nLod, IPhysicalEntity* pent, const Vec3& offset)
{
	IStatObj* pStatObj;
...
void CAttachmentUpr::DephysicalizeAttachment(int idx, IPhysicalEntity* pent)
{
	if (!pent)
		if (!(pent = m_pSkelInstance->GetISkeletonPose()->GetCharacterPhysics()))
			return;
...
int CAttachmentUpr::UpdatePhysicalizedAttachment(int idx, IPhysicalEntity* pent, const QuatT& offset)
{
	IStatObj* pStatObj;
...
int CAttachmentUpr::UpdatePhysAttachmentHideState(int idx, IPhysicalEntity* pent, const Vec3& offset)
{
	IStatObj* pStatObj;
	IAttachment* pIAttachment;
...
int32 CAttachmentUpr::RemoveAttachmentByInterface(const IAttachment* pAttachment, uint32 loadingFlags)
{
	return RemoveAttachmentByName(pAttachment->GetName(), loadingFlags);
}

int32 CAttachmentUpr::RemoveAttachmentByName(const char* szName, uint32 loadingFlags)
{
...
int32 CAttachmentUpr::RemoveAttachmentByNameCRC(uint32 nameCRC, uint32 loadingFlags)
{
...
void CAttachmentUpr::RemoveAttachmentByIndex(uint32 index, uint32 loadingFlags)
{
...
uint32 CAttachmentUpr::ProjectAllAttachment()
{
...
IAttachment* CAttachmentUpr::GetInterfaceByName(const char* szName) const
{
...
IAttachment* CAttachmentUpr::GetInterfaceByIndex(uint32 c) const
{
...
int32 CAttachmentUpr::GetIndexByName(const char* szName) const
{
	const uint32 nameCRC = CCrc32::ComputeLowercase(szName);
	return GetIndexByNameCRC(nameCRC);
}

IAttachment* CAttachmentUpr::GetInterfaceByNameCRC(uint32 nameCRC) const
{
...
int32 CAttachmentUpr::GetIndexByNameCRC(uint32 nameCRC) const
{
...
IAttachment* CAttachmentUpr::GetInterfaceByPhysId(int id) const
{
...
int CAttachmentUpr::GenerateAttachedInstanceContexts()
{
	if (m_TypeSortingRequired)
		SortByType();
	     ...
	     i < m_sortedRanges[eRange_BoneExecuteUnsafe].end; i++)
	{
		...
		if (pIAttachmentObject->GetAttachmentType() == IAttachmentObject::eAttachment_Skeleton)
		{
			...
			if (pChildInstance)
			{
				...
				result += 1;
				int numChildren = pChildInstance->m_AttachmentUpr.GenerateAttachedInstanceContexts();
				result += numChildren;
...
uint32 CAttachmentUpr::RemoveAllAttachments()
{
...
IProxy* CAttachmentUpr::CreateProxy(const char* szProxyName, const char* szJointName)
{
...
IProxy* CAttachmentUpr::GetProxyInterfaceByIndex(uint32 c) const
{
...
IProxy* CAttachmentUpr::GetProxyInterfaceByName(const char* szName) const
{
...
IProxy* CAttachmentUpr::GetProxyInterfaceByCRC(uint32 nameCRC) const
{
...
int32 CAttachmentUpr::GetProxyIndexByName(const char* szName) const
{
...
int32 CAttachmentUpr::GetProxyIndexByCRC(uint32 nameCRC) const
{
	int num = GetProxyCount();
	for (int i = 0; i < num; i++)
	{
...
int32 CAttachmentUpr::RemoveProxyByInterface(const IProxy* pIProxy)
{
	const char* pName = pIProxy->GetName();
	return RemoveProxyByName(pName);
}

int32 CAttachmentUpr::RemoveProxyByName(const char* szName)
{
...
int32 CAttachmentUpr::RemoveProxyByNameCRC(uint32 nameCRC)
{
...
void CAttachmentUpr::RemoveProxyByIndex(uint32 n)
{
	size_t size = m_arrProxies.size();
	if (size == 0)
		return;
...
void CAttachmentUpr::SortByType()
{
	     ...
	     r < m_sortedRanges[eRange_VertexClothOrPendulumRow].end; r++)
	{
		...
		const char* pstr = DrxStringUtils::stristr(psrc, "_x00_");
		if (pstr == 0)
...
const char* CAttachmentUpr::GetProcFunctionName(uint32 idx) const
{
	return 0;
}

const char* CAttachmentUpr::ExecProcFunction(uint32 nCRC32, Skeleton::CPoseData* pPoseData, const char* pstrFunction) const
{
	return 0;
}

void CAttachmentUpr::OnHideAttachment(const IAttachment* pAttachment, uint32 nHideType, bool bHide)
{
...
void* CAttachmentUpr::CModificationCommandBuffer::Alloc(size_t size, size_t align)
{
	DRX_ASSERT(align <= 32);
...
void CAttachmentUpr::ClearAttachmentObject(SAttachmentBase* pAttachment, uint32 nLoadingFlags)
{
...
void CAttachmentUpr::AddAttachmentObject(SAttachmentBase* pAttachment, IAttachmentObject* pModel, ISkin* pISkin /*= 0*/, uint32 nLoadingFlags /*= 0*/)
{
