...
#include <DinrusX/DinrusXAnimation/LoaderTCB.h>
#include <DinrusX/DinrusXAnimation/ModelAnimationSet.h>
#include <DinrusX/DinrusXAnimation/LoaderCGA.h>
#include <DinrusX/DinrusXAnimation/LoaderDBA.h>
#include <DinrusX/CoreX/StlUtils.h>
#include <DinrusX/DinrusXAnimation/ControllerPQ.h>
#include <DinrusX/DinrusXAnimation/ControllerOpt.h>
...
int CAnimationUpr::GetGlobalIDbyFilePath_CAF(const char* szAnimFileName) const
{
...
int CAnimationUpr::GetGlobalIDbyFilePath_AIM(const char* szAnimFilePath) const
{
...
int CAnimationUpr::GetGlobalIDbyFilePath_LMG(const char* szAnimFilePath) const
{
...
int CAnimationUpr::GetGlobalIDbyFilePathCRC_LMG(uint32 crc32) const
{
...
int CAnimationUpr::CreateGAH_CAF(const string& strFilePath)
{
...
int CAnimationUpr::CreateGAH_AIM(const string& strFilePath)
{
...
int CAnimationUpr::CreateGAH_LMG(const string& strFilePath)
{
...
CGlobalHeaderDBA* CAnimationUpr::FindGlobalHeaderByCRC_DBA(uint32 crc)
{
...
bool CAnimationUpr::LoadAnimationTCB(int nAnimId, DynArray<CControllerTCB>& arrNodeAnims, DrxCGALoader* pCGA, const IDefaultSkeleton* pIDefaultSkeleton)
{
...
//////////////////////////////////////////////////////////////////////////
IAnimEventList* CAnimationUpr::GetAnimEventList(const char* animationFilePath)
{
...
const IAnimEventList* CAnimationUpr::GetAnimEventList(const char* animationFilePath) const
{
...
void CAnimationUpr::InitializeSegmentationDataFromAnimEvents(const char* animationFilePath)
{
	const int32 globalCafId = GetGlobalIDbyFilePath_CAF(animationFilePath);
	if (globalCafId < 0)
		return;
...
//---------------------------------------------------------------------------------------------
//---------------------------------------------------------------------------------------------
bool CAnimationUpr::DBA_PreLoad(const char* pFilePathDBA, bool highPriority)
{
...
bool CAnimationUpr::DBA_LockStatus(const char* pFilePathDBA, uint32 status, bool highPriority)
{
	stack_string strPath = pFilePathDBA;
	PathUtil::UnifyFilePath(strPath);
...
bool CAnimationUpr::DBA_Unload(const char* pFilePathDBA)
{
	stack_string strPath = pFilePathDBA;
	PathUtil::UnifyFilePath(strPath);
...
EReloadCAFResult CAnimationUpr::ReloadCAF(const char* szFilePathCAF)
{
			...
			if (status)
			{
				...
				if (pDefaultSkeleton)
				{
					...
					for (uint32 d = 0; d < numDB; d++)
					{
						...
						uint32 IsAIM = (DrxStringUtils::stristr(rAIM.m_FilePath.c_str(), strAnimToken) != 0);
						if (IsAIM)
						{
					...
					for (uint32 d = 0; d < numLB; d++)
					{
						...
						uint32 IsLOOK = (DrxStringUtils::stristr(rAIM.m_FilePath.c_str(), strAnimToken) != 0);
						if (IsLOOK)
						{
...
int CAnimationUpr::ReloadLMG(const char* szFilePath)
{
...
uint32 CAnimationUpr::GetDBACRC32fromFilePath(const char* szFilePathCAF)
{
	uint32 nCRC32 = CCrc32::ComputeLowercase(szFilePathCAF);
...
bool CAnimationUpr::IsDatabaseInMemory(uint32 nDBACRC32)
{
...
void CAnimationUpr::UnloadAnimationAIM(int nGLobalAnimID)
{
...
void CAnimationUpr::DebugAnimUsage(uint32 printtxt)
{
