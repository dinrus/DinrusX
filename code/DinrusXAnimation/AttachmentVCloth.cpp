...
#include <DinrusX/DinrusXAnimation/AttachmentBase.h>
	...
	if (swSkin && pRenderMesh)
	{
		...
		IF(pRenderMesh && bNewFrame, 1)
		{
			...
			else
			{
				...
				void *pUpdatedJobSkinningData = DrxInterlockedCompareExchangePointer(alias_cast<void *volatile *>(pD->m_pSkinningData->pMasterSkinningDataList), pD->m_pSkinningData, pCurrentJobSkinningData);
...
void CClothSimulator::StartStep(float time_interval, const QuatT& location)
{
	...
	uint32 numProxies = m_pAttachmentUpr->GetProxyCount();
...
bool CClothSimulator::CheckAnimationRewind()
{
	...
	int isAnimPlaying = m_pAttachmentUpr->m_pSkelInstance->m_SkeletonAnim.m_IsAnimPlaying;
...
void CClothSimulator::DrawHelperInformation()
{
	...
	if ((m_config.debugPrint >= 100) && (noOfProxies > 0))
	{
		...
		uint32 numAllProxies = m_pAttachmentUpr->GetProxyCount();
		int numClothProxy = -1;
