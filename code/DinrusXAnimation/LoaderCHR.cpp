...
#include <DinrusX/DinrusXAnimation/ModelSkin.h>
...
#include<DinrusX/DinrusXAnimation/FaceEffectorLibrary.h>
...
static bool InitializeBones(CDefaultSkeleton* pDefaultSkeleton, CSkinningInfo* pSkinningInfo, const char* szFilePath)
{
	...
	if (IsIdentiy == 0)
	{
		g_pISystem->Warning(VALIDATOR_MODULE_ANIMATION, VALIDATOR_WARNING, VALIDATOR_FLAG_FILE, szFilePath, "DinrusXAnimation: Default-Skeleton has wrong orientation. Please use the DinrusX conventions when building a model");
...
#ifndef _RELEASE
	{
		uint32 isSKEL = stricmp(szExt, DRX_SKEL_FILE_EXT) == 0;
		if (isSKEL)
		{
...
bool CDefaultSkeleton::LoadAnimations(CParamLoader& paramLoader)
{
	...
	uint32 dddd = g_AnimationUpr.m_arrGlobalCAF.size();
