...
void CGlobalHeaderDBA::CompleteStreamingDBA(CInternalDatabaseInfo* pInfo)
{
	...
	if (g_pCharacterUpr->m_pStreamingListener)
	{
		...
		for (uint32 i = 0; i < numHeadersCAF; i++)
		{
			...
			{
				int32 globalID = g_AnimationUpr.GetGlobalIDbyFilePath_CAF(rGAH.GetFilePath());
...
bool CInternalDatabaseInfo::ReadControllers(IChunkFile::ChunkDesc* pChunkDesc, bool bStreaming)
{
	...
	DrxFatalError("DinrusXAnimation: version 0x%03x of controllers is not supported. Use DBAs generated with latest RC", (int)pChunkDesc->chunkVersion);
	return true;
}
