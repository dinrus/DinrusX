...
#include  <DinrusX/DinrusXNetwork/SocketIOUprIOCP.h>
...
bool CSocketIOUprIOCP::PollWait(uint32 waitTime)
{
	if (m_pWatchdog)
	{
...
int CSocketIOUprIOCP::PollWork(bool& performedWork)
{
	ULONG_PTR sockid_fromOS;
...
void CSocketIOUprIOCP::CheckBackoff()
{
	if (m_completedIO.size() > COMFORTABLE_COMPLETED_BACKLOG && m_completedIO.back().bSuccess && m_completedIO.back().pOverlapped)
	{
		...
		if (pReq->iort == eIORT_RecvFrom)
		{
			...
			{
				NetLog("[CSocketIOUprIOCP::CheckBackoff] %s: Requesting Backoff. Backlog: %i", gEnv->IsDedicated() ? "Server" : "Client", (int)m_completedIO.size());
...
int CSocketIOUprIOCP::PollInnerLoop(int& retval)
{
...
int CSocketIOUprIOCP::PollInner(const SCompletedIO& io)
{
	ASSERT_GLOBAL_LOCK;
...
int CSocketIOUprIOCP::CompleteSuccess(SRegisteredSocket* pSock, SIORequest* pReq, DWORD nIOSize)
{
...
int CSocketIOUprIOCP::CompleteEmptySuccess(SRegisteredSocket* pSock, SIORequest* pReq)
{
	DRX_PROFILE_FUNCTION(PROFILE_NETWORK);
...
int CSocketIOUprIOCP::CompleteFailure(SRegisteredSocket* pSock, SIORequest* pReq, DWORD err)
{
	DRX_PROFILE_FUNCTION(PROFILE_NETWORK);
...
void CSocketIOUprIOCP::PushUserMessage(int msg)
{
...
SSocketID CSocketIOUprIOCP::RegisterSocket(DRXSOCKET sock, int protocol)
{
	ASSERT_GLOBAL_LOCK;
	SCOPED_COMM_LOCK;
...
bool CSocketIOUprIOCP::RequestRecvFrom(SSocketID sockid)
{
	...
	int recvret = WSARecvFrom(rs->sock, pReq->wsaBuffer, 1, &pReq->bytesReceived, &pReq->flags, (DRXSOCKADDR*)pReq->addrBuffer, &pReq->addrLength, &(pReq->overlapped), NULL);
	switch (recvret)
	{
	case 0:
...
bool CSocketIOUprIOCP::RequestSendTo(SSocketID sockid, const TNetAddress& addr, const uint8* pData, size_t len)
{
	SCOPED_COMM_LOCK;
...
bool CSocketIOUprIOCP::RequestSendVoiceTo(SSocketID sockid, const TNetAddress& addr, const uint8* pData, size_t len)
{
	return RequestSendTo(sockid, addr, pData, len);
}

bool CSocketIOUprIOCP::DoRequestSendTo(SRegisteredSocket* rs, const TNetAddress& addr, const uint8* pData, size_t len, CTimeValue now)
{
	ASSERT_COMM_LOCK;
	...
	if (!ConvertAddr(addr, (DRXSOCKADDR*)pReq->addrBuffer, &pReq->addrLength))
		return false;
	int sendret = WSASendTo(rs->sock, pReq->wsaBuffer, 1, &pReq->bytesReceived, 0, (DRXSOCKADDR*)pReq->addrBuffer, pReq->addrLength, &pReq->overlapped, NULL);
...
bool CSocketIOUprIOCP::RequestSend(SSocketID sockid, const uint8* pData, size_t len)
{
	SCOPED_COMM_LOCK;
