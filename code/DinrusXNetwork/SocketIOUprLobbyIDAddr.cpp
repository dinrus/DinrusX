...
#include  <DinrusX/DinrusXNetwork/SocketIOUprLobbyIDAddr.h>
...
bool CSocketIOUprLobbyIDAddr::PollWait(uint32 waitTime)
{
	bool haveData = false;
	m_recvSize = 0;
...
void CSocketIOUprLobbyIDAddr::RecvPacket(void* privateRef, uint8* recvBuffer, uint32 recvSize, DRXSOCKET recvSocket, TNetAddress& recvAddr)
{
...
int CSocketIOUprLobbyIDAddr::PollWork(bool& performedWork)
{
	IDrxMatchMakingPrivate* pMMPrivate = gEnv->pLobby ? gEnv->pLobby->GetMatchMakingPrivate() : nullptr;
	if (pMMPrivate)
	{
...
SSocketID CSocketIOUprLobbyIDAddr::RegisterSocket(DRXSOCKET sock, int protocol)
{
	ASSERT_GLOBAL_LOCK;
	SCOPED_COMM_LOCK;
...
bool CSocketIOUprLobbyIDAddr::RequestSendTo(SSocketID sockid, const TNetAddress& addr, const uint8* pData, size_t len)
{
	return true;
}

bool CSocketIOUprLobbyIDAddr::RequestSendVoiceTo(SSocketID sockid, const TNetAddress& addr, const uint8* pData, size_t len)
{
	return true;
}
...
bool CSocketIOUprLobbyIDAddr::RequestSend(SSocketID sockid, const uint8* pData, size_t len)
{
	return true;
}
