...
TNetAddress ConvertAddr(const DRXSOCKADDR* pSockAddr, int addrLength)
{
...
{
public:
	CConvertAddrVisitor(DRXSOCKADDR* pOut, int* pOutLen) : m_pOut(pOut), m_pOutLen(pOutLen), m_result(false) {}
...
bool ConvertAddr(const TNetAddress& addrIn, DRXSOCKADDR* pSockAddr, int* addrLen)
{
