...
bool MakeSocketNonBlocking(DRXSOCKET sock)
{
	...
	int nonblocking = 1;
	if (DrxSock::setsockopt(sock, SOL_SOCKET, SO_NBIO, (const char*)&nonblocking, sizeof(nonblocking)) == -1)
		return false;
#else
...
template<class T>
static bool SetSockOpt(DRXSOCKET s, int level, int optname, const T& value)
{
	return 0 == DrxSock::setsockopt(s, level, optname, (const char*)&value, sizeof(T));
}
...
#endif
		int bindret = DrxSock::bind(m_socket, static_cast<DRXSOCKADDR*>(pSockAddr), sizeSockAddr);
		if (bindret)
			return InitWinError();
	}
#if (USE_LOBBYIDADDR)
...
{
	int error = DrxSock::GetLastSocketError();
	LogWinError(error);
}
