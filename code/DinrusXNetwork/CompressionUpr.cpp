...
#include  <DinrusX/DinrusXNetwork/SerializationChunk.h>
#include  <DinrusX/DinrusXNetwork/RangedIntPolicy.h>
#include  <DinrusX/DinrusXNetwork/OwnChannelCompressionPolicy.h>
#include  <DinrusX/DinrusXNetwork/ErrorDistributionEncoding.h>
#include <queue>
...
void CCompressionUpr::LoadCompressionPolicy(const char* fileName, XmlNodeRef rootNode, bool bUseCompression)
{
...
ICompressionPolicyPtr CCompressionUpr::CreatePolicy(XmlNodeRef node, const string& filename, uint32 key)
{
	if (!node)
...
ICompressionPolicyPtr CCompressionUpr::GetCompressionPolicy(uint32 key)
{
...
ICompressionPolicyPtr CCompressionUpr::GetCompressionPolicyFallback(uint32 key)
{
...
ChunkID CCompressionUpr::GameContextGetChunkID(IGameContext* pCtx, EntityId id, NetworkAspectType nAspect, uint8 nProfile, NetworkAspectType skipCompression)
{
...
void CCompressionUpr::BufferToStream(ChunkID chunk, uint8 profile, CByteInputStream& in, CNetOutputSerializeImpl& out)
{
	if (CSerializationChunkPtr pChunk = GetChunk(chunk))
		pChunk->EncodeToStream(in, out, chunk, profile);
}

void CCompressionUpr::StreamToBuffer(ChunkID chunk, uint8 profile, CNetInputSerializeImpl& in, CByteOutputStream& out)
{
	if (CSerializationChunkPtr pChunk = GetChunk(chunk))
		pChunk->DecodeFromStream(in, out, chunk, profile);
}

ICompressionPolicyPtr CCompressionUpr::CreateRangedInt(int nMax, uint32 key)
{
