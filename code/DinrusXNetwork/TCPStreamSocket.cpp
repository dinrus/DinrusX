...
//	sain.sin_addr.s_addr = htonl(addr.addr);
//	sain.sin_port = htons(addr.port);
//	int ir = DrxSock::connect( m_pStreamSocket->m_socket, (DRXSOCKADDR*)&sain, sizeof(sain) );
...
void CTCPStreamSocket::OnAccept(const TNetAddress& from, DRXSOCKET s)
{
	...
	int nonblocking = 1;
	DrxSock::setsockopt(s, SOL_SOCKET, SO_NBIO, (const char*)&nonblocking, sizeof(nonblocking));
