...
#include  <DinrusX/DinrusXNetwork/MementoMemoryUpr.h>
...
int CMementoMemoryUpr::CMementoMemoryUprAllocator::m_numCMementoMemoryUprs = 0;
#if MMM_MUTEX_ENABLE
...
void* CMementoMemoryUpr::CMementoMemoryUprAllocator::AllocPtr(size_t sz)
{
	MMM_ASSERT_GLOBAL_LOCK();
	SHandleData hd;
	PREVENT_ZERO_ALLOCS(sz);
...
void CMementoMemoryUpr::CMementoMemoryUprAllocator::FreePtr(void* p, size_t sz)
{
	MMM_ASSERT_GLOBAL_LOCK();
	PREVENT_ZERO_ALLOCS(sz);
#if MMM_MUTEX_ENABLE
...
void CMementoMemoryUpr::CMementoMemoryUprAllocator::DebugDraw(int x, int& y, size_t& totalAllocated)
{
#if ENABLE_NETWORK_MEM_INFO
...
CMementoMemoryUpr::Hdl CMementoMemoryUpr::AllocHdl(size_t sz, void* callerOverride)
{
	MMM_ASSERT_GLOBAL_LOCK();
...
void* CMementoMemoryUpr::AllocPtr(size_t sz, void* callerOverride)
{
	MMM_ASSERT_GLOBAL_LOCK();
	...
	void* ret = CMementoMemoryUprAllocator::GetAllocator()->AllocPtr(sz);
...
void CMementoMemoryUpr::FreePtr(void* p, size_t sz)
{
	MMM_ASSERT_GLOBAL_LOCK();
