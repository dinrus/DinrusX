...
#include  <DinrusX/DinrusXNetwork/SocketIOUprSelect.h>
#include  <DinrusX/DinrusXNetwork/Network.h>
#include  <DinrusX/DinrusXNetwork/UDPDatagramSocket.h>
...
bool CSocketIOUprSelect::PollWait(uint32 waitTime)
{
	bool haveData = false;
...
int CSocketIOUprSelect::PollWork(bool& performedWork)
{
	int r = 0;
	...
	for (size_t i = 0; i < numSockets; i++)
	{
		...
		if (si.NeedRead() && FD_ISSET(si.sock, &m_read))
		{
			if (si.nRecvFrom)
			{
				...
				default:
					...
					si.pRecvFromTarget->OnRecvFromComplete(ConvertAddr((DRXSOCKADDR*)address, addrlen), (uint8*)buffer, r);
					si.nRecvFrom--;
					break;
				}
			}
		...
		if (si.NeedWrite() && FD_ISSET(si.sock, &m_write))
		{
			...
			{
				r = DrxSock::send(si.sock, (char*)si.outgoing.front().data, si.outgoing.front().nLength, 0);
				switch (r)
				{
				case 0:
			...
			while (!done && !si.outgoingAddressed.empty())
			{
				...
				{
					r = DrxSock::sendto(si.sock, (char*)si.outgoingAddressed.front().data, si.outgoingAddressed.front().nLength, 0, (DRXSOCKADDR*)address, _addrlen);
					switch (r)
					{
					case 0:
...
void CSocketIOUprSelect::PushUserMessage(int msg)
{
	...
	#endif // LOCK_NETWORK_FREQUENCY
	DrxSock::sendto(m_wakeupSocket, reinterpret_cast<char*>(&message), sizeof(message), 0, (DRXSOCKADDR*)&m_wakeupAddr, sizeof(m_wakeupAddr));
}
...
SSocketID CSocketIOUprSelect::RegisterSocket(DRXSOCKET sock, int protocol)
{
	uint32 id;
...
bool CSocketIOUprSelect::RequestSendTo(SSocketID sockid, const TNetAddress& addr, const uint8* pData, size_t len)
{
...
delaysend:
	...
	#endif
					int r = DrxSock::sendto(pSI->sock, (char*)pData, len, 0, (DRXSOCKADDR*)address, addrlen);
					switch (r)
					{
					case 0:
...
bool CSocketIOUprSelect::RequestSendVoiceTo(SSocketID sockid, const TNetAddress& addr, const uint8* pData, size_t len)
{
...
bool CSocketIOUprSelect::RequestSend(SSocketID sockid, const uint8* pData, size_t len)
{
	if (SSocketInfo* pSI = GetSocketInfo(sockid))
	{
		if (pSI->pSendTarget)
