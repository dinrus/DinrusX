...
#include  <DinrusX/DinrusXNetwork/NetContextState.h>
...
#include  <DinrusX/DinrusXNetwork/UpdateAspectDataContext.h>
#include  <DinrusX/DinrusXNetwork/AutoFreeHandle.h>
#include  <DinrusX/DinrusXNetwork/Config.h>

#include <DinrusX/CoreX/Game/IGameFramework.h>       // LOCAL_PLAYER_ENTITY_ID
...
CNetContextState::CNetContextState(CNetContext* pContext, int token, CNetContextState* pPrev) : m_pMMM(new CMementoMemoryUpr(string().Format("NetContextState[%d]", token)))
{
	MMM_REGION(m_pMMM);
	...
	else
	{
		...
		if (m_multiplayer)
		{
			...
			for (NetworkAspectID i = 0; i < NumAspects; i++)
			{
				...
				{
					objx.vAspectProfileChunks[i][c] = CNetwork::Get()->GetCompressionUpr().GameContextGetChunkID(m_pGameContext, userId, nAspect, static_cast<uint8>(c), skipCompression);
				}
				nAspect <<= 1;
			}
