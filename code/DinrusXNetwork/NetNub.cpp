...
#include  <DinrusX/DinrusXNetwork/INubMember.h>
...
public:
	...
	void OnCompleted(int cookie, bool success, DRXSOCKADDR_IN* addr)
	{
	...
	void GetAddr(int cookie, DRXSOCKADDR_IN& a) const
	{
		NegVector::const_iterator it = FindNeg(cookie);
		if (it != m_negotiations.end())
			a = it->m_addr;
...
bool CNetNub::SendPendingConnect(SPendingConnection& pc)
{
	...
	if (pc.kes == eKES_SetupInitiated)
	{
		...
		PREFAST_SUPPRESS_WARNING(6255) uint8 * pBuffer = (uint8*) alloca(1 + (VERSION_SIZE)*sizeof(uint32) + 2 * sizeof(uint32) + sizeof(DrxSessionHandle) + cslen);
...
void CNetNub::DisconnectChannel(EDisconnectionCause cause, DrxSessionHandle session, const char* pReason)
{
	SCOPED_GLOBAL_LOCK;
...
#endif
	uint32 socketCaps = htonl(CNetwork::Get()->GetExternalSocketIOUpr().caps);
...
void CNetNub::ProcessSetup(const TNetAddress& from, const uint8* pData, uint32 nSize)
{
	...
	if (nSize < 1 + VERSION_SIZE * sizeof(uint32) + 2 * sizeof(uint32) + sizeof(DrxSessionHandle))
	{
	...
	memcpy(&session, (char*)pData + 1 + VERSION_SIZE * sizeof(uint32) + 2 * sizeof(uint32), sizeof(DrxSessionHandle));
	...
	string connectionString((char*)pData + 1 + VERSION_SIZE * sizeof(uint32) + 2 * sizeof(uint32) + sizeof(DrxSessionHandle), (char*)pData + nSize);
...
void CNetNub::CreateChannel(const TNetAddress& ip, const CExponentialKeyExchange::KEY_TYPE& key, const string& connectionString, uint32 remoteSocketCaps, uint32 proifle, DrxSessionHandle session, CNubConnectingLock conlk)
{
...
void CNetNub::AddDisconnectEntry(const TNetAddress& ip, DrxSessionHandle session, EDisconnectionCause cause, const char* reason)
{
