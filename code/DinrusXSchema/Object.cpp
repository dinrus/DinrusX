...
bool CObject::Init(DrxGUID classGUID, const IObjectPropertiesPtr& pProperties)
{
	...
	DRX_ASSERT_MESSAGE(pClass, "Schematyc class '%s' couldn't be found in class registry.");
	if (!pClass)
	{
		return false;
	}
...
{
	DRX_ASSERT_MESSAGE(m_pClass, "Runtime class of Schematyc Object must be not null.");
	return *m_pClass;
}
