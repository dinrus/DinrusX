...
void CObjectProperties::AddComponent(const DrxGUID& guid, const char* szName, const CClassProperties& properties)
{
	m_components.insert(Components::value_type(guid, SComponent(szName, properties, EOverridePolicy::Default)));
}

void CObjectProperties::AddVariable(const DrxGUID& guid, const char* szName, const CAnyConstRef& value)
{
