...
void CScriptGraphReceiveSignalNode::Register(CScriptGraphNodeFactory& factory)
{
	...
	private:
		...
		public:
			...
			CCreationCommand(const char* szSubject, const SElementId& signalId, const DrxGUID& objectGUID = DrxGUID())
				: m_subject(szSubject)
				, m_signalId(signalId)
				, m_objectGUID(objectGUID)
			{}
		...
		struct SComponent
		{
			inline SComponent(const DrxGUID& _guid, const DrxGUID& _typeGUID, const char* szName)
				: guid(_guid)
				, typeGUID(_typeGUID)
				, name(szName)
			{}
