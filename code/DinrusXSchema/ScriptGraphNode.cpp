...
inline ColorB GetPortColor(const DrxGUID& typeGUID)
{
	...
	for (uint32 portColorIdx = 0; portColorIdx < DRX_ARRAY_COUNT(portColors); ++portColorIdx)
	{
...
SScriptGraphNodePort::SScriptGraphNodePort(const CUniqueId& _id, const char* _szName, const DrxGUID& _typeGUID, const ScriptGraphPortFlags& _flags, const CAnyValuePtr& _pData)
	: id(_id)
	, name(_szName)
	, typeGUID(_typeGUID)
	, flags(_flags)
...
void CScriptGraphNodeLayout::AddInput(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyValuePtr& pData)
{
	// #SchematycTODO : Check for id collisions!!!
	m_inputs.push_back(SScriptGraphNodePort(id, szName, typeGUID, flags, pData));
}

void CScriptGraphNodeLayout::AddOutput(const CUniqueId& id, const char* szName, const DrxGUID& typeGUID, const ScriptGraphPortFlags& flags, const CAnyValuePtr& pData)
{
	// #SchematycTODO : Check for id collisions!!!
...
DrxGUID CScriptGraphNode::GetInputTypeGUID(uint32 inputIdx) const
{
	const ScriptGraphNodePorts& inputs = m_layout.GetInputs();
...
DrxGUID CScriptGraphNode::GetOutputTypeGUID(uint32 outputIdx) const
{
	const ScriptGraphNodePorts& outputs = m_layout.GetOutputs();
