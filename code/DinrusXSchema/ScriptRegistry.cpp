...
IScriptStateMachine* CScriptRegistry::AddStateMachine(const char* szName, EScriptStateMachineLifetime lifetime, const DrxGUID& contextGUID, const DrxGUID& partnerGUID, IScriptElement* pScope)
{
	SCHEMATYC_CORE_ASSERT(gEnv->IsEditor() && szName);
	if (gEnv->IsEditor() && szName)
	{
...
IScriptVariable* CScriptRegistry::AddVariable(const char* szName, const SElementId& typeId, const DrxGUID& baseGUID, IScriptElement* pScope)
{
	SCHEMATYC_CORE_ASSERT(gEnv->IsEditor() && szName);
	if (gEnv->IsEditor() && szName)
	{
...
IScriptSignalReceiver* CScriptRegistry::AddSignalReceiver(const char* szName, EScriptSignalReceiverType type, const DrxGUID& signalGUID, IScriptElement* pScope)
{
	SCHEMATYC_CORE_ASSERT(gEnv->IsEditor());
	if (gEnv->IsEditor())
	{
...
IScriptComponentInstance* CScriptRegistry::AddComponentInstance(const char* szName, const DrxGUID& typeGUID, IScriptElement* pScope)
{
	SCHEMATYC_CORE_ASSERT(gEnv->IsEditor() && szName);
	if (gEnv->IsEditor() && szName)
	{
...
IScriptActionInstance* CScriptRegistry::AddActionInstance(const char* szName, const DrxGUID& actionGUID, const DrxGUID& contextGUID, IScriptElement* pScope)
{
	SCHEMATYC_CORE_ASSERT(gEnv->IsEditor() && szName);
	if (gEnv->IsEditor() && szName)
	{
...
void CScriptRegistry::SaveScript(CScript& script)
{
	...
	DRX_ASSERT_MESSAGE(hasFileName, "Trying to save Schematyc script without a defined file name.");
	if (hasFileName)
	{
