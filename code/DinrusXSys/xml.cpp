...
void CXmlNode::setAttr(const char* key, const DrxGUID& value)
{
	setAttr(key, value.ToString());
}

bool CXmlNode::getAttr(const char* key, DrxGUID& value) const
{
	const char* svalue = GetValue(key);
	if (svalue)
	{
...
inline static char* drx_stpcpy(char* dst, const char* src)
{
	while (src[0])
	{
		dst[0] = src[0];
...
XmlNodeRef XmlParserImp::ParseBuffer(const char* buffer, size_t bufLen, XmlString& errorString, bool bCleanPools)
{
	...
	// This is not binary XML, so let's use text XML parser.
	{
		...
		{
			char str[1024];
			drx_sprintf(str, "%s%s at line %d", errorPrefix, XML_ErrorString(XML_GetErrorCode(m_parser)), (int)XML_GetCurrentLineNumber(m_parser));
			errorString = str;
...
XmlNodeRef XmlParserImp::ParseFile(const char* filename, XmlString& errorString, bool bCleanPools)
{
...

	{
		...
		{
			drx_sprintf(str, "%s%s at line %d (%s)", errorPrefix, XML_ErrorString(XML_GetErrorCode(m_parser)), (int)XML_GetCurrentLineNumber(m_parser), filename);
			errorString = str;
			DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_WARNING, "%s", str);
		}
