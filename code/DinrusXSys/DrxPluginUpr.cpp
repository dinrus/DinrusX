...
#include <DinrusX/CoreX/Extension/IDrxFactoryRegistry.h>
...
struct SNativePluginModule
{
	...
	SNativePluginModule(const char* szPath, IDrxFactory* pFactory = nullptr)
...
CDrxPluginUpr::CDrxPluginUpr(const SSystemInitParams& initParams)
...
void CDrxPluginUpr::LoadProjectPlugins()
{
	...
	for (const SPluginDefinition& pluginDefinition : pluginDefinitions)
	{
		...
		for (const std::pair<uint8, SPluginDefinition>& defaultPlugin : CDrxPluginUpr::GetDefaultPlugins())
		{
...
#if DrxSharedLibrarySupported
bool CDrxPluginUpr::LoadPluginFromDisk(EType type, const char* szPath, bool notifyUserOnFailure)
{
...
{
	for (uint8 i = 0; i < static_cast<uint8>(Drx::IEnginePlugin::EUpdateStep::Count); ++i)
	{
...
void CDrxPluginUpr::OnPluginUpdateFlagsChanged(Drx::IEnginePlugin& plugin, uint8 newFlags, uint8 changedStep)
{
