...
void* GSysAllocDrxMem::Alloc(UPInt size, UPInt align)
{
	const size_t granularity = m_pHeap->GetGranularity();
	size = (size + granularity - 1) & ~(granularity - 1);

	const bool allocateInHeap = size < FlashHeapAllocSizeThreshold;
	void* ptr = allocateInHeap ? m_pHeap->Map(size) : DrxGetIMemoryUpr()->AllocPages(size);
	...
	IF (!ptr, 0)
	{
		DrxGFxLog::GetAccess().LogError("Allocation request for %d bytes failed. Unexpected behavior to occur!", (int)size);
...
bool GSysAllocDrxMem::Free(void* ptr, UPInt size, UPInt align)
{
	#if defined(TRACK_ALLOCATIONS)
