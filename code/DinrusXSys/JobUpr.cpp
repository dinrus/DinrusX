...
#include <DinrusX/DinrusXSys/ThreadBackEnd.h>
#include <DinrusX/DinrusXSys/BlockingBackEnd.h>

#include <DinrusX/DinrusXSys/System.h>
#include <DinrusX/DinrusXSys/CPUDetect.h>

#include <DinrusX/3rdParty/concqueue/concqueue.hpp>
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::Init(uint16 numWorkers)
{
	// Init Job Stats
	ZeroMemory(m_JobStatsInfo.m_pJobStats, sizeof(m_JobStatsInfo.m_pJobStats));

	// Init Worker Stats
	for (uint32 i = 0; i < JobUpr::detail::eJOB_FRAME_STATS; ++i)
	{
	...
	int nWorkerStatsBufSize = sizeof(JobUpr::SWorkerStats) * numWorkers * JobUpr::detail::eJOB_FRAME_STATS;
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::Update(const uint32 curTimeSample)
{
...
void JobUpr::CWorkerBackEndProfiler::GetFrameStatsSummary(SJobFrameStatsSummary& rStats) const
{
	...
	for (uint16 i = 0; i < JobUpr::detail::eJOB_FRAME_STATS_MAX_SUPP_JOBS; ++i)
	{
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::RegisterJob(const uint32 jobId, const char* jobName)
{
	...
	for (uint8 i = 0; i < JobUpr::detail::eJOB_FRAME_STATS; ++i)
		m_JobStatsInfo.m_pJobStats[(i* JobUpr::detail::eJOB_FRAME_STATS_MAX_SUPP_JOBS) +jobId].cpName = jobName;
}

///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::RecordJob(const uint16 profileIndex, const uint8 workerId, const uint32 jobId, const uint32 runTimeMicroSec)
{
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&jobStats.count), nCount + 1, nCount) != nCount);
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&jobStats.usec), nUsec + runTimeMicroSec, nUsec) != nUsec);
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&workerStats.nExecutionPeriod), threadExcutionTime + runTimeMicroSec, threadExcutionTime) != threadExcutionTime);
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&workerStats.nNumJobsExecuted), numJobsExecuted + 1, numJobsExecuted) != numJobsExecuted);
}

///////////////////////////////////////////////////////////////////////////////
uint16 JobUpr::CWorkerBackEndProfiler::GetProfileIndex() const
{
	return m_nCurBufIndex;
}

///////////////////////////////////////////////////////////////////////////////
uint32 JobUpr::CWorkerBackEndProfiler::GetNumWorkers() const
{
	return m_WorkerStatsInfo.m_nNumWorkers;
}

///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::GetWorkerStats(const uint8 nBufferIndex, JobUpr::CWorkerFrameStats& rWorkerStats) const
{
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::GetJobStats(const uint8 nBufferIndex, TJobFrameStatsContainer& rJobStatsContainer, IWorkerBackEndProfiler::EJobSortOrder jobSortOrder) const
{
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::ResetWorkerStats(const uint8 nBufferIndex, const uint32 curTimeSample)
{
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CWorkerBackEndProfiler::ResetJobStats(const uint8 nBufferIndex)
{
...
ColorB JobUpr::CJobUpr::GenerateColorBasedOnName(const char* name)
{
	ColorB color;
...
const JobUpr::TJobHandle JobUpr::CJobUpr::GetJobHandle(const char* cpJobName, const uint32 cStrLen, JobUpr::Invoker pInvoker)
{
...
const char* JobUpr::CJobUpr::GetJobName(JobUpr::Invoker invoker)
{
...
void JobUpr::CJobUpr::AddJob(JobUpr::CJobDelegator& crJob, const JobUpr::TJobHandle cJobHandle)
{
	...
	//reset info block
	unsigned int flagSet = cNoQueue ? 0 : (unsigned int)JobUpr::SInfoBlock::scHasQueue;
...
void JobUpr::CJobUpr::AddLambdaJob(const char* jobName, const std::function<void()>& callback, TPriorityLevel priority, SJobState* pJobState)
{
...
void JobUpr::CJobUpr::Init(uint32 nSysMaxWorker)
{
	// only init once
	if (m_Initialized)
		return;
...
bool JobUpr::CJobUpr::InvokeAsJob(const char* cpJobName) const
{
#if defined(_RELEASE)
...
uint32 JobUpr::CJobUpr::GetWorkerThreadId() const
{
	return JobUpr::detail::GetWorkerThreadId();
}

JobUpr::SJobProfilingData* JobUpr::CJobUpr::GetProfilingData(uint16 nProfilerIndex)
{
#if defined(JOBMANAGER_SUPPORT_PROFILING)
...
uint16 JobUpr::CJobUpr::ReserveProfilingData()
{
#if defined(JOBMANAGER_SUPPORT_PROFILING)
	uint32 nFrameIdx = m_profilingData.GetFillFrameIdx();
	uint32 nProfilingDataEntryIndex = DrxInterlockedIncrement((volatile int*)&m_profilingData.nProfilingDataCounter[nFrameIdx]);
...
void JobUpr::CJobUpr::Update(int nJobSystemProfiler)
{
...
void JobUpr::CJobUpr::PushProfilingMarker(const char* pName)
{
#if defined(JOBMANAGER_SUPPORT_PROFILING)
	// get used thread ids
...
///////////////////////////////////////////////////////////////////////////////
JobUpr::TSemaphoreHandle JobUpr::CJobUpr::AllocateSemaphore(volatile const void* pOwner)
{
	// static checks
	STATIC_CHECK(sizeof(JobUpr::TSemaphoreHandle) == 2, ERROR_SIZE_OF_SEMAPHORE_HANDLE_IS_NOT_2);
	STATIC_CHECK(static_cast<int>(nSemaphorePoolSize) < JobUpr::Detail::nSemaphoreSaltBit, ERROR_SEMAPHORE_POOL_IS_BIGGER_THAN_SALT_HANDLE);
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CJobUpr::DeallocateSemaphore(JobUpr::TSemaphoreHandle nSemaphoreHandle, volatile const void* pOwner)
{
	AUTO_LOCK(m_JobUprLock);
	uint32 nIndex = JobUpr::Detail::SemaphoreHandleToIndex(nSemaphoreHandle);
...
///////////////////////////////////////////////////////////////////////////////
bool JobUpr::CJobUpr::AddRefSemaphore(JobUpr::TSemaphoreHandle nSemaphoreHandle, volatile const void* pOwner)
{
	AUTO_LOCK(m_JobUprLock);
	uint32 nIndex = JobUpr::Detail::SemaphoreHandleToIndex(nSemaphoreHandle);
	assert(nIndex < nSemaphorePoolSize);
...
///////////////////////////////////////////////////////////////////////////////
SJobFinishedConditionVariable* JobUpr::CJobUpr::GetSemaphore(JobUpr::TSemaphoreHandle nSemaphoreHandle, volatile const void* pOwner)
{
	uint32 nIndex = JobUpr::Detail::SemaphoreHandleToIndex(nSemaphoreHandle);
	assert(nIndex < nSemaphorePoolSize);
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::CJobUpr::AddBlockingFallbackJob(JobUpr::SInfoBlock* pInfoBlock, uint32 nWorkerThreadID)
{
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::detail::SetWorkerThreadId(uint32 nWorkerThreadId)
{
	TLS_SET(gWorkerThreadId, (size_t)mark_worker_thread_id(nWorkerThreadId));
}

///////////////////////////////////////////////////////////////////////////////
uint32 JobUpr::detail::GetWorkerThreadId()
{
