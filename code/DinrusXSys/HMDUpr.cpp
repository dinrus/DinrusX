...
// ------------------------------------------------------------------------
void CHmdUpr::RegisterDevice(const char* szDeviceName, IHmdDevice& device)
{
...
// ------------------------------------------------------------------------
void CHmdUpr::UnregisterDevice(const char* szDeviceName)
{
	...
	case EHmdSetupAction::eHmdSetupAction_Init:
		{
			if (gEnv->pConsole)
			{
				...
				if (pVrSupportVar->GetIVal() > 0)
				{
					...
					const char *selectedHmdName = DrxVR::CVars::pSelectedHmdNameVar->GetString();
...
// ------------------------------------------------------------------------
HMDCameraSetup CHmdUpr::GetHMDCameraSetup(int nEye, float projRatio, float fnear) const
{
	if (m_pHmdDevice)
