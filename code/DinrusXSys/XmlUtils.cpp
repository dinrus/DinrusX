...
#include <DinrusX/DinrusXSys/SerializeXMLReader.h>
#include <DinrusX/DinrusXSys/SerializeXMLWriter.h>

#include <DinrusX/DinrusXSys/XMLBinaryWriter.h>
#include <DinrusX/DinrusXSys/XMLBinaryReader.h>

#include <DinrusX/DinrusXSys/XMLPatcher.h>
...
{
public:
	CXmlBinaryDataWriterFile(const char* file) { m_file = gEnv->pDrxPak->FOpen(file, "wb"); }
	~CXmlBinaryDataWriterFile() { if (m_file) gEnv->pDrxPak->FClose(m_file); };
	virtual bool IsOk()                                { return m_file != 0; };
	virtual void Write(const void* pData, size_t size) { if (m_file) gEnv->pDrxPak->FWrite(pData, size, 1, m_file); }
private:
	FILE* m_file;
};
