...
public:
	virtual void OnNotificationNetworkReceive(const void* pBuffer, size_t length)
	{
		...
		const char* path = gEnv->pDrxPak->GetGameFolder();
...
bool CConnectionBase::Validate()
{
	if (m_socket != DRX_INVALID_SOCKET)
	{
		...
		timeval timeOut = { 0, 0 };
		int r = (int)::select(int(m_socket + 1), NULL, &stWriteSockets, &stExceptions, &timeOut); // Ian: DRXSOCKET possible truncation on x64
		if (r < 0)
		{
...
bool CConnectionBase::Send(const void* pBuffer, size_t length)
{
	...
	while (sent < length)
	{
		int r = DrxSock::send(m_socket, (const char*)pBuffer + sent, length - sent, 0);
		if (r < 0)
		{
...
bool CConnectionBase::ReceiveMessage(CListeners& listeners)
{
	...
	int r = DrxSock::recv(m_socket, (char*)&m_bufferHeader[NN_PACKET_HEADER_LENGTH - m_dataLeft], m_dataLeft, 0);
	if (!r)
	{
...
{
	int r = DrxSock::recv(m_socket, (char*)&m_buffer.pData[m_buffer.length - m_dataLeft], m_dataLeft, 0);
	if (!r)
	{
...
CNotificationNetwork* CNotificationNetwork::Create()
{
	...
	if (DrxSock::setsockopt(s, SOL_SOCKET, SO_NBIO, (const char*)&yes, sizeof(int)) < 0)
	{
		DrxSock::closesocket(s);
		s = DRX_INVALID_SOCKET;
		DrxLog("CNotificationNetworkClient::Connect: Failed to set socket to asynchronous operation.");
...
void CNotificationNetwork::ProcessSockets()
{
	...
	timeval timeOut = { 1, 0 };
	int r = (int)DrxSock::select(int(socketMax + 1), &read, NULL, NULL, &timeOut);
	if (r == 0)
		return;
