...
protected:
	virtual void* QueryInterface(const DrxInterfaceID& iid) const
	{
		if (iid == drxiidof<IDrxUnknown>())
			return (void*)(IDrxUnknown*) this;
...
IDrxUnknownPtr CDontLikeMacrosFactory::CreateClassInstance() const
{
	...
	return IDrxUnknownPtr(*static_cast<std::shared_ptr<IDrxUnknown>*>(static_cast<void*>(&p)));
}
