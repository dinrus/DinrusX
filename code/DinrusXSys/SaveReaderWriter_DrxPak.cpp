...
// -------------------------------------------------------------------------
//  File name:   SaveReaderWriter_DrxPak.cpp
...
#include <DinrusX/DinrusXSys/SaveReaderWriter_DrxPak.h>
#include <DinrusX/DinrusXSys/SaveReaderWriter_Memory.h>
...
int TranslateSeekMode(IPlatformOS::ISaveReader::ESeekMode mode)
{
	...
	default:
		{
			DRX_ASSERT_TRACE(false, ("Unrecognized seek mode %i", static_cast<int>(mode)));
			return INVALID_SEEK;
		}
	}
}
...
NO_INLINE_WEAK FILE* CDrxPakFile::FOpen(const char* pName, const char* mode, unsigned nFlags)
{
	return gEnv->pDrxPak->FOpen(pName, mode, nFlags);
}

NO_INLINE_WEAK int CDrxPakFile::FClose(FILE* fp)
{
...
NO_INLINE_WEAK size_t CDrxPakFile::FReadRaw(void* data, size_t length, size_t elems, FILE* fp)
{
	return gEnv->pDrxPak->FReadRaw(data, length, elems, fp);
}

NO_INLINE_WEAK size_t CDrxPakFile::FWrite(const void* data, size_t length, size_t elems, FILE* fp)
{
	return gEnv->pDrxPak->FWrite(data, length, elems, fp);
}

NO_INLINE_WEAK size_t CDrxPakFile::FSeek(FILE* fp, long offset, int mode)
{
...
CDrxPakFile::CDrxPakFile(const char* fileName, const char* szMode)
	: m_fileName(fileName)
	, m_filePos(0)
...
CSaveReader_DrxPak::CSaveReader_DrxPak(const char* fileName)
...
IPlatformOS::EFileOperationCode CSaveReader_DrxPak::ReadBytes(void* data, size_t numBytes)
{
	DRX_ASSERT(gEnv);
	DRX_ASSERT(gEnv->pDrxPak);
...
CSaveWriter_DrxPak::CSaveWriter_DrxPak(const char* fileName)
...
IPlatformOS::EFileOperationCode CSaveWriter_DrxPak::AppendBytes(const void* data, size_t length)
{
	DRX_ASSERT(gEnv);
	DRX_ASSERT(gEnv->pDrxPak);
