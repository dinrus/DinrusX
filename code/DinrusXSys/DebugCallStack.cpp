...
#include <chrono>
	...
	#include <DinrusX/DinrusXSys/JiraClient.h>
	#include <DinrusX/DinrusXSys/System.h>
	#include <DinrusX/CoreX/DrxCustomTypes.h>

	#include <DinrusX/DinrusXSys/resource.h>
...
extern LONG WINAPI DinrusXExceptionFilterMiniDump(struct _EXCEPTION_POINTERS* pExceptionPointers, const char* szDumpPath, MINIDUMP_TYPE mdumpValue);
...
/*
   ...
   {
   DrxLogAlways( "<SymModule> %s: %x",ModuleName,(uint32)BaseOfDll );
   return TRUE;
   }
   ...
   {
   DrxLogAlways( "<Symbol> %08X Size=%08X  :%s",(uint32)pSymInfo->Address,(uint32)pSymInfo->Size,pSymInfo->Name );
   return TRUE;
   }
 */
...
void DebugCallStack::LogExceptionInfo(EXCEPTION_POINTERS* pex)
{
...

	{
		...
		const char* cThreadName = gEnv->pThreadUpr->GetThreadName(nThreadId);
...
void DebugCallStack::MinimalExceptionReport(EXCEPTION_POINTERS* exception_pointer)
{
	...
	if (gEnv->pRenderer)
	{
		...
		gEnv->pThreadUpr->ForEachOtherThread(ResumeTargetThread, (void*)(&g_screenShotThread.GetThreadId()));
		gEnv->pThreadUpr->ForEachOtherThread(ResumeTargetThread, (void*)(&renderThread));
