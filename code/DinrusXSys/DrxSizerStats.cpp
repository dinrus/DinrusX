...
DrxSizerStatsBuilder::DrxSizerStatsBuilder(DrxSizerImpl* pSizer, int nMinSubcomponentBytes) :
	m_pSizer(pSizer),
...
DrxSizerStatsRenderer::DrxSizerStatsRenderer(ISystem* pSystem, DrxSizerStats* pStats, unsigned nMaxSubcomponentDepth, int nMinSubcomponentBytes) :
	m_pStats(pStats),
...
void DrxSizerStatsRenderer::render(bool bRefreshMark)
{
	...
	const char* szOverheadNames[DrxSizerStats::g_numTimers] =
	{
		".Collection",
		".Transformation",
		".Cleanup"
...
void DrxSizerStatsRenderer::dump(bool bUseKB)
{
	...
	for (unsigned i = 0; i < m_pStats->size(); ++i)
	{
		...
		if (rComp.numObjects)
			drx_sprintf(szCount, "%8u", (unsigned int)rComp.numObjects);
		else
			szCount[0] = '\0';
