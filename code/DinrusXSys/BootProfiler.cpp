...
public:
	...
	ILINE unsigned int GetThreadIndexByID(threadID threadID)
	{
		...
		const unsigned int counter = DrxInterlockedIncrement(&m_threadCounter) - 1;     //count to index
...
void CBootProfiler::StopFrame()
{
	...
	if (CV_sys_bp_frames_threshold > 0.0f || bDisablingThresholdMode)
	{
		...
		{
			const int nextOffset = drx_random(0, CV_sys_bp_frames_sample_period_rnd);
			m_countdownToNextSaveSesssion = drxmath::clamp(CV_sys_bp_frames_sample_period + nextOffset, 0, std::numeric_limits<int>::max());
		}
