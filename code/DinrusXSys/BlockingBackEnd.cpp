...
///////////////////////////////////////////////////////////////////////////////
JobUpr::BlockingBackEnd::CBlockingBackEnd::CBlockingBackEnd(JobUpr::SInfoBlock** pRegularWorkerFallbacks, uint32 nRegularWorkerThreads) :
...
///////////////////////////////////////////////////////////////////////////////
bool JobUpr::BlockingBackEnd::CBlockingBackEnd::Init(uint32 nSysMaxWorker)
{
		...
		else
		{
			...
			for (uint32 i = 0; i < m_nRegularWorkerThreads; ++i)
			{
				if (m_pRegularWorkerFallbacks[i])
				{
					...
					do
					{
						pRegularWorkerFallback = const_cast<JobUpr::SInfoBlock*>(*(const_cast<volatile JobUpr::SInfoBlock**>(&m_pRegularWorkerFallbacks[i])));
					}
					while (DrxInterlockedCompareExchangePointer(alias_cast<void* volatile*>(&m_pRegularWorkerFallbacks[i]), pRegularWorkerFallback->pNext, alias_cast<void*>(pRegularWorkerFallback)) != pRegularWorkerFallback);
			...
			if (bFoundBlockingFallbackJob == false)
			{
				...
				do
				{
					...
					currentPullIndex = DrxInterlockedCompareExchange64(alias_cast<volatile int64*>(&m_rJobQueue.pull.index), 0, 0);
					currentPushIndex = DrxInterlockedCompareExchange64(alias_cast<volatile int64*>(&m_rJobQueue.push.index), 0, 0);
#else
					...
					if (DrxInterlockedCompareExchange64(alias_cast<volatile int64*>(&m_rJobQueue.pull.index), newPullIndex, currentPullIndex) == currentPullIndex)
						break;
				...
				uint32 nExtractedCurIndex = static_cast<uint32>(JobUpr::SJobQueuePos::ExtractIndex(currentPullIndex, nPriorityLevel));
	...
	do
	{
		...
		*pQueuePull = curPullPtr;
		...
		do
		{
			...
			unsigned char ret = DrxInterlockedCompareExchange128((volatile int64*)pQueue, exchangeValue[1], exchangeValue[0], resultValue);
		...
		do
		{
			...
			resultValue.doubleWord = DrxInterlockedCompareExchange64((volatile int64*)pQueue, exchangeValue.doubleWord, compareValue.doubleWord);
...
///////////////////////////////////////////////////////////////////////////////
JobUpr::BlockingBackEnd::CBlockingBackEndWorkerThread::CBlockingBackEndWorkerThread(CBlockingBackEnd* pBlockingBackend, DrxFastSemaphore& rSemaphore, JobUpr::SJobQueue_BlockingBackEnd& rJobQueue, JobUpr::SInfoBlock** pRegularWorkerFallbacks, uint32 nRegularWorkerThreads, uint32 nID) :
...
///////////////////////////////////////////////////////////////////////////////
void JobUpr::BlockingBackEnd::CBlockingBackEnd::AddBlockingFallbackJob(JobUpr::SInfoBlock* pInfoBlock, uint32 nWorkerThreadID)
{
	volatile JobUpr::SInfoBlock* pCurrentWorkerFallback = NULL;
	do
	{
		pCurrentWorkerFallback = *(const_cast<volatile JobUpr::SInfoBlock**>(&m_pRegularWorkerFallbacks[nWorkerThreadID]));
		pInfoBlock->pNext = const_cast<JobUpr::SInfoBlock*>(pCurrentWorkerFallback);
	}
	while (DrxInterlockedCompareExchangePointer(alias_cast<void* volatile*>(&m_pRegularWorkerFallbacks[nWorkerThreadID]), pInfoBlock, alias_cast<void*>(pCurrentWorkerFallback)) != pCurrentWorkerFallback);
}
