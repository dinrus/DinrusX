...
#include <DinrusX/DinrusXSys/UnitTestSystem.h>
#include <DinrusX/CoreX/Memory/HeapAllocator.h>
#include <DinrusX/CoreX/String/StringUtils.h>  // drx_strXXX()
#include <DinrusX/CoreX/DrxCustomTypes.h> // DRX_ARRAY_COUNT
...
#include <DinrusX/DinrusXSys/UnitTestExcelReporter.h>
...
int CUnitTestUpr::RunAllTests(EReporterType reporterType)
{
...
void CUnitTestUpr::RunAutoTests(const char* szSuiteName, const char* szTestName)
{
...
void CUnitTestUpr::SetExceptionCause(const char* szExpression, const char* szFile, int line)
{
	if (m_bRunningTest)
	{
...
void DrxUnitTest::CUnitTestUpr::OnAssert(const char* szCondition, const char* szMessage, const char* szFileName, unsigned int fileLineNumber)
{
	if (m_bRunningTest)
	{
		string sCause;
...
void DrxUnitTest::CUnitTestUpr::OnFatalError(const char* szMessage)
{
	if (m_bRunningTest)
	{
		string sCause;
...
DRX_UNIT_TEST_SUITE(BITFIDDLING)
{
	...
	DRX_UNIT_TEST(CUT_BITFIDDLING)
	{
		...
		{
			DRX_UNIT_TEST_ASSERT(IntegerLog2        (uint8(j           )) ==  i     );
			DRX_UNIT_TEST_ASSERT(IntegerLog2        (uint8(j + (j >> 1))) ==  i     );
			DRX_UNIT_TEST_ASSERT(IntegerLog2_RoundUp(uint8(j           )) ==  i     );
			DRX_UNIT_TEST_ASSERT(IntegerLog2_RoundUp(uint8(j + (j >> 1))) == (i + 1));
		}
	}
}
...
DRX_UNIT_TEST_SUITE(Strings)
{
	DRX_UNIT_TEST(CUT_Strings)
	{
		...
		// DrxStringUtils_Internal::compute_length_formatted()
		{
			...
			{
				c.resize(length + 1);
				const int count = DrxStringUtils_Internal::compute_length_formatted("%*s", length, "");
...
DRX_UNIT_TEST(CUT_SimplifyFilePath)
{
	...
	#include <DinrusX/CoreX/RingBuffer.h>
...
DRX_UNIT_TEST(CUT_RingBuffer)
{
	...
	#include <DinrusX/CoreX/DrxVariant.h>

DRX_UNIT_TEST(CUT_Variant)
{
	// Default initialization
	{
		DrxVariant<string, Vec3, short, int> v;
	...
	// Emplace & Get
	{
		DrxVariant<int, string> v;

		v.emplace<int>(5);
		DRX_UNIT_TEST_ASSERT(stl::get<int>(v) == 5);
		DRX_UNIT_TEST_ASSERT(stl::get<0>(v) == 5);
		DRX_UNIT_TEST_ASSERT(stl::get_if<int>(&v) != nullptr);
		DRX_UNIT_TEST_ASSERT((*stl::get_if<int>(&v)) == 5);
	...
	// Copy & Move
	{
		DrxVariant<int, string> v1, v2;

		v1.emplace<int>(0);
		v2.emplace<int>(1);
		v1 = v2;
		DRX_UNIT_TEST_ASSERT(stl::get<int>(v1) == 1);
	...
	// Swap
	{
		...
		v2.emplace<int>(1);
		v1.swap(v2);
		DRX_UNIT_TEST_ASSERT(stl::holds_alternative<int>(v1));
		DRX_UNIT_TEST_ASSERT(stl::get<int>(v1) == 1);
		DRX_UNIT_TEST_ASSERT(stl::holds_alternative<int>(v2));
		DRX_UNIT_TEST_ASSERT(stl::get<int>(v2) == 0);
	...
	// Visit
	{
		auto visitor = [](DrxVariant<int, string>& v)
		{
		...
		DRX_UNIT_TEST_ASSERT(stl::get<int>(v1) == 10);
