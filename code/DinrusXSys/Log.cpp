...
#include <DinrusX/DinrusXNetwork/INetwork.h>  // EvenBalance - M. Quinn
#include <DinrusX/DinrusXSys/System.h>
#include <DinrusX/CoreX/String/Path.h>          // PathUtil::ReplaceExtension()
...
ILINE void LockNoneExclusiveAccess(SExclusiveThreadAccessLock* pExclusiveLock)
{
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&pExclusiveLock->counter), nCurCount + 1, nCurCount) != nCurCount);
}

//////////////////////////////////////////////////////////////////////////
ILINE void UnlockNoneExclusiveAccess(SExclusiveThreadAccessLock* pExclusiveLock)
{
	DrxInterlockedDecrement(alias_cast<volatile int*>(&pExclusiveLock->counter));
}
...
ILINE void LockExclusiveAccess(SExclusiveThreadAccessLock* pExclusiveLock)
{
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&pExclusiveLock->counter), (nCurCount | LOG_EXCLUSIVE_ACCESS_SINGLE_WRITER_LOCK_VALUE), nCurCountNoLock) != nCurCountNoLock);
...
ILINE void UnlockExclusiveAccess(SExclusiveThreadAccessLock* pExclusiveLock)
{
	...
	while (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&pExclusiveLock->counter), (nCurCount & (~LOG_EXCLUSIVE_ACCESS_SINGLE_WRITER_LOCK_VALUE)), nCurCount) != nCurCount);
}
...
void CLog::LogV(const ELogType type, int flags, const char* szFormat, va_list args)
{
	...
	{
		const int sz = DRX_ARRAY_COUNT(m_history);
		int i, j;
