...
#include <DinrusX/DinrusXSys/ConfigScaleform.h>
	...
	#include "GImage.h"
	#include <DinrusX/DinrusXSys/GFxVideoWrapper.h>
	#include <DinrusX/DinrusXSys/GImageInfo_Impl.h>
	#include <DinrusX/DinrusXSys/GTexture_Impl.h>
...
GFile* DrxGFxFileOpener::OpenFile(const char* pUrl, SInt flags, SInt /*mode*/)
{
	assert(pUrl);
...
SInt64 DrxGFxFileOpener::GetFileModifyTime(const char* pUrl)
{
...
void DrxGFxTranslator::SetWordWrappingMode(const char* pLanguage)
{
...
int DrxGFxLog::ms_sys_flash_debuglog(0);
...
void DrxGFxLog::LogMessageVarg(LogMessageType messageType, const char* pFmt, va_list argList)
{
	char logBuf[1024];
	{
...
void DrxGFxLog::SetContext(const char* pFlashContext)
{
	#if defined(DRXGFXLOG_USE_IMPLICT_TLS)
...
const char* DrxGFxLog::GetContext() const
{
	#if defined(DRXGFXLOG_USE_IMPLICT_TLS)
...
void DrxGFxFSCommandHandler::Callback(GFxMovieView* pMovieView, const char* pCommand, const char* pArgs)
{
...
void DrxGFxExternalInterface::Callback(GFxMovieView* pMovieView, const char* pMethodName, const GFxValue* pArgs, UInt numArgs)
{
	CFlashPlayer* pFlashPlayer(static_cast<CFlashPlayer*>(pMovieView->GetUserData()));
	if (pFlashPlayer)
...
GImageInfoBase* DrxGFxImageLoader::LoadImage(const char* pUrl)
{
