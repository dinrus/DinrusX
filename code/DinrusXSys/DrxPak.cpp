...
#include <DinrusX/DinrusXSys/CustomMemoryHeap.h>
...
#include <DinrusX/DinrusXSys/ZipEncrypt.h>
#endif
...
#if DRX_PLATFORM_IOS
	#include <DinrusX/DinrusXSys/SystemUtilsApple.h>
#endif
...
void CDrxPak::SetDecryptionKey(const uint8* pKeyData, uint32 keyLength)
{
...
bool CDrxPak::CheckFileAccessDisabled(const char* name, const char* mode)
{
	...
	if (logInvalidFileAccess | msgInvalidFileAccess)
	{
		...
		if (threadIndex >= 0 && m_disableRuntimeFileAccess[threadIndex])
		{
			...
			{
				const char* sThreadName = gEnv->pThreadUpr->GetThreadName(currentThreadId);
...
void CDrxPak::LogFileAccessCallStack(const char* name, const char* nameFull, const char* mode)
{
...
//////////////////////////////////////////////////////////////////////////
void CDrxPak::AddMod(const char* szMod)
{
...
//////////////////////////////////////////////////////////////////////////
void CDrxPak::RemoveMod(const char* szMod)
{
	DrxPathString strPrepend = szMod;
	strPrepend.replace(g_cNativeSlash, g_cNonNativeSlash);
	strPrepend.MakeLower();
...
//////////////////////////////////////////////////////////////////////////
const char* CDrxPak::GetMod(int index)
{
...
bool CDrxPak::IsInstalledToHDD(const char* acFilePath) const
{
...
//////////////////////////////////////////////////////////////////////////
void CDrxPak::ParseAliases(const char* szCommandLine)
{
...
const char* CDrxPak::GetAlias(const char* szName, bool bReturnSame)
{
...
void CDrxPak::SetGameFolder(const char* szFolder)
{
	assert(szFolder);
...
const char* CDrxPak::GetGameFolder() const
{
...
//////////////////////////////////////////////////////////////////////////
void CDrxPak::SetAlias(const char* szName, const char* szAlias, bool bAdd)
{
...
char* CDrxPak::BeautifyPath(char* dst, bool bMakeLowercase)
{
...
void CDrxPak::RemoveRelativeParts(char* dst)
{
	char* q = dst;
	char* p = NULL;
...
//////////////////////////////////////////////////////////////////////////
inline IDrxPak::SignedFileSize GetFileSizeOnDisk(const char* filename)
{
...
const char* CDrxPak::AdjustFileName(const char* src, char dst[g_nMaxPath], unsigned nFlags)
{
	DRX_ASSERT(src);
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::AdjustAliases(char* dst)
{
...
void CDrxPak::GetCachedPakCDROffsetSize(const char* szName, uint32& offset, uint32& size)
{
...
const char* CDrxPak::AdjustFileNameInternal(const char* src, char* dst, unsigned nFlags)
{
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::CopyFileOnDisk(const char* source, const char* dest, bool bFailIfExist)
{
#if DRX_PLATFORM_WINDOWS
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::IsFileExist(const char* sFilename, EFileSearchLocation fileLocation)
{
	// lock-less check
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::IsFolder(const char* sPath)
{
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
IDrxPak::SignedFileSize CDrxPak::GetFileSizeOnDisk(const char* filename)
{
#if DRX_PLATFORM_ORBIS
	char buf[512];
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::IsFileCompressed(const char* filename)
{
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
//////////////////////////////////////////////////////////////////////////
FILE* CDrxPak::FOpenRaw(const char* pName, const char* mode)
{
...
//////////////////////////////////////////////////////////////////////////
FILE* CDrxPak::FOpen(const char* pName, const char* szMode, char* szFileGamePath, int nLen)
{
	LOADING_TIME_PROFILE_SECTION_ARGS(pName);
...
//////////////////////////////////////////////////////////////////////////
FILE* CDrxPak::FOpen(const char* pName, const char* szMode, unsigned nInputFlags)
{
	LOADING_TIME_PROFILE_SECTION_ARGS(pName);
...
CCachedFileDataPtr CDrxPak::GetFileData(const char* szName, unsigned int& nArchiveFlags)
{
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
bool CDrxPak::WillOpenFromPak(const char* szPath)
{
	int nVarPakPriority = m_pPakVars->nPriority;
...
// the path must be absolute normalized lower-case with forward-slashes
ZipDir::FileEntry* CDrxPak::FindPakFileEntry(const char* szPath, unsigned int& nArchiveFlags, ZipDir::CachePtr* pZip, bool bSkipInMemoryPaks)
{
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
const char* CDrxPak::GetFileArchivePath(FILE* hFile)
{
	AUTO_READLOCK(m_csOpenFiles);
	INT_PTR nPseudoFile = ((INT_PTR)hFile) - g_nPseudoFileIdxOffset;
	if ((UINT_PTR)nPseudoFile < m_arrOpenFiles.size())
...
//////////////////////////////////////////////////////////////////////////
size_t CDrxPak::FGetSize(const char* sFilename, bool bAllowUseFileSystem)
{
	// lock-less GetSize
	DRX_PROFILE_FUNCTION(PROFILE_SYSTEM);
...
size_t CDrxPak::FSeek(FILE* hFile, long seek, int mode)
{
	SAutoCollectFileAcessTime accessTime(this);
...
size_t CDrxPak::FWrite(const void* data, size_t length, size_t elems, FILE* hFile)
{
	SAutoCollectFileAcessTime accessTime(this);
...
//////////////////////////////////////////////////////////////////////////
size_t CDrxPak::FReadRaw(void* pData, size_t nSize, size_t nCount, FILE* hFile)
{
	LOADING_TIME_PROFILE_SECTION;
...
//////////////////////////////////////////////////////////////////////////
size_t CDrxPak::FReadRawAll(void* pData, size_t nFileSize, FILE* hFile)
{
	LOADING_TIME_PROFILE_SECTION;
...
//////////////////////////////////////////////////////////////////////////
void* CDrxPak::FGetCachedFileData(FILE* hFile, size_t& nFileSize)
{
	LOADING_TIME_PROFILE_SECTION;
...
int CDrxPak::FGetErrno()
{
	return errno;
}

int CDrxPak::FScanf(FILE* hFile, const char* format, ...)
{
	AUTO_READLOCK(m_csOpenFiles);
	SAutoCollectFileAcessTime accessTime(this);
	va_list arglist;
...
int CDrxPak::FPrintf(FILE* hFile, const char* szFormat, ...)
{
	SAutoCollectFileAcessTime accessTime(this);
	{
		AUTO_READLOCK(m_csOpenFiles);
...
char* CDrxPak::FGets(char* str, int n, FILE* hFile)
{
	SAutoCollectFileAcessTime accessTime(this);
	{
		AUTO_READLOCK(m_csOpenFiles);
...
int CDrxPak::Ungetc(int c, FILE* hFile)
{
	SAutoCollectFileAcessTime accessTime(this);
	{
		AUTO_READLOCK(m_csOpenFiles);
...
//////////////////////////////////////////////////////////////////////////
intptr_t CDrxPak::FindFirst(const char* pDir, _finddata_t* fd,
...
//////////////////////////////////////////////////////////////////////////
int CDrxPak::FindNext(intptr_t handle, struct _finddata_t* fd)
{
...
int CDrxPak::FindClose(intptr_t handle)
{
	AUTO_MODIFYLOCK(m_csFindData);
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::LoadPakToMemory(const char* pName, IDrxPak::EInMemoryPakLocation nLoadPakToMemory, IMemoryBlock* pMemoryBlock)
{
	LOADING_TIME_PROFILE_SECTION_ARGS(pName);
...
//////////////////////////////////////////////////////////////////////////
void CDrxPak::LoadPaksToMemory(int nMaxPakSize, bool bLoadToMemory)
{
	LOADING_TIME_PROFILE_SECTION;
...
//======================================================================
bool CDrxPak::OpenPack(const char* szBindRootIn, const char* szPath, unsigned nFlags, IMemoryBlock* pData, DrxFixedStringT<g_nMaxPath>* pFullPath)
{
	assert(szBindRootIn);
	char szFullPathBuf[g_nMaxPath];
...
bool CDrxPak::OpenPack(const char* szPath, unsigned nFlags, IMemoryBlock* pData, DrxFixedStringT<g_nMaxPath>* pFullPath)
{
	char szFullPathBuf[g_nMaxPath];
...
bool CDrxPak::OpenPackCommon(const char* szBindRoot, const char* szFullPath, unsigned int nPakFlags, IMemoryBlock* pData)
{
	...
	int flags = IDrxArchive::FLAGS_OPTIMIZED_READ_ONLY | IDrxArchive::FLAGS_ABSOLUTE_PATHS;
...
bool CDrxPak::ClosePack(const char* pName, unsigned nFlags)
{
...
bool CDrxPak::FindPacks(const char* pWildcardIn)
{
...
bool CDrxPak::OpenPacks(const char* pWildcardIn, unsigned nFlags, std::vector<DrxFixedStringT<g_nMaxPath>>* pFullPaths)
{
	char cWorkBuf[g_nMaxPath];
...
bool CDrxPak::OpenPacks(const char* szBindRoot, const char* pWildcardIn, unsigned nFlags, std::vector<DrxFixedStringT<g_nMaxPath>>* pFullPaths)
{
	char cWorkBuf[g_nMaxPath];
	AdjustFileName(pWildcardIn, cWorkBuf, nFlags | FLAGS_COPY_DEST_ALWAYS);
...
bool CDrxPak::OpenPacksCommon(const char* szDir, const char* pWildcardIn, char* cWork, int nPakFlags, std::vector<DrxFixedStringT<g_nMaxPath>>* pFullPaths)
{
...
bool CDrxPak::ClosePacks(const char* pWildcardIn, unsigned nFlags)
{
	__finddata64_t fd;
	char cWorkBuf[g_nMaxPath];
...
bool CDrxPak::ReOpenPack(const char* pPath)
{
	AUTO_MODIFYLOCK(m_csZips);
...
int CDrxPak::RemountPacks(DynArray<FILE*>& outHandlesToClose, CBShouldPackReOpen pShouldPackReOpen)
{
	AUTO_MODIFYLOCK(m_csZips);
...
bool CDrxPak::InitPack(const char* szBasePath, unsigned nFlags)
{
#if DRX_PLATFORM_IOS
	char buffer[1024];
...
/////////////////////////////////////////////////////
bool CDrxPak::Init(const char* szBasePath)
{
...
CCachedFileData::CCachedFileData(class CDrxPak* pPak, ZipDir::Cache* pZip, unsigned int nArchiveFlags, ZipDir::FileEntry* pFileEntry, const char* szFilename)
{
	m_pPak = pPak;
...
{
	const int nCount = DrxInterlockedDecrement(&m_nRefCounter);
	assert(nCount >= 0);
	if (nCount == 0)
	{
...
//////////////////////////////////////////////////////////////////////////
void CDrxPakFindData::Scan(class CDrxPak* pPak, const char* szDir, bool bAllowUseFS)
{
	// get the priority into local variable to avoid it changing in the course of
	// this function execution
...
CDrxPakFindData::CDrxPakFindData(const char* szNameFilter)
	: m_szNameFilter(szNameFilter)
{
}

//////////////////////////////////////////////////////////////////////////
void CDrxPakFindData::ScanFS(CDrxPak* pPak, const char* szDirIn)
{
...
//////////////////////////////////////////////////////////////////////////
void CDrxPakFindData::ScanZips(CDrxPak* pPak, const char* szDir)
{
...
bool CDrxPak::MakeDir(const char* szPath, bool bGamePathMapping)
{
	if (0 == szPath[0])
	{
		return true;
...
uint32 CDrxPak::ComputeCRC(const char* szPath, uint32 nFileOpenFlags)
{
	assert(szPath);
...
bool CDrxPak::ComputeMD5(const char* szPath, unsigned char* md5, uint32 nFileOpenFlags)
{
	if (!szPath || !md5)
		return false;
...
int CDrxPak::ComputeCachedPakCDR_CRC(const char* filename, bool useDrxFile /*=true*/, IMemoryBlock* pData /*=NULL*/)
{
	uint32 offset = 0;
	uint32 size = 0;
	int retval = 0;
...
IDrxArchive* CDrxPak::FindArchive(const char* szFullPath)
{
	AUTO_READLOCK(m_csZips);
...
const IDrxArchive* CDrxPak::FindArchive(const char* szFullPath) const
{
	AUTO_READLOCK(m_csZips);
...
// MT-safe
int CDrxPak::RawCompress(const void* pUncompressed, unsigned long* pDestSize, void* pCompressed, unsigned long nSrcSize, int nLevel)
{
...
int CDrxPak::RawUncompress(void* pUncompressed, unsigned long* pDestSize, const void* pCompressed, unsigned long nSrcSize)
{
...
void CDrxPak::RecordFileOpen(const ERecordFileOpenList eList)
{
	...
	//DrxLog("RecordFileOpen(%d)",(int)eList);
...
//////////////////////////////////////////////////////////////////////////
void CDrxPak::RecordFile(FILE* in, const char* szFilename)
{
	//if (m_pLog)
...
void CDrxPak::OnMissingFile(const char* szPath)
{
	AUTO_LOCK_CS(m_csMissingFiles);
...
static char* drx_strdup(const char* szSource)
{
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::RemoveFile(const char* pName)
{
	char szFullPathBuf[g_nMaxPath];
	const char* szFullPath = AdjustFileName(pName, szFullPathBuf, 0);
	uint32 dwAttr = DrxGetFileAttributes(szFullPath);
	bool ok = false;
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::RemoveDir(const char* pName, bool bRecurse)
{
	char szFullPathBuf[g_nMaxPath];
	const char* szFullPath = AdjustFileName(pName, szFullPathBuf, 0);
...
//////////////////////////////////////////////////////////////////////////
bool CDrxPak::IsAbsPath(const char* pPath)
{
...
void* CDrxPak::PoolMalloc(size_t size)
{
	return g_pPakHeap->TempAlloc(size, "CDrxPak::GetPoolRealloc");
}

void CDrxPak::PoolFree(void* p)
{
...
IMemoryBlock* CDrxPak::PoolAllocMemoryBlock(size_t nSize, const char* sUsage, size_t nAlign)
{
...
uint64 CDrxPak::GetFileOffsetOnMedia(const char* sFilename)
{
	char szFullPathBuf[g_nMaxPath];
	const char* szFullPath = AdjustFileName(sFilename, szFullPathBuf, FOPEN_HINT_QUIET);
...
EStreamSourceMediaType CDrxPak::GetFileMediaType(const char* szName)
{
	char szFullPathBuf[g_nMaxPath];
	const char* szFullPath = gEnv->pDrxPak->AdjustFileName(szName, szFullPathBuf, IDrxPak::FOPEN_HINT_QUIET);
...
EStreamSourceMediaType CDrxPak::GetMediaType(ZipDir::Cache* pZip, unsigned int nArchiveFlags)
{
...
bool CDrxPak::SetPacksAccessible(bool bAccessible, const char* pWildcard, unsigned nFlags)
{
	__finddata64_t fd;
	char cWorkBuf[g_nMaxPath];
...
bool CDrxPak::SetPackAccessible(bool bAccessible, const char* pName, unsigned nFlags)
{
	char szZipPathBuf[g_nMaxPath];
	const char* szZipPath = AdjustFileName(pName, szZipPathBuf, nFlags);
...
void CDrxPak::SetPacksAccessibleForLevel(const char* sLevelName)
{
	LOADING_TIME_PROFILE_SECTION;
...
bool CDrxPak::ForEachArchiveFolderEntry(const char* szArchivePath, const char* szFolderPath, const ArchiveEntrySinkFunction& callback)
{
	char szFullPathBuf[CDrxPak::g_nMaxPath];
