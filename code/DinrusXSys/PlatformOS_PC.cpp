...
#include <DinrusX/DinrusXSys/StdAfx.h>
	...
	#include <DinrusX/DinrusXSys/PlatformOS_PC.h>
...
bool CPlatformOS_PC::DecryptAndCheckSigning(const char* pInData, int inDataLen, const char** pOutData, int* pOutDataLen, const uint8 key[16])
{
	...
	if (inDataLen > 16)
	{
		if (cipher)
		{
			pOutput = new unsigned char[inDataLen];
			if (pOutput)
			{
				...
				pZLib->MD5Init(&ctx);
				pZLib->MD5Update(&ctx, (const unsigned char*)(key + 8), 8); // Use the last 8 bytes of the key as the signing key
		...
		if (nFS != -1)
		{
			do
			{
				...
				if (pFile)
				{
					...
					int decryptedSize = 0;
					if (DecryptAndCheckSigning(pData, size, (const char**) &pDecryptedData, &decryptedSize, keyData))
					{
...
IPlatformOS::ECDP_Open CPlatformOS_PC::DoesCachePakExist(const char* const filename, const size_t size, unsigned char md5[16])
{
	...
	const uint32 fileOpenFlags = IDrxPak::FLAGS_NEVER_IN_PAK | IDrxPak::FLAGS_PATH_REAL | IDrxPak::FOPEN_ONDISK;
...
IPlatformOS::ECDP_Open CPlatformOS_PC::OpenCachePak(const char* const filename, const char* const bindRoot, const size_t size, unsigned char md5[16])
{
	...
	const uint32 fileOpenFlags = IDrxPak::FLAGS_NEVER_IN_PAK | IDrxPak::FLAGS_PATH_REAL | IDrxPak::FOPEN_ONDISK;
	const uint32 pakOpenFlags = fileOpenFlags | IDrxArchive::FLAGS_OVERRIDE_PAK;
...
IPlatformOS::ECDP_Close CPlatformOS_PC::CloseCachePak(const char* const filename)
{
	...
	DrxFixedStringT<128> realFileName;
	realFileName.Format("%%USER%%/cache/%s", filename);
	const uint32 pakOpenFlags = IDrxPak::FLAGS_NEVER_IN_PAK | IDrxPak::FLAGS_PATH_REAL | IDrxPak::FOPEN_ONDISK | IDrxArchive::FLAGS_OVERRIDE_PAK;
...
IPlatformOS::ECDP_Write CPlatformOS_PC::WriteCachePak(const char* const filename, const void* const pData, const size_t numBytes)
{
	...
	DrxFixedStringT<128> realFileName;
	realFileName.Format("%%USER%%/cache/%s", filename);
	const uint32 nWriteFlags = IDrxPak::FLAGS_PATH_REAL | IDrxPak::FOPEN_ONDISK;
	...
	const int ret = gEnv->pDrxPak->FClose(file);
	if (ret != 0)
	{
