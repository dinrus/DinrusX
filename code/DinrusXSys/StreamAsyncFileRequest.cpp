...
#include <DinrusX/DinrusXSys/StreamAsyncFileRequest.h>
...
uint32 CAsyncIOFileRequest::OpenFile(CDrxFile& file)
{
...
uint32 CAsyncIOFileRequest::ReadFileInPages(CStreamingIOThread* pIOThread, CDrxFile& file)
{
...
void CAsyncIOFileRequest::ComputeSortKey(uint64 nCurrentKeyInProgress)
{
	...
	const int g_nMaxPath = 0x800;
	char szFullPathBuf[g_nMaxPath];
	const char* szFullPath = gEnv->pDrxPak->AdjustFileName(m_strFileName.c_str(), szFullPathBuf, IDrxPak::FOPEN_HINT_QUIET);
