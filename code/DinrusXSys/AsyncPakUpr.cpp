...
bool CAsyncPakUpr::LoadLayerPak(const char* sLayerName)
{
...
bool CAsyncPakUpr::LoadPakToMemAsync(const char* pPath, bool bLevelLoadOnly)
{
	...
	else
	{
		...
		const char* szFullPath = gEnv->pDrxPak->AdjustFileName(pPath, szFullPathBuf, IDrxPak::FOPEN_HINT_QUIET | IDrxPak::FLAGS_PATH_REAL);
...
void CAsyncPakUpr::UnloadLayerPak(const char* sLayerName)
{
	TPakMap::iterator findResult = m_paks.find(sLayerName);
	if (findResult == m_paks.end())
		return;
...
void* CAsyncPakUpr::StreamOnNeedStorage(IReadStream* pStream, unsigned nSize, bool& bAbortOnFailToAlloc)
{
