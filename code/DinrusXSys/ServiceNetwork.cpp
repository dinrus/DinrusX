...
#include <DinrusX/DinrusXSys/RemoteCommandHelpers.h>
...
bool CServiceNetworkConnection::TryReconnect()
{
	...
	// Connect (blocking)
	const int result = DrxSock::connect(m_socket, (const DRXSOCKADDR*)&addr, sizeof(addr));
	if (result == 0)
	{
...
void CServiceNetworkConnection::ProcessSendingQueue()
{
	...
	if (m_messageDataSentSoFar >= endOfDataOffset)
	{
		...
		DrxInterlockedAdd(&m_sendQueueDataSize, -(int)m_pSendedMessages->GetSize());

		// stats (not collected in release builds)
#ifndef RELEASE
		DrxInterlockedIncrement((volatile int*) &m_statsNumPacketsSend);
#endif
...
void CServiceNetworkConnection::ProcessReceivingQueue()
{
	...
	const uint32 kReceivedDataLimit = m_pUpr->GetReceivedDataQueueLimit();
	...
	if (m_messageDataReceivedSoFar >= kOffsetData)
	{
		...
		if (m_messageDataReceivedSoFar == (kOffsetData + m_pCurrentReceiveMessage->GetSize()))
		{
			...
			// Stats (not collected in release builds)
#ifndef RELEASE
			DrxInterlockedIncrement((volatile int*) &m_statsNumPacketsReceived);
#endif
...
{
	// Send the data
	const int ret = DrxSock::send(m_socket, (const char*) dataBuffer, dataSize, 0);
	...
	// Update stats
#ifndef RELEASE
	DrxInterlockedAdd((volatile int*) &m_statsNumDataSend, ret);
#endif
...
{
	// Send the data
	const int ret = DrxSock::recv(m_socket, (char*) dataBuffer, dataSize, 0);
	const DrxSock::eDrxSockError errCode = DrxSock::TranslateSocketError(ret);
	if (errCode != DrxSock::eCSE_NO_ERROR)
	{
		// We would block, that's not an error
	...
	// Update stats
#ifndef RELEASE
	DrxInterlockedAdd((volatile int*) &m_statsNumDataReceived, ret);
#endif
...
bool CServiceNetworkConnection::SendMsg(IServiceNetworkMessage* message)
{
	...
	// Process data size limits
	{
		...
		const uint32 sendQueueLimit = m_pUpr->GetSendDataQueueLimit();
...
bool CServiceNetworkConnection::HandleReconnect(DRXSOCKET socket, const uint32 tryCount)
{
...
void CServiceNetworkListener::ProcessPendingConnections()
{
	     ...
	     it != m_pPendingConnections.end(); /*++it*/)
	{
		...
		const int size = DrxSock::recv(con.m_socket, (char*)&con.m_initHeader + con.m_dataReceivedSoFar, dataLeft, 0);
...
IServiceNetworkMessage* CServiceNetwork::AllocMessageBuffer(const uint32 size)
{
	...
	const uint32 bufferID = DrxInterlockedIncrement(&m_bufferID);
...
IServiceNetworkListener* CServiceNetwork::CreateListener(uint16 localPort)
{
...
#if DRX_PLATFORM_WINDOWS
	const int yes = 1;
	const int ret3 = DrxSock::setsockopt(createdSocket, SOL_SOCKET, TCP_NODELAY, (const char*)&yes, sizeof(int));
	if (ret3 < 0)
	{
		// Connection has other problems
	...
	{
		int valYes = 1;
		if (DrxSock::setsockopt(createdSocket, SOL_SOCKET, SO_REUSEADDR, (const char*)&valYes, sizeof(int)) < 0)
		{
			DrxSock::closesocket(createdSocket);
	...
	// Bind socket
	const int ret = DrxSock::bind(createdSocket, (DRXSOCKADDR*) &service, sizeof(service));
	if (ret < 0)
	{
		// Connection has other problems
	...
	const int ret2 = DrxSock::listen(createdSocket, 64 /*backLogSize*/);
	if (ret2 < 0)
	{
		// Connection has other problems
...
IServiceNetworkConnection* CServiceNetwork::Connect(const ServiceNetworkAddress& remoteAddress)
{
	...
	// Connect (blocking)
	const int result = DrxSock::connect(socket, (const DRXSOCKADDR*)&addr, sizeof(addr));
	if (result != 0)
	{
		// Spew to log (important info)
