...
public:
	...
	~CFlashFunctionProfilerLight()
	{
		...
		while (true)
		{
			...
			IF (curAccum == (uint32) DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&ms_deltaTicksAccum), (LONG) (curAccum + delta), (LONG) curAccum), 1)
				break;
		}
		#else
	...
	static void Update()
	{
		...
		{
			const uint32 curAccum = ms_deltaTicksAccum;
			IF (curAccum == (uint32) DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&ms_deltaTicksAccum), 0, (LONG) curAccum), 1)
				break;
		}
		#else
...
{
public:
	CFlashLogContext(DrxGFxLog* pLog, const char* pFlashContext)
		: m_pLog(pLog)
		, m_pPrevLogContext(0)
	{
...
CFlashPlayer::~CFlashPlayer()
{
	...
	// release ref counts of all shared resources
	{
		...
		if (extRefs > 0)
		{
			DrxGFxLog::GetAccess().LogError("Releasing flash player object while still holding %d associated flash variable object reference(s)! Enforce breaking into the debugger...", (int) extRefs);
			//__debugbreak();
		}
	#endif
