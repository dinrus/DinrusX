...
void CZLibCompressor::MD5Update(SMD5Context* pIOCtx, const unsigned char* pInBuff, unsigned int len)
{
	::MD5Update(pIOCtx, pInBuff, len);
}

void CZLibCompressor::MD5Final(SMD5Context* pIOCtx, unsigned char outDigest[16])
{
...
EZDeflateState CZLibDeflateStream::RunDeflate()
{
	...
	if (runDeflate)
	{
		if (!m_streamOpened)
		{
			...
			int error = deflateInit2(&m_compressStream, m_level, Lookup(m_method, k_methodMap, DRX_ARRAY_COUNT(k_methodMap)), m_windowBits, m_memLevel, Lookup(m_strategy, k_stratMap, DRX_ARRAY_COUNT(k_stratMap)));
			if (error != Z_OK)
			{
