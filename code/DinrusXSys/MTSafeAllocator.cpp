	...
	m_TempAllocationsTime()
{
	...
	uint8* backingStorage = reinterpret_cast<uint8*>(DrxModuleMemalign(MTSAFE_TEMPORARY_POOL_SIZE, 128));
	if (!backingStorage)
	{
...
#if MTSAFE_PROFILE
	DrxInterlockedAdd((volatile int*)&m_TempAllocationsFailed, (int)nSize);
#endif
	{
		// provide hint to mem replay on debug source
		MEMSTAT_CONTEXT(EMemStatContextTypes::MSC_Other, 0, szDbgSource);
