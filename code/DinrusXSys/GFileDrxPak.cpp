...
GFileDrxPak::GFileDrxPak(const char* pPath, unsigned int openFlags)
	: GFile()
	, m_pPak(0)
	, m_fileHandle(0)
...
const char* GFileDrxPak::GetFilePath()
{
...
inline long GFileDrxPak::SeekInternal(long offset, int origin)
{
	if (IsValidInternal())
	{
		int newOrigin = 0;
...
GFileDrxStream::GFileDrxStream(const char* pPath, EStreamTaskType streamType)
	: GFile()
	, m_pStreamEngine(0)
	, m_relativeFilePath(pPath)
	, m_fileSize(0)
...
const char* GFileDrxStream::GetFilePath()
{
	return m_relativeFilePath.ToCStr();
}
...
inline long GFileDrxStream::SeekInternal(long offset, int origin)
{
	if (IsValidInternal())
	{
		switch (origin)
...
GFileInMemoryDrxStream::GFileInMemoryDrxStream(const char* pPath, EStreamTaskType streamType)
	: GFile()
	, m_pMemFile(0)
	, m_pData(0)
	, m_dataSize(0)
...
const char* GFileInMemoryDrxStream::GetFilePath()
{
