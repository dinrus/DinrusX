...
const char* CProjectUpr::GetCurrentProjectName() const
{
...
const char* CProjectUpr::GetCurrentProjectDirectoryAbsolute() const
{
	return m_project.rootDirectory;
}

const char* CProjectUpr::GetCurrentAssetDirectoryRelative() const
{
	return gEnv->pDrxPak->GetGameFolder();
}

const char* CProjectUpr::GetCurrentAssetDirectoryAbsolute() const
{
	return m_project.assetDirectoryFullPath;
}

const char* CProjectUpr::GetProjectFilePath() const
{
	return m_project.filePath;
}

void CProjectUpr::StoreConsoleVariable(const char* szCVarName, const char* szValue)
{
...
bool CProjectUpr::ParseProjectFile()
{
	...
	int flags = IDrxPak::FOPEN_ONDISK;
#else
	int flags = 0;
#endif
	CDrxFile file;
...
void CProjectUpr::OnLoadConfigurationEntry(const char* szKey, const char* szValue, const char* szGroup)
{
...
void CProjectUpr::AddDefaultPlugins(unsigned int previousVersion)
{
...
string CProjectUpr::LoadTemplateFile(const char* szPath, std::function<string(const char* szAlias)> aliasReplacementFunc) const
{
...
void CProjectUpr::GetPluginInfo(uint16 index, Drx::IPluginUpr::EType& typeOut, string& pathOut, DynArray<EPlatform>& platformsOut) const
{
