...
inline uint32 DrxGetCurrentThreadId32()
{
	...
	return *reinterpret_cast<uint32*>(DrxGetCurrentThreadId());
}
#else
inline uint32 DrxGetCurrentThreadId32()
{
	return static_cast<uint32>(DrxGetCurrentThreadId());
}
#endif
...
bool ReplaySocketWriter::Open()
{
	...
	int connectErr = DRX_SOCKET_ERROR;
	for (uint16 port = m_port; (connectErr == DRX_SOCKET_ERROR) && port < (29494 + 32); ++port)
	{
...
void CMemReplay::RecordModuleLoad(void* pSelf, const CReplayModules::ModuleLoadDesc& mld)
{
	...
	const uint32 threadId = DrxGetCurrentThreadId32();
...
MemReplayDrxSizer::MemReplayDrxSizer(ReplayLogStream& stream, const char* name)
	: m_stream(&stream)
	, m_totalSize(0)
	, m_totalCount(0)
{
...
bool MemReplayDrxSizer::AddObject(const void* pIdentifier, size_t nSizeBytes, int nCount)
{
	bool ret = false;
...
void MemReplayDrxSizer::Push(const char* szComponentName)
{
	if (m_stream->IsOpen())
	{
		DrxAutoLock<DrxCriticalSection> lock(GetLogMutex());
...
void MemReplayDrxSizer::PushSubcomponent(const char* szSubcomponentName)
{
