...
#include <DinrusX/DinrusXSys/XMLBinaryNode.h>
...
void CBinaryXmlNode::setAttr(const char* key, const DrxGUID& value)
{
	setAttr(key, value.ToString());
}

bool CBinaryXmlNode::getAttr(const char* key, DrxGUID& value) const
{
	const char* svalue = GetValue(key);
	if (svalue)
	{
