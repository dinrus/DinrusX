...
IRemoteCommandServer* CRemoteCommandUpr::CreateServer(uint16 localPort)
{
	// Create the listener
...
#ifndef RELEASE
bool CRemoteCommandUpr::CheckVerbose(const uint32 level) const
{
	const int verboseLevel = m_pVerboseLevel->GetIVal();
	return (int)level < verboseLevel;
}

void CRemoteCommandUpr::Log(const char* txt, ...)  const
{
...
void CRemoteCommandUpr::SetVerbosityLevel(const uint32 level)
{
...
bool CRemoteCommandUpr::FindClassId(IRemoteCommandClass* commandClass, uint32& outClassId) const
{
