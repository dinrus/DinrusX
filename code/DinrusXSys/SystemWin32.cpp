...
#include <DinrusX/DinrusXSys/IDebugCallStack.h>
...
#include <DinrusX/DinrusXSys/DrxSizerStats.h>
...
#include <DinrusX/DinrusXSys/XmlUtils.h>
...
//////////////////////////////////////////////////////////////////////////
const char g_szGroupCore[] = "DinrusX";
const char* g_szModuleGroups[][2] = {
	{ "DinrusSandbox.exe",         g_szGroupCore },
	{ "DinrusXSys.dll",       g_szGroupCore },
	{ "DinrusXScript.dll", g_szGroupCore },
	{ "DinrusXNetwork.dll",      g_szGroupCore },
	{ "DinrusXPhys.dll",      g_szGroupCore },
	{ "DinrusXMovie.dll",        g_szGroupCore },
	{ "DinrusXInput.dll",        g_szGroupCore },
	{ "DinrusXAudio.dll",  g_szGroupCore },
	{ "DinrusXFont.dll",         g_szGroupCore },
	{ "DinrusXAI.dll",     g_szGroupCore },
	{ "DinrusXEntitySys.dll", g_szGroupCore },
	{ "DinrusX3dEng.dll",     g_szGroupCore },
	{ "DinrusXGame.dll",            g_szGroupCore },
	{ "DinrusXAct.dll",       g_szGroupCore },
	{ "DinrusXAnimation.dll",    g_szGroupCore },
	{ "DinrusXRenderD3D9.dll",   g_szGroupCore },
	{ "DinrusXRenderD3D10.dll",  g_szGroupCore },
	{ "DinrusXRenderD3D11.dll",  g_szGroupCore },
	{ "DinrusXRenderD3D12.dll",  g_szGroupCore },
	{ "DinrusXRenderOpenGL.dll", g_szGroupCore },
	{ "DinrusXRenderVulkan.dll", g_szGroupCore },
	{ "DinrusXRenderNULL.dll",   g_szGroupCore },
	{ "DinrusXFlowGraph.dll",    g_szGroupCore }
};
...
void CSystem::CollectMemStats(IDrxSizer* pSizer, MemStatsPurposeEnum nPurpose, std::vector<SmallModuleInfo>* pStats)
{
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusSystem");
		{
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXAnimation");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXAnimation.dll");
				if (info)
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXPhys");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXPhys.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	if (m_env.p3DEngine)
	{
		...
		SIZER_COMPONENT_NAME(pSizer, "DinrusX3dEng");
		{
			m_env.p3DEngine->GetMemoryUsage(pSizer);
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusX3dEng.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	if (m_env.pRenderer)
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXRenderer");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste D3D9");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXRenderD3D9.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste D3D10");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXRenderD3D10.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXFont");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXFont.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXAudioSys");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXAudioSys.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
		}
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXScriptSys");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXScriptSys.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	if (m_env.pAISystem)
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXAISys");
		{
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXAISys.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	{
		{
			...
			{
				SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
				const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXAct.dll");
				if (info)
					pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
			}
	...
	if (m_env.pNetwork)
	{
		...
		{
			SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
			const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXNetwork.dll");
			if (info)
				pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
		}
	...
	{
		SIZER_COMPONENT_NAME(pSizer, "DinrusXEntitySys");
		{
			SIZER_COMPONENT_NAME(pSizer, "$Allocations waste");
			const SmallModuleInfo* info = FindModuleInfo(stats, "DinrusXEntitySys.dll");
			if (info)
				pSizer->AddObject(info, static_cast<size_t>(info->memInfo.allocated - info->memInfo.requested));
		}
...
void CSystem::DebugStats(bool checkpoint, bool leaks)
{
	...
	for (int i = 0; i < (int)(dbgmodules.size()); i++)
	{
		...
		if (fpDrxModuleGetAllocatedMemory)
		{
			int allocatedMemory = fpDrxModuleGetAllocatedMemory();
...
static const char* GetLastSystemErrorMessage()
{
	...
	if (dwError)
	{
		...
		if (FormatMessage(
		      ...
		      NULL))
		{
			drx_strcpy(szBuffer, (char*)lpMsgBuf);
			LocalFree(lpMsgBuf);
		}
		else return 0;
