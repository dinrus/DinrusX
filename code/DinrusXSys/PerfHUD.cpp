...
#include <DinrusX/DinrusXSys/StdAfx.h>
	...
	#include <DinrusX/DinrusXSys/MiniMenu.h>
...
{
	assert(pWidget);
	static int s_widgetUID = IDrxPerfHUDWidget::eWidget_Num;
...
void CPerfHUD::EnableWidget(IDrxPerfHUDWidget::EWidgetID id, int mode)
{
	const int nWidgets = m_widgets.size();
...
void CRenderStatsWidget::Update()
{
	...
	if (pParticleMgr)
	{
		...
		drx_sprintf(entryBuffer, "Num Particles Rendered: %d (%u)", (int)particleCounts.particles.rendered, m_particlesBudget);
		...
		{
			m_pInfoBox->AddEntry(entryBuffer, CPerfHUD::COL_ERROR, CPerfHUD::TEXT_SIZE_NORM);
			DrxPerfHUDWarning(1.f, "Too Many Particles: %d", (int)particleCounts.particles.rendered);
		}
	}
