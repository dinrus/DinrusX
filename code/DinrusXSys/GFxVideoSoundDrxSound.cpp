...
static float s_sys_flash_video_subaudiovolume = 1.0f;
		...
		#include <DinrusX/CoreX/Thread/IThreadConfigUpr.h>
...
{
	const char* threadName = "DrxGFxVideo_SoundUpdate";
	assert(pSound);
...
int GFxVideoDrxSoundSystemImpl::UpdateFunc(GThread*, void* h)
{
...
unsigned int GFxVideoDrxSound::ReadAndFillSound(bool fillup)
{
	assert(m_pSound);
	assert(m_pInterleavedBuffer);
...
unsigned int GFxVideoDrxSound::GetPCMData()
{
