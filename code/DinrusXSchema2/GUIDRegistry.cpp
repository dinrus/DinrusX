...
#include <DinrusX/DinrusXSchema2/Framework.h>
...
namespace Schematyc
{
	namespace
	{
		...
		class CSchematycFrameworkCreator : public IGameFrameworkExtensionCreator
		{
			...
			DRXGENERATE_SINGLETONCLASS_GUID(CSchematycFrameworkCreator, SCHEMATYC_FRAMEWORK_CREATOR_CLASS_NAME, "0f296b01-ff3f-4f55-aa10-a724a89c73cd"_drx_guid)
			...
			virtual IDrxUnknown* Create(IGameFramework* pIGameFramework) OVERRIDE
			{
				...
				if(DrxCreateClassInstance(SCHEMATYC_FRAMEWORK_CLASS_NAME, pSchematycFramework) == true)
				{
					...
					DrxLogAlways("Schematyc framework initialized");
	...
	IFramework* CreateFramework(IGameFramework* pIGameFramework)
	{
		...
		if(DrxCreateClassInstance(SCHEMATYC_FRAMEWORK_CREATOR_CLASS_NAME, pCreator) == true)
		{
