...
namespace Schematyc2
{
	namespace LogUtils
	{
		...
		inline ECriticalErrorStatus DisplayCriticalErrorMessage(const char* szMessage)
		{
			...
			switch (DrxMessageBox(message.c_str(), "Schematyc - Critical Error!", eMB_YesNoCancel))
			{
			case eQR_Yes:
				{
		...
		private:
			...
			void Initialize()
			{
				if(!m_pFile)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_UNKNOWN, VALIDATOR_WARNING, "Failed to write Schematyc2 log to disk!");
					}
