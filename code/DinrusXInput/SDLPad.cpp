...
#include <DinrusX/DinrusXInput/StdAfx.h>
	...
	#include <DinrusX/DinrusXInput/SDLPad.h>
...
void CSDLPad::HandleButtonEvent(const SDL_JoyButtonEvent& btEvt)
{
	...
	if (pSymbol)
	{
		...
		//DrxLogAlways("BTEVENT SYM: name: %s state:%d=>%d", pSymbol->name.c_str(), int(pSymbol->state), int(currState));
		bool isPost = false;
		switch (btEvt.button)
		{
...
bool CSDLPadUpr::AddGamePad(int deviceIndex)
{
...
bool CSDLPadUpr::RemovGamePad(int instanceId)
{
...
CSDLPad* CSDLPadUpr::FindPadByInstanceId(int instanceId)
{
...
CSDLPad* CSDLPadUpr::FindPadByDeviceIndex(int deviceIndex)
{
