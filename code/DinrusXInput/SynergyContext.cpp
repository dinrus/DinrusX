...
static bool synergyConnectFunc(CSynergyContext* pContext)
{
	...
	if (pContext->m_socket >= 0)
	{
		...
		{
			const int result = DrxSock::connect(pContext->m_socket, (const DRXSOCKADDR*)&sa, sizeof(sa));
...
{
	int ret = DrxSock::send(pContext->m_socket, buffer, length, 0);
...
{
	int ret = DrxSock::recv(pContext->m_socket, buffer, maxLength, 0);
	*outLength = ret;
