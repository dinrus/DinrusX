...
#include <DinrusX/DinrusXEntity/AffineParts.h>
#include <DinrusX/DinrusXEntity/EntitySystem.h>
#include <DinrusX/DinrusXEntity/IEntityClass.h>
#include <DinrusX/DinrusXEntity/EntityAudioProxy.h>
#include <DinrusX/DinrusXEntity/AreaProxy.h>
#include <DinrusX/DinrusXEntity/FlowGraphProxy.h>
#include <DinrusX/DinrusXNetwork/ISerialize.h>
#include <DinrusX/DinrusXEntity/NetEntity.h>
#include <DinrusX/DinrusXEntity/TriggerProxy.h>
#include <DinrusX/DinrusXEntity/EntityNodeProxy.h>
#include <DinrusX/DinrusXEntity/PartitionGrid.h>
#include <DinrusX/DinrusXEntity/ProximityTriggerSystem.h>
#include <DinrusX/DinrusXEntity/CameraProxy.h>
...
#include <DinrusX/DinrusXAI/IAIObjectUpr.h>
#include <DinrusX/DinrusXAI/IAIActor.h>
#include <DinrusX/DinrusXEntity/EntityLayer.h>
#include <DinrusX/DinrusXEntity/GeomCacheAttachmentUpr.h>
#include <DinrusX/DinrusXEntity/CharacterBoneAttachmentUpr.h>
#include <DinrusX/CoreX/String/StringUtils.h>
#include <DinrusX/DinrusXEntity/ClipVolumeProxy.h>
...
void CEntity::MoveSlot(IEntity* targetIEnt, int nSlot)
{
	...
	if (pCharacterInstance)
	{
		...
		if (srcPhysicsProxy && (pCharacterPhysics == srcPhysicsProxy->GetPhysicalEntity()))
		{
			...
			const int attachmentCount = pAttachmentUpr->GetAttachmentCount();
...
void CEntity::LogEvent(const SEntityEvent& event, CTimeValue dt)
{
	...
	char sNameId[32];
	const char* sName = "";
	for (int i = 0; i < DRX_ARRAY_COUNT(s_eventsName); i++)
	{
	...
	if (!*sName)
	{
		drx_sprintf(sNameId, "%d", (int)event.event);
		sName = sNameId;
	}
	s_LastLoggedFrame = nFrameId;

	float fTimeMs = dt.GetMilliSeconds();
	DrxLogAlways("<Frame:%d><EntityEvent> [%s](%X)\t[%.2fms]\t%s", nFrameId, sName, (int)event.nParam[0], fTimeMs, GetEntityTextDescription().c_str());
}
