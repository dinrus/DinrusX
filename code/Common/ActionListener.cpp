...
class CMyActionListener final : public IActionListener
{
	...
	virtual ~CMyActionListener()
	{
		...
		gEnv->pGameFramework->GetIActionMapUpr()->RemoveExtraActionListener(this, m_szMyActionMapName);
	}
	...
	void RegisterAction()
	{
		...
		IActionMap* pActionMap = pActionMapUpr->CreateActionMap(m_szMyActionMapName);
		// Register a listener to receive callbacks when actions in our action map are triggered
		pActionMapUpr->AddExtraActionListener(this, m_szMyActionMapName);
	...
	virtual void OnAction(const ActionId &actionId, int activationMode, float value) override
	{
		...
		if (actionId == m_myActionId)
		{
			if (isInputPressed)
			{
				DrxLogAlways("Action pressed!");
			}
			else if (isInputReleased)
			{
				DrxLogAlways("Action released!");
			}
		}
	}
};