...
#define POOLALLOCTESTSUIT
	...
	#error DRX_PLATFORM is not defined, probably #include <DinrusX/stdafx.h> is missing.
#endif
...
#include <DinrusX/AllocTrace.hpp>
#include <DinrusX/TGASave.h>
...
uint8 g_TestArea[sizeof(NDrxPoolAlloc::CFirstFit < NDrxPoolAlloc::CReferenced < NDrxPoolAlloc::CMemoryStatic < TEST_AREA_SIZE >, TEST_NODE_COUNT >, NDrxPoolAlloc::CListItemReference >)];
...
void TestRSXAllocDumb(TContainer& rMemory)
{
	...
	const uint32 Size = DRX_ARRAY_COUNT(g_Trace) / 3;
...
typedef NDrxPoolAlloc::CSTLPoolAllocWrapper<std::string, tdMySTLAllocator2>                                               tdMySTLAllocatorWrapped2;
