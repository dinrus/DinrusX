...
#include "DrxSharedLobbyPacket.h"
	...
	#include "DrxPSN2MatchMaking.h"

	#if USE_PSN
		#if USE_DRX_MATCHMAKING

			<DinrusX/CoreX/DrxEndian.h>
...
DrxMatchMakingConnectionID CDrxPSNMatchMaking::FindConnectionIDFromRoomMemberID(DrxLobbySessionHandle sessionHandle, SceNpMatching2RoomMemberId memberId)
{
	if ((sessionHandle != DrxLobbyInvalidSessionHandle) && (memberId != INVALID_PSN_ROOM_MEMBER_ID))
	{
...
CDrxPSNMatchMaking::SSession::SRoomMember* CDrxPSNMatchMaking::FindRoomMemberFromRoomMemberID(DrxLobbySessionHandle sessionHandle, SceNpMatching2RoomMemberId memberId)
{
	if ((sessionHandle != DrxLobbyInvalidSessionHandle) && (memberId != INVALID_PSN_ROOM_MEMBER_ID))
	{
		SSession* pSession = &m_sessions[sessionHandle];
...
uint64 CDrxPSNMatchMaking::GetSIDFromSessionHandle(DrxLobbySessionHandle h)
{
	DRX_ASSERT_MESSAGE((h < MAX_MATCHMAKING_SESSIONS) && m_sessions[h].IsUsed(), "CDrxPSNMatchMaking::GetSIDFromSessionHandle: invalid session handle");

	return m_sessions[h].m_roomId;
}

DrxSessionID CDrxPSNMatchMaking::SessionGetDrxSessionIDFromDrxSessionHandle(DrxSessionHandle gh)
{
	DrxLobbySessionHandle h = GetSessionHandleFromGameSessionHandle(gh);

	if ((h < MAX_MATCHMAKING_SESSIONS) && m_sessions[h].IsUsed())
	{
		SDrxPSNSessionID* pID = new SDrxPSNSessionID;
...
TNetAddress CDrxPSNMatchMaking::GetHostAddressFromSessionHandle(DrxSessionHandle gh)
{
	LOBBY_AUTO_LOCK;
...
void CDrxPSNMatchMaking::SessionDisconnectRemoteConnection(DrxSessionHandle gh, const TNetAddress& addr)
{
	// At present i don't need to worry about this
}

void CDrxPSNMatchMaking::FreeRemoteConnection(DrxLobbySessionHandle h, DrxMatchMakingConnectionID id)
{
...
uint32 CDrxPSNMatchMaking::GetSessionIDSizeInPacket() const
{
	return sizeof(SSynchronisedSessionID);
}

EDrxLobbyError CDrxPSNMatchMaking::WriteSessionIDToPacket(DrxSessionID sessionId, CDrxLobbyPacket* pPacket) const
{
	if (pPacket)
	{
		SDrxPSNSessionID blank;

		if (sessionId != DrxSessionInvalidID)
		{
			SDrxPSNSessionID* psnID = (SDrxPSNSessionID*)sessionId.get();
			if (psnID)
			{
...
DrxSessionID CDrxPSNMatchMaking::ReadSessionIDFromPacket(CDrxLobbyPacket* pPacket) const
{
	if (pPacket)
	{
		DrxSessionID returnId = new SDrxPSNSessionID;
		SDrxPSNSessionID* psnID = (SDrxPSNSessionID*)returnId.get();
		if (psnID)
		{
			SDrxPSNSessionID blank;
			...
			//-- Cannot use the SDrxPSNSessionID equals operator because we write more than just the roomId to the packet
...
void CDrxPSNMatchMaking::DumpMemberInfo(SSession::SRoomMember* pMember)
{
...
void CDrxPSNMatchMaking::ClearRoomMember(DrxLobbySessionHandle sessionHandle, uint32 nMember)
{
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		SSession* pSession = &m_sessions[sessionHandle];
		...
		if (pMember->IsUsed())
		{
			if (pMember->IsValid(ePSNMI_Other))
			{
				...
				DrxMatchMakingConnectionID id = FindConnectionIDFromRoomMemberID(sessionHandle, pMember->m_memberId);
				if (id != DrxMatchMakingInvalidConnectionID)
				{
...
void CDrxPSNMatchMaking::AddRoomMember(DrxLobbySessionHandle sessionHandle, const SDrxPSNRoomMemberDataInternal& memberInfo, EDrxPSNMemberInfo info)
{
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		SSession* pSession = &m_sessions[sessionHandle];
		if (pSession->IsUsed())
		{
			...
			for (uint32 a = 0; a < MAX_PSN_ROOM_MEMBERS; a++)
			{
				if (pSession->m_members[a].IsUsed())
				{
					...
					{
						//-- already exists!
						DRX_ASSERT_MESSAGE(pSession->m_members[a].m_memberId != memberInfo.m_memberId, "Adding a room member who already exists!");
						return;
					}
				}
				else
			...
			if (pMember)
			{
				...
				{
					memcpy(pMember->m_userData, memberInfo.m_binAttributes[0].m_data, DRXLOBBY_USER_DATA_SIZE_IN_BYTES);
				}
				...
				if (pMember->IsValid(ePSNMI_Other))
				{
					...
					SDrxMatchMakingConnectionUID matchingUID;

					matchingUID.m_uid = pMember->m_memberId;
					matchingUID.m_sid = pSession->m_roomId;

					DrxMatchMakingConnectionID id = CDrxMatchMaking::AddRemoteConnection(sessionHandle, DrxLobbyInvalidConnectionID, matchingUID, 1);
					if (id != DrxMatchMakingInvalidConnectionID)
					{
						SSession::SRConnection* pConnection = &pSession->remoteConnection[id];
						...
						if ((pSession->localFlags & DRXSESSION_LOCAL_FLAG_HOST) != DRXSESSION_LOCAL_FLAG_HOST)
						{
							if (!pMember->IsValid(ePSNMI_Owner))
							{
...
void CDrxPSNMatchMaking::RemoveRoomMember(DrxLobbySessionHandle sessionHandle, const SDrxPSNRoomMemberDataInternal& memberInfo)
{
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		SSession* pSession = &m_sessions[sessionHandle];
...
void CDrxPSNMatchMaking::UpdateRoomMember(DrxLobbySessionHandle sessionHandle, const SDrxPSNRoomMemberDataInternal& updatedMemberInfo)
{
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		SSession* pSession = &m_sessions[sessionHandle];
		if (pSession->IsUsed())
		{
			for (uint32 a = 0; a < MAX_PSN_ROOM_MEMBERS; a++)
			{
				...
				if (pMember->m_valid && (pMember->m_memberId == updatedMemberInfo.m_memberId))
				{
					...
					{
						memcpy(pMember->m_userData, updatedMemberInfo.m_binAttributes[0].m_data, DRXLOBBY_USER_DATA_SIZE_IN_BYTES);
					}
...
void CDrxPSNMatchMaking::ClearSessionInfo(DrxLobbySessionHandle sessionHandle)
{
	assert(sessionHandle != DrxLobbyInvalidSessionHandle);
	...
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		...
		if (pSession->IsUsed() && (pSession->m_roomId != INVALID_PSN_ROOM_ID))
		{
			...
			if (pSession->m_sessionId.data[0] != 0)
			{
				...
				// m_pPSNSupport->GetWebApiInterface().AddJob(CDrxPSNOrbisWebApiThread::LeaveSession, leaveSessionParamsHdl);
			}
		}
...
void CDrxPSNMatchMaking::AddInitialRoomMembers(DrxLobbySessionHandle sessionHandle, const SDrxPSNRoomMemberDataInternalList& memberList)
{
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		SSession* pSession = &m_sessions[sessionHandle];
		if (pSession->IsUsed())
		{
			...
			{
				SDrxPSNRoomMemberDataInternal& memberInfo = memberList.m_pRoomMembers[a];
				...
				AddRoomMember(sessionHandle, memberInfo, (EDrxPSNMemberInfo)info);
			}
		}
	}
}

void CDrxPSNMatchMaking::DispatchRoomOwnerChangedEvent(TMemHdl mh)
{
...
void CDrxPSNMatchMaking::ChangeRoomOwner(DrxLobbySessionHandle sessionHandle, SceNpMatching2RoomMemberId prevOwner, SceNpMatching2RoomMemberId newOwner)
{
	if (sessionHandle != DrxLobbyInvalidSessionHandle)
	{
		SSession* pSession = &m_sessions[sessionHandle];
				...
				if (pRoomMember->IsValid(ePSNMI_Other) && (pRoomMember->m_signalingState == ePSNCS_Active))
				{
					if (m_lobby)
					{
						...
						if (mh != TMemInvalidHdl)
						{
							...
							TO_GAME_FROM_LOBBY(&CDrxPSNMatchMaking::DispatchRoomOwnerChangedEvent, this, mh);
						}
					}
				}
			}
...
void CDrxPSNMatchMaking::UpdateSignaling(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];

	if (pTask->used && pTask->running && (pTask->session != DrxLobbyInvalidSessionHandle))
	{
				...
				if (pMember->IsUsed())
				{
					...
					if (bTestTimeout)
					{
						if ((g_time.GetMilliSecondsAsInt64() - pMember->m_signalingTimer.GetMilliSecondsAsInt64()) > SIGNALING_TIMEOUT)
						{
							...
							DrxMatchMakingConnectionID id = FindConnectionIDFromRoomMemberID(pTask->session, pMember->m_memberId);
							if (id != DrxMatchMakingInvalidConnectionID)
							{
								SSession::SRConnection* pConnection = &pSession->remoteConnection[id];
								NetLog("[Lobby] CDrxPSNMatchMaking::UpdateSignaling() : Signaling dead event setting connection " PRFORMAT_LCID " to eCLCS_NotConnected", PRARG_LCID(pConnection->connectionID));
...
CDrxPSNMatchMaking::EDrxPSNConnectionState CDrxPSNMatchMaking::CheckSignaling(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
...
bool CDrxPSNMatchMaking::ProcessCorrectTaskForRequest(SDrxPSNSupportCallbackEventData& data, ETask subTaskCheck, const char* pRequestString, EventRequestResponseCallback pCB)
{
		...
		if (pTask->used && pTask->running)
		{
			if (pTask->m_reqId == data.m_requestEvent.m_reqId)
			{
				...
				if (pTask->subTask == subTaskCheck)
				{
					...
					if (bHandled)
					{
						data.m_requestEvent.m_returnFlags |= SDrxPSNSupportCallbackEventData::SRequestEvent::REQUEST_EVENT_HANDLED;
					}
					else
					{
						data.m_requestEvent.m_returnFlags = SDrxPSNSupportCallbackEventData::SRequestEvent::REQUEST_EVENT_NOT_HANDLED;
...
bool CDrxPSNMatchMaking::ProcessCorrectTaskForSearch(SDrxPSNSupportCallbackEventData& data, ETask subTaskCheck, const char* pRequestString, EventRequestResponseCallback pCB)
{
	for (uint32 i = 0; i < MAX_MATCHMAKING_TASKS; i++)
	{
		STask* pTask = &m_task[i];
...
void CDrxPSNMatchMaking::ProcessRequestEvent(SDrxPSNSupportCallbackEventData& data)
{
...
void CDrxPSNMatchMaking::ProcessSignalingEvent(SDrxPSNSupportCallbackEventData& data)
{
				...
				case SIGNALING_EVENT_DEAD:
					{
						...
						DumpMemberInfo(pMember);
						DrxMatchMakingConnectionID id = FindConnectionIDFromRoomMemberID(i, pMember->m_memberId);
						if (id != DrxMatchMakingInvalidConnectionID)
						{
							SSession::SRConnection* pConnection = &pSession->remoteConnection[id];
							NetLog("[Lobby] CDrxPSNMatchMaking::ProcessSignalingEvent() : SCE_NP_SIGNALING_EVENT_DEAD : Signaling dead event setting connection " PRFORMAT_LCID " to eCLCS_NotConnected", PRARG_LCID(pConnection->connectionID));
				...
				case SIGNALING_EVENT_CONNECTED:
					{
						...
						if ((ret == PSN_OK) && (conStatus == SCE_NP_MATCHING2_SIGNALING_CONN_STATUS_ACTIVE))
						{
							...
							if (id != DrxMatchMakingInvalidConnectionID)
							{
								...
								if (pConnection->connectionID == DrxLobbyInvalidConnectionID)
								{
									...
									DrxLobbyConnectionID lID = m_lobby->FindConnection(netAddr);
									if (lID == DrxLobbyInvalidConnectionID)
									{
									...
									DumpMemberInfo(pMember);
									NetLog("[Lobby] CDrxPSNMatchMaking::ProcessSignalingEvent() : SIGNALING_EVENT_CONNECTED : Signaling established for " PRFORMAT_SHMMCINFO, PRARG_SHMMCINFO(DrxLobbySessionHandle(i), id, pConnection->connectionID, pConnection->uid));
									...
									if (pSession->localFlags & DRXSESSION_LOCAL_FLAG_HOST)
									{
										...
										const uint32 maxBufferSize = DrxLobbyPacketHeaderSize + sizeof(SceNpSessionId);
										...
										Send(DrxMatchMakingInvalidTaskID, &packet, i, id);
									}
								}
							}
						}
						else if (ret < PSN_OK)
						{
							...
							{
								SSession::SRConnection* pConnection = &pSession->remoteConnection[id];
								NetLog("[Lobby] CDrxPSNMatchMaking::ProcessSignalingEvent() : SIGNALING_EVENT_CONNECTED(1) : Signaling connected failure event, setting " PRFORMAT_SHMMCINFO " to eCLCS_NotConnected", PRARG_SHMMCINFO(DrxLobbySessionHandle(i), id, pConnection->connectionID, pConnection->uid));
								m_lobby->ConnectionSetState(pConnection->connectionID, eCLCS_NotConnected);
							}
						...
						else
						{
							...
							{
								SSession::SRConnection* pConnection = &pSession->remoteConnection[id];
								NetLog("[Lobby] CDrxPSNMatchMaking::ProcessSignalingEvent() : SIGNALING_EVENT_CONNECTED(2) : Signaling connected failure event, setting " PRFORMAT_SHMMCINFO " to eCLCS_NotConnected", PRARG_SHMMCINFO(DrxLobbySessionHandle(i), id, pConnection->connectionID, pConnection->uid));
								m_lobby->ConnectionSetState(pConnection->connectionID, eCLCS_NotConnected);
							}
						}
						data.m_signalEvent.m_bHandled = true;
...
void CDrxPSNMatchMaking::DispatchForcedFromRoomEvent(DrxSessionHandle gh, SceNpMatching2Event how, SceNpMatching2EventCause why)
{
	if (m_lobby)
	{
		UDrxLobbyEventData eventData;
		SDrxLobbyForcedFromRoomData eventRoomData;
...
void CDrxPSNMatchMaking::ProcessRoomEvent(SDrxPSNSupportCallbackEventData& data)
{
	for (uint32 i = 0; i < MAX_MATCHMAKING_SESSIONS; i++)
	{
		SSession* pSession = &m_sessions[i];
			...
			case ROOM_EVENT_MEMBER_JOINED:
				{
					if ((data.m_roomEvent.m_error == 0) &&
					    ...
					    (data.m_roomEvent.m_dataSize == sizeof(SDrxPSNRoomMemberUpdateInfoResponse)))
					{
						SDrxPSNRoomMemberUpdateInfoResponse* pMemberUpdateInfo = (SDrxPSNRoomMemberUpdateInfoResponse*)m_pPSNSupport->GetLobby()->MemGetPtr(data.m_roomEvent.m_dataHdl);
						...
						data.m_roomEvent.m_returnFlags |= SDrxPSNSupportCallbackEventData::SRoomEvent::ROOM_EVENT_HANDLED;
					}
					else
					{
			...
			case ROOM_EVENT_ROOM_OWNER_CHANGED:
				{
					if ((data.m_roomEvent.m_error == 0) &&
					    (data.m_roomEvent.m_dataHdl != TMemInvalidHdl) &&
					    (data.m_roomEvent.m_dataSize == sizeof(SDrxPSNRoomOwnerUpdateInfoResponse)))
					{
						SDrxPSNRoomOwnerUpdateInfoResponse* pOwnerInfo = (SDrxPSNRoomOwnerUpdateInfoResponse*)m_pPSNSupport->GetLobby()->MemGetPtr(data.m_roomEvent.m_dataHdl);
			...
			case ROOM_EVENT_KICKED_OUT:
				{
					...
					if ((data.m_roomEvent.m_error == 0) &&
					    (data.m_roomEvent.m_dataHdl != TMemInvalidHdl) &&
					    (data.m_roomEvent.m_dataSize == sizeof(SDrxPSNRoomUpdateInfoResponse)))
					{
						ClearSessionInfo(i);
						FreeSessionHandle(i);

						SDrxPSNRoomUpdateInfoResponse* pUpdateInfo = (SDrxPSNRoomUpdateInfoResponse*)m_pPSNSupport->GetLobby()->MemGetPtr(data.m_roomEvent.m_dataHdl);

						TO_GAME_FROM_LOBBY(&CDrxPSNMatchMaking::DispatchForcedFromRoomEvent, this, CreateGameSessionHandle(i, pSession->localConnection.uid), data.m_roomEvent.m_event, pUpdateInfo->m_eventCause);
			...
			case ROOM_EVENT_UPDATED_ROOM_MEMBER_DATA_INTERNAL:
				{
					    ...
					    (data.m_roomEvent.m_dataSize == sizeof(SDrxPSNRoomMemberUpdateInfoResponse)))
					{
						...
						const SDrxPSNRoomMemberUpdateInfoResponse* const pUpdateInfo = (SDrxPSNRoomMemberUpdateInfoResponse*)m_pPSNSupport->GetLobby()->MemGetPtr(data.m_requestEvent.m_dataHdl);
...
void CDrxPSNMatchMaking::ProcessErrorEvent(SDrxPSNSupportCallbackEventData& data)
{
	for (uint32 i = 0; i < MAX_MATCHMAKING_TASKS; i++)
	{
		STask* pTask = &m_task[i];
...
void CDrxPSNMatchMaking::ProcessWebApiEvent(SDrxPSNSupportCallbackEventData& data)
{
	for (uint32 i = 0; i < MAX_MATCHMAKING_TASKS; i++)
	{
		STask* pTask = &m_task[i];
...
void CDrxPSNMatchMaking::SupportCallback(EDrxPSNSupportCallbackEvent ecb, SDrxPSNSupportCallbackEventData& data, void* pArg)
{
	CDrxPSNMatchMaking* _this = static_cast<CDrxPSNMatchMaking*>(pArg);
...
EDrxLobbyError CDrxPSNMatchMaking::Initialise()
{
	...
	if (error == eCLE_Success)
	{
		...
		for (uint32 i = 0; i < MAX_MATCHMAKING_SESSIONS; i++)
		{
			m_sessions[i].localFlags &= ~DRXSESSION_LOCAL_FLAG_USED;

			for (uint32 j = 0; j < MAX_LOBBY_CONNECTIONS; j++)
			{
				m_sessions[i].remoteConnection[j].connectionID = DrxLobbyInvalidConnectionID;
...
EDrxLobbyError CDrxPSNMatchMaking::Terminate()
{
	for (uint32 i = 0; i < MAX_MATCHMAKING_SESSIONS; i++)
	{
		SSession* pSession = &m_sessions[i];
...
CDrxPSNMatchMaking::CDrxPSNMatchMaking(CDrxLobby* lobby, CDrxLobbyService* service, CDrxPSNSupport* pSupport, EDrxLobbyService serviceType) : CDrxMatchMaking(lobby, service, serviceType)
{
...
CDrxPSNMatchMaking::~CDrxPSNMatchMaking()
{
}

EDrxLobbyError CDrxPSNMatchMaking::StartTask(ETask etask, bool startRunning, uint32 user, DrxMatchMakingTaskID* mmTaskID, DrxLobbyTaskID* lTaskID, DrxLobbySessionHandle h, void* cb, void* cbArg)
{
...
void CDrxPSNMatchMaking::ResetTask(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	pTask->returnTaskID = DrxMatchMakingInvalidTaskID;
	pTask->m_reqId = INVALID_PSN_REQUEST_ID;
}

const char* CDrxPSNMatchMaking::SSession::GetLocalUserName(uint32 localUserIndex) const
{
...
void CDrxPSNMatchMaking::SSession::Reset()
{
	PARENT::Reset();

	started = false;
	remoteConnectionProcessingToDo = false;
	remoteConnectionTaskID = DrxMatchMakingInvalidTaskID;
...
const char* CDrxPSNMatchMaking::GetConnectionName(CDrxMatchMaking::SSession::SRConnection* pConnection, uint32 localUserIndex) const
{
...
void CDrxPSNMatchMaking::StartTaskRunning(DrxMatchMakingTaskID mmTaskID)
{
	LOBBY_AUTO_LOCK;
...
void CDrxPSNMatchMaking::EndTask(DrxMatchMakingTaskID mmTaskID)
{
	LOBBY_AUTO_LOCK;
	...
	if (pTask->used)
	{
		if (m_pPSNSupport->GetNetInfo().natStatus != m_oldNatType)
		{
			...
			UDrxLobbyEventData eventData;
			SDrxLobbyNatTypeData natTypeData;
		...
		if (pTask->error != eCLE_Success)
		{
			if ((pTask->startedTask == eT_SessionCreate) || (pTask->startedTask == eT_SessionJoin))
			{
				if (pTask->session != DrxLobbyInvalidSessionHandle)
				{
					...
					pTask->session = DrxLobbyInvalidSessionHandle;
				}
			}
...
void CDrxPSNMatchMaking::StopTaskRunning(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	...
	{
		pTask->running = false;
		TO_GAME_FROM_LOBBY(&CDrxPSNMatchMaking::EndTask, this, mmTaskID);
	}
}

EDrxLobbyError CDrxPSNMatchMaking::CreateSessionHandle(DrxLobbySessionHandle* h, bool host, uint32* users, int numUsers, uint32 flags)
{
	EDrxLobbyError error = CDrxMatchMaking::CreateSessionHandle(h, host, flags, numUsers);
...
void CDrxPSNMatchMaking::Tick(CTimeValue tv)
{
...
EDrxLobbyError CDrxPSNMatchMaking::SessionRegisterUserData(SDrxSessionUserData* data, uint32 numData, DrxLobbyTaskID* taskID, DrxMatchmakingCallback cb, void* cbArg)
{
	...
	if (numData < MAX_MATCHMAKING_SESSION_USER_DATA)
	{
		...
		if (error == eCLE_Success)
		{
			...
			FROM_GAME_TO_LOBBY(&CDrxPSNMatchMaking::StartTaskRunning, this, mmTaskID);
		}
	}
	else
	{
...
uint32 CDrxPSNMatchMaking::SortWorlds(const SDrxPSNGetWorldInfoListResponse* pWorldList, SceNpMatching2WorldId* pReturnIdList, uint32 nMaxListSize, bool bIsForSearch)
{
	...
	if (pReturnIdList && (nMaxListSize > 0))
	{
		...
		if (pWorldList && numTotalWorlds)
		{
			...
			if (forceWorld == 0)
			{
				...
				{
					SDrxPSNWorld& world = pWorldList->m_pWorlds[i];

					world.m_score = 0;
					world.m_sortState = SDrxPSNWorld::SORT_WORLD_NONE;
				...
				for (uint32 i = 0; i < numTotalWorlds; i++)
				{
					...
					if (bIsForSearch)
					{
						...
						if (world.m_score > 0)
						{
							world.m_sortState = SDrxPSNWorld::SORT_WORLD_NEEDS_SORTING;
						}
						...
						//scoreC = (drx_rand() % CNetCVars::Get().psnBestWorldRandomBonus);
				...
				while (bSorting)
				{
					SDrxPSNWorld* pBestWorld = NULL;
					uint32 highestWorldScore = 0;
					uint32 lowestWorldScore = maxBonus;

					for (uint32 l = 0; l < numTotalWorlds; l++)
					{
						SDrxPSNWorld* pWorld = &pWorldList->m_pWorlds[l];

						if (pWorld->m_sortState == SDrxPSNWorld::SORT_WORLD_NEEDS_SORTING)
						{
							if (bIsForSearch)
							{
					...
					if (pBestWorld)
					{
						...
						pBestWorld->m_sortState = SDrxPSNWorld::SORT_WORLD_SORTED;
...
void CDrxPSNMatchMaking::ProcessExternalRoomData(const SDrxPSNRoomDataExternal& curRoom, SDrxSessionSearchResult& result, SDrxSessionUserData userData[MAX_MATCHMAKING_SESSION_USER_DATA])
{
	SDrxPSNSessionID* pRoomInfo = new SDrxPSNSessionID;
...
void CDrxPSNMatchMaking::BuildUserSessionParams(SSession* pSession, SDrxSessionUserData* pData, uint32 numData,
...
EDrxLobbyError CDrxPSNMatchMaking::SessionCreate(uint32* users, int numUsers, uint32 flags, SDrxSessionData* data, DrxLobbyTaskID* taskID, DrxMatchmakingSessionCreateCallback cb, void* cbArg)
{
	LOBBY_AUTO_LOCK;

	DrxLobbySessionHandle h;
	EDrxLobbyError error = CreateSessionHandle(&h, true, users, numUsers, flags);
	...
	if (error == eCLE_Success)
	{
		...
		if (error == eCLE_Success)
		{
			...
			if ((flags & DRXSESSION_CREATE_FLAG_SEARCHABLE) == 0)
			{
			...
			error = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_CREATE_PARAMS_SLOT, data, 1, sizeof(SDrxSessionData));
			...
			if (error == eCLE_Success)
			{
				...
				if (error == eCLE_Success)
				{
					...
					if (error == eCLE_Success)
					{
						...
						error = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_WEBAPI_CREATE_SLOT, NULL, 1, sizeof(SDrxPSNOrbisWebApiCreateSessionInput));
						if (error == eCLE_Success)
						{
...
void CDrxPSNMatchMaking::TickSessionCreate(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
...
bool CDrxPSNMatchMaking::EventRequestResponse_GetWorldInfoListForCreate(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	...
	assert(data.m_requestEvent.m_dataSize == sizeof(SDrxPSNGetWorldInfoListResponse));

	if ((data.m_requestEvent.m_error == 0) &&
	    (data.m_requestEvent.m_dataHdl != TMemInvalidHdl) &&
	    (data.m_requestEvent.m_dataSize == sizeof(SDrxPSNGetWorldInfoListResponse)))
	{
		...
		if (pCreateParams)
		{
			...
			const SDrxPSNGetWorldInfoListResponse* pWorldList = (SDrxPSNGetWorldInfoListResponse*)_this->m_lobby->MemGetPtr(data.m_requestEvent.m_dataHdl);
...
void CDrxPSNMatchMaking::TickSessionCreateRequest(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
		...
		if (pTask->params[PSN_MATCHMAKING_CREATE_PARAMS_SLOT] != TMemInvalidHdl)
		{
			...
			{
				SDrxSessionData* pData = (SDrxSessionData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_CREATE_PARAMS_SLOT]);
				if (pTask->params[PSN_MATCHMAKING_CREATE_DATA_SLOT] != TMemInvalidHdl)
				{
					pData->m_data = (SDrxSessionUserData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_CREATE_DATA_SLOT]);
				}
				else
				{
					pData->m_data = NULL;
				...
				uint16 sessionCreateGameFlags = (pSession->createFlags & DRXSESSION_CREATE_FLAG_GAME_MASK) >> DRXSESSION_CREATE_FLAG_GAME_FLAGS_SHIFT;
...
bool CDrxPSNMatchMaking::EventRequestResponse_CreateRoom(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
	...
	assert(data.m_requestEvent.m_dataSize == sizeof(SDrxPSNCreateJoinRoomResponse));

	if ((data.m_requestEvent.m_error == 0) &&
	    (data.m_requestEvent.m_dataHdl != TMemInvalidHdl) &&
	    (data.m_requestEvent.m_dataSize == sizeof(SDrxPSNCreateJoinRoomResponse)))
	{
		const SDrxPSNCreateJoinRoomResponse* pCreateResponse = (SDrxPSNCreateJoinRoomResponse*)_this->m_pPSNSupport->GetLobby()->MemGetPtr(data.m_requestEvent.m_dataHdl);
...
void CDrxPSNMatchMaking::TickSessionCreateSignaling(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
		...
		case ePSNCS_Active:
			{
				...
				if (pTask->session != DrxLobbyInvalidSessionHandle)
				{
					...
					if (pSession->IsUsed())
					{
						if (!pTask->canceled)
						{
							if (pSession->createFlags & DRXSESSION_CREATE_FLAG_INVITABLE)
							{
								SDrxSessionData* pSessionData = (SDrxSessionData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_CREATE_PARAMS_SLOT]);
								SCreateParamData* pCreateParams = (SCreateParamData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_PSNSUPPORT_CREATE_REQUEST_SLOT]);
								SDrxPSNOrbisWebApiCreateSessionInput* pWebApiCreateParams = (SDrxPSNOrbisWebApiCreateSessionInput*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_WEBAPI_CREATE_SLOT]);
								if (pSessionData && pCreateParams && pWebApiCreateParams)
								{
									StartSubTask(eST_WaitingForCreateRoomWebApiCallback, mmTaskID);

									memset(pWebApiCreateParams, 0, sizeof(SDrxPSNOrbisWebApiCreateSessionInput));

									// Some of the SDrxLobbySessionAdvertisement we can fill in now.
									SDrxLobbySessionAdvertisement& advertisement = pWebApiCreateParams->advertisement;
									...
									else
									{
										...
										advertisement.m_sizeofData = DRX_WEBAPI_CREATESESSION_LINKDATA_MAX_SIZE;
									}
									...
									// And now we should have a completed SDrxLobbySessionAdvertisement structure available to us. We'll be using this in CDrxPSNOrbisWebApiThread::CreateSession later on.
									pCreateParams->m_createJobId = m_pPSNSupport->GetWebApiInterface().AddJob(CDrxPSNOrbisWebApiThread::CreateSession, pTask->params[PSN_MATCHMAKING_WEBAPI_CREATE_SLOT]);
...
void CDrxPSNMatchMaking::EventWebApiCreateSession(DrxMatchMakingTaskID mmTaskID, SDrxPSNSupportCallbackEventData& data)
{
	STask* pTask = &m_task[mmTaskID];
...
void CDrxPSNMatchMaking::EndSessionCreate(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	SSession* pSession = &m_sessions[pTask->session];
	...
	((DrxMatchmakingSessionCreateCallback)pTask->cb)(pTask->lTaskID, pTask->error, CreateGameSessionHandle(pTask->session, pSession->localConnection.uid), pTask->cbArg);
...
EDrxLobbyError CDrxPSNMatchMaking::SessionMigrate(DrxSessionHandle gh, uint32* pUsers, int numUsers, uint32 flags, SDrxSessionData* pData, DrxLobbyTaskID* pTaskID, DrxMatchmakingSessionCreateCallback pCB, void* pCBArg)
{
	EDrxLobbyError error = eCLE_Success;
...
void CDrxPSNMatchMaking::TickSessionMigrate(DrxMatchMakingTaskID mmTaskID)
{
	UpdateTaskError(mmTaskID, eCLE_Success);
	StopTaskRunning(mmTaskID);
}

EDrxLobbyError CDrxPSNMatchMaking::SessionUpdate(DrxSessionHandle gh, SDrxSessionUserData* pData, uint32 numData, DrxLobbyTaskID* pTaskID, DrxMatchmakingCallback pCB, void* pCBArg)
{
	EDrxLobbyError error = eCLE_Success;
	...
	if ((h < MAX_MATCHMAKING_SESSIONS) && (m_sessions[h].localFlags & DRXSESSION_LOCAL_FLAG_USED))
	{
		...
		if (session->localFlags & DRXSESSION_LOCAL_FLAG_HOST)
		{
			DrxMatchMakingTaskID mmTaskID;
...
void CDrxPSNMatchMaking::TickSessionUpdate(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
		...
		if (pTask->params[PSN_MATCHMAKING_PSNSUPPORT_UPDATE_REQUEST_SLOT] != TMemInvalidHdl)
		{
			...
			SDrxSessionUserData* pData = (SDrxSessionUserData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_UPDATE_DATA_SLOT]);
			SSession* pSession = &m_sessions[pTask->session];
...
bool CDrxPSNMatchMaking::EventRequestResponse_SetRoomDataExternal(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
...
EDrxLobbyError CDrxPSNMatchMaking::SessionUpdateSlots(DrxSessionHandle gh, uint32 numPublic, uint32 numPrivate, DrxLobbyTaskID* pTaskID, DrxMatchmakingCallback pCB, void* pCBArg)
{
	EDrxLobbyError error = eCLE_Success;
	...
	if ((h < MAX_MATCHMAKING_SESSIONS) && (m_sessions[h].localFlags & DRXSESSION_LOCAL_FLAG_USED))
	{
		...
		if (error == eCLE_Success)
		{
			FROM_GAME_TO_LOBBY(&CDrxPSNMatchMaking::StartTaskRunning, this, tid);
		}
	}
	else
	{
	...
	return error;
			...
			#if NETWORK_HOST_MIGRATION
EDrxLobbyError CDrxPSNMatchMaking::SendHostHintExternal(DrxLobbySessionHandle h)
{
	EDrxLobbyError error = eCLE_Success;
	LOBBY_AUTO_LOCK;
...
void CDrxPSNMatchMaking::StartSessionSendHostHintExternal(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	...
	if (pTask->params[PSN_MATCHMAKING_PSNSUPPORT_HOSTHINT_REQUEST_SLOT] != TMemInvalidHdl)
	{
		...
		{
			pHostHintParams->m_memberId[index] = ((CDrxPSNMatchMaking::SSession::SRConnection*)(pSession->newHostPriority[index]))->memberId;
		}
...
void CDrxPSNMatchMaking::TickSessionSendHostHintExternal(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
...
bool CDrxPSNMatchMaking::EventRequestResponse_SetRoomDataInternal(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
...
void CDrxPSNMatchMaking::EndSessionSendHostHintExternal(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
...
EDrxLobbyError CDrxPSNMatchMaking::SessionQuery(DrxSessionHandle gh, DrxLobbyTaskID* pTaskID, DrxMatchmakingSessionQueryCallback pCB, void* pCBArg)
{
	EDrxLobbyError error = eCLE_Success;
...
void CDrxPSNMatchMaking::TickSessionQuery(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
...
bool CDrxPSNMatchMaking::EventRequestResponse_GetRoomDataExternalList(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
	...
	assert(data.m_requestEvent.m_dataHdl != TMemInvalidHdl);
	assert(data.m_requestEvent.m_dataSize == sizeof(SDrxPSNSearchRoomResponse));

	if ((data.m_requestEvent.m_error == 0) &&
	    (data.m_requestEvent.m_dataHdl != TMemInvalidHdl) &&
	    (data.m_requestEvent.m_dataSize == sizeof(SDrxPSNSearchRoomResponse)))
	{
		...
		data.m_requestEvent.m_dataHdl = TMemInvalidHdl;
		data.m_requestEvent.m_returnFlags |= SDrxPSNSupportCallbackEventData::SRequestEvent::REQUEST_EVENT_OWNS_MEMORY;
...
void CDrxPSNMatchMaking::EndSessionQuery(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	SDrxPSNSearchRoomResponse* pResponse = NULL;

	if (pTask->params[PSN_MATCHMAKING_PSNSUPPORT_QUERY_RESPONSE_SLOT] != TMemInvalidHdl)
	{
		pResponse = (SDrxPSNSearchRoomResponse*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_PSNSUPPORT_QUERY_RESPONSE_SLOT]);
	}
	...
	if (pTask->error == eCLE_Success)
	{
		if (pResponse)
		{
			for (uint16 i = 0; i < pResponse->m_numRooms; i++)
			{
				...
				((DrxMatchmakingSessionQueryCallback)pTask->cb)(pTask->lTaskID, eCLE_SuccessContinue, &result, pTask->cbArg);
			}
		}
		else
		{
...
EDrxLobbyError CDrxPSNMatchMaking::SessionGetUsers(DrxSessionHandle gh, DrxLobbyTaskID* pTaskID, DrxMatchmakingSessionGetUsersCallback pCB, void* pCBArg)
{
	EDrxLobbyError error = eCLE_Success;
...
void CDrxPSNMatchMaking::GetRoomMemberData(DrxLobbySessionHandle sessionHandle, SDrxUserInfoResult& result, SDrxMatchMakingConnectionUID& uid)
{
	...
	if (pRoomMember)
	{
		drx_strcpy(result.m_userName, pRoomMember->m_npId.handle.data);
		memcpy(result.m_userData, pRoomMember->m_userData, DRXLOBBY_USER_DATA_SIZE_IN_BYTES);

		((SDrxPSNUserID*)result.m_userID.get())->npId = pRoomMember->m_npId;
	}
	else
	{
...
void CDrxPSNMatchMaking::TickSessionGetUsers(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];

	SDrxUserInfoResult temp;
	temp.m_userID = new SDrxPSNUserID;
	...
	((DrxMatchmakingSessionGetUsersCallback)pTask->cb)(pTask->lTaskID, pTask->error, &temp, pTask->cbArg);
...
EDrxLobbyError CDrxPSNMatchMaking::SessionStart(DrxSessionHandle gh, DrxLobbyTaskID* taskID, DrxMatchmakingCallback cb, void* cbArg)
{
	EDrxLobbyError error = eCLE_Success;
...
EDrxLobbyError CDrxPSNMatchMaking::SessionEnd(DrxSessionHandle gh, DrxLobbyTaskID* taskID, DrxMatchmakingCallback cb, void* cbArg)
{
	EDrxLobbyError error = eCLE_Success;
...
EDrxLobbyError CDrxPSNMatchMaking::SessionDelete(DrxSessionHandle gh, DrxLobbyTaskID* taskID, DrxMatchmakingCallback cb, void* cbArg)
{
	EDrxLobbyError error = eCLE_Success;
	...
	if ((h < MAX_MATCHMAKING_SESSIONS) && m_sessions[h].IsUsed())
	{
		...
		SessionDisconnectRemoteConnectionViaNub(h, DrxMatchMakingInvalidConnectionID, eDS_Local, DrxMatchMakingInvalidConnectionID, eDC_UserRequested, "Session deleted");
		...
		if (error == eCLE_Success)
		{
			...
			if (error == eCLE_Success)
			{
				error = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_WEBAPI_LEAVE_REQUEST_SLOT, NULL, 1, sizeof(SDrxPSNOrbisWebApiLeaveSessionInput));
				if (error == eCLE_Success)
				{
					m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
...
void CDrxPSNMatchMaking::StartSessionDelete(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	SSession* pSession = &m_sessions[pTask->session];
...
void CDrxPSNMatchMaking::TickSessionDelete(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
		...
		if (pTask->params[PSN_MATCHMAKING_PSNSUPPORT_LEAVE_REQUEST_SLOT] != TMemInvalidHdl)
		{
			...
			else
			{
				...
				if ((ret == SCE_NP_MATCHING2_ERROR_ROOM_NOT_FOUND) || (ret == SCE_NP_MATCHING2_SERVER_ERROR_NO_SUCH_ROOM) || (ret == SCE_NP_MATCHING2_SERVER_ERROR_NO_SUCH_ROOM_INSTANCE))
				{
					...
					SDrxPSNSupportCallbackEventData data;
					memset(&data, 0, sizeof(SDrxPSNSupportCallbackEventData));
...
bool CDrxPSNMatchMaking::EventRequestResponse_LeaveRoom(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
		...
		if (pTask->session != DrxLobbyInvalidSessionHandle)
		{
			...
			if (pSession->IsUsed())
			{
				if (pSession->m_sessionId.data[0] != 0)
				{
					...
					SDrxPSNOrbisWebApiLeaveSessionInput* pLeaveWebApiParams = (SDrxPSNOrbisWebApiLeaveSessionInput*)_this->m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_WEBAPI_LEAVE_REQUEST_SLOT]);
					if (pLeaveWebApiParams)
					{
						...
						pLeaveParams->m_leaveJobId = _this->m_pPSNSupport->GetWebApiInterface().AddJob(CDrxPSNOrbisWebApiThread::LeaveSession, pTask->params[PSN_MATCHMAKING_WEBAPI_LEAVE_REQUEST_SLOT]);
					}
					else
					{
						// failed!
...
void CDrxPSNMatchMaking::EventWebApiLeaveSession(DrxMatchMakingTaskID mmTaskID, SDrxPSNSupportCallbackEventData& data)
{
	STask* pTask = &m_task[mmTaskID];
...
EDrxLobbyError CDrxPSNMatchMaking::SessionSearch(uint32 user, SDrxSessionSearchParam* pParam, DrxLobbyTaskID* pTaskID, DrxMatchmakingSessionSearchCallback cb, void* cbArg)
{
	LOBBY_AUTO_LOCK;

	DrxMatchMakingTaskID mmTaskID;

	EDrxLobbyError error = StartTask(eT_SessionSearch, false, user, &mmTaskID, pTaskID, 0, (void*)cb, cbArg);
	if (error == eCLE_Success)
	{
		error = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_SEARCH_PARAMS_SLOT, pParam, 1, sizeof(SDrxSessionSearchParam));
		if (error == eCLE_Success)
		{
...
uint32 CDrxPSNMatchMaking::BuildSearchFilterParams(SDrxSessionSearchData* pData, uint32 numData,
...
void CDrxPSNMatchMaking::StartSessionSearch(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	...
	if (pTask->params[PSN_MATCHMAKING_SEARCH_PARAMS_SLOT] != TMemInvalidHdl)
	{
		...
		{
			SDrxSessionSearchParam* pData = (SDrxSessionSearchParam*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_SEARCH_PARAMS_SLOT]);
			if (pTask->params[PSN_MATCHMAKING_SEARCH_DATA_SLOT] != TMemInvalidHdl)
			{
				pData->m_data = (SDrxSessionSearchData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_SEARCH_DATA_SLOT]);
			}
			else
			{
				pData->m_data = NULL;
...
void CDrxPSNMatchMaking::TickSessionSearch(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
...
bool CDrxPSNMatchMaking::EventRequestResponse_GetWorldInfoListForSearch(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
...
void CDrxPSNMatchMaking::TickSessionSearchRequest(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
...
bool CDrxPSNMatchMaking::EventRequestResponse_SearchRoom(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
	...
	if ((data.m_requestEvent.m_error == 0) &&
	    ...
	    (data.m_requestEvent.m_dataSize == sizeof(SDrxPSNSearchRoomResponse)))
	{
		const SDrxPSNSearchRoomResponse* const pResponse = (SDrxPSNSearchRoomResponse*)_this->m_lobby->MemGetPtr(data.m_requestEvent.m_dataHdl);
		SSearchInfoData* pSearchInfo = (SSearchInfoData*)_this->m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_PSNSUPPORT_SEARCH_INFO_SLOT]);
		SDrxSessionSearchParam* pSearchParams = (SDrxSessionSearchParam*)_this->m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_SEARCH_PARAMS_SLOT]);
...
void CDrxPSNMatchMaking::TickSessionWaitForQOS(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
...
void CDrxPSNMatchMaking::TickSessionQOSSearch(DrxMatchMakingTaskID mmTaskID)
{
	int qosIndex = 0;
	...
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
		...
		{
			SDrxPSNSessionID* pId = (SDrxPSNSessionID*)pSearchInfo->m_results[qosIndex].m_id.get();
			if (pId)
			{
...
bool CDrxPSNMatchMaking::EventRequestResponse_SearchQOS(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	...
	assert(data.m_requestEvent.m_dataHdl != TMemInvalidHdl);
	assert(data.m_requestEvent.m_dataSize == sizeof(SDrxPSNSignalingGetPingInfoResponse));

	SSearchInfoData* pSearchInfo = (SSearchInfoData*)_this->m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_PSNSUPPORT_SEARCH_INFO_SLOT]);
	SDrxSessionSearchResult* pSearchResult = NULL;
	...
	if ((data.m_requestEvent.m_error == 0) &&
	    (data.m_requestEvent.m_dataHdl != TMemInvalidHdl) &&
	    (data.m_requestEvent.m_dataSize == sizeof(SDrxPSNSignalingGetPingInfoResponse)) &&
	    pSearchResult)
	{
		const SDrxPSNSignalingGetPingInfoResponse* const pQOSResponse = (SDrxPSNSignalingGetPingInfoResponse*)_this->m_lobby->MemGetPtr(data.m_requestEvent.m_dataHdl);
...
void CDrxPSNMatchMaking::EndSessionSearch(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	...
	if (pTask->error == eCLE_Success)
	{
		if (pTask->params[PSN_MATCHMAKING_PSNSUPPORT_SEARCH_INFO_SLOT] != TMemInvalidHdl)
		{
			...
			{
				SDrxSessionSearchResult* pResult = &pSearchInfo->m_results[i];
				((DrxMatchmakingSessionSearchCallback)pTask->cb)(pTask->lTaskID, eCLE_SuccessContinue, pResult, pTask->cbArg);

				//-- remove reference to DrxSessionID, since it's a smart pointer inside a larger alloc that wouldn't be cleaned up otherwise.
				pResult->m_id = NULL;
			}
		}
		else
...
EDrxLobbyError CDrxPSNMatchMaking::SessionJoin(uint32* users, int numUsers, uint32 flags, DrxSessionID id, DrxLobbyTaskID* taskID, DrxMatchmakingSessionJoinCallback cb, void* cbArg)
{
	LOBBY_AUTO_LOCK;

	DrxLobbySessionHandle h;
	EDrxLobbyError error = CreateSessionHandle(&h, false, users, numUsers, flags);
	if (error == eCLE_Success)
	{
		DrxMatchMakingTaskID mmTaskID;
		...
		if (error == eCLE_Success)
		{
			...
			if (error == eCLE_Success)
			{
				error = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_WEBAPI_JOIN_SLOT, NULL, 1, sizeof(SDrxPSNOrbisWebApiJoinSessionInput));
				if (error == eCLE_Success)
				{
					SDrxPSNSessionID* psnID = (SDrxPSNSessionID*)id.get();
					SSession* pSession = &m_sessions[h];
...
void CDrxPSNMatchMaking::TickSessionJoin(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
...
bool CDrxPSNMatchMaking::EventRequestResponse_JoinRoom(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
	assert(pTask->subTask == eST_WaitingForJoinRoomRequestCallback);
	assert(data.m_requestEvent.m_event == REQUEST_EVENT_JOIN_ROOM);
	assert(data.m_requestEvent.m_dataHdl != TMemInvalidHdl);
	assert(data.m_requestEvent.m_dataSize == sizeof(SDrxPSNJoinRoomResponse));

	if ((data.m_requestEvent.m_error == 0) &&
	    (data.m_requestEvent.m_dataHdl != TMemInvalidHdl) &&
	    (data.m_requestEvent.m_dataSize == sizeof(SDrxPSNJoinRoomResponse)))
	{
		if (pTask->session != DrxLobbyInvalidSessionHandle)
		{
			SSession* pSession = &_this->m_sessions[pTask->session];
			if (pSession->IsUsed())
			{
				const SDrxPSNJoinRoomResponse* const pJoinResponse = (SDrxPSNJoinRoomResponse*)_this->m_lobby->MemGetPtr(data.m_requestEvent.m_dataHdl);
...
void CDrxPSNMatchMaking::ProcessSessionJoinHostAck(const TNetAddress& addr, CDrxSharedLobbyPacket* pPacket)
{
	DrxLobbySessionHandle h = DrxLobbyInvalidSessionHandle;
	SDrxMatchMakingConnectionUID hostUID = pPacket->GetFromConnectionUID();
...
void CDrxPSNMatchMaking::TickSessionJoinSignaling(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
					...
					if (pTask->session != DrxLobbyInvalidSessionHandle)
					{
						...
						if (pSession->IsUsed())
						{
							if (pSession->createFlags & DRXSESSION_CREATE_FLAG_INVITABLE)
							{
								...
								{
									SJoinParamData* pJoinParams = (SJoinParamData*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_PSNSUPPORT_JOIN_REQUEST_SLOT]);
									SDrxPSNOrbisWebApiJoinSessionInput* pWebApiJoinParams = (SDrxPSNOrbisWebApiJoinSessionInput*)m_pPSNSupport->GetLobby()->MemGetPtr(pTask->params[PSN_MATCHMAKING_WEBAPI_JOIN_SLOT]);
									if (pJoinParams && pWebApiJoinParams)
									{
										...
										pJoinParams->m_joinJobId = m_pPSNSupport->GetWebApiInterface().AddJob(CDrxPSNOrbisWebApiThread::JoinSession, pTask->params[PSN_MATCHMAKING_WEBAPI_JOIN_SLOT]);
...
void CDrxPSNMatchMaking::EventWebApiJoinSession(DrxMatchMakingTaskID mmTaskID, SDrxPSNSupportCallbackEventData& data)
{
	STask* pTask = &m_task[mmTaskID];
...
void CDrxPSNMatchMaking::EndSessionJoin(DrxMatchMakingTaskID mmTaskID)
{
	uint32 myIP = 0, myPort = 0;
	STask* pTask = &m_task[mmTaskID];
	SSession* pSession = &m_sessions[pTask->session];

	SDrxPSNJoinRoomResponse* pResponse = NULL;

	if (pTask->params[PSN_MATCHMAKING_PSNSUPPORT_JOIN_RESPONSE_SLOT] != TMemInvalidHdl)
	{
		pResponse = (SDrxPSNJoinRoomResponse*)m_lobby->MemGetPtr(pTask->params[PSN_MATCHMAKING_PSNSUPPORT_JOIN_RESPONSE_SLOT]);
	}
		...
		if (pResponse)
		{
			...
			if (pSession->m_ownerMemberId != pSession->m_myMemberId)
			{
				if (pMember && (pMember->m_signalingState == ePSNCS_Active))
				{
					...
					//						{
					//							((CDrxPSNLobbyUI*)m_pService->GetLobbyUI())->SetStoredInviteSessionHandle(CreateGameSessionHandle(pTask->session, DrxMatchMakingInvalidConnectionUID));
					//						}
					//					}
				}
				else
	...
	((DrxMatchmakingSessionJoinCallback)pTask->cb)(pTask->lTaskID, pTask->error, CreateGameSessionHandle(pTask->session, pSession->localConnection.uid), myIP, myPort, pTask->cbArg);
...
EDrxLobbyError CDrxPSNMatchMaking::SessionSetLocalUserData(DrxSessionHandle gh, DrxLobbyTaskID* pTaskID, uint32 user, uint8* pData, uint32 dataSize, DrxMatchmakingCallback pCB, void* pCBArg)
{
	EDrxLobbyError error = eCLE_Success;
...
void CDrxPSNMatchMaking::TickSessionUserData(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
...
bool CDrxPSNMatchMaking::EventRequestResponse_SetRoomMemberDataInternal(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity tests
...
void CDrxPSNMatchMaking::OnPacket(const TNetAddress& addr, CDrxLobbyPacket* pPacket)
{
...
void CDrxPSNMatchMaking::SessionUserDataEvent(EDrxLobbySystemEvent event, DrxLobbySessionHandle h, DrxMatchMakingConnectionID id)
{
	SSession* pSession = &m_sessions[h];
	...
	if (pSession->localFlags & DRXSESSION_LOCAL_FLAG_USER_DATA_EVENTS_STARTED)
	{
		...
		userInfo.m_userID = new SDrxPSNUserID;
...
void CDrxPSNMatchMaking::InitialUserDataEvent(DrxLobbySessionHandle h)
{
	SSession* pSession = &m_sessions[h];
	...
	for (uint32 i = 0; i < MAX_LOBBY_CONNECTIONS; i++)
	{
		...
		if (pConnection->used)
		{
			...
			#if NETWORK_HOST_MIGRATION
void CDrxPSNMatchMaking::HostMigrationInitialise(DrxLobbySessionHandle h, EDisconnectionCause cause)
{
	SSession* pSession = &m_sessions[h];
...
EDrxLobbyError CDrxPSNMatchMaking::HostMigrationServer(SHostMigrationInfo* pInfo)
{
	EDrxLobbyError error = eCLE_Success;
	...
	if (error != eCLE_Success)
	{
		NetLog("[Host Migration]: CDrxPSNMatchMaking::HostMigrationServer(): " PRFORMAT_SH ", error %d", PRARG_SH(h), error);
	}

	return error;
}

void CDrxPSNMatchMaking::HostMigrationServerNT(DrxMatchMakingTaskID mmTaskID)
{
	STask* task = &m_task[mmTaskID];
	task->StartTimer();
}

void CDrxPSNMatchMaking::TickHostMigrationServerNT(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	SSession* pSession = &m_sessions[pTask->session];
	...
	packet.StartWrite(ePSNPT_HostMigrationServer, true);
				...
				#if !defined(DRXNETWORK_RELEASEBUILD)
	int count = 0;
	CDrxMatchMaking::SSession* pSession = CDrxMatchMaking::m_sessions[pTask->session];
	for (uint32 i = 0; i < MAX_LOBBY_CONNECTIONS; i++)
	{
		CDrxMatchMaking::SSession::SRConnection* pConnection = pSession->remoteConnection[i];
		...
		if (pConnection->used)
		{
			...
			{
				NetLog("    sending to " PRFORMAT_ADDR " client, " PRFORMAT_MMCINFO, PRARG_ADDR(ip), PRARG_MMCINFO(DrxMatchMakingConnectionID(i), pConnection->connectionID, pConnection->uid));
			}
	...
	SendToAll(mmTaskID, &packet, pTask->session, DrxMatchMakingInvalidConnectionID);
...
void CDrxPSNMatchMaking::ProcessHostMigrationFromServer(const TNetAddress& addr, CDrxSharedLobbyPacket* pPacket)
{
	DrxLobbySessionHandle h = DrxLobbyInvalidSessionHandle;
	SDrxMatchMakingConnectionUID hostUID = pPacket->GetFromConnectionUID();
	DrxMatchMakingConnectionID mmCxID = DrxMatchMakingInvalidConnectionID;
...
bool CDrxPSNMatchMaking::GetNewHostAddress(char* address, SHostMigrationInfo* pInfo)
{
	bool success = false;
	...
	if ((pSession->localFlags & DRXSESSION_LOCAL_FLAG_USED) && pPrivateInfo->m_newHostAddressValid)
	{
		uint32 myIP = 0;
		uint16 myPort = 0;
		...
		if (pMember)
		{
			if (pSession->serverConnectionID != DrxMatchMakingInvalidConnectionID)
			{
				if (pSession->desiredHostUID != DrxMatchMakingInvalidConnectionUID)
				{
					if ((pMember->m_valid & ePSNMI_Me) == 0)      // Make sure we are not the new host before updating the connection information
					{
						...
						{
							DrxMatchMakingConnectionID remoteConnectionID = FindConnectionIDFromRoomMemberID(sessionIndex, pSession->m_ownerMemberId);
							if (remoteConnectionID != DrxMatchMakingInvalidConnectionID)
							{
			...
			else
			{
				...
				drx_sprintf(address, HOST_MIGRATION_MAX_SERVER_NAME_SIZE, "%s:%d", LOCAL_CONNECTION_STRING, port);
			}
		}
...
void CDrxPSNMatchMaking::HostMigrationStartNT(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	SSession* pSession = &m_sessions[pTask->session];
...
void CDrxPSNMatchMaking::TickHostMigrationStartNT(DrxMatchMakingTaskID mmTaskID)
{
	m_pPSNSupport->ResumeTransitioning(ePSNOS_Matchmaking_Available);
	if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
	{
...
bool CDrxPSNMatchMaking::EventRequestResponse_GrantRoomOwner(CDrxPSNMatchMaking* _this, STask* pTask, DrxMatchMakingTaskID taskId, SDrxPSNSupportCallbackEventData& data)
{
	// sanity
...
void CDrxPSNMatchMaking::ProcessHostMigrationStart(const TNetAddress& addr, CDrxSharedLobbyPacket* pPacket)
{
	DrxLobbySessionHandle h = DrxLobbyInvalidSessionHandle;
	SDrxMatchMakingConnectionUID hostUID = pPacket->GetFromConnectionUID();
	DrxMatchMakingConnectionID mmCxID = DrxMatchMakingInvalidConnectionID;
	...
	if (FindConnectionFromUID(hostUID, &h, &mmCxID))
	{
		...
		if (pSession->IsUsed() && !(pSession->localFlags & DRXSESSION_LOCAL_FLAG_HOST))
		{
			if (pSession->hostMigrationInfo.m_state == eHMS_Idle)
			{
...
EDrxLobbyError CDrxPSNMatchMaking::SessionEnsureBestHost(DrxSessionHandle gh, DrxLobbyTaskID* pTaskID, DrxMatchmakingCallback pCB, void* pCBArg)
{
...
void CDrxPSNMatchMaking::AddVoiceUser(SSession::SRoomMember* pMember)
{
			#if USE_PSN_VOICE
	CDrxPSNVoice* pVoice = (CDrxPSNVoice*)m_pService->GetVoice();
	if (pVoice)
	{
...
void CDrxPSNMatchMaking::ClearVoiceUser(SSession::SRoomMember* pMember)
{
			#if USE_PSN_VOICE
	CDrxPSNVoice* pVoice = (CDrxPSNVoice*)m_pService->GetVoice();
	if (pVoice)
...
void CDrxPSNMatchMaking::UpdateVoiceUsers()
{
			#if USE_PSN_VOICE
	CDrxPSNVoice* pVoice = (CDrxPSNVoice*)m_pService->GetVoice();
	if (pVoice)
...
EDrxLobbyError CDrxPSNMatchMaking::CopySessionIDToBuffer(DrxSessionID sessionId, void* pBuffer, size_t& bufferSize)
{
	if (SDrxPSNSessionID* pData = (SDrxPSNSessionID*)sessionId.get())
	{
		size_t requiredSize = sizeof(SDrxPSNSessionID);
...
EDrxLobbyError CDrxPSNMatchMaking::CopySessionIDFromBuffer(const void* pBuffer, size_t bufferSize, DrxSessionID& sessionId)
{
	size_t requiredSize = sizeof(SDrxPSNSessionID);
	if (bufferSize == requiredSize)
	{
		SDrxPSNSessionID* pData = new SDrxPSNSessionID();
...
void CDrxPSNMatchMaking::ForceFromInvite(DrxSessionID sessionId)
{
	if (SDrxPSNSessionID* pData = (SDrxPSNSessionID*)sessionId.get())
	{
...
EDrxLobbyError CDrxPSNMatchMaking::SessionSetAdvertisementData(DrxSessionHandle gh, uint8* pData, uint32 dataSize, DrxLobbyTaskID* pTaskID, DrxMatchmakingCallback pCb, void* pCbArg)
{
	LOBBY_AUTO_LOCK;

	EDrxLobbyError rc = eCLE_InvalidSession;
	DrxLobbySessionHandle h = GetSessionHandleFromGameSessionHandle(gh);
	if (h != DrxLobbyInvalidSessionHandle)
	{
		DrxMatchMakingTaskID mmTaskID;
		...
		if (rc == eCLE_Success)
		{
			...
			if (rc == eCLE_Success)
			{
				rc = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_WEBAPI_UPDATEADVERT_REQUEST_SLOT, NULL, 1, sizeof(SDrxPSNOrbisWebApiUpdateSessionInput));
				if (rc == eCLE_Success)
				{
					SDrxPSNOrbisWebApiUpdateSessionInput* pAdvertInput = (SDrxPSNOrbisWebApiUpdateSessionInput*)m_lobby->MemGetPtr(m_task[mmTaskID].params[PSN_MATCHMAKING_WEBAPI_UPDATEADVERT_REQUEST_SLOT]);
					memset(pAdvertInput, 0, sizeof(SDrxPSNOrbisWebApiUpdateSessionInput));
					...
					pAdvertInput->advertisement.m_sizeofData = MIN(dataSize, DRX_WEBAPI_UPDATESESSION_LINKDATA_MAX_SIZE);
...
void CDrxPSNMatchMaking::TickSessionSetAdvertisementData(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	...
	if (pTask->error == eCLE_Success)
	{
		...
		if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
		{
			    ...
			    (pTask->params[PSN_MATCHMAKING_WEBAPI_UPDATEADVERT_REQUEST_SLOT] != TMemInvalidHdl))
			{
				...
				pParam->m_jobId = m_pPSNSupport->GetWebApiInterface().AddJob(CDrxPSNOrbisWebApiThread::UpdateSession, pTask->params[PSN_MATCHMAKING_WEBAPI_UPDATEADVERT_REQUEST_SLOT]);
...
void CDrxPSNMatchMaking::EventWebApiUpdateSessionAdvertisement(DrxMatchMakingTaskID mmTaskID, SDrxPSNSupportCallbackEventData& data)
{
	STask* pTask = &m_task[mmTaskID];
...
void CDrxPSNMatchMaking::EndSessionSetAdvertisementData(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	((DrxMatchmakingCallback)pTask->cb)(pTask->lTaskID, pTask->error, pTask->cbArg);
}

EDrxLobbyError CDrxPSNMatchMaking::SessionGetAdvertisementData(DrxSessionID sessionId, DrxLobbyTaskID* pTaskID, DrxMatchmakingSessionGetAdvertisementDataCallback pCb, void* pCbArg)
{
	LOBBY_AUTO_LOCK;
	...
	{
		DrxMatchMakingTaskID mmTaskID;
		rc = StartTask(eT_SessionGetAdvertisementData, false, 0, &mmTaskID, pTaskID, DrxLobbyInvalidSessionHandle, (void*)pCb, pCbArg);
		if (rc == eCLE_Success)
		{
			...
			if (rc == eCLE_Success)
			{
				rc = CreateTaskParam(mmTaskID, PSN_MATCHMAKING_WEBAPI_GETADVERT_REQUEST_SLOT, NULL, 1, sizeof(SDrxPSNOrbisWebApiGetSessionLinkDataInput));
				if (rc == eCLE_Success)
				{
					SDrxPSNSessionID* pPSNSessionId = (SDrxPSNSessionID*)sessionId.get();
					SDrxPSNOrbisWebApiGetSessionLinkDataInput* pAdvertInput = (SDrxPSNOrbisWebApiGetSessionLinkDataInput*)m_lobby->MemGetPtr(m_task[mmTaskID].params[PSN_MATCHMAKING_WEBAPI_GETADVERT_REQUEST_SLOT]);
...
void CDrxPSNMatchMaking::TickSessionGetAdvertisementData(DrxMatchMakingTaskID mmTaskID)
{
	STask* pTask = &m_task[mmTaskID];
	...
	if (pTask->error == eCLE_Success)
	{
		...
		if (m_pPSNSupport->HasTransitioningReachedState(ePSNOS_Matchmaking_Available))
		{
			    ...
			    (pTask->params[PSN_MATCHMAKING_WEBAPI_GETADVERT_REQUEST_SLOT] != TMemInvalidHdl))
			{
				...
				pParam->m_jobId = m_pPSNSupport->GetWebApiInterface().AddJob(CDrxPSNOrbisWebApiThread::GetSessionLinkData, pTask->params[PSN_MATCHMAKING_WEBAPI_GETADVERT_REQUEST_SLOT]);
				if (pParam->m_jobId != INVALID_WEBAPI_JOB_ID)
				{
					//-- waiting for callback
				}
...
void CDrxPSNMatchMaking::EventWebApiGetSessionLinkData(DrxMatchMakingTaskID mmTaskID, SDrxPSNSupportCallbackEventData& data)
{
	STask* pTask = &m_task[mmTaskID];
	...
	if (data.m_webApiEvent.m_error == PSN_OK)
	{
		if (data.m_webApiEvent.m_pResponseBody)
		{
			if ((data.m_webApiEvent.m_pResponseBody->eType == SDrxPSNWebApiResponseBody::E_RAW) &&
...
void CDrxPSNMatchMaking::EndSessionGetAdvertisementData(DrxMatchMakingTaskID mmTaskID)
{
	uint8* pData = NULL;
	uint32 dataSize = 0;
	...
	((DrxMatchmakingSessionGetAdvertisementDataCallback)pTask->cb)(pTask->lTaskID, pTask->error, pData, dataSize, pTask->cbArg);
}
