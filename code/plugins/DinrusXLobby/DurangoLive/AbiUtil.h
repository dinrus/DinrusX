...
public:
	...
	virtual /* propget */ HRESULT STDMETHODCALLTYPE get_HasCurrent(_Outboolean* hasCurrent) noexcept override
	{
		if (!hasCurrent)
			return E_INVALIDARG;
	...
	virtual HRESULT STDMETHODCALLTYPE MoveNext(_Outboolean* hasCurrent) noexcept override
	{
		if (!hasCurrent)
			return E_INVALIDARG;
...
public:
	...
	virtual HRESULT STDMETHODCALLTYPE IndexOf(_In_opt_ T_abi value, _Out_ unsigned* index, _Outboolean* found) noexcept override
	{
		if (!index || !found)
			return E_INVALIDARG;
