...
//////////////////////////////////////////////////////////////////////////
CDrxDurangoLiveOnlineStorage::CDrxDurangoLiveOnlineStorage(CDrxLobby* pLobby, CDrxLobbyService* pService)
	: m_pendingCount(0)
	, m_status(eCOSS_Idle)
	...
	, m_pService(pService)
{
	memset(&m_pendingQueries, 0, sizeof(DrxOnlineStorageQueryData) * MAXIMUM_PENDING_QUERIES);
}
...
void DebugProfileLogCurrentUser(const char* reason, int queryId)
{
	...
	DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[>PROFILE_DEBUG<][ONLINE] = = = = = = %s = = = = = (?)", reason);
	DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[>PROFILE_DEBUG<][ONLINE] signed in = %s", isSignedIn ? "true" : "false");
	DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[>PROFILE_DEBUG<][ONLINE] guest = %s", isGuest ? "true" : "false");
	DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[>PROFILE_DEBUG<][ONLINE] userId = %s", user.Get() ? (const char*)XboxUserId.Get() : "#####NoId#####");
	DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[>PROFILE_DEBUG<][ONLINE] (optional query id = %d)", queryId);
	DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "[>PROFILE_DEBUG<][ONLINE] ~ ~ ~ ~ ~ ~ %s ~ ~ ~ ~ ~", reason);
	#endif // DEBUG_ONLINE_STORAGE_ALLOW_TRACE
}
}

void CDrxDurangoLiveOnlineStorage::Tick(CTimeValue tv)
{
	...
	case eCOSS_Idle:
		if (m_pendingCount)
		{
			DrxOnlineStorageQueryData* pQueryData = &m_pendingQueries[0];
			...
			{
				DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_WARNING, "CDrxDurangoLiveOnlineStorage::Tick - Trying to query title managed storage while offline");
			...
			case eCOSO_Upload:
				{
					...
					if (SUCCEEDED(hr))
					{
						...
						DrxLogAlways("CDrxDurangoLiveOnlineStorage - Upload: started");
						...
						if (SUCCEEDED(hr) && hr != S_FALSE)
						{
							...
							if (S_FALSE == hr)
							{
								DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: UploadBlobAsync returned S_FALSE");
								liveStatus = !ERROR_SUCCESS;
							}
							...
							{
								DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: UploadBlobAsync failed: %08X", hr);
								liveStatus = !ERROR_SUCCESS;
							}
						}
						else
						{
							DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: get_TitleStorageService failed: %08X", hr);
							liveStatus = !ERROR_SUCCESS;
						}
					}
					else
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: failed to create buffer for upload: %08X", hr);
						liveStatus = !ERROR_SUCCESS;
					}
				}
				break;
			...
			case eCOSO_Download:
				{
					...
					if (SUCCEEDED(hr))
					{
						DrxLogAlways("CDrxDurangoLiveOnlineStorage - Download: started");
						...
						if (SUCCEEDED(hr))
						{
							...
							{
								DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: DownloadBlobAsync2 failed: %08X", hr);
								liveStatus = !ERROR_SUCCESS;
							}
						}
						else
					...
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: failed to create buffer for download: %08X", hr);
						liveStatus = !ERROR_SUCCESS;
					}
				}
				break;
	...
	case eCOSS_Working:
		...
		case ABI::Windows::Foundation::AsyncStatus::Completed:
			if (m_pendingCount)
			{
				...
				{
				case eCOSO_Upload:
					DrxLogAlways("CDrxDurangoLiveOnlineStorage: successfully saved.");
					liveStatus = ERROR_SUCCESS;
					break;
				case eCOSO_Download:
					{
						...
						if (SUCCEEDED(hr))
						{
								...
								if (bytes)
								{
									...
									if (SUCCEEDED(hr))
									{
										if (pQueryData->bufferSize >= readBytes)
										{
											...
											DrxLogAlways("CDrxDurangoLiveOnlineStorage: successfully downloaded.");
										}
										else
										{
											DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: bad buffer size: %u < %u", (uint)pQueryData->bufferSize, (uint)readBytes);
											liveStatus = !ERROR_SUCCESS;
										}
									}
									else
									{
										DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: failed to get length of buffer: %08X", hr);
										liveStatus = !ERROR_SUCCESS;
									}
								}
							}
							...
							{
								DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: failed to get buffer from result: %08X", hr);
								liveStatus = !ERROR_SUCCESS;
							}
						}
						else
						{
							DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: failed to fetch result: %08X", hr);
							liveStatus = !ERROR_SUCCESS;
						}
					}
					break;
		...
		default:
			if (m_pendingCount)
			{
				...
				{
				case eCOSO_Upload:
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: UploadBlobAsync failed for status=%d,hr=%08X", status, hr);
					break;
				case eCOSO_Download:
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage: DownloadBlobAsync failed for status=%d,hr=%08X", status, hr);
					break;
				}
	...
	case ERROR_SUCCESS:
		{
			...
			TO_GAME_FROM_LOBBY(&CDrxDurangoLiveOnlineStorage::TriggerCallbackOnGameThread, this, m_pendingQueries[0]);
			...
			{
				memcpy(&m_pendingQueries[i], &m_pendingQueries[i + 1], sizeof(DrxOnlineStorageQueryData));
			}
...
EDrxLobbyError CDrxDurangoLiveOnlineStorage::OnlineStorageDataQuery(DrxOnlineStorageQueryData* pQueryData)
{
	LOBBY_AUTO_LOCK;

	if (m_pendingCount == (MAXIMUM_PENDING_QUERIES - 1))
	{
		DrxWarning(VALIDATOR_MODULE_NETWORK, VALIDATOR_ERROR, "CDrxDurangoLiveOnlineStorage::OnlineStorageDataQuery ran out of storage, too many queries at once");
	...
	if (pQueryData->lTaskID != DrxLobbyInvalidTaskID)
	{
		memcpy(&m_pendingQueries[m_pendingCount], pQueryData, sizeof(DrxOnlineStorageQueryData));
		m_pendingCount++;
	}
	else
	{
...
void CDrxDurangoLiveOnlineStorage::TriggerCallbackOnGameThread(const DrxOnlineStorageQueryData& QueryData)
{
