...
<DinrusX/DinrusXSys/Profilers/IStatoscope.h>
...
void CHostMigration::Start(SHostMigrationInfo_Private* pInfo)
{
	...
	{
		DrxLogAlways("Host migration for dedicated servers is disabled");
...
void CHostMigration::Update(SHostMigrationInfo_Private* pInfo)
{
	...
	bool done = true;
	THostMigrationEventListenerContainer* pListeners = ((CDrxLobby*)CDrxLobby::GetLobby())->GetHostMigrationListeners();
	...
	if (pListeners)
	{
		while (done)
		{
			...
			DrxLobbySessionHandle sh = pMatchMaking->GetSessionHandleFromGameSessionHandle(pInfo->m_session);
			DRX_ASSERT(sh != DrxLobbyInvalidSessionHandle);
					...
					case eHMS_WaitForNewServer:
						{
							...
							drx_strcpy(address, pInfo->m_newServer.c_str());
			...
			if (done)
			{
				...
				case eHMS_StateCheck:
	...
	#endif
					{
						...
						gEnv->bHostMigrating = static_cast<CDrxMatchMaking*>(CDrxLobby::GetLobby()->GetMatchMaking())->AreAnySessionsMigrating();
...
void CHostMigration::HandleListenerReturn(SHostMigrationEventListenerInfo& li, int sessionIndex, SHostMigrationInfo_Private* pInfo, IHostMigrationEventListener::EHostMigrationReturn listenerReturn, bool ignoreTerminate)
{
	...
	case IHostMigrationEventListener::Listener_Terminate:
		...
		break;
	default:
		DrxFatalError("Unknown host migration listener return value!!");
		break;
	}
}
