...
CDrxFriends::CDrxFriends(CDrxLobby* pLobby, CDrxLobbyService* pService)
{
	DRX_ASSERT_MESSAGE(pLobby, "CDrxFriends::CDrxFriends: Lobby not specified");
	DRX_ASSERT_MESSAGE(pService, "CDrxFriends::CDrxFriends: Service not specified");
...
EDrxLobbyError CDrxFriends::Initialise()
{
	for (uint32 i = 0; i < MAX_FRIENDS_TASKS; i++)
	{
		DRX_ASSERT_MESSAGE(m_pTask[i], "CDrxFriends: Task base pointers not setup");
...
EDrxLobbyError CDrxFriends::Terminate()
{
	for (uint32 i = 0; i < MAX_FRIENDS_TASKS; i++)
	{
		DRX_ASSERT_MESSAGE(m_pTask[i], "CDrxFriends: Task base pointers not setup");
...
EDrxLobbyError CDrxFriends::StartTask(uint32 eTask, bool startRunning, DrxFriendsTaskID* pFTaskID, DrxLobbyTaskID* pLTaskID, DrxLobbySessionHandle h, void* pCb, void* pCbArg)
{
	...
	if (lobbyTaskID != DrxLobbyInvalidTaskID)
	{
		for (uint32 i = 0; i < MAX_FRIENDS_TASKS; i++)
		{
			...
			DRX_ASSERT_MESSAGE(pTask, "CDrxFriends: Task base pointers not setup");
...
void CDrxFriends::FreeTask(DrxFriendsTaskID fTaskID)
{
...
void CDrxFriends::CancelTask(DrxLobbyTaskID lTaskID)
{
	LOBBY_AUTO_LOCK;
...
EDrxLobbyError CDrxFriends::CreateTaskParamMem(DrxFriendsTaskID fTaskID, uint32 param, const void* pParamData, size_t paramDataSize)
{
	STask* pTask = m_pTask[fTaskID];
...
void CDrxFriends::UpdateTaskError(DrxFriendsTaskID fTaskID, EDrxLobbyError error)
{
	STask* pTask = m_pTask[fTaskID];
