...
#endif
{
	...
	REGISTER_CVAR2("cld_enable", &cldEnable, 0, VF_CHEAT, "Enable drx lobby debug tests");
	REGISTER_CVAR2("cld_error_percentage", &cldErrorPercentage, 10, VF_CHEAT, "How often a drx lobby task will return with a time out error");
	REGISTER_CVAR2("cld_min_delay_time", &cldMinDelayTime, 2000, VF_CHEAT, "The minimum time a drx lobby task will be delayed before processing in milliseconds");
	REGISTER_CVAR2("cld_max_delay_time", &cldMaxDelayTime, 5000, VF_CHEAT, "The maximum time a drx lobby task will be delayed before processing in milliseconds");
#endif
...
void CLobbyCVars::EnsureBestHostForSessionCmd(IConsoleCmdArgs* pArgs)
{
	...
	if (argCount == 2)
	{
		DrxLobbySessionHandle h = atoi(pArgs->GetArg(1));
		if (h != DrxLobbyInvalidSessionHandle)
		{
			CDrxLobby* pLobby = (CDrxLobby*)CDrxLobby::GetLobby();
			if (pLobby != NULL)
			{
				CDrxMatchMaking* pMatchMaking = (CDrxMatchMaking*)pLobby->GetMatchMaking();
				if (pMatchMaking != NULL)
				{
					DrxSessionHandle gh = pMatchMaking->CreateGameSessionHandle(h, DrxMatchMakingInvalidConnectionUID);
	...
	{
		DrxLog("Bad arguments for net_ensure_best_host_for_session");
	}
}
#endif
...
void CLobbyCVars::DedicatedServerArbitratorListFreeServersCmd(IConsoleCmdArgs* pArgs)
{
	if (gEnv->pLobby)
	{
		...
		if (pLobbyService)
		{
			CDrxDedicatedServerArbitrator* pArbitrator = pLobbyService->GetDedicatedServerArbitrator();
