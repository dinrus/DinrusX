		...
		namespace Sce
		{
			CPlugin::~CPlugin()
			{
				...
				if (m_iHTTPLibId > 0)
				{
					...
					if (result < 0)
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpEnd() error: 0x%08X\n", result);
					}
				}
				...
				if (m_iSSLLibId > 0)
				{
					...
					if (result < 0)
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceSslEnd() error: 0x%08X\n", result);
					}
				}
				...
				if (m_iNetLibId > 0)
				{
					...
					if (result > 0)
					{
						...
						if (result < 0)
						{
							DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] Failed to terminate net");
						}
					}
					else
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] Failed to destroy net pool");
					}
				}	
			}
			...
			bool CPlugin::Initialize(SSystemGlobalEnvironment& env, const SSystemInitParams& initParams)
			{
				...
				if (result < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceNetInit() error: 0x%08X\n", result);
					return false;
				}

				constexpr int netPoolHeapSize = 64 * 1024;
				m_iNetLibId = sceNetPoolCreate(gEnv->pSystem->GetIProjectUpr()->GetCurrentProjectName(), netPoolHeapSize, 0);
				if (result < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceNetPoolCreate() error: 0x%08X\n", result);
					return false;
				}
				...
				if (m_iSSLLibId < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceSslInit() error: 0x%08X\n", m_iSSLLibId);
					return false;
				}
				...
				if (m_iHTTPLibId < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpInit() error: 0x%08X\n", m_iHTTPLibId);
					return false;
				}
			...
			bool CPlugin::Send(ERequestType requestType, const char* szURL, const char* szBody, TResponseCallback resultCallback, const THeaders& headers)
			{
				...
				DrxFixedStringT<32> productVersion = gEnv->pSystem->GetProductVersion().ToString();

				string userAgent;
				userAgent.Format("%s/%s", gEnv->pSystem->GetIProjectUpr()->GetCurrentProjectName(), productVersion.c_str());

				pRequest->templateId = sceHttpCreateTemplate(m_iHTTPLibId, userAgent, SCE_HTTP_VERSION_1_1, SCE_TRUE);
				if (pRequest->templateId < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpCreateTemplate() error: 0x%08X\n", pRequest->templateId);
					return false;
				}
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpSetNonblock() error: 0x%08X\n", returnCode);
					return false;
				}
				...
				if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpCreateEpoll() error: 0x%08X\n", returnCode);
					return false;
				}
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpCreateConnectionWithURL() error: 0x%08X\n", pRequest->connectionId);
					return false;
				}
				...
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpCreateRequestWithURL() error: 0x%08X\n", pRequest->requestId);
					return false;
				}
				...
				for(const std::pair<const char*, const char*>& headerPair : headers)
				{
					...
					if (returnCode < 0)
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpAddRequestHeader() error: 0x%08X\n", returnCode);
						return false;
					}
				}
			...
			bool CPlugin::WaitForRequest(SSceHttpRequest& request)
			{
				...
				if (returnCode > 0 && nbEvent.id == request.requestId)
				{
					...
					{
						DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] Request wait failed!");
						return false;
					}
				}
				else if (returnCode < 0)
				{
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpWaitRequest() error: 0x%08X\n", returnCode);
					return false;
				}
			...
			ERequestStateResult CPlugin::OnReceiveResponse(SSceHttpRequest& request)
			{
				...
				else if (returnCode < 0)
				{
					...
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpReadData() error: 0x%08X\n", returnCode);
			...
			ERequestStateResult CPlugin::OnGetResponseLength(SSceHttpRequest& request)
			{
				...
				if (returnCode < 0)
				{
					...
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpGetContentLength() error: 0x%08X\n", returnCode);
					return ERequestStateResult::Abort;
				}
			...
			ERequestStateResult CPlugin::OnGetStatus(SSceHttpRequest& request)
			{
				...
				if (result < 0)
				{
					...
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpGetStatusCode() error: 0x%08X\n", result);
					return ERequestStateResult::Abort;
				}
			...
			ERequestStateResult CPlugin::OnSendRequest(SSceHttpRequest& request)
			{
				...
				if (returnCode < 0)
				{
					...
					DrxWarning(VALIDATOR_MODULE_SYSTEM, VALIDATOR_ERROR, "[SceHttp] sceHttpSendRequest() error: 0x%08X\n", returnCode);
					return ERequestStateResult::Abort;
				}
