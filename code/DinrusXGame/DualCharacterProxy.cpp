...
void CAnimationProxyDualCharacterBase::Generate1P3PPairFile()
{
	...
	for (uint32 i=0; i<numModels; ++i)
	{
		...
		uint32 numInstances = gEnv->pCharacterUpr->GetNumInstancesPerModel(rIDefaultSkeleton);
		if (numInstances > 0)
		{
			...
			for (uint32 anm = 0; anm < numAnims; anm++)
			{
				...
				if (s_animCrCHashMap.find(animCRC) == s_animCrCHashMap.end())
				{
					...
					const char *pos = DrxStringUtils::stristr(name, "_1p");
					if (pos)
					{
						char name3P[256];
...
bool CAnimationProxyDualCharacterBase::StartAnimation(IEntity *entity, const char* szAnimName, const DrxCharAnimationParams& Params, float speedMultiplier)
{
...
bool CAnimationProxyDualCharacter::StartAnimationById(IEntity *entity, int animId, const DrxCharAnimationParams& Params, float speedMultiplier)
{
	ICharacterInstance* pICharacter = entity->GetCharacter(m_characterMain);
	if (pICharacter)
	{
...
bool CAnimationProxyDualCharacterUpper::StartAnimationById(IEntity *entity, int animId, const DrxCharAnimationParams& Params, float speedMultiplier)
{
	ICharacterInstance* pICharacter = entity->GetCharacter(m_characterMain);
	if (pICharacter)
	{
