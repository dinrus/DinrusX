...
void CGameRulesStandardTwoTeams::RequestChangeTeam(EntityId playerId, int teamId, bool onlyIfUnassigned)
{
	...
	{
		DrxLog("Player %d already on team %d, onlyIfUnassigned is set", (int)playerId, teamId);
		return;
	}

	if (currentTeam == teamId)
	{
		DrxLog("Player %d already on team %d, not changing", (int)playerId, teamId);
		return;
	}
	...
	{
		DrxLog("Team %i is full, cannot switch player id %d", teamId, (int)playerId);
