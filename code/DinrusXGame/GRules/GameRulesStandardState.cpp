...
#include <DinrusX/DinrusXGame/GameRulesStandardState.h>
...
void CGameRulesStandardState::OwnClientEnteredGame(const CPlayer& rPlayer)
{
	...
	if(!rPlayer.ShouldPlayIntro() && m_state == IGameRulesStateModule::EGRS_Intro)
	{
		...
		DrxLog("CGameRulesStandardState::OwnClientEnteredGame(), state changed from %i to %i", (int)m_state, (int)m_lastReceivedServerState);
...
void CGameRulesStandardState::EnterPostGameState( EPostGameState state )
{
	...
	if(g_pGameCVars->g_hud_postgame_debug)
	{
		DrxLog("HUD PostGame - Entering new state %i from %i after %.2f seconds", (int)state, (int)m_postGameState, m_timeInCurrentPostGameState);
	}
#endif
			...
			if(!m_pGameRules->IsIntroSequenceCurrentlyPlaying())
			{
				...
				if(bApplyChange)
				{
					if (m_state != m_lastReceivedServerState)
					{
						...
						DrxLog("CGameRulesStandardState::NetSerialize(), state changed from %i to %i", (int)m_state, (int)m_lastReceivedServerState);
						ChangeState(m_lastReceivedServerState);
...
void CGameRulesStandardState::OnIntroCompleted()
{
	...
	else
	{
		...
		DrxLog("CGameRulesStandardState::OnIntroCompleted(), state changed from %i to %i", (int)m_state, (int)m_lastReceivedServerState);
		ChangeState(m_lastReceivedServerState);
	}
}
