...
#include <DinrusX/DinrusXGame/Audio/GameAudioUtils.h>
...
void CBattlechatter::InitData()
{
	...
	if(xmlData)
	{
		if( (int)BC_Last != xmlData->getChildCount() )
			DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Missing Battlechatter Valid Data, expected %d nodes, got %d!", (int)BC_Last, xmlData->getChildCount());
...
void CBattlechatter::Update(const float dt)
{
	if(m_clientPlayer)
	{
		...
		const int nNumActors = pActorUpr->GetNumActors();
		if(nNumActors > 0)
		{
...
EntityId CBattlechatter::GetNearestHearableFriendlyActor(CActor& rActor) const
{
	...
	if(nActorTeam != 0)
	{
		...
		const int hearableActorCount = pActorUpr->GetNumActors();
...
//-------------------------------------------------------
void CBattlechatter::UpdateActorBattlechatter(int actorIndex, const CActorUpr * pActorUpr)
{
	SActorData actorData;
	...
	else if(CPlayer * pPlayer = static_cast<CPlayer*>(gEnv->pGame->GetIGameFramework()->GetIActorSystem()->GetActor(actorData.entityId)))
	{
		...
		const static int updateSize = DRX_ARRAY_COUNT(s_updateFunctions);
	...
	const unsigned int inFlags)
{
	...
	for (int i=0, nNumActors = pActorUpr->GetNumActors(); i < nNumActors; i++)
	{
		SActorData actorData;
		pActorUpr->GetNthActorData(i, actorData);
