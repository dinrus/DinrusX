...
struct SPlaylistVariantAutoComplete : public IConsoleArgumentAutoComplete
{
	virtual int GetCount() const 
	{
		...
		if (pPlaylistUpr)
		{
			const int numPlaylists = pPlaylistUpr->GetNumPlaylists();
...
void SPlaylist::ResolveVariants()
{
	if (!rotExtInfo.m_resolvedVariants)
	{
		...
		if (pPlaylistUpr)
		{
			...
			int defaultIndex = pPlaylistUpr->GetDefaultVariant();

			int numVariants = rotExtInfo.m_supportedVariants.size();
			for (int i = 0; i < numVariants; ++ i)
			{
				int variantIndex = pPlaylistUpr->GetVariantIndex(rotExtInfo.m_supportedVariants[i].m_name.c_str());
				if (variantIndex >= 0)
				{
...
bool SPlaylistRotationExtension::LoadFromXmlNode(const XmlNodeRef xmlNode)
{
	...
	if (const XmlNodeRef infoNode=xmlNode->findChild("ExtendedInfo"))
	{
		...
		const char*  pCurrentLang = gEnv->pSystem->GetLocalizationUpr()->GetLanguage();
...
//---------------------------------------
void CPlaylistUpr::Init(const char* pPath)
{
	DRX_ASSERT(!m_inited);
...
//---------------------------------------
void CPlaylistUpr::AddGameModeOptionInt( const char *pCVarName, const char *pProfileOption, bool gameModeSpecific, int iDefault )
{
	DRX_ASSERT(gEnv);
...
//---------------------------------------
void CPlaylistUpr::AddGameModeOptionFloat( const char *pCVarName, const char *pProfileOption, bool gameModeSpecific, float netMultiplyer, float profileMultiplyer, float fDefault, int floatPrecision )
{
	DRX_ASSERT(gEnv);
	m_options.push_back(SGameModeOption());
	SGameModeOption &newOption = m_options[m_options.size() - 1];
...
//---------------------------------------
void CPlaylistUpr::AddGameModeOptionString( const char *pCVarName, const char *pProfileOption, bool gameModeSpecific )
{
	DRX_ASSERT(gEnv);
	m_options.push_back(SGameModeOption());
	SGameModeOption &newOption = m_options[m_options.size() - 1];
...
//---------------------------------------
SGameModeOption *CPlaylistUpr::GetGameModeOptionStruct( const char *pOptionName )
{
	DRX_ASSERT(gEnv);
...
//---------------------------------------
void CPlaylistUpr::AddConfigVar(const char *pCVarName, bool bNetSynched)
{
	ICVar *pCVar = gEnv->pConsole->GetCVar(pCVarName);
	if (pCVar)
	{
...
//---------------------------------------
bool CPlaylistUpr::HaveUnlockedPlaylistVariant(const SPlaylist *pPlaylist, const int variantId, DrxFixedStringT<128>* pUnlockString/*=NULL*/)
{
	bool result = true;
...
//---------------------------------------
void  CPlaylistUpr::SetActiveVariant(int variantIndex)
{
	DRX_ASSERT((variantIndex >= 0) && (variantIndex < (int)m_variants.size()));
...
//---------------------------------------
bool CPlaylistUpr::DisablePlaylist(const char* uniqueName)
{
	bool  disabled = false;
...
//---------------------------------------
void CPlaylistUpr::AddPlaylistsFromPath(const char* pPath)
{
...
//---------------------------------------
ILevelRotation::TExtInfoId CPlaylistUpr::GetPlaylistId( const char *pUniqueName ) const
{
...
//---------------------------------------
SPlaylist* CPlaylistUpr::FindPlaylistByUniqueName(const char* uniqueName)
{
	DRX_ASSERT(gEnv);
...
//---------------------------------------
const int CPlaylistUpr::GetNumPlaylists() const
{
	return m_playlists.size();
}

//---------------------------------------
const SPlaylist* CPlaylistUpr::GetPlaylist(const int index)
{
...
//---------------------------------------
int CPlaylistUpr::GetPlaylistIndex( ILevelRotation::TExtInfoId findId ) const
{
	int iFound = -1;
...
//---------------------------------------
const char *CPlaylistUpr::GetActiveVariant()
{
	const char *pResult = "";
...
//---------------------------------------
const int CPlaylistUpr::GetActiveVariantIndex() const
{
	return m_activeVariantIndex;
}

//---------------------------------------
const char* CPlaylistUpr::GetVariantName(const int index)
{
...
//---------------------------------------
int CPlaylistUpr::GetVariantIndex(const char *pName)
{
	int result = -1;
...
//---------------------------------------
const int CPlaylistUpr::GetNumVariants() const
{
	return m_variants.size();
}

//---------------------------------------
bool CPlaylistUpr::ChoosePlaylist(const int chooseIdx)
{
...
//---------------------------------------
bool CPlaylistUpr::ChooseVariant(int selectIdx)
{
	if (HavePlaylistSet())
	{
		if (SPlaylist* pList=FindPlaylistById(m_currentPlaylistId))
...
//---------------------------------------
void CPlaylistUpr::AddVariantsFromPath(const char* pPath)
{
	DrxStringLocal  indexPath;
...
//---------------------------------------
void CPlaylistUpr::LoadVariantsFile(const char *pPath)
{
	DRX_ASSERT(gEnv);
...
//---------------------------------------
SGameVariant *CPlaylistUpr::FindVariantByName(const char *pName)
{
...
//---------------------------------------
bool CPlaylistUpr::DisableVariant(const char *pName)
{
	bool result = false;
...
//---------------------------------------
void CPlaylistUpr::GetVariantsForGameMode( const char *pGameMode, TVariantsPtrVec &result )
{
...
//---------------------------------------
void CPlaylistUpr::GetTotalPlaylistCounts(uint32 *outUnlocked, uint32 *outTotal)
{
...
//---------------------------------------
void CPlaylistUpr::SetGameModeOption( const char *pOption, const char *pValue )
{
	if (m_activeVariantIndex != m_customVariantIndex)
	{
...
//---------------------------------------
void CPlaylistUpr::GetGameModeOption(const char *pOption, DrxFixedStringT<32> &result)
{
	if (m_activeVariantIndex != m_customVariantIndex)
	{
...
//---------------------------------------
void CPlaylistUpr::GetGameModeProfileOptionName(const uint32 index, DrxFixedStringT<32> &result)
{
...
uint16 CPlaylistUpr::PackCustomVariantOption(uint32 index)
{
	if (index < m_options.size())
	{
		SGameModeOption &option = m_options[index];
...
int CPlaylistUpr::UnpackCustomVariantOptionProfileValues(uint16 value, uint32 index, int* pIntValue, float* pFloatValue, int* pFloatPrecision)
{
	if (index < m_options.size())
	{
		SGameModeOption &option = m_options[index];
...
int CPlaylistUpr::UnpackCustomVariantOption(uint16 value, uint32 index, int* pIntValue, float* pFloatValue)
{
	if (index < m_options.size())
	{
		SGameModeOption &option = m_options[index];
...
void CPlaylistUpr::ReadDetailedServerInfo(uint16 *pOptions, uint32 numOptions)
{
...
void CPlaylistUpr::WriteSetCustomVariantOptions(CDrxLobbyPacket* pPacket, CPlaylistUpr::TOptionsVec pOptions, uint32 numOptions)
{
	for (uint32 i = 0; i < numOptions; ++ i)
	{
...
void CPlaylistUpr::WriteSetVariantPacket( CDrxLobbyPacket *pPacket )
{
	...
	if (variantNum > -1)
	{
		...
		if (!bIsCustomVariant)
		{
			const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size + GetSynchedVarsSize();
		...
		else
		{
			...
			const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size + (DrxLobbyPacketUINT16Size * numOptions) + GetSynchedVarsSize();
			if (pPacket->CreateWriteBuffer(bufferSize))
			{
				pPacket->StartWrite(eGUPD_SetGameVariant, true);
				pPacket->WriteUINT8((uint8) variantNum);
...
//---------------------------------------
int CPlaylistUpr::GetSynchedVarsSize()
{
	int size = 0;
...
//---------------------------------------
void CPlaylistUpr::ReadSetCustomVariantOptions(CDrxLobbyPacket* pPacket, CPlaylistUpr::TOptionsVec pOptions, uint32 numOptions)
{
	m_bIsSettingOptions = true;
	for (uint32 i = 0; i < numOptions; ++ i)
	{
...
void CPlaylistUpr::ReadSetVariantPacket( CDrxLobbyPacket *pPacket )
{
	...
	DRX_ASSERT((variantNum >= 0) && (variantNum < (int)m_variants.size()));
...
//---------------------------------------
bool CPlaylistUpr::AdvanceRotationUntil(const int newNextIdx)
{
	bool  ok = false;
...
//---------------------------------------
void CPlaylistUpr::DbgPlaylistsChoose(const int chooseIdx)
{
	const int  n = m_playlists.size();
	if ((chooseIdx >= 0) && (chooseIdx < n))
	{
...
//---------------------------------------
void CPlaylistUpr::DbgPlaylistsSelectVariant(int selectIdx)
{
	if (HavePlaylistSet())
	{
		if (SPlaylist* pList=FindPlaylistById(m_currentPlaylistId))
...
//---------------------------------------
// static
bool CPlaylistUpr::DoStartPlaylistCommand( const char *pPlaylistArg )
{
	DRX_ASSERT(gEnv);
		...
		if (gl_PlaylistVariantAutoComplete.GetPlaylistAndVariant(i, &pPlaylist, &pVariant))
		{
			...
			if (!stricmp(pPlaylistArg, pOptionName))
			{
				...
				if (pPlaylistUpr)
				{
					...
					int numOptions = pPlaylistUpr->m_options.size();
					int numConfigOptions = pPlaylistUpr->m_configVars.size();
...
// [static]
template <class T>
void CPlaylistUpr::GetLocalisationSpecificAttr(const XmlNodeRef& rNode, const char* pAttrKey, const char* pLang, T* pOutVal)
{
	DRX_ASSERT(rNode!=0);
	DRX_ASSERT(pAttrKey);
	DRX_ASSERT(pOutVal);
...
//---------------------------------------
ILevelRotation::TExtInfoId CPlaylistUpr::CreateCustomPlaylist(const char *pName)
{
...
//---------------------------------------
void CPlaylistUpr::ReadServerInfo(XmlNodeRef xmlNode, int &outVariantIndex, int &outMaxPlayers)
{
