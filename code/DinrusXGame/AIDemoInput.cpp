	...
	if (m_timeBeforeChangeMode <= 0.f)
	{
		...
		if (shouldMove == true)
		{
			if (m_pPlayer->IsClient() && (cry_random(0, 3) != 0))
			{
				...
				if (binWeapon)
				{
					int whichMode = cry_random(0, (int)(DRX_ARRAY_COUNT(s_visionModes)) - 1);
...
void CDedicatedInput::GiveItems()
{
	...
	const int numberItems = (DRX_ARRAY_COUNT(itemNames));
	...
	const int numberExplosiveItems = (DRX_ARRAY_COUNT(explosiveItemNames));
