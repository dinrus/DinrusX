...
// -------------------------------------------------------------------------
//  File name:   GameCodeCoverageUpr.cpp
...
#include <DinrusX/DinrusXGame/GameCodeCoverage/IGameCodeCoverageListener.h>
...
{
	virtual int GetCount() const { return CGameCodeCoverageUpr::GetInstance()->GetTotalCustomListsReadFromFile(); }
	virtual const char* GetValue( int nIndex ) const { return CGameCodeCoverageUpr::GetInstance()->GetCustomListName(nIndex); }
} s_gameCodeCoverageListAutoComplete;

//---------------------------------------------------------------------------------------
CGameCodeCoverageUpr::CGameCodeCoverageUpr(const char * filename):
...
bool CGameCodeCoverageUpr::ReadCodeCoverageContext( IItemParamsNode *paramNode )
{
	...
	if (bOk)
	{
		...
		if (bOk)
		{
			...
			cry_displayMemInHexAndAscii("[CODECOVERAGE] <str> ", m_singleAllocTextBlock.GetMem(), m_singleAllocTextBlock.GetSizeNeeded(), CDrxLogAlwaysOutputHandler());
			cry_displayMemInHexAndAscii("[CODECOVERAGE] <grp> ", m_pAutoNamedCheckpointGroups_Array, sizeof(CNamedCheckpointGroup) * tempGroupsFound, CDrxLogAlwaysOutputHandler(), sizeof(CNamedCheckpointGroup));
			cry_displayMemInHexAndAscii("[CODECOVERAGE] <lst> ", m_pCustomCheckpointLists_Array, sizeof(SCustomCheckpointList) * numListDefinitions, CDrxLogAlwaysOutputHandler(), sizeof(SCustomCheckpointList));
			cry_displayMemInHexAndAscii("[CODECOVERAGE] <hit> ", m_pExpectedCheckpoints_Array, sizeof(SLabelInfoFromFile) * m_nTotalCheckpointsReadFromFile, CDrxLogAlwaysOutputHandler(), sizeof(SLabelInfoFromFile));
	#endif
		}
	}
...
//---------------------------------------------------------------------------------------
void CGameCodeCoverageUpr::ParseCustomList(const char * name, const char * listString, int numListsParsedSoFar)
{
...
//---------------------------------------------------------------------------------------
void CGameCodeCoverageUpr::AddCheckpointToCustomList(SCustomCheckpointList * checkpointList, int checkpointNum, TGameCodeCoverageCustomListBitfield bit)
{
...
//---------------------------------------------------------------------------------------
void CGameCodeCoverageUpr::GetRemainingCheckpointLabels( std::vector < const char * > &vLabels ) const
{
...
//---------------------------------------------------------------------------------------
SLabelInfoFromFile * CGameCodeCoverageUpr::FindWatchedCheckpointStructByName(const char *szCheckPoint) const
{
	for (int i = 0; i < m_nTotalCheckpointsReadFromFile; ++ i)
	{
...
//---------------------------------------------------------------------------------------
void CGameCodeCoverageUpr::IgnoreAllCheckpointsWhichMatchPattern(bool onOff, const char * argument)
{
...
bool CGameCodeCoverageUpr::GetMatchThisPatternData(const char * patternTxt, SMatchThisPattern * pattOut)
{
...
//---------------------------------------------------------------------------------------
bool CGameCodeCoverageUpr::MatchesPattern(const char * checkpointName, const SMatchThisPattern * patt)
{
...
//---------------------------------------------------------------------------------------
const CNamedCheckpointGroup * CGameCodeCoverageUpr::GetAutoNamedCheckpointGroup(int i) const
{
...
//---------------------------------------------------------------------------------------
void CGameCodeCoverageUpr::AddToRecentlyHitList(const char * which, bool expected)
{
