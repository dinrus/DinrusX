...
#include <DinrusX/DinrusXGame/IActorSystem.h>
...
#include <DinrusX/DinrusXGame/IMovementController.h>
...
#include <DinrusX/DinrusXGame/EquipmentSystemInterface.h>
...
void CEditorGame::InitActionInputEnums( IGameToEditorInterface* pGTE )
{
	...
	if (pActionMapUpr)
	{
		...
		int actionCount = pActionMapUpr->GetActionsCount();
		if (actionCount > 0)
		{
...
void CEditorGame::InitReadabilityEnums( IGameToEditorInterface* pGTE )
{
	...
	if (pCommunicationUpr)
	{
		...
		const uint32 configCount = pCommunicationUpr->GetConfigCount();
		if (configCount > 0)
		{
			...
			for (uint32 configIndex = 0; configIndex < configCount; ++configIndex)
			{
				...
				const uint32 communicationCount = pCommunicationUpr->GetConfigCommunicationCount(commConfigId);
				if (communicationCount > 0)
				{
					...
					{
						const char* szName = pCommunicationUpr->GetConfigCommunicationNameByIndex(commConfigId, communicationIndex);
