	...
	if (m_pRemoteArchive && pDrxPak)
	{
		...
		if (pFile)
		{
			...
			else
			{
				...
				int iResult = m_pRemoteArchive->UpdateFile(PathUtil::GetFile(szFile), pBuffer, iFileSize, IDrxArchive::METHOD_DEFLATE, IDrxArchive::LEVEL_BETTER);
				if (0 != iResult)
				{
...
bool CGameAIRecorder::SendRemoteArchive(const char* szRecordingFile)
{
	...
	if (pPAKFile)
	{
		...
		if (pDestFile)
		{
			...
			while (true)
			{
				const int iReadAmount = pDrxPak->FReadRaw(pBuffer, 1, 512, pPAKFile);
				if (iReadAmount <= 0)
					break;
