...
#include <DinrusX/DinrusXGame/Agent.h>
#include <DrxRenderer/IRenderer.h>
...
#include <DinrusX/DinrusXGame/GameAISystem.h>
...
namespace
{
	...
	protected:
		virtual void OnInitialize(const UpdateContext& context) override
		{
			...
			const uint32 squadCount = squadUpr.GetSquadMemberCount(squadId);
...
bool AISquadUpr::EnterSquadScope( const SquadScopeID& scopeId, const EntityId entityID, const uint32 concurrentUsersAllowed )
{
...
uint32 AISquadUpr::GetCurrentSquadMatesInScope(const SquadId squadId, const SquadScopeID& squadScopeId) const
{
...
void AISquadUpr::SendSquadEvent( const EntityId sourcEntityId, const char* eventName )
{
...
uint32 AISquadUpr::GetSquadMemberCount( const SquadId squadId ) const
{
	SquadAgentMap::const_iterator it = m_registeredAgents.begin();
	SquadAgentMap::const_iterator end = m_registeredAgents.end();
