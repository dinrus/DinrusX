...
#include <DinrusX/CoreX/Lobby/IDrxTCPService.h>
#include <DinrusX/DinrusXAct/ILevelSystem.h>
#include <DinrusX/DinrusXGame/IPlayerProfiles.h>
...
#include <DinrusX/DinrusXSys/ZLib/IZLibCompressor.h>
	...
	const char *inTargetFilename)
{
	...
	if (gEnv->pDrxPak->IsFileExist(inSourceFilename, IDrxPak::eFileLocation_OnDisk))
	{
		...
		const char *sourceFullPath = gEnv->pDrxPak->AdjustFileName(inSourceFilename, sourceFullPathBuf, IDrxPak::FOPEN_HINT_QUIET);
		char targetFullPathBuf[IDrxPak::g_nMaxPath];
		const char *targetFullPath = gEnv->pDrxPak->AdjustFileName(newTargetFilename.c_str(), targetFullPathBuf, IDrxPak::FOPEN_HINT_QUIET);
...
bool CTelemetryCollector::UploadFileForPreviousSession(const char *inFileName, const char *inRemoteFileName)
{
	...
	if (gEnv->pDrxPak->IsFileExist(inFileName, IDrxPak::eFileLocation_OnDisk))
	{
		...
		int fileSize = gEnv->pDrxPak->FGetSize(inFileName);
	...
	if (gEnv->pDrxPak->IsFileExist(inFileName, IDrxPak::eFileLocation_OnDisk))
	{
		...
		if (gEnv->pDrxPak->IsFileExist(k_hintFileName, IDrxPak::eFileLocation_OnDisk))
		{
			...
			if (file)
			{
				...
				int readSize = gEnv->pDrxPak->FRead(hintFileData, sizeof(hintFileData), file);
				gEnv->pDrxPak->FClose(file);
...
// static
bool CTelemetryCollector::SubmitFileTelemetryCallback(EDrxTCPServiceResult res, void* arg, STCPServiceDataPtr pInUploadData, const char *pData, size_t dataLen, bool endOfStream)
{
	...
	TTelemetrySubmitFlags inFlags)
{
	...
	if (pInProducer)
	{
		...
		if (pLargeSubmitData)
		{
			...
			DRX_ASSERT_MESSAGE(inLen<=int(sizeof(pLargeSubmitData->m_postHeaderContents)),"http post header too long, truncating - message liable to get lost");
...
void CTelemetryCollector::OutputMemoryUsage(const char *message, const char *newLevelName)
{
	...
	if (file)
	{
		int fileSize=gEnv->pDrxPak->FGetSize(file);
...
void CTelemetryCollector::SetAbortedSessionId( const char* abortedSessionName, DrxSessionID& abortedSessionID )
{
#if defined(USE_SESSION_SEARCH_SIMULATOR)
	ICVar* pSearchSimuCVar = gEnv->pConsole->GetCVar( "gl_searchSimulatorEnabled" );
	if( pSearchSimuCVar != NULL && pSearchSimuCVar->GetIVal() )
