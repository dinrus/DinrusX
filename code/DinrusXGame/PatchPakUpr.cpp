	...
	for (unsigned int i=0; i<numPatchPaks; i++)
	{
		...
		DrxLog("CPatchPakUpr::~CPatchPakUpr() pak[%d] url=%s; state=%d; m_downloadableResource=%p", i, patchPakData.m_url.c_str(), patchPakData.m_state, patchPakData.m_downloadableResource.get());
		{
			...
			else if (patchPakData.m_state == SPatchPakData::es_PakLoaded)
			{
				...
				uint32 nFlags = IDrxPak::FLAGS_NEVER_IN_PAK|IDrxPak::FLAGS_PATH_REAL|IDrxArchive::FLAGS_OVERRIDE_PAK;
...
uint32 CPatchPakUpr::GetXOROfPatchPakCRCs()
{
	uint32 result=0;
...
int CPatchPakUpr::GetPatchPakDataIndexFromDownloadableResource(CDownloadableResourcePtr inResource)
{
	int retIndex=-1;
...
void CPatchPakUpr::StartNewDownload(const char *inServerName, const int inPort, const char *inURLPrefix, const char *inURL, const char *inPakBindRoot, const int inDownloadSize, const char *inMD5, const bool inMD5FileName, const char *inDescName)
{
	DRX_ASSERT(m_enabled);
...
bool CPatchPakUpr::CheckForNewDownload(const char *inServerName, const int inPort, const char *inURLPrefix, const char *inURL, const int inDownloadSize, const char *inMD5, const char *inDescName)
{		
...
void CPatchPakUpr::GeneratePakFileNameFromURLName(DrxFixedStringT<64> &outPakFileName, const char *inUrlName)
{
	const char *gameFolder = gEnv->pDrxPak->GetGameFolder();
