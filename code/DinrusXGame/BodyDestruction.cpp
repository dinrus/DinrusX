...
void CBodyDestructibilityProfile::PreparePartsAndEventsBeforeLoad( const XmlNodeRef& rootNode, SParsingHelper& parsingHelper )
{
	...
	for (TPartNames::const_iterator cit = attachmentList.begin(); cit != attachmentList.end(); ++cit)
	{
		...
		parsingHelper.attachmentIndices.insert(TNameHashToIdxMap::value_type(DrxStringUtils::HashString(bodyPart.name.c_str()), (int)(m_attachments.size() - 1)));
	}
	...
	for (TPartNames::const_iterator cit = boneList.begin(); cit != boneList.end(); ++cit)
	{
		...
		parsingHelper.boneIndices.insert(TNameHashToIdxMap::value_type(DrxStringUtils::HashString(bodyPart.name.c_str()), (int)(m_bones.size() - 1)));
	}
}
...
void CBodyDestructibilityProfile::PrepareInstance( CBodyDestrutibilityInstance& instance, float instanceBaseHealth, const SBodyCharacterInfo& bodyInfo )
{
	...
	instance.ReserveSlots((int)bodyInfo.pAttachmentUpr->GetAttachmentCount(), (int)m_attachments.size(), (int)m_bones.size(), (int)m_destructionEvents.size());
...
{
	DRX_ASSERT((destructionEventId >= 0) && (destructionEventId < (int)m_destructionEvents.size()));
...
void CBodyDestructibilityProfile::ProcessDestructionEvent( IEntity& characterEntity, CBodyDestrutibilityInstance& instance, const HitInfo& hitInfo, const TDestructionEventId destructionEventId, IAttachmentUpr* pAttachmentUpr, const char* targetEffectBone,  const Vec3& noAttachEffectPos /*= ZERO*/ )
{
	DRX_ASSERT(pAttachmentUpr);
	...
	while(attachmentToHideCit != attachmentToHideEnd)
	{
		...
		DRX_ASSERT((attachmentId >= 0) && (attachmentId < (int)m_attachments.size()));
	...
	while (eventToStopCit != eventToStopEnd)
	{
		...
		DRX_ASSERT((eventId >= 0) && (eventId < (int)m_destructionEvents.size()));
...
void CBodyDestructibilityProfile::HideAttachment( IAttachmentUpr* pAttachmentUpr, const char* attachmentName, uint32 hide )
{
