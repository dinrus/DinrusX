...
#include <DinrusX/DinrusXGame/Player.h> // for CPlayer, IMovementController, SActorFrameMovementParams, ...
#include <DinrusX/DinrusXGame/MovementTransitionsUtil.h>
#include <DinrusX/DinrusXGame/MovementTransitionsController.h>
#include <DinrusX/DinrusXGame/MovementTransitions.h>

#include <DinrusX/DinrusXAI/IAIActor.h> // for IAIObject, IAIActor, IAIPathAgent

#include <DinrusX/CoreX/Game/GameUtils.h> // for AngleDifference
#include <DinrusX/CoreX/TypeInfo_impl.h> // for DRX_ARRAY_COUNT
...
namespace MovementTransitionsUtil
{
	...
	const int NUM_PSEUDOSPEEDS = DRX_ARRAY_COUNT(s_pseudoSpeeds);
	...
	const int NUM_TRANSITIONTYPES = DRX_ARRAY_COUNT(s_transitionTypes);
}
	...
	const char**const pBodyTargetType ) const
{
...
#ifndef _RELEASE
	{
		...
		{
			DRX_ASSERT(index < (int)m_transitions.size());
