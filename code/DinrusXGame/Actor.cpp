...
#include <DinrusX/DinrusXAct/IInteractor.h>
...
#include <DinrusX/DinrusXGame/Utility/StringUtils.h>
...
#include <DinrusX/DinrusXGame/BodyUprCVars.h>
...
#include <DinrusX/DinrusXGame/EntityUtility/EntityScriptCalls.h>
...
#include <DinrusX/DinrusXGame/AnimActionBlendFromRagdoll.h>
...
//------------------------------------------------------------------------
IAttachment* CActor::GetOrCreateAttachment(IAttachmentUpr *pAttachmentUpr, const char *boneName, const char *attachmentName)
{
...
bool CActor::UpdateStance()
{
	if (m_stance != m_desiredStance)
	{
		...
		if (!TrySetStance(m_desiredStance))
		{
			...
			if (m_stance == STANCE_NULL && GetEntity()->GetPhysics() && IsPlayer())
			{
				...
				switch (m_tryToChangeStanceCounter)
				{
					...
					default:
					++ m_tryToChangeStanceCounter;
					DrxLog("%s '%s' (%g %g %g) can't change stance from %d '%s' to %d '%s' [Attempt %u]", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), pos.x, pos.y, pos.z, (int) m_stance, GetStanceName(m_stance), (int) m_desiredStance, GetStanceName(m_desiredStance), m_tryToChangeStanceCounter);
					break;
				}
			}
#endif
		...
		{
			const Vec3 pos = GetEntity()->GetWorldPos();
			DrxLog("%s '%s' (%g %g %g) has finally changed stance from %d '%s' to %d '%s'", GetEntity()->GetClass()->GetName(), GetEntity()->GetName(), pos.x, pos.y, pos.z, (int) oldStance, GetStanceName(oldStance), (int) newStance, GetStanceName(m_desiredStance));
			m_tryToChangeStanceCounter = 0;
		}
#endif
...
void CActor::CloakSyncAttachments(bool bFade)
{
	...
	if (ICharacterInstance *pOwnerCharacter = pEntity->GetCharacter(0))
	{
		...
		{
			const uint32 attachmentCount = pAttachmentUpr->GetAttachmentCount();
