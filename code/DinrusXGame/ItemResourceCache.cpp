...
bool CItemAnimationDBAUpr::IsValidDBAPath( const char* dbaPath ) const
{
	return (dbaPath && dbaPath[0]);
}

int CItemAnimationDBAUpr::IsDbaAlreadyInUse( const ItemString& animationGroup ) const
{
...
int CItemAnimationDBAUpr::IsDbaAlreadyInPreloadList( const ItemString& animationGroup ) const
{
...
CItemAnimationDBAUpr::SItemDBAInfo& CItemAnimationDBAUpr::GetDbaInUse( const int index )
{
	DRX_ASSERT((index >= 0) && (index < (int)m_inUseDBAs.size()));

	return m_inUseDBAs[index].second;
}

CItemAnimationDBAUpr::SItemDBAInfo& CItemAnimationDBAUpr::GetDbaInPreloadList( const int index )
{
	DRX_ASSERT((index >= 0) && (index < (int)m_preloadedDBASlots.size()));
