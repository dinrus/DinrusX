...
#include <DinrusX/DinrusXGame/LedgeUpr.h>
...
void CLedgeUprEdit::RegisterLedge( EntityId entityId, const SLedgeMarker* pMarkersArray, const uint32 markerCount, ELedgeFlagBitfield ledgeInFlags, ELedgeFlagBitfield ledgeOutFlags, float ledgeCornerMaxAngle, float ledgeCornerEndAdjustAmount )
{
	if (markerCount == 0)
		return;
...
void CLedgeUprEdit::UpdateLedgeMarkers( const EntityId entityId, const SLedgeMarker* pMarkersArray, const uint32 markerCount )
{
	TLedgeObjectsEditorContainer::iterator ledgeIt = std::find(m_ledgeObjects.begin(), m_ledgeObjects.end(), entityId);
...
void CLedgeUprEdit::Export( const char* fileName ) const
{
...
void CLedgeUpr::Load( const char* fileName )
{
...
void CLedgeUpr::UpdateLedgeMarkers( const EntityId entityId, const SLedgeMarker* pMarkersArray, const uint32 markerCount )
{
	if (m_editorUpr.IsInEditorMode())
	{
