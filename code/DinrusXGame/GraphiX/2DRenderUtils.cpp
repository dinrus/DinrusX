...
#include <DinrusX/DinrusXGame/2DRenderUtils.h>
...
void C2DRenderUtils::RenderTest_Grid( float fTime, const ColorF& color )
{
	...
	const int ylines = (int)(m_pLayoutUpr->GetVirtualHeight()/rowheight);
	float fY = 0;
	...
	const int xlines = (int)(m_pLayoutUpr->GetVirtualWidth()/colwidth);
	float fX = 0;
...
void C2DRenderUtils::RenderTest_Tris( float fTime, const ColorF& color )
{
	...
	const int xtris = (int)(m_pLayoutUpr->GetVirtualWidth()/tri_width);
	const int ytris = (int)(m_pLayoutUpr->GetVirtualHeight()/tri_height);

	float fX = 0;
	float fY = 0;
	for( int x=0; x<xtris; x++ ) //float fX=0; fX<=m_pLayoutUpr->GetVirtualWidth()-tri_width; fX+=tri_width )
	{
		for( int y=0; y<ytris; y++ ) //float fY=0; fY<=m_pLayoutUpr->GetVirtualHeight()-tri_height; fY+=tri_height )
		{
			float t1x = fX;
			float t1y = fY;
...
void C2DRenderUtils::RenderTest_Quads( float fTime, const ColorF& color )
{
	...
	const int xquads = (int)(m_pLayoutUpr->GetVirtualWidth()/quad_width);
	const int yquads = (int)(m_pLayoutUpr->GetVirtualHeight()/quad_height);

	float fX = 0;
	float fY = 0;
	for( int ix=0; ix<xquads; ix++ ) //float fX=0; fX<=m_pLayoutUpr->GetVirtualWidth()-quad_width; fX+=quad_width )
	{
		for( int iy=0; iy<yquads; iy++ ) //float fY=0; fY<=m_pLayoutUpr->GetVirtualHeight()-quad_height; fY+=quad_height )
		{
			float x = fX;
			float y = fY;
			float sx = quad_width;
