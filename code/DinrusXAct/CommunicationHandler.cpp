...
#include <DinrusX/DinrusXAct/CommunicationHandler.h>
...
SCommunicationSound CommunicationHandler::PlaySound(CommPlayID playID, const char* name, ESoundType type, IEventListener* listener)
{
	...
	if (pEntity)
	{
		...
		if (pIEntityAudioComponent)
		{
			...
			DrxAudio::SRequestUserData const userData(DrxAudio::ERequestFlags::DoneCallbackOnExternalThread, this, reinterpret_cast<void*>(static_cast<UINT_PTR>(m_entityId)), this);
