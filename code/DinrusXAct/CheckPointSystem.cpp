...
bool CCheckpointSystem::SaveExternalEntity(EntityId id)
{
	...
	if (!CHECKPOINT_SAVE_XML_NODE)
	{
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "Tried writing external entity %i while savegame was not open.", (int)id);
		return false;
	}
...
void CCheckpointSystem::WriteMetaData(EntityId checkpointId, XmlNodeRef parentNode, SCheckpointData& outMetaData)
{
	...
	const char* levelName = CDrxAction::GetDrxAction()->GetLevelName();
		...
		for (int i = 0; i < actorFlags; ++i)
		{
			...
			if (foundEntity)
			{
				...
				if (pActor)
				{
					...
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "Failed finding actor %i from checkpoint.", (int)id);
			}
			else
