...
#include <DinrusX/DinrusXAI/IAIActor.h>
#include <DinrusX/DinrusXAct/AnimActionTriState.h>
		...
		if (bFound)
		{
			...
			if (CDrxActionCVars::Get().aiLogSignals)
			{
				...
				if (start.GetMilliSeconds() > CDrxActionCVars::Get().aiMaxSignalDuration)
				{
					...
					char buffer[256];
					drx_sprintf(buffer, "%s.%s,%g,%d,%s", behaviour, funcName, start.GetMilliSeconds(), (int)pagefaults, m_pEntity->GetName());
...
bool CAIHandler::GetMostLikelyTable(IScriptTable* table, SmartScriptTable& destTable)
{
	...
	int probability = drx_random(0, maxProb - 1);
