...
#include <DinrusX/DinrusXAct/ICooperativeAnimationUpr.h>
...
{
	const char* pActualGameModeName = CDrxAction::GetDrxAction()->GetIGameRulesSystem()->GetGameRulesName(gameMode);
...
void CLevelRotation::ShallowShuffle()
{
	...
	else
	{
		for (int i = 0; i < nEntries; ++i)
		{
			int idx = drx_random(0, nEntries - 1);
			Log_LevelRotation("Called drx_random()");
