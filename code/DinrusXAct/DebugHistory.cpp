...
//--------------------------------------------------------------------------------
IDebugHistory* CDebugHistoryUpr::CreateHistory(const char* id, const char* name)
{
	if (!name)
		name = id;
...
void CDebugHistoryUpr::RemoveHistory(const char* id)
{
	MapIterator it = m_histories.find(CONST_TEMP_STRING(id));
...
IDebugHistory* CDebugHistoryUpr::GetHistory(const char* id)
{
	MapIterator it = m_histories.find(CONST_TEMP_STRING(id));
	CDebugHistory* history = (it != m_histories.end()) ? (*it).second : NULL;
	return history;
...
void CDebugHistoryUpr::LayoutHelper(const char* id, const char* name, bool visible, float minout, float maxout, float minin, float maxin, float x, float y, float w /*=1.0f*/, float h /*=1.0f*/)
{
	if (!visible)
	{
