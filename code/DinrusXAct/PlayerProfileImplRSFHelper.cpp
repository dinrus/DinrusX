...
#include <DinrusX/DinrusXAct/RichSaveGameTypes.h>
...
{
	size_t maxCount = (size_t) std::min( (int) DRX_ARRAY_COUNT(t) - 1, (int) str.length());
...
bool WriteXMLNode(const uint32 tag, const XmlNodeRef& node, FILE* pFile, bool bCompress, const char* debugFilename = "")
{
	...
	if (node != 0)
	{
		...
		if (debugFilename && *debugFilename)
		{
			...
			if (pDebugFile)
			{
				pDrxPak->FWrite((void*)pXmlStrData->GetString(), pXmlStrData->GetStringLength(), 1, pDebugFile);
		...
		if (bCompress == false)
		{
			...
			if (xmlDataLength > 0)
			{
				pDrxPak->FWrite((const void*)pXmlStrData->GetString(), xmlDataLength, 1, pFile);
			}
		}
		else
		{
			char* compressedBuf = static_cast<char*>(pDrxPak->PoolMalloc(xmlDataLength));
			...
			pDrxPak->FWrite((void*) compressedBuf, compressedLength, 1, pFile);
			pDrxPak->PoolFree(compressedBuf);
		}
	}
	else
...
bool ReadRichGameMetaData(const string& filename, CPlayerProfileUpr::SSaveGameMetaData& metaData)
{
	...
	{
		pDrxPak->FSeek(pFile, (int)thumbNailOffset, SEEK_CUR);
	}
	if (thumbNailSize > 0)
	{
...
bool CRichSaveGameHelper::GetSaveGames(CPlayerProfileUpr::SUserEntry* pEntry, CPlayerProfileUpr::TSaveGameInfoVec& outVec, const char* altProfileName)
{
...
bool CRichSaveGameHelper::DeleteSaveGame(CPlayerProfileUpr::SUserEntry* pEntry, const char* name)
{
	string filename;
...
bool CRichSaveGameHelper::MoveSaveGames(CPlayerProfileUpr::SUserEntry* pEntry, const char* oldProfileName, const char* newProfileName)
{
...
bool CRichSaveGameHelper::GetSaveGameThumbnail(CPlayerProfileUpr::SUserEntry* pEntry, const char* saveGameName, CPlayerProfileUpr::SThumbnail& thumbnail)
{
