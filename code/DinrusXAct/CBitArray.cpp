#include <DinrusX/DinrusXAct/SerializeBits.h>
...
void CBitArray::PushBit(int bit)
{
#if !defined(_RELEASE)
...
int CBitArray::NumberOfBitsPushed()
{
	return m_bytePos * 8 + m_bitPos + 1;
}

int CBitArray::PopBit()  /* from the front */
{
	m_bitPos++;
	if (m_bitPos == 8)
	{
...
void CBitArray::ReadBits(unsigned char* out, int numBits)
{
	unsigned char byte;
	while (numBits >= 8)
	{
...
void CBitArray::WriteBits(const unsigned char* in, int numBits)
{
	unsigned char v;
	while (numBits >= 8)
	{
