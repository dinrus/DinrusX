...
#include <DinrusX/DinrusXAct/ActionController.h>
#include <DinrusX/DinrusXAct/ActionScope.h>

#include <DinrusX/DinrusXAct/AnimationDatabase.h>

#include <DinrusX/CoreX/Extension/DrxCreateClassInstance.h>

#include <DinrusX/DinrusXAct/MannequinUtils.h>
#include <DinrusX/DinrusXAct/MannequinDebug.h>
...
bool CActionController::UpdateScopeContextValidity(uint32 scopeContextID)
{
	...
	if (hasInvalidEntity)
	{
		...
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[CActionController::UpdateScopeContextValidity] Dangling Entity %p (expected %p for id=%u) in context '%s'", (void*)scopeContext.pCachedEntity, (void*)expectedEntity, scopeContext.entityId, m_context.controllerDef.m_scopeContexts.GetTagName(scopeContextID));
...
bool CActionController::UpdateRootEntityValidity()
{
	...
	{
#if DRXMANNEQUIN_WARN_ABOUT_VALIDITY()
		DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_ERROR, "[CActionController::UpdateRootEntityValidity] Dangling Entity %p (expected %p for id=%u) in actioncontroller for '%s'", (void*)m_cachedEntity, (void*)expectedEntity, m_entityId, m_context.controllerDef.m_filename.c_str());
