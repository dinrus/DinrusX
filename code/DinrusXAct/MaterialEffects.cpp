...
#include <DinrusX/DinrusXAct/MaterialEffectsDebug.h>
...
void CMaterialEffects::LoadSpreadSheet()
{
	...
	for (;; )
	{
		...
		for (;; )
		{
			...
			{
				const int matId = gEnv->p3DEngine->GetMaterialUpr()->GetSurfaceTypeUpr()->GetSurfaceTypeByName(cellString.c_str(), "MFX", true)->GetId();
			...
			else if (rowCount > maxCol && colIndex == 0)
			{
				...
				//DrxLog("[MFX] Object class ID: %d", (int)pEntityClass);
				if (pEntityClass)
				{
