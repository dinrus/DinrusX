...
#include <DinrusX/DinrusXAct/PlayerProfileImplRSFHelper.h>
...
#include <DinrusX/DinrusXAct/BMPHelper.h>
...
//------------------------------------------------------------------------
bool CPlayerProfileImplFS::GetSaveGames(SUserEntry* pEntry, CPlayerProfileUpr::TSaveGameInfoVec& outVec, const char* altProfileName)
{
	if (CPlayerProfileUpr::sUseRichSaveGames)
	{
		CRichSaveGameHelper sgHelper(this);
...
//------------------------------------------------------------------------
bool CPlayerProfileImplFSDir::GetSaveGames(SUserEntry* pEntry, CPlayerProfileUpr::TSaveGameInfoVec& outVec, const char* altProfileName)
{
	if (CPlayerProfileUpr::sUseRichSaveGames)
	{
		CRichSaveGameHelper sgHelper(this);
...
bool CCommonSaveGameHelper::GetSaveGameThumbnail(CPlayerProfileUpr::SUserEntry* pEntry, const char* saveGameName, CPlayerProfileUpr::SThumbnail& thumbnail)
{
...
bool CCommonSaveGameHelper::GetSaveGames(CPlayerProfileUpr::SUserEntry* pEntry, CPlayerProfileUpr::TSaveGameInfoVec& outVec, const char* altProfileName = "")
{
...
bool CCommonSaveGameHelper::DeleteSaveGame(CPlayerProfileUpr::SUserEntry* pEntry, const char* name)
{
	string filename;
	m_pImpl->InternalMakeFSSaveGamePath(pEntry, pEntry->pCurrentProfile->GetName(), filename, true);
	string strippedName = PathUtil::GetFile(name);
...
bool CCommonSaveGameHelper::MoveSaveGames(CPlayerProfileUpr::SUserEntry* pEntry, const char* oldProfileName, const char* newProfileName)
{
...
ILevelRotationFile* CCommonSaveGameHelper::GetLevelRotationFile(CPlayerProfileUpr::SUserEntry* pEntry, const char* name)
{
	string rootFolder;
...
XmlNodeRef CSerializerXML::CreateNewSection(CPlayerProfileUpr::EPlayerProfileSection section, const char* name)
{
	int sectionIdx = (int)section;
	assert(sectionIdx >= 0 && sectionIdx < (int)CPlayerProfileUpr::ePPS_Num);
