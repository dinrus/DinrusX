...
#include <DinrusX/DinrusXAct/PlayerProfile.h>
...
const char* CPlayerProfileUpr::FACTORY_DEFAULT_NAME = "default";
const char* CPlayerProfileUpr::PLAYER_DEFAULT_PROFILE_NAME = "default";

namespace TESTING
{
void DumpProfiles(IPlayerProfileUpr* pFS, const char* userId)
{
...
int CPlayerProfileUpr::sUseRichSaveGames = 0;
int CPlayerProfileUpr::sRSFDebugWrite = 0;
int CPlayerProfileUpr::sRSFDebugWriteOnLoad = 0;
int CPlayerProfileUpr::sLoadOnlineAttributes = 1;
...
//------------------------------------------------------------------------
int CPlayerProfileUpr::GetUserCount()
{
	return m_userVec.size();
}

//------------------------------------------------------------------------
bool CPlayerProfileUpr::GetUserInfo(int index, IPlayerProfileUpr::SUserInfo& outInfo)
{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::LoginUser(const char* userId, bool& bOutFirstTime)
{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::LogoutUser(const char* userId)
{
	if (!m_bInitialized)
		return false;
...
//------------------------------------------------------------------------
int CPlayerProfileUpr::GetProfileCount(const char* userId)
{
	SUserEntry* pEntry = FindEntry(userId);
	if (pEntry == 0)
		return 0;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::GetProfileInfo(const char* userId, int index, IPlayerProfileUpr::SProfileDescription& outInfo)
{
	if (!m_bInitialized)
		return false;
...
//------------------------------------------------------------------------
void CPlayerProfileUpr::SetProfileLastLoginTime(const char* userId, int index, time_t lastLoginTime)
{
	if (!m_bInitialized)
		return;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::CreateProfile(const char* userId, const char* profileName, bool bOverride, IPlayerProfileUpr::EProfileOperationResult& result)
{
	if (!m_bInitialized)
	{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::DeleteProfile(const char* userId, const char* profileName, IPlayerProfileUpr::EProfileOperationResult& result)
{
	if (!m_bInitialized)
	{
		result = ePOR_NotInitialized;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::RenameProfile(const char* userId, const char* newName, IPlayerProfileUpr::EProfileOperationResult& result)
{
	if (!m_bInitialized)
	{
		result = ePOR_NotInitialized;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::SaveProfile(const char* userId, IPlayerProfileUpr::EProfileOperationResult& result, unsigned int reason)
{
	if (!m_bInitialized)
	{
		result = ePOR_NotInitialized;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::SaveInactiveProfile(const char* userId, const char* profileName, EProfileOperationResult& result, unsigned int reason)
{
	if (!m_bInitialized)
	{
		result = ePOR_NotInitialized;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::LoadProfile(SUserEntry* pEntry, CPlayerProfile* pProfile, const char* name, bool bPreview /*false*/)
{
	if (m_loadingProfile || m_savingProfile)
	{
		return false;
...
//------------------------------------------------------------------------
const IPlayerProfile* CPlayerProfileUpr::PreviewProfile(const char* userId, const char* profileName)
{
	SUserEntry* pEntry = FindEntry(userId);
...
//------------------------------------------------------------------------
IPlayerProfile* CPlayerProfileUpr::ActivateProfile(const char* userId, const char* profileName)
{
	SUserEntry* pEntry = FindEntry(userId);
	if (pEntry == 0) // no such user
	{
...
//------------------------------------------------------------------------
IPlayerProfile* CPlayerProfileUpr::GetCurrentProfile(const char* userId)
{
	SUserEntry* pEntry = FindEntry(userId);
...
//------------------------------------------------------------------------
const char* CPlayerProfileUpr::GetCurrentUser()
{
	return m_curUserID.c_str();
}

//------------------------------------------------------------------------
int CPlayerProfileUpr::GetCurrentUserIndex()
{
	return m_curUserIndex;
}

void CPlayerProfileUpr::SetExclusiveControllerDeviceIndex(unsigned int exclusiveDeviceIndex)
{
	m_exclusiveControllerDeviceIndex = exclusiveDeviceIndex;
}

unsigned int CPlayerProfileUpr::GetExclusiveControllerDeviceIndex() const
{
	return m_exclusiveControllerDeviceIndex;
}

//------------------------------------------------------------------------
bool CPlayerProfileUpr::ResetProfile(const char* userId)
{
	SUserEntry* pEntry = FindEntry(userId);
	if (pEntry == 0) // no such user
	{
...
CPlayerProfileUpr::FindEntry(const char* userId) const
{
...
//------------------------------------------------------------------------
CPlayerProfileUpr::SUserEntry*
CPlayerProfileUpr::FindEntry(const char* userId, CPlayerProfileUpr::TUserVec::iterator& outIter)
{
...
CPlayerProfileUpr::GetLocalProfileInfo(SUserEntry* pEntry, const char* profileName) const
{
...
//------------------------------------------------------------------------
CPlayerProfileUpr::SLocalProfileInfo*
CPlayerProfileUpr::GetLocalProfileInfo(SUserEntry* pEntry, const char* profileName, std::vector<SLocalProfileInfo>::iterator& outIter) const
{
	std::vector<SLocalProfileInfo>::iterator iter = pEntry->profileDesc.begin();
	while (iter != pEntry->profileDesc.end())
	{
...
//------------------------------------------------------------------------
ISaveGameEnumeratorPtr CPlayerProfileUpr::CreateSaveGameEnumerator(const char* userId, CPlayerProfile* pProfile)
{
	if (!m_bInitialized)
		return 0;
...
//------------------------------------------------------------------------
ISaveGame* CPlayerProfileUpr::CreateSaveGame(const char* userId, CPlayerProfile* pProfile)
{
	if (!m_bInitialized)
		return 0;
...
//------------------------------------------------------------------------
ILoadGame* CPlayerProfileUpr::CreateLoadGame(const char* userId, CPlayerProfile* pProfile)
{
	if (!m_bInitialized)
		return 0;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::DeleteSaveGame(const char* userId, CPlayerProfile* pProfile, const char* name)
{
	if (!m_bInitialized)
		return false;
...
//------------------------------------------------------------------------
ILevelRotationFile* CPlayerProfileUpr::GetLevelRotationFile(const char* userId, CPlayerProfile* pProfile, const char* name)
{
	if (!m_bInitialized)
		return 0;
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::ResolveAttributeBlock(const char* userId, const char* attrBlockName, IResolveAttributeBlockListener* pListener, int reason)
{
	SUserEntry* pEntry = FindEntry(userId);
	if (pEntry == 0) // no such user
	{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::ResolveAttributeBlock(const char* userId, const SResolveAttributeRequest& attrBlockNameRequest, IResolveAttributeBlockListener* pListener, int reason)
{
	SUserEntry* pEntry = FindEntry(userId);
	if (pEntry == 0) // no such user
	{
...
//------------------------------------------------------------------------
void CPlayerProfileUpr::SetSharedSaveGameFolder(const char* sharedSaveGameFolder)
{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::IsOnlineOnlyAttribute(const char* name)
{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::RegisterOnlineAttribute(const char* name, const char* type, const bool onlineOnly, const SDrxLobbyUserData& defaultValue, CCrc32& crc)
{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::SetUserDataType(SDrxLobbyUserData* data, const char* type)
{
...
//------------------------------------------------------------------------
void CPlayerProfileUpr::GetDefaultValue(const char* type, XmlNodeRef attributeNode, SDrxLobbyUserData* pOutData)
{
	if (strcmpi(type, "int") == 0)
	{
...
void CPlayerProfileUpr::ReloadProfile(IPlayerProfile* pProfile, unsigned int reason)
{
...
void CPlayerProfileUpr::SetOnlineAttributes(IPlayerProfile* pProfile, const SDrxLobbyUserData* pData, const int32 onlineDataCount)
{
	DRX_ASSERT(pProfile);
...
//------------------------------------------------------------------------
uint32 CPlayerProfileUpr::GetOnlineAttributes(SDrxLobbyUserData* pData, uint32 numData)
{
	uint32 numCopied = 0;
...
//------------------------------------------------------------------------
uint32 CPlayerProfileUpr::GetDefaultOnlineAttributes(SDrxLobbyUserData* pData, uint32 numData)
{
	uint32 numCopied = 0;
	...
	case eCLUDT_Int32:
		{
			...
			if (pInt)
			{
				...
				{
					DRX_ASSERT_MESSAGE(false, "TFlowInputData int didn't match UserData types");
					return false;
				}
				return true;
			}
			...
			{
				DRX_ASSERT_MESSAGE(false, "TFlowInputData didn't contain expected int/bool");
				return false;
			}
		}
		break;
...
//------------------------------------------------------------------------
uint32 CPlayerProfileUpr::UserDataSize(const SDrxLobbyUserData* data)
{
	switch (data->m_type)
	{
	case eCLUDT_Int32:
...
//static------------------------------------------------------------------------
void CPlayerProfileUpr::ReadUserDataCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, SDrxLobbyUserData* pData, uint32 numData, void* pArg)
{
...
//-----------------------------------------------------------------------------
void CPlayerProfileUpr::ReadOnlineAttributes(IPlayerProfile* pProfile, const SDrxLobbyUserData* pData, const uint32 numData)
{
...
//------------------------------------------------------------------------
int CPlayerProfileUpr::GetOnlineAttributesVersion() const
{
	return m_onlineAttributeAutoGeneratedVersion + m_onlineAttributeDefinedVersion;
}

//------------------------------------------------------------------------
int CPlayerProfileUpr::GetOnlineAttributeIndexByName(const char* name)
{
	TOnlineAttributeMap::const_iterator end = m_onlineAttributeMap.end();
...
//------------------------------------------------------------------------
void CPlayerProfileUpr::GetOnlineAttributesDataFormat(SDrxLobbyUserData* pData, uint32 numData)
{
...
//------------------------------------------------------------------------
uint32 CPlayerProfileUpr::GetOnlineAttributeCount()
{
	return m_onlineDataCount;
}

//------------------------------------------------------------------------
int CPlayerProfileUpr::ChecksumConvertValueToInt(const SDrxLobbyUserData* pData)
{
	int value = 0;
...
//------------------------------------------------------------------------
void CPlayerProfileUpr::ApplyChecksums(SDrxLobbyUserData* pData, uint32 numData)
{
...
//------------------------------------------------------------------------
bool CPlayerProfileUpr::ValidChecksums(const SDrxLobbyUserData* pData, uint32 numData)
{
	for (int i = 0; i < k_onlineChecksums; i++)
	{
...
//------------------------------------------------------------------------
int CPlayerProfileUpr::Checksum(const int checksum, const SDrxLobbyUserData* pData, uint32 numData)
{
	switch (checksum)
	{
	case 0:
...
//------------------------------------------------------------------------
int CPlayerProfileUpr::ChecksumHash(const int seed, const int value) const
{
	int hash = seed + value;
	hash += (hash << 10);
	hash ^= (hash >> 6);
...
//------------------------------------------------------------------------
int CPlayerProfileUpr::ChecksumHash(const int value0, const int value1, const int value2, const int value3, const int value4) const
{
...
//static------------------------------------------------------------------------
void CPlayerProfileUpr::RegisterUserDataCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg)
{
	CPlayerProfileUpr* pUpr = static_cast<CPlayerProfileUpr*>(pArg);
	DRX_ASSERT(pUpr);
...
//static------------------------------------------------------------------------
void CPlayerProfileUpr::WriteUserDataCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg)
{
	DRX_ASSERT(error == eCLE_Success);
			...
			case 2:
				{
					...
					int returned = memcmp(pPlayerProfMan->m_testData, pPlayerProfMan->m_onlineData, pPlayerProfMan->m_onlineDataCount * sizeof(SDrxLobbyUserData));
...
bool CPlayerProfileUpr::MoveFileHelper(const char* existingFileName, const char* newFileName)
{
