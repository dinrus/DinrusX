...
#include <DinrusX/DinrusXAct/StdAfx.h>
	...
	#include <DinrusX/DinrusXAct/ProceduralBreakingBaseListener.h>
...
void DumpSpawnRecs(const DynArray<SProceduralSpawnRec>& recs, const DynArray<SJointBreakRec>& joints, const char* title)
{
	if (CNetworkCVars::Get().BreakageLog)
	{
		...
		{
	#if BREAK_HIERARCHICAL_TRACKING
			DrxLog("[brk]   .break %d on %d epipart=%d @ frame %d", int(joints[i].id), int(joints[i].idxRef), joints[i].epicenter, int(joints[i].frame));
	#else
			DrxLog("[brk]   .break %d on %d epipart=%d", int(joints[i].id), int(joints[i].idxRef), joints[i].epicenter);
	#endif
		}
	#if BREAK_HIERARCHICAL_TRACKING
		int frameno = 0;
