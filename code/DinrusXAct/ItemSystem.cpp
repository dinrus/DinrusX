...
#include <DinrusX/DinrusXAct/ItemParams.h>
...
void CItemSystem::SetActorItem(IActor* pActor, EntityId itemId, bool keepHistory)
{
	...
	CDrxAction::GetDrxAction()->GetIGameplayRecorder()->Event(pActor->GetEntity(), GameplayEvent(eGE_ItemSelected, 0, 0, (void*)(EXPAND_PTR)itemId));
...
void* CItemSystem::Query(IItemSystemQuery query, const void* param /* = NULL */)
{
	...
	case eISQ_Find_Item_By_Name:
		{
			DrxFixedStringT<32> filterName = param ? (const char*)(param) : "";
			if (filterName.empty())
				return NULL;
