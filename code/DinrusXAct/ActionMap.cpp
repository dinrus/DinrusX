...
{
	DRX_ASSERT(iIndex >= 0 && iIndex < (int)m_actionInputs.size());
...
//------------------------------------------------------------------------
CActionMap::CActionMap(CActionMapUpr* pActionMapUpr, const char* name)
	...
	if (currentMode)
	{
		         ...
		         (!m_pActionMapUpr->IsCurrentlyRefiringInput()))   // Input event is from refire, dont try to refire a refired event
		{
			...
			else // Not repeated key, must wait for delay before firing
			{
				...
				int currentHighestDelayPriority = m_pActionMapUpr->GetHighestPressDelayPriority();
...
{
	const int iNumDeviceData = m_pActionMapUpr->GetNumInputDeviceData();
	bool bResult;
	...
	if (inputBlockData.blockType != eAIBT_None)
	{
		...
		else // Normal blocking inputs
		{
			...
			for (size_t i = 0; i < inputBlockData.inputs.size(); i++)
			{
				...
				if (!szKeyName)
				{
					DrxWarning(VALIDATOR_MODULE_GAME, VALIDATOR_WARNING, "CActionMap::LoadActionInputAttributesFromXML: Failed to translate keyname for key id: %d", (int)inputBlocker.keyId);
					return false;
				}
