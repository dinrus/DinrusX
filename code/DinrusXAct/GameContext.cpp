...
#include <DinrusX/DinrusXAct/VoiceController.h>
...
#include <DinrusX/DinrusXAct/CET_ClassRegistry.h>
#include <DinrusX/DinrusXAct/CET_CVars.h>
#include <DinrusX/DinrusXAct/CET_EntitySystem.h>
#include <DinrusX/DinrusXAct/CET_GameRules.h>
#include <DinrusX/DinrusXAct/CET_LevelLoading.h>
#include <DinrusX/DinrusXAct/CET_NetConfig.h>
#include <DinrusX/DinrusXAct/CET_ClientConnections.h>

#include <DinrusX/DinrusXAct/NetDebug.h>
...
string CGameContext::GetConnectionString(DrxFixedStringT<HOST_MIGRATION_MAX_PLAYER_NAME_SIZE>* pNameOverride, bool fake) const
{
	...
	else
	{
		...
		if (IPlayerProfileUpr* pProfileUpr = CDrxAction::GetDrxAction()->GetIPlayerProfileUpr())
		{
			...
			uint32 userIndex = pProfileUpr->GetExclusiveControllerDeviceIndex();
	...
	drx_sprintf(buf, "%d", (int)time(NULL));
	constr = buf + constr;
	int len = constr.length();
	int r = drx_random(0, 19) + CLAMP(64 - len, 0, 64);
