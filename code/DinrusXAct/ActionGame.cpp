...
#include <DinrusX/DinrusXAct/ServerTimer.h>
#include <DinrusX/DinrusXAct/DeformingBreak.h> // for debug function
...
#include <DinrusX/DinrusXAct/GameStats.h>
#include <DinrusX/DinrusXAct/IKTorsoAim.h>
...
bool CActionGame::Init(const SGameStartParams* pGameStartParams)
{
	...
	else
	{
		...
		{
			const char* pLevelName = pLobby->GetDinrusXLevelNameHint();
			if (strlen(pLevelName) != 0)
			{
				m_pGameContext->SetLevelName(pLevelName);
			}
			const char* pRulesName = pLobby->GetDinrusXRulesNameHint();
...
SBreakEvent& CActionGame::StoreBreakEvent(const SBreakEvent& breakEvent)
{
	...
	CDrxAction::GetDrxAction()->OnBreakEvent(static_cast<uint16>(m_breakEvents.size() - 1));
