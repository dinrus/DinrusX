...
#include <DinrusX/DinrusXAct/AIFaceUpr.h>
...
//
//------------------------------------------------------------------------------
int CAIFaceUpr::SelectExpressionTime() const
{
...
//
//----------------------------------------------------------------------------------------------------------
bool CAIFaceUpr::Load(const char* szPackName)
{
	string sPath = PathUtil::Make("Libs/Readability/Faces", "Faces.xml");
...
//
//------------------------------------------------------------------------------
CAIFaceUpr::e_ExpressionEvent CAIFaceUpr::StringToEnum(const char* szFaceName)
{
...
void CAIFaceUpr::SetExpression(e_ExpressionEvent expression, bool forceChange)
{
	...
	if (!theState.empty())
	{
		m_CurrentExprIdx = drx_random(0, (int)theState.size() - 1);
...
//
//------------------------------------------------------------------------------
void CAIFaceUpr::MakeFace(const char* pFaceName)
{
	IActorSystem* pASystem = gEnv->pGameFramework->GetIActorSystem();
	if (pASystem)
	{
