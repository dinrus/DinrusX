...
bool CDurangoGPUMemoryUpr::Init(size_t size, size_t bankSize, size_t reserveSize, uint32 xgMemType, bool allowAdditionalBanks)
{
	DrxAutoLock<DrxCriticalSectionNonRecursive> lock(m_lock);
...
void* CDurangoGPUMemoryUpr::WeakPin(SGPUMemHdl hdl)
{
	if (!hdl.IsFixed())
	{
...
void* CDurangoGPUMemoryUpr::Pin(SGPUMemHdl hdl)
{
	if (!hdl.IsFixed())
	{
...
uint32 CDurangoGPUMemoryUpr::BeginCopy(void* pContext, UINT_PTR dstOffset, UINT_PTR srcOffset, UINT_PTR size, IDefragAllocatorCopyNotification* pCompleteNotification)
{
...
void CDurangoGPUMemoryUpr::Relocate(uint32 userMoveId, void* pContext, UINT_PTR newOffset, UINT_PTR oldOffset, UINT_PTR size)
{
	// Assume called from CollectGarbage
...
void CDurangoGPUMemoryUpr::CancelCopy(uint32 userMoveId, void* pContext, bool bSync)
{
...
void CDurangoGPUMemoryUpr::SyncCopy(void* pContext, UINT_PTR dstOffset, UINT_PTR srcOffset, UINT_PTR size)
{
	CDeviceTexture* pDevTex = reinterpret_cast<CDeviceTexture*>(pContext);
...
uint32 CDurangoGPUMemoryUpr::Hash(UINT_PTR offset, UINT_PTR size)
{
...
void CDurangoGPUMemoryUpr::Relocate_Int(CDeviceTexture* pDevTex, char* pOldTexBase, char* pTexBase, UINT_PTR size)
{
//	pDevTex->Unbind();
...
char* CDurangoGPUMemoryUpr::GetPhysicalAddress(UINT_PTR offs)
{
