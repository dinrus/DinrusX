...
#include <DinrusX/DinrusXRend/DeviceRenderPass.h>
...
bool CDeviceRenderPassDesc::OnResourceInvalidated(void* pThis, SResourceBindPoint bindPoint, UResourceReference pResource, uint32 flags) threadsafe
{
	...
	if (flags & eResourceDestroyed)
	{
		...
		{
			pFoundResource = DrxInterlockedCompareExchangePointer((void* volatile*)&pPass->m_outputUAVs[bindPoint.slotNumber], nullptr, pResource.pAnonymous);
		}
		else if (bindPoint.slotType == SResourceBindPoint::ESlotType::TextureAndBuffer)
		{
			if ((int8)bindPoint.slotNumber >= 0)
				pFoundResource = DrxInterlockedCompareExchangePointer((void* volatile*)&pPass->m_renderTargets[bindPoint.slotNumber], nullptr, pResource.pAnonymous);
			else
				pFoundResource = DrxInterlockedCompareExchangePointer((void* volatile*)&pPass->m_depthTarget, nullptr, pResource.pAnonymous);
		}
