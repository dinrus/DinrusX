...
#include <DinrusX/DinrusXRend/VKHeap.hpp>
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void* VKAPI_CALL NDrxVulkan::CHostAllocator::CpuMalloc(void* pUserData, size_t size, size_t alignment, VkSystemAllocationScope scope)
{
	void* const pResult = DrxModuleMemalign(size, alignment);
#if !defined(_RELEASE)
	if (pResult)
	{
...
void* VKAPI_CALL NDrxVulkan::CHostAllocator::CpuRealloc(void* pUserData, void* pOriginal, size_t size, size_t alignment, VkSystemAllocationScope scope)
{
#if !defined(_RELEASE)
	volatile SPoolStats& stats = static_cast<CHostAllocator*>(pUserData)->AtStats(scope);
	if (pOriginal)
...
#endif
	void* const pResult = DrxModuleReallocAlign(pOriginal, size, alignment);
#if !defined(_RELEASE)
	if (pResult)
	{
		const size_t actualSize = DrxModuleMemSize(pResult, alignment);
...
void VKAPI_CALL NDrxVulkan::CHostAllocator::CpuFree(void* pUserData, void* pMemory)
{
#if !defined(_RELEASE)
	if (pMemory)
	{
...
void VKAPI_CALL NDrxVulkan::CHostAllocator::CpuForeignMalloc(void* pUserData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope)
{
#if !defined(_RELEASE)
	if (size)
	{
...
void VKAPI_CALL NDrxVulkan::CHostAllocator::CpuForeignFree(void* pUserData, size_t size, VkInternalAllocationType type, VkSystemAllocationScope scope)
{
#if !defined(_RELEASE)
	if (size)
	{
...
const char* NDrxVulkan::CHeap::FormatSize(char buffer[10], uint64_t bytes)
{
...
void* NDrxVulkan::CHeap::MapBlock(uint32_t memoryType, TBlockHandle blockHandle)
{
...
void NDrxVulkan::CHeap::UnmapBlock(uint32_t memoryType, TBlockHandle blockHandle, void* pAddress)
{
	(void)pAddress;
