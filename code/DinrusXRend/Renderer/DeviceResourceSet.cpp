...
#include <DinrusX/DinrusXRend/D3DRender/GraphicsPipeline/Common/GraphicsPipelineStateSet.h>
#include <DinrusX/DinrusXRend/DeviceResourceSet.h>
...
bool CDeviceResourceSetDesc::OnResourceInvalidated(void* pThis, SResourceBindPoint bindPoint, UResourceReference pResource, uint32 flags) threadsafe
{
	...
	if (flags & eResourceDestroyed)
	{
		...
		if (it != pSet->m_resources.end())
		{
			...
			UResourceReference pFoundResource = DrxInterlockedCompareExchangePointer((void* volatile*)&existingBinding.fastCompare, nullptr, pResource.pAnonymous);
