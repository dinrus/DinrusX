...
buffer_handle_t CRenderAuxGeomD3D::CBufferUpr::update(BUFFER_BIND_TYPE type, const void* data, size_t size)
{
...
buffer_handle_t CRenderAuxGeomD3D::CBufferUpr::fill(buffer_handle_t buf, BUFFER_BIND_TYPE type, const void* data, size_t size)
{
...
{
	DRX_ASSERT(m_pCurCBRawData && idx >= 0 && idx < (int)m_pCurCBRawData->m_auxOrthoMatrices.size());
	return m_pCurCBRawData->m_auxOrthoMatrices[idx];
}

inline const Matrix34A& CRenderAuxGeomD3D::GetAuxWorldMatrix(int idx) const
{
	DRX_ASSERT(m_pCurCBRawData && idx >= 0 && idx < (int)m_pCurCBRawData->m_auxWorldMatrices.size());
