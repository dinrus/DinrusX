...
#include <DinrusX/DinrusXRend/D3DOculus.h>
...
#include <DinrusXSys/VR/IHMDDevice.h>
	...
	#include <IBenchmarkRendererSensorUpr.h>
		...
		#include <DinrusX/DinrusXRend/DX12/Resource/Texture/CDrxDX12Texture2D.hpp>
			...
			#include <DinrusX/DinrusXRend/DX12/API/Redirections/D3D12Device.inl"
			#include <DinrusX/DinrusXRend/DX12/API/Workarounds/OculusMultiGPU.inl"
		#endif
	#endif
...
bool CD3DOculusRenderer::InitializeTextureSwapSet(ID3D11Device* pD3d11Device, EEyeType eye, STextureSwapChainRenderData& eyeRenderData, DrxVR::Oculus::TextureDesc desc, const std::string& name)
{
#if (DRX_RENDERER_DIRECT3D >= 120)
...
bool CD3DOculusRenderer::InitializeTextureSwapSet(ID3D11Device* pD3d11Device, EEyeType eye, DrxVR::Oculus::TextureDesc desc, const std::string& name)
{
	return InitializeTextureSwapSet(pD3d11Device, eye, m_scene3DRenderData[eye], desc, name);
}

bool CD3DOculusRenderer::InitializeQuadTextureSwapSet(ID3D11Device* d3dDevice, RenderLayer::EQuadLayers id, DrxVR::Oculus::TextureDesc desc, const std::string& name)
{
	return InitializeTextureSwapSet(d3dDevice, static_cast<EEyeType>(-1), m_quadLayerRenderData[id], desc, name);
}

bool CD3DOculusRenderer::InitializeMirrorTexture(ID3D11Device* pD3d11Device, DrxVR::Oculus::TextureDesc desc, const std::string& name)
{
	#if (DRX_RENDERER_DIRECT3D >= 120)
	ID3D12CommandQueue* pD3d12Queue = GetDeviceObjectFactory().GetNativeCoreCommandQueue();
