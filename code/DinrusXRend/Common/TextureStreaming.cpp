...
#include <DinrusX/DinrusXRend/PlanningTextureStreamer.h>

#include <DinrusX/DinrusXRend/StatoscopeTextureStreaming.h>
...
void STexStreamInState::StreamAsyncOnComplete(IReadStream* pStream, unsigned nError)
{
	...
	const int nRef = DrxInterlockedDecrement(&m_nAsyncRefCount);
	...
	if (pTex->TryAddRef() > 0)
	{
		...
		if (pStreamState)
		{
			...
			if (nStreams)
			{
				...
				DrxInterlockedAdd(&s_nMipsSubmittedToStreaming, (int)nStreams);
