...
void CRenderPolygonDataPool::UpdateAPIBuffers()
{
	...
	if (m_indices.size() > 0)
	{
		m_indexBuffer = CDeviceBufferUpr::Instance()->Create(BBT_INDEX_BUFFER, BU_STATIC, m_indices.size() * sizeof(uint16));
		CDeviceBufferUpr::Instance()->UpdateBuffer(m_indexBuffer, &m_indices[0], CDeviceBufferUpr::AlignBufferSizeForStreaming(m_indices.size() * sizeof(uint16)));
	}
}
