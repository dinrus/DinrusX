...
#include <DinrusX/DinrusXRend/ResFileLookupDataMan.h>
...
CDrxNameTSCRC CResFileLookupDataMan::AdjustName(const char* szName)
{
	char acTmp[1024];
  ...
  const char* acFilename, bool bSwapEndianRead, bool bReadOnly)
{
	...
	int nFlags = bReadOnly ? 0 : IDrxPak::FLAGS_NEVER_IN_PAK | IDrxPak::FLAGS_PATH_REAL | IDrxPak::FOPEN_ONDISK;
	...
	gEnv->pDrxPak->FReadRaw(&hid, sizeof(uint32), 1, handle);
	...
	gEnv->pDrxPak->FReadRaw(&uiCount, sizeof(unsigned int), 1, handle);
	...
	for (ui = 0; ui < uiCount; ++ui)
	{
		...
		gEnv->pDrxPak->FReadRaw(&dirData.m_NumOfFilesUnique, sizeof(int), 1, handle);
		gEnv->pDrxPak->FReadRaw(&dirData.m_NumOfFilesRef, sizeof(int), 1, handle);
		gEnv->pDrxPak->FReadRaw(&dirData.m_OffsetDir, sizeof(uint32), 1, handle);
		gEnv->pDrxPak->FReadRaw(&dirData.m_CRC32, sizeof(uint32), 1, handle);
		...
		gEnv->pDrxPak->FReadRaw(&uiDirSize, sizeof(unsigned int), 1, handle);
		if (bSwapEndianRead)
		{
  ...
  const char* acFilename, bool bSwapEndianWrite) const
{
	...
	gEnv->pDrxPak->FWrite(&hid, sizeof(uint32), 1, handle);
	...
	gEnv->pDrxPak->FWrite(&uiCount, sizeof(unsigned int), 1, handle);
	...
	for (TFileResDirDataMap::const_iterator it = m_Data.begin(); it != m_Data.end(); ++it)
	{
		...
		gEnv->pDrxPak->FWrite(&numOfFilesUnique, sizeof(int), 1, handle);
		gEnv->pDrxPak->FWrite(&numOfFilesRef, sizeof(int), 1, handle);
		gEnv->pDrxPak->FWrite(&offsetDir, sizeof(uint32), 1, handle);
		gEnv->pDrxPak->FWrite(&crc, sizeof(uint32), 1, handle);
		...
		if (header.m_ContainsResDir)
		{
			...
			gEnv->pDrxPak->FWrite(&uiDirSize, sizeof(unsigned int), 1, handle);
