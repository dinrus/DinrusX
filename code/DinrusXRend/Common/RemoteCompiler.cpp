...
bool CShaderSrv::Send(DRXSOCKET Socket, const char* pBuffer, uint32 Size)  const
{
	//size_t w;
	size_t wTotal = 0;
	while (wTotal < Size)
	{
		int result = DrxSock::send(Socket, pBuffer + wTotal, Size - wTotal, 0);
		DrxSock::eDrxSockError sockErr = DrxSock::TranslateSocketError(result);
		if (sockErr != DrxSock::eCSE_NO_ERROR)
		{
			;
			iLog->Log("ERROR:CShaderSrv::Send failed (%d, %d)\n", (int)result, DrxSock::TranslateToSocketError(sockErr));
			return false;
		}
...
bool CShaderSrv::Send(DRXSOCKET Socket, std::vector<uint8>& rCompileData) const
{
...
EServerError CShaderSrv::Recv(DRXSOCKET Socket, std::vector<uint8>& rCompileData)  const
{
	...
	while (rCompileData.size() < SIZELIMIT)
	{
		...
		int Recived = DRX_SOCKET_ERROR;
		int waitingtime = 0;
		while (Recived < 0)
		{
			Recived = DrxSock::recv(Socket, reinterpret_cast<char*>(&rCompileData[CurrentPos]), BLOCKSIZE, 0);
			...
			if (sockErr != DrxSock::eCSE_NO_ERROR)
			{
				...
				else
				{
					...
					iLog->LogError("ERROR: CShaderSrv::Recv:  error in recv() from remote server at offset %lu: error %li, sys_net_errno=%i\n",
					               (unsigned long)rCompileData.size(), (long)Recived, DrxSock::TranslateToSocketError(sockErr));
					return ESRecvFailed;
				}
			}
		}
...
EServerError CShaderSrv::Send(std::vector<uint8>& rCompileData) const
{
	...
	for (uint32 nRetries = m_LastWorkingServer; nRetries < m_LastWorkingServer + ServerVec.size() + 6; nRetries++)
	{
		...
		int arg = 1;
		DrxSock::setsockopt(Socket, SOL_SOCKET, SO_REUSEADDR, (char*)&arg, sizeof arg);
