  ...
  uint32 nSourceCRC32, const char* szName, bool bInclude, FILE* fpSrc)
{
	...
	int nSize = gEnv->pDrxPak->FTell(fpSrc);
	...
	if (fpDst)
	{
		...
		gEnv->pDrxPak->FWrite((void*)pHD, sizeof(Header), 1, fpDst);
		if (CParserBin::m_bEndians)
		{
			...
			gEnv->pDrxPak->FWrite((void*)pT, pBin->m_Tokens.size() * sizeof(DWORD), 1, fpDst);
			delete[] pT;
		}
		else
...
bool CShaderManBin::SaveBinShaderLocalInfo(SShaderBin* pBin, uint32 dwName, uint64 nMaskGenFX, TArray<int32>& Funcs, std::vector<SFXParam>& Params, std::vector<SFXSampler>& Samplers, std::vector<SFXTexture>& Textures)
{
	...
	int nSeek = gEnv->pDrxPak->FTell(fpBin);
	assert(nSeek > 0);
	if (nSeek <= 0)
		return false;
	...
	if (EParams.size())
		gEnv->pDrxPak->FWrite(pParams, EParams.size(), sizeof(int32), fpBin);
	if (ESamplers.size())
		gEnv->pDrxPak->FWrite(pSamplers, ESamplers.size(), sizeof(int32), fpBin);
	if (ETextures.size())
		gEnv->pDrxPak->FWrite(pTextures, ETextures.size(), sizeof(int32), fpBin);
	gEnv->pDrxPak->FWrite(pFuncs, Funcs.size(), sizeof(int32), fpBin);
...
SShaderBin* CShaderManBin::LoadBinShader(FILE* fpBin, const char* szName, const char* szNameBin, bool bReadParams)
{
	...
	sizeRead = gEnv->pDrxPak->FReadRaw(&pBin->m_Tokens[0], sizeof(uint32), Header.m_nTokens, fpBin);
	...
	if (bReadParams)
	{
		...
		{
			SShaderBinParamsHeader sd;
			int nSize = gEnv->pDrxPak->FReadRaw(&sd, 1, sizeof(sd), fpBin);
			if (nSize != sizeof(sd))
			{
				break;
			}
			...
			if (sd.nParams)
			{
				nSize = gEnv->pDrxPak->FReadRaw(&prc.m_AffectedParams[0], sizeof(int32), sd.nParams, fpBin);
				if (nSize != sd.nParams)
				{
			...
			if (sd.nSamplers)
			{
				nSize = gEnv->pDrxPak->FReadRaw(&prc.m_AffectedSamplers[0], sizeof(int32), sd.nSamplers, fpBin);
			...
			if (sd.nTextures)
			{
				nSize = gEnv->pDrxPak->FReadRaw(&prc.m_AffectedTextures[0], sizeof(int32), sd.nTextures, fpBin);
			...
			assert(sd.nFuncs > 0);
			nSize = gEnv->pDrxPak->FReadRaw(&prc.m_AffectedFuncs[0], sizeof(int32), sd.nFuncs, fpBin);
			if (nSize != sd.nFuncs)
			{
...
void SFXParam::GetCompName(uint32 nId, DrxFixedStringT<128>& name)
{
	if (nId > 3)
	{
		name = "";
...
void SFXParam::GetParamComp(uint32 nOffset, DrxFixedStringT<128>& param)
{
