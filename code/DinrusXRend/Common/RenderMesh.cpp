...
int CRenderMesh::GetTextureMemoryUsage( const IMaterial* pMaterial, IDrxSizer* pSizer, bool bStreamedIn ) const
{
...
void CRenderMesh::Size( uint32 nFlags, IDrxSizer* pSizer ) const
{
	uint32 i;
...
void CRenderMesh::ShutDown()
{
	...
	for (int i=0; i<DRX_ARRAY_COUNT(CRenderMesh::s_MeshGarbageList); ++i) // Make sure there are no pending deletes in garbage list
