...
#include <DinrusX/DinrusXRend/D3DRender/GraphicsPipeline/ShadowMap.h>
#include <DinrusX/DinrusXRend/D3DRender/GraphicsPipeline/ClipVolumes.h>
#include <DinrusX/DinrusXRend/CompiledRenderObject.h>
#include <DinrusX/DinrusXRend/D3D_SVO.h>

#include <DinrusX/CoreX/Renderer/branchmask.h>
...
void CRenderView::InspectRenderOutput()
{
	if (CRenderOutput* pRenderOutput = m_pRenderOutput.get())
	{
		...
		int outputWidth  = (vp.width  - vp.x); DRX_ASSERT(pRenderOutput->GetOutputResolution()[0] == outputWidth);
		int outputHeight = (vp.height - vp.y); DRX_ASSERT(pRenderOutput->GetOutputResolution()[1] == outputHeight);
...
inline void UpdateRenderListBatchFlags(volatile uint32& listFlags, int newFlags) threadsafe
{
	if (bConcurrent)
	{
		DrxInterlockedExchangeOr((volatile LONG*)&listFlags, newFlags);
	}
	else
	{
		listFlags |= newFlags;
	...
	for (auto record : m_permanentObjects)
	{
		...
		while (pRenderObject)
		{
				...
				for (size_t i = 0; i < numItems; i++)
				{
					...
					if (!pri.m_pCompiledObject)
					{
						...
						if (bRequireCompiledRenderObject)
						{
							...
							if (DrxInterlockedCompareExchangePointer((void* volatile*)&pri.m_pCompiledObject, allocatedCompiledObject, nullptr) != nullptr)
							{
							...
							else 
							{
								...
								if (i == 0)
									DrxInterlockedCompareExchangePointer((void* volatile*)&pRenderObject->m_pCompiledObject, pri.m_pCompiledObject, nullptr);
							}
						}
					}
			    ...
			    !pRenderObject->m_bAllCompiledValid)
			{
				...
				DrxInterlockedExchangeAnd((volatile LONG*)&pRenderObject->m_compiledReadyMask, ~passMask);      // This compiled masks invalid
				if (auto compiledObj = pRenderObject->m_pCompiledObject)
					DrxInterlockedExchangeAnd((volatile LONG*)&compiledObj->m_compiledFlags, ~compilationOptions);
...
void CRenderView::CompileModifiedRenderObjects()
{
	...
	for (const auto &compilationData : m_permanentRenderObjectsToCompile)
	{
		...
		if (bAllCompiled)
		{
			DrxInterlockedExchangeOr((volatile LONG*)&pRenderObject->m_compiledReadyMask, passMask);
		}
		else
		{
			DrxInterlockedExchangeAnd((volatile LONG*)&pRenderObject->m_passReadyMask, ~passMask); // reset passReadyMask if compilation failed
...
void CRenderView::CheckAndScheduleForUpdate(const SShaderItem& shaderItem) threadsafe
{
	...
	if (pSR && pShader)
	{
		...
		{
			if (DrxInterlockedExchange((volatile LONG*)&pSR->m_nUpdateFrameID, (uint32)m_frameId) != (uint32)m_frameId)
			{
