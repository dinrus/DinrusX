...
bool CResFile::mfActivate(bool bFirstTime)
{
	...
	if (!m_bActive)
	{
		...
		int nFlags = !m_pLookupDataMan || m_pLookupDataMan->IsReadOnly() ? 0 : IDrxPak::FLAGS_NEVER_IN_PAK | IDrxPak::FLAGS_PATH_REAL | IDrxPak::FOPEN_ONDISK;
...
int CResFile::mfGetResourceSize()
{
	...
	int length = gEnv->pDrxPak->FTell(m_handle);
...
bool CResFile::mfCloseEntry(const CDrxNameTSCRC &Name, uint32 flags, bool bEraseOpenEntry)
{
	ResDirOpenIt it = std::lower_bound(m_DirOpen.begin(), m_DirOpen.end(), Name, ResDirOpenSortByName());
	if (it == m_DirOpen.end() || Name != (*it).Name)
		return false;
...
int CResFile::mfFileClose(const CDrxNameTSCRC &Name, uint32 flags)
{
...
int CResFile::mfFileRead(CDirEntry* de)
{
	...
	{
		//gEnv->pDrxPak->FSeek(m_handle, 0, SEEK_END);
		//int nSize = gEnv->pDrxPak->FTell(m_handle);
		//int nnn = 0;
	}
...
byte* CResFile::mfFileReadCompressed(CDirEntry* de, uint32& nSizeDecomp, uint32& nSizeComp)
{
	...
	if (de->flags & RF_COMPRESS)
	{
		...
		{
			int nSize = -1;
			gEnv->pDrxPak->FReadRaw(&nSize, 1, sizeof(int), m_handle);
			if (m_bSwapEndianRead)
...
int CResFile::mfFileRead(CDrxNameTSCRC name)
{
	CDirEntry* de = mfGetEntry(name);
...
int CResFile::mfFileWrite(CDrxNameTSCRC name, void* data)
{
	CDirEntry* de = mfGetEntry(name);
...
void CResFile::mfFileRead2(CDrxNameTSCRC name, int size, void* buf)
{
	CDirEntry* de = mfGetEntry(name);
	if (!de)
	{
...
void* CResFile::mfFileGetBuf(CDrxNameTSCRC name)
{
	CDirEntry* de = mfGetEntry(name);
	if (!de)
	{
...
int CResFile::mfFileSeek(CDrxNameTSCRC name, int ofs, int type)
{
	CDirEntry* de = mfGetEntry(name);
...
int CResFile::mfFileLength(CDrxNameTSCRC name)
{
	CDirEntry* de = mfGetEntry(name);
