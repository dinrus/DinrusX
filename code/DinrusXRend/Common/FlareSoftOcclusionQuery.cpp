...
void CSoftOcclusionUpr::Init()
{
	...
	if (m_indexBuffer != ~0u)
	{
		DrxStackAllocWithSizeVector(uint16, maxIndexCount, pDeviceIBAddr, CDeviceBufferUpr::AlignBufferSizeForStreaming);
...
bool CSoftOcclusionUpr::PrepareGatherPrimitive(CRenderPrimitive& primitive, const CPrimitiveRenderPass& targetPass, SRenderViewInfo* pViewInfo, int viewInfoCount)
{
...
CFlareSoftOcclusionQuery* CSoftOcclusionUpr::GetSoftOcclusionQuery(int nIndex) const
{
...
bool CSoftOcclusionUpr::Update(SRenderViewInfo* pViewInfo, int viewInfoCount)
{
	if (GetSize() > 0)
	{
