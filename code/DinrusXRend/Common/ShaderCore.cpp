	...
	if (!m_bInitialized)
	{
		...
		if (!CRenderer::CV_r_shadersAllowCompilation)
		{
...

			{
				...
				//				"files are write protected / media is read only / windows user setting don't allow file changes","DinrusX",MB_ICONEXCLAMATION|MB_OK);
...
void CShaderMan::mfGatherFilesList(const char* szPath, std::vector<CDrxNameR>& Names, int nLevel, bool bUseFilter, bool bMaterial)
{
	struct _finddata_t fileinfo;
	intptr_t handle;
	char nmf[256];
