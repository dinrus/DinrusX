...
/*static*/ void CHUDEventDispatcher::CheckRegisteredEvents( void )
{
	...
	DRX_ASSERT_MESSAGE( regdEventCount == eHUDEvent_LAST-1, string().Format("HUD: Wrong number of events registered (%d) when compared to the number defined (%d)! /FH", regdEventCount, (int)eHUDEvent_LAST - 1 ).c_str() );
