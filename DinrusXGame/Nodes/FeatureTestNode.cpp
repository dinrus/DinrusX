...
#include <DinrusX/DinrusXGame/FeatureTestNode.h>

#include <DinrusX/DinrusXGame/Testing/FeatureTester.h>	// For ENABLE_FEATURE_TESTER definition(!)
#include <DinrusX/DinrusXAI/ICommunicationUpr.h>
#include <DinrusX/DinrusXSys/ICodeCheckpointMgr.h>			// For ICodeCheckpointMgr & CCodeCheckPoint
#include <DinrusX/DinrusXSys/IStatoscope.h>
#include <DinrusX/DinrusXAI/IGoalPipe.h>							// For Entity debug info
	...
	if (index >= -1 && index < SEQ_ENTITY_COUNT)
	{
		...
		if (id)
		{
			...
			else if (pEntitySystem->GetIEntityPoolUpr()->IsEntityBookmarked(id))
			{
				...
				IEntityPoolUpr *pEntityPoolUpr = pEntitySystem->GetIEntityPoolUpr();
				const char* entityName = pEntityPoolUpr->GetBookmarkedEntityName(id);
				outData.m_name = entityName;				
				const char* className = pEntityPoolUpr->GetBookmarkedClassName(id);
