...
void DrxGame_AIActionSequenceFlowNodeBase::FinishSequenceActionAndActivateOutputPort(int port)
{
	gEnv->pAISystem->GetSequenceUpr()->ActionCompleted(GetAssignedSequenceId());
	ActivateOutput(&m_actInfo, port, true);
}

void DrxGame_AIActionSequenceFlowNodeBase::CancelSequenceAndActivateOutputPort(int port)
{
