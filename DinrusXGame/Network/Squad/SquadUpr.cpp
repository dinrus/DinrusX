...
#include <DinrusX/DinrusXGame/SquadUpr.h>
...
{
	DrxLog("CSquadUpr::GameSessionIdChanged() eventType=%i", (int) eventType);
...
{
	DrxLog("[tlh] CSquadUpr::SetSquadHandle(DrxSessionHandle handle = %d) previousHandle=%d", (int)handle, int(m_squadHandle));
...
//static---------------------------------------
void CSquadUpr::UpdateLocalUserDataCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, void* arg )
{
	DrxLog("CSquadUpr::UpdateLocalUserDataCallback(DrxLobbyTaskID taskID, EDrxLobbyError error = %d, void* arg)", error);
...
EDrxLobbyError CSquadUpr::DoCreateSquad(IDrxMatchMaking *pMatchMaking, DrxLobbyTaskID &taskId)
{
	...
	{
		cry_strcpy(session.m_name, pPlayerProfileUpr->GetCurrentUser());
...
//static---------------------------------------
void CSquadUpr::CreateCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle handle, void* arg)
{
	DrxLog("CSquadUpr::CreateCallback(DrxLobbyTaskID taskID, EDrxLobbyError error = %d, DrxSessionHandle handle = %d, void* arg)", (int)error, (int)handle);
	...
	if (pSquadUpr->CallbackReceived(taskID, error))
	{
		...
		DrxLog("CSquadUpr session created %d", (int)handle);
...
void CSquadUpr::JoinSessionFinished(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle hdl)
{
	if (CallbackReceived(taskID, error))
	{
		...
		DrxLog("CSquadUpr session joined %d", (int)hdl);
...
//static---------------------------------------
void CSquadUpr::JoinCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle handle, uint32 ip, uint16 port, void* arg)
{
	DrxLog("CSquadUpr::JoinCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle handle = %d, uint32 ip, uint16 port, void* arg)", (int)handle);
...
//static---------------------------------------
void CSquadUpr::SessionChangeSlotTypeCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg)
{
...
//static---------------------------------------
void CSquadUpr::DeleteCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* arg)
{

	DrxLog("CSquadUpr::DeleteCallback(DrxLobbyTaskID taskID, EDrxLobbyError error = %d, void* arg)", error);
...
//static---------------------------------------
void CSquadUpr::JoinUserCallback(UDrxLobbyEventData eventData, void *arg)
{
	CSquadUpr* pSquadUpr = (CSquadUpr*)arg;
	DRX_ASSERT(pSquadUpr);
...
//static---------------------------------------
void CSquadUpr::LeaveUserCallback(UDrxLobbyEventData eventData, void *arg)
{
	CSquadUpr* pSquadUpr = (CSquadUpr*)arg;
	DRX_ASSERT(pSquadUpr);
	if(eventData.pSessionUserData->session == pSquadUpr->m_squadHandle)
...
//static---------------------------------------
void CSquadUpr::UpdateUserCallback(UDrxLobbyEventData eventData, void *arg)
{
	CSquadUpr* pSquadUpr = (CSquadUpr*)arg;
	DRX_ASSERT(pSquadUpr);
	if(eventData.pSessionUserData->session == pSquadUpr->m_squadHandle)
...
//static---------------------------------------
void CSquadUpr::OnlineCallback(UDrxLobbyEventData eventData, void *arg)
{
	DrxLog("CSquadUpr::OnlineCallback(UDrxLobbyEventData eventData, void *arg)");
	...
	if(eventData.pOnlineStateData)
	{
		...
		DrxLog("    onlineState=%i, currentTask=%i", int(onlineState), int(pSquadUpr->m_taskQueue.GetCurrentTask()));
...
//static---------------------------------------
void CSquadUpr::UserPacketCallback(UDrxLobbyEventData eventData, void *userParam)
{
	if(!sm_enable)
		return;
	...
	case eGUPD_SquadJoinGame:
		{
			...
			const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + pMatchmaking->GetSessionIDSizeInPacket() + DrxLobbyPacketBoolSize + DrxLobbyPacketUINT32Size + DrxLobbyPacketUINT32Size + DrxLobbyPacketUINT32Size;
			if (packet.CreateWriteBuffer(MaxBufferSize))
			{
				packet.StartWrite(packetType, true);
				...
				if(pPlaylistUpr)
				{
					const int activeVariant = pPlaylistUpr->GetActiveVariantIndex();
	...
	case eGUPD_SquadKick:
		{
			...
			if (pLobby)
			{
				...
				if (pMatchmaking)
				{
					...
					if (pPlayerProfileUpr)
					{
						const char *pCurrentUser = pPlayerProfileUpr->GetCurrentUser();
...
//---------------------------------------
DrxUserID CSquadUpr::GetUserIDFromChannelID(int channelId)
{
...
//-------------------------------------------------------------------------
bool CSquadUpr::GetSquadCommonDLCs(uint32 &commonDLCs)
{
	if(InSquad())
	{
...
//-------------------------------------------------------------------------
void CSquadUpr::TaskStartedCallback( CLobbyTaskQueue::ESessionTask task, void *pArg )
{
...
//static---------------------------------------
void CSquadUpr::HandleCustomError(const char* dialogName, const char* msgPreLoc, const bool deleteSession, const bool returnToMainMenu)
{
	if (deleteSession)
	{
...
//-------------------------------------------------------------------------
IHostMigrationEventListener::EHostMigrationReturn CSquadUpr::OnPromoteToServer( SHostMigrationInfo& hostMigrationInfo, uint32& state )
{
...
//-------------------------------------------------------------------------
IHostMigrationEventListener::EHostMigrationReturn CSquadUpr::OnFinalise( SHostMigrationInfo& hostMigrationInfo, uint32& state )
{
	if (m_squadHandle == hostMigrationInfo.m_session)
	{
...
//-------------------------------------------------------------------------
void CSquadUpr::SessionStartCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg )
{
...
//-------------------------------------------------------------------------
void CSquadUpr::SessionEndCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg )
{
...
//-------------------------------------------------------------------------
void CSquadUpr::SessionClosedCallback( UDrxLobbyEventData eventData, void *userParam )
{
...
//-------------------------------------------------------------------------
void CSquadUpr::ForcedFromRoomCallback(UDrxLobbyEventData eventData, void *pArg)
{
	CSquadUpr *pSquadUpr = static_cast<CSquadUpr*>(pArg);
	if (pSquadUpr)
	{
		...
		DrxLog("CSquadUpr::ForcedFromRoomCallback session %d reason %d", (int)pEventData->m_session, pEventData->m_why);
...
//-------------------------------------------------------------------------
IHostMigrationEventListener::EHostMigrationReturn CSquadUpr::OnTerminate( SHostMigrationInfo& hostMigrationInfo, uint32& state )
{
	if (m_squadHandle == hostMigrationInfo.m_session)
	{
...
//-------------------------------------------------------------------------
IHostMigrationEventListener::EHostMigrationReturn CSquadUpr::OnReset(SHostMigrationInfo& hostMigrationInfo, uint32& state)
{
	return IHostMigrationEventListener::Listener_Done;
}
...
//-------------------------------------------------------------------------
void CSquadUpr::SessionSetLocalFlagsCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle h, uint32 flags, void* pArg )
{
...
//-------------------------------------------------------------------------
bool CSquadUpr::OnWarningReturn( THUDWarningId id, const char* returnValue )
{
...
//-------------------------------------------------------------------------
void CSquadUpr::SquadJoinGame(DrxSessionID sessionID, bool isMatchmakingGame, uint32 playlistID, int restrictRank, int requireRank)
{
