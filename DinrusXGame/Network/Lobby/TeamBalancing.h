...
public:
	...
	void OnPlayerSwitchedTeam(const SDrxMatchMakingConnectionUID &uid, uint8 teamId);
	void OnGameFinished(EUpdateTeamType updateType);

	uint8 GetTeamId(const SDrxMatchMakingConnectionUID &uid);
	...
	void ReadPacket(CDrxLobbyPacket *pPacket, uint32 packetType);
...
private:
	...
	void UpdatePlayer( STeamBalancing_PlayerSlot *pSlot, uint16 skillRank, uint32 squadLeaderUID, DrxFixedStringT<CLAN_TAG_LENGTH> &clanTag, bool bBalanceTeams );
