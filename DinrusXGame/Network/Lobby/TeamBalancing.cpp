...
//-----------------------------------------------------------------------------
void CTeamBalancing::UpdatePlayer( STeamBalancing_PlayerSlot *pSlot, uint16 skillRank, uint32 squadLeaderUID, DrxFixedStringT<CLAN_TAG_LENGTH> &clanTag, bool bBalanceTeams )
{
	...
	else
	{
		...
		int variantIdx = pPlaylistUpr ?	pPlaylistUpr->GetActiveVariantIndex() : -1;
		if (variantIdx >= 0)
		{
...
//-----------------------------------------------------------------------------
void CTeamBalancing::OnPlayerSwitchedTeam( const SDrxMatchMakingConnectionUID &uid, uint8 teamId )
{
	if(teamId)
	{
		STeamBalancing_PlayerSlot *pSlot = FindPlayerSlot(uid);
...
//-----------------------------------------------------------------------------
uint8 CTeamBalancing::GetTeamId( const SDrxMatchMakingConnectionUID &uid )
{
	STeamBalancing_PlayerSlot *pSlot = FindPlayerSlot(uid);
	if (pSlot)
	{
...
void CTeamBalancing::WritePacket(CDrxLobbyPacket *pPacket, GameUserPacketDefinitions packetType, SDrxMatchMakingConnectionUID playerUID) 
{
	...
	switch(packetType)
	{
		case eGUPD_TeamBalancingSetup:
		{
			...
			const uint32 dataSize = (DrxLobbyPacketConnectionUIDSize + DrxLobbyPacketUINT16Size + DrxLobbyPacketUINT32Size + DrxLobbyPacketUINT8Size + DrxLobbyPacketBoolSize) * numPlayers;
			const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT32Size + dataSize;
		...
		case eGUPD_TeamBalancingAddPlayer:
		{
			...
			const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketConnectionUIDSize + DrxLobbyPacketUINT16Size + DrxLobbyPacketUINT32Size + DrxLobbyPacketUINT8Size + DrxLobbyPacketBoolSize;
			if(pPacket->CreateWriteBuffer(bufferSize))
			{
				pPacket->StartWrite(packetType, true);
		...
		{
			const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketConnectionUIDSize;
...
//-----------------------------------------------------------------------------
void CTeamBalancing::ReadPacket(CDrxLobbyPacket *pPacket, uint32 packetType)
{
	switch(packetType)
	{
		case eGUPD_TeamBalancingSetup:
