...
#include <DinrusX/DinrusXGame/GameBrowser.h>
#include <DinrusX/DinrusXGame/GameServerLists.h>

#include <DinrusX/CoreX/Lobby/IDrxLobbyUI.h>
#include <DinrusX/CoreX/Lobby/IDrxMatchMaking.h>
#include <DinrusX/CoreX/Lobby/IDrxVoice.h>
#include <DinrusX/CoreX/Game/IGameFramework.h>
#include <DinrusX/DinrusXGame/IGameRulesSystem.h>
#include <DinrusX/DinrusXGame/IGameSessionHandler.h>
#include <DinrusX/CoreX/Lobby/IDrxStats.h>
#include <DinrusX/DinrusXSys/File/IResourceUpr.h>
...
#include <DinrusX/DinrusXGame/GameRulesModules/IGameRulesPlayerStatsModule.h>
#include <DinrusX/DinrusXGame/UI/UIUpr.h>
#include <DinrusX/DinrusXGame/UI/WarningsUpr.h>
#include <DinrusX/DinrusXGame/UI/HUD/HUDEventWrapper.h>
#include <DinrusX/DinrusXGame/UI/HUD/HUDUtils.h>
#include <DinrusX/DinrusXGame/UI/UICVars.h>

#include <DinrusX/DinrusXGame/PlaylistUpr.h>
#include <DinrusX/DinrusXGame/PlaylistActivityTracker.h>

#include <DinrusX/CoreX/TypeInfo_impl.h>

#include <DinrusX/DinrusXGame/TelemetryCollector.h>
#include <DinrusX/DinrusXGame/DLCUpr.h>
#include <DinrusX/DinrusXGame/Network/GameNetworkUtils.h>
...
#include <DinrusX/DinrusXGame/RecordingSystem.h>

#include <DinrusX/DinrusXGame/GameLobbyCVars.h>
#include <DinrusX/DinrusXGame/UI/ProfileOptions.h>
#include #include <DinrusX/DinrusXGame/../UI/Menu3dModels/FrontEndModelCache.h>
#include <DinrusX/DinrusXGame/UI/UILobbyMP.h>
#include <DinrusX/DinrusXGame/UI/UIMultiPlayer.h>
...
void CGameLobby::CmdFillReservationSlots(IConsoleCmdArgs *pArgs)
{
	...
	if (pLobby)
	{
		...
		for (int i = 0; i < DRX_ARRAY_COUNT(pLobby->m_slotReservations); ++ i)
		{
...
CGameLobby::CGameLobby( CGameLobbyUpr* pMgr )
{
	...
	for (int i=0; i<DRX_ARRAY_COUNT(m_slotReservations); ++i)
	{
...
void CGameLobby::Update(float dt)
{
	...
	{
		DrxWatch("GameLobby state: %d, StartedGameContext: %d", (int)m_state, g_pGame->GetIGameFramework()->StartedGameContext());
	...
	if (s_pGameLobbyCVars->gl_voip_debug)
	{
		switch (m_autoVOIPMutingType)
		{
			...
			default:								DrxWatch("Current mute state %d", (int)m_autoVOIPMutingType);	break;
		};
	}
#endif
...
//---------------------------------------
// [static]
void CGameLobby::MatchmakingEnsureBestHostCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, void* arg )
{
	ENSURE_ON_MAIN_THREAD;

	CGameLobby *pLobby = static_cast<CGameLobby *>(arg);

	DrxLog("CGameLobby::MatchmakingEnsureBestHostCallback error %d Lobby: %p", (int)error, pLobby);
...
const char* CGameLobby::GetMapDescription(const char* levelFileName, DrxFixedStringT<64>* pOutLevelDescription)
{
...
const char* CGameLobby::GetMapImageName(const char* levelFileName, DrxFixedStringT<128>* pOutLevelImageName)
{
...
bool CGameLobby::CheckRankRestrictions()
{
	...
	if (!gEnv->IsDedicated())
	{
		...
		if (pPlaylistUpr)
		{
			int activeVariant = pPlaylistUpr->GetActiveVariantIndex();
...
void CGameLobby::EnterState(ELobbyState prevState, ELobbyState newState)
{
	...
	DrxLog("[CG] CGameLobby::EnterState() entering state %i from %i Lobby: %p", int(newState), int(prevState), this);
	INDENT_LOG_DURING_SCOPE();
...
void CGameLobby::SetupSessionData()
{
	...
	if (pServerNameCVar)
	{
		...
		else
		{
			...
			if (pProfileUpr)
			{
				const uint32 userIndex = pProfileUpr->GetExclusiveControllerDeviceIndex();
...
{
	uint32 flags = DRXSESSION_CREATE_FLAG_NUB | DRXSESSION_CREATE_FLAG_CAN_SEND_SERVER_PING;
...
void CGameLobby::SetLocalUserData(uint8 * localUserData)
{
	...
	localUserData[eLUD_LoadedDLCs] = (uint8)g_pGame->GetDLCUpr()->GetLoadedDLCs();
...
EDrxLobbyError CGameLobby::DoUpdateLocalUserData(IDrxMatchMaking *pMatchMaking, DrxLobbyTaskID &taskId)
{
	...
	uint8 localUserData[DRXLOBBY_USER_DATA_SIZE_IN_BYTES] = {0};
...
//-------------------------------------------------------------------------
// Connect to a server
bool CGameLobby::JoinServer(DrxSessionID sessionId, const char *sessionName, const SDrxMatchMakingConnectionUID &reservationId, bool bRetryIfPassworded )
{
...
//-------------------------------------------------------------------------
// Migrate callback.
void CGameLobby::MatchmakingSessionMigrateCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle h, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
...
void CGameLobby::MatchmakingSessionCreateCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle h, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
		...
		if(!pLobby->m_bCancelling)
		{
...

			{
				...
				if (pPlaylistUpr)
				{
					...
					{
						// Set default variant
						int defaultVariant = pPlaylistUpr->GetDefaultVariant();
						if (defaultVariant != -1)
						{
...
// arg is CGameLobby*
void CGameLobby::MatchmakingSessionJoinCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle h, uint32 ip, uint16 port, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
...
{
	DrxLog("GameLobby session join error %d", (int)error);
...
// arg is CGameLobby*
void CGameLobby::MatchmakingSessionDeleteCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
...
{
	DrxLog("CGameLobby::FinishDelete() lobby:%p result from delete = %d", this, (int) result);
...
//static-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionUpdateCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
...
//static-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionQueryCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, SDrxSessionSearchResult* session, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
...
//static-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionUserPacketCallback(UDrxLobbyEventData eventData, void *arg)
{
	ENSURE_ON_MAIN_THREAD;
...
static int GetDrxLobbyUserTypeSize(EDrxLobbyUserDataType type)
{
	switch(type)
	{
		case eCLUDT_Int8:
...
//static-------------------------------------------------------------------------
void CGameLobby::WriteOnlineAttributeData(CGameLobby *pLobby, CDrxLobbyPacket *pPacket, GameUserPacketDefinitions packetType, SDrxLobbyUserData *pData, int32 numData, SDrxMatchMakingConnectionUID connnectionUID)
{
	DRX_ASSERT(pData);
	...
	{
		int bufferSz = DrxLobbyPacketHeaderSize;
...
void CGameLobby::SendPacket(GameUserPacketDefinitions packetType, SDrxMatchMakingConnectionUID connectionUID /*=DrxMatchMakingInvalidConnectionUID*/)
{
	...
	case eGUPD_SetupJoinCommand:	// needs to be sent before LobbyGameHasStarted
		{
			...
			const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize +  DrxLobbyPacketUINT32Size + DrxLobbyPacketUINT16Size;
			if (packet.CreateWriteBuffer(MaxBufferSize))
			{
				packet.StartWrite(packetType, true);
	...
	case eGUPD_SetupArbitratedLobby:
		{
			...
			const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize +  DrxLobbyPacketUINT8Size + DrxLobbyPacketUINT16Size + (DrxLobbyPacketUINT8Size * ipSize);
			if (packet.CreateWriteBuffer(MaxBufferSize))
			{
				packet.StartWrite(packetType, true);
	...
	case eGUPD_LobbyGameHasStarted:
		{
			DRX_ASSERT(m_server);
			const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + (3 * DrxLobbyPacketUINT32Size) + (DrxLobbyPacketBoolSize * 2);
			if (packet.CreateWriteBuffer(MaxBufferSize))
			{
				packet.StartWrite(packetType, true);
				CDLCUpr* pDLCUpr = g_pGame->GetDLCUpr();
	...
	case eGUPD_LobbyMoveSession:
		{
			...
			IDrxMatchMaking *pMatchmaking = gEnv->pNetwork->GetLobby()->GetLobbyService()->GetMatchMaking();
			const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + pMatchmaking->GetSessionIDSizeInPacket();
			if (packet.CreateWriteBuffer(MaxBufferSize))
			{
				packet.StartWrite(packetType, true);
	...
	case eGUPD_SetTeamAndRank:
		{
			...
			const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size + (numPlayers * (DrxLobbyPacketUINT8Size * 2 + DrxLobbyPacketConnectionUIDSize));
			if (packet.CreateWriteBuffer(MaxBufferSize))
			{
				packet.StartWrite(packetType, true);
	...
	case eGUPD_SendChatMessage:
		{
			...
			{
				const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size + DrxLobbyPacketUINT8Size + messageLength;
				if (packet.CreateWriteBuffer(MaxBufferSize))
				{
					packet.StartWrite(packetType, true);
	...
	case eGUPD_ChatMessage:
		{
			...
			if (messageLength>0 && messageLength<MAX_CHATMESSAGE_LENGTH)
			{
				const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketConnectionUIDSize + DrxLobbyPacketUINT8Size + DrxLobbyPacketUINT8Size + messageLength;
				if (packet.CreateWriteBuffer(MaxBufferSize))
				{
					packet.StartWrite(packetType, true);
	...
	case eGUPD_ReservationRequest:
		{
			...
			const int  numReservationsToRequest = BuildReservationsRequestList(reservationRequests, DRX_ARRAY_COUNT(reservationRequests), m_reservationList);

			const SSessionNames*  members = m_reservationList;

			const int  bufferSz = (DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size + (numReservationsToRequest * DrxLobbyPacketConnectionUIDSize));
	...
	case eGUPD_ReservationClientIdentify:
		{
			...
			const int  bufferSz = DrxLobbyPacketHeaderSize + DrxLobbyPacketConnectionUIDSize + DrxLobbyPacketBoolSize + pMatchmaking->GetSessionIDSizeInPacket();
	...
	case eGUPD_ReservationsMade:
		{
			...
			const int  bufferSz = (DrxLobbyPacketHeaderSize);
	...
	case eGUPD_SyncPlaylistRotation:
		{
			...
			const uint32  bufferSize = (DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT32Size + DrxLobbyPacketUINT8Size + (2 * DrxLobbyPacketBoolSize));
	...
	case eGUPD_UpdateCountdownTimer:
		{
			const int bufferSz = (DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size);
			if (packet.CreateWriteBuffer(bufferSz))
			{
				packet.StartWrite(packetType, true);
	...
	case eGUPD_SyncPlaylistContents:
		{
			...
			if (pRotation)
			{
				...
				const uint32 bufferSize = (DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size + (DrxLobbyPacketUINT32Size * (2 * numLevels)));
				if (packet.CreateWriteBuffer(bufferSize))
				{
					packet.StartWrite(packetType, true);
	...
	case eGUPD_SyncExtendedServerDetails:
		{
			...
			uint32 bufferSize = (DrxLobbyPacketHeaderSize);
	...
	case eGUPD_SetAutoStartingGame:
		{
			...
			const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketBoolSize;
			if (packet.CreateWriteBuffer(bufferSize))
			{
				packet.StartWrite(packetType, true);
...
void CGameLobby::SendPacket(CDrxLobbyPacket *pPacket, GameUserPacketDefinitions packetType, SDrxMatchMakingConnectionUID connectionUID)
{
	...
	if (packetSize > 0)
	{
		...
		{
			DrxLog("GameLobby::SendPacket ERROR sending the packet %d type %d",(int)result,(int)packetType);
		}
	}
	else
	{
		DrxLog("GameLobby::SendPacket ERROR trying to send an invalid sized packet %d typed %d",packetSize,(int)packetType);
	}
}
...
//static ------------------------------------------------------------------------
DrxFixedStringT<32> CGameLobby::GetValidMapForGameRules(const char* inLevel, const char* gameRules, bool returnBackup/*=false*/)
{
	if (!gameRules)
		return "";
...
//static-------------------------------------------------------------------------
void CGameLobby::SendChatMessageTeamCheckProfanityCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, const char* pString, bool isProfanity, void* pArg )
{
	CGameLobby* pLobby = g_pGame->GetGameLobby();
	if(pLobby)
	{
...
//static-------------------------------------------------------------------------
void CGameLobby::SendChatMessageAllCheckProfanityCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, const char* pString, bool isProfanity, void* pArg )
{
	CGameLobby* pLobby = g_pGame->GetGameLobby();
	if(pLobby)
	{
...
//-------------------------------------------------------------------------
void CGameLobby::SendChatMessageCheckProfanityCallback( const bool team, DrxLobbyTaskID taskID, EDrxLobbyError error, const char* pString, bool isProfanity, void* pArg )
{
...
void CGameLobby::CmdStartGame(IConsoleCmdArgs* pCmdArgs)
{
	...
	if (pGameLobby)
	{
		if(pGameLobby->m_server)
		{
			...
			uint32 requiredDLCs = g_pGame->GetDLCUpr()->GetRequiredDLCs();
			for (int i=0; i<nameSize; ++i)
			{
...
//---------------------------------------
void CGameLobby::MatchmakingSessionRoomOwnerChangedCallback( UDrxLobbyEventData eventData, void *userParam )
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
void CGameLobby::MatchmakingSessionJoinUserCallback( UDrxLobbyEventData eventData, void *userParam )
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
void CGameLobby::MatchmakingSessionLeaveUserCallback( UDrxLobbyEventData eventData, void *userParam )
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
void CGameLobby::MatchmakingSessionUpdateUserCallback( UDrxLobbyEventData eventData, void *userParam )
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
void CGameLobby::MatchmakingLocalUserDataCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* arg)
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
void CGameLobby::InviteAcceptedCallback( UDrxLobbyEventData eventData, void *userParam )
{
...
//------------------------------------------------------------------------
void CGameLobby::GetPlayerNameFromChannelId(int channelId, DrxFixedStringT<DRXLOBBY_USER_NAME_LENGTH> &name)
{
...
void CGameLobby::GetClanTagFromChannelId(int channelId, DrxFixedStringT<CLAN_TAG_LENGTH> &name)
{
	SDrxMatchMakingConnectionUID temp = gEnv->pNetwork->GetLobby()->GetMatchMaking()->GetConnectionUIDFromGameSessionHandleAndChannelID(m_currentSession, channelId);
	SSessionNames::SSessionName *pPlayer = m_nameList.GetSessionName(temp, true);
	if (pPlayer)
...
//---------------------------------------
bool CGameLobby::GetPlayerDisplayNameFromChannelId(int channelId, DrxFixedStringT<DISPLAY_NAME_LENGTH> &displayName)
{
...
//---------------------------------------
int CGameLobby::GetTeamByConnectionUID( SDrxMatchMakingConnectionUID uid )
{
	return m_teamBalancing.GetTeamId(uid);
}

//---------------------------------------
SDrxMatchMakingConnectionUID CGameLobby::GetConnectionUIDFromChannelID(int channelId)
{
...
void CGameLobby::MakeReservations(SSessionNames* nameList, bool squadReservation)
{
	...
	{
		SDrxMatchMakingConnectionUID  reservationRequests[MAX_RESERVATIONS];
		int numReservationsToRequest = BuildReservationsRequestList(reservationRequests, DRX_ARRAY_COUNT(reservationRequests), nameList);
...
//---------------------------------------
int CGameLobby::BuildReservationsRequestList(SDrxMatchMakingConnectionUID reservationRequests[], const int maxRequests, const SSessionNames*  members)
{
	...
	for (int i=0; i<numReservationsToRequest; ++i)
	{
		DRX_ASSERT((i + 1) < (int)members->m_sessionNames.size());
...
//---------------------------------------
EReservationResult CGameLobby::DoReservations(const int numReservationsRequested, const SDrxMatchMakingConnectionUID requestedReservations[])
{
...
//---------------------------------------
DrxUserID CGameLobby::GetUserIDFromChannelID(int channelId)
{
...
//---------------------------------------
int CGameLobby::GetPlayerMuteReason(DrxUserID userId)
{
	int result = 0;
...
void CGameLobby::SetAutomaticMutingState(ELobbyAutomaticVOIPType newType)
{
	...
	if(s_pGameLobbyCVars->gl_voip_debug)
	{
		DrxLog("SetAutomaticMutingState %d -> %d Lobby: %p", (int)m_autoVOIPMutingType, (int)newType, this);
		...
		default:								DrxLog("Unknown automatic muting %d Lobby: %p", (int)newType, this);	break;
		};
	}
#endif
...
//---------------------------------------
void CGameLobby::MatchmakingSessionStartCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg )
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
void CGameLobby::MatchmakingSessionEndCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg)
{
	ENSURE_ON_MAIN_THREAD;
...
//---------------------------------------
// [static]
THUDWarningId CGameLobby::ShowErrorDialog(const EDrxLobbyError error, const char* pDialogName, const char* pDialogParam, IGameWarningsListener* pWarningsListener)
{
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionTerminateHostHintingForGroupCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg )
{
	ENSURE_ON_MAIN_THREAD;
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionClosedCallback( UDrxLobbyEventData eventData, void *userParam )
{
	MatchmakingSessionClosed(eventData, userParam, CErrorHandling::eNFE_HostMigrationFailed);
}

//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionKickedCallback( UDrxLobbyEventData eventData, void *userParam )
{
	MatchmakingSessionClosed(eventData, userParam, CErrorHandling::eNFE_Kicked);
}

//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionKickedHighPingCallback( UDrxLobbyEventData eventData, void *userParam )
{
	MatchmakingSessionClosed(eventData, userParam, CErrorHandling::eNFE_KickedHighPing);
}

//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionKickedReservedUserCallback( UDrxLobbyEventData eventData, void *userParam )
{
	MatchmakingSessionClosed(eventData, userParam, CErrorHandling::eNFE_KickedReservedUser);
}

//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionClosed(UDrxLobbyEventData eventData, void *userParam, CErrorHandling::ENonFatalError error)
{
	ENSURE_ON_MAIN_THREAD;
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingForcedFromRoomCallback(UDrxLobbyEventData eventData, void *pArg)
{
	ENSURE_ON_MAIN_THREAD;
	...
	if (pGameLobby)
	{
		...
		DrxLog("CGameLobby::ForcedFromRoomCallback session %d reason %d", (int)pEventData->m_session, pEventData->m_why);
...
EDrxLobbyError CGameLobby::DoSessionDetailedInfo( IDrxMatchMaking *pMatchMaking, DrxLobbyTaskID &taskId )
{
	...
	CDrxLobbyPacket packet;
	const uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size;
...
void CGameLobby::MatchmakingSessionDetailedInfoRequestCallback(UDrxLobbyEventData eventData, void *pArg)
{
	ENSURE_ON_MAIN_THREAD;
	...
	if (pGameLobby)
	{
		...
		DrxLog("CGameLobby::MatchmakingSessionDetailedInfoRequestCallback session %d", (int)pEventData->session);
		...
		if (pEventData->session != DrxSessionInvalidHandle && pGameLobby->m_currentSession == pEventData->session)
		{
			...
			if (pRequest)
			{
				...
				CDrxLobbyPacket packet;
				uint32 bufferSize = DrxLobbyPacketHeaderSize + DrxLobbyPacketUINT8Size;
				...
				if (packet.CreateWriteBuffer(bufferSize))
				{
					...
					if (flags & eDSIRF_IncludeCustomFields)
					{
						if (pPlaylistUpr)
						{
							...
							{
								uint16 value = pPlaylistUpr->PackCustomVariantOption(j);
...
void CGameLobby::MatchmakingSessionDetailedInfoResponseCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, CDrxLobbyPacket* pPacket, void* pArg)
{
	ENSURE_ON_MAIN_THREAD;
...
void CGameLobby::ConnectionFailed(CErrorHandling::ENonFatalError error)
{
	...
	DrxLog("CGameLobby::ConnectionFailed() error=%d", (int) error);
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSessionSetLocalFlagsCallback( DrxLobbyTaskID taskID, EDrxLobbyError error, DrxSessionHandle h, uint32 flags, void* pArg )
{
	ENSURE_ON_MAIN_THREAD;
...
void CGameLobby::CreateSessionFromSettings(const char *pGameRules, const char *pLevelName)
{
	...
	if (pPlaylistUpr)
	{
		int defaultPlaylistId = pPlaylistUpr->GetDefaultVariant();
...
EDrxLobbyError CGameLobby::DoSetupDedicatedServer(IDrxMatchMaking *pMatchMaking, DrxLobbyTaskID &taskId, bool &bTaskStartedOut)
{
	...
	if (m_gameLobbyMgr->IsPrimarySession(this))
	{
		const uint32 MaxBufferSize = DrxLobbyPacketHeaderSize + (5 * DrxLobbyPacketUINT32Size);
		CDrxLobbyPacket packet;
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingSetupDedicatedServerCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, uint32 ip, uint16 port, void* pArg)
{
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingDedicatedServerSetup(UDrxLobbyEventData eventData, void *userParam)
{
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingReleaseDedicatedServerCallback(DrxLobbyTaskID taskId, EDrxLobbyError error, void *pArg)
{
...
//-------------------------------------------------------------------------
void CGameLobby::MatchmakingDedicatedServerRelease(UDrxLobbyEventData eventData, void *userParam)
{
