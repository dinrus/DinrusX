...
public:
	...
	EOnlineState GetOnlineState(uint32 userIndex) const { DRX_ASSERT(userIndex < MAX_LOCAL_USERS); return m_onlineState[userIndex]; }
...
protected:
	...
	// lobby callbacks
	static void OnlineCallback(UDrxLobbyEventData eventData, void *arg);
	static void EthernetStateCallback(UDrxLobbyEventData eventData, void *arg);
	static void ChatRestrictedCallback(UDrxLobbyEventData eventData, void *arg);
	static void UserSignoutCallback(DrxLobbyTaskID taskID, EDrxLobbyError error, uint32 user, void *pArg); 
};
