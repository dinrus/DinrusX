...
struct SGameVariant
{
	...
	typedef DrxFixedArray<int, 16> TIntArray;
...
public:
	...
	bool HaveUnlockedPlaylistVariant(const SPlaylist *pPlaylist, const int variantId, DrxFixedStringT<128>* pUnlockString=NULL);
	...
	void GetGameModeOption(const char *pOption, DrxFixedStringT<32> &result);
	uint32 GetGameModeOptionCount()
	{
		return (uint32)m_options.size();
	}
	void GetGameModeProfileOptionName(const uint32 index, DrxFixedStringT<32> &result);
	...
	void WriteSetCustomVariantOptions(CDrxLobbyPacket* pPacket, CPlaylistUpr::TOptionsVec pOptions, uint32 numOptions);
	void WriteSetVariantPacket(CDrxLobbyPacket *pPacket);

	void ReadSetCustomVariantOptions(CDrxLobbyPacket* pPacket, CPlaylistUpr::TOptionsVec pOptions, uint32 numOptions);
