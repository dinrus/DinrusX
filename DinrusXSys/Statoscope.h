...
protected:
	...
	bool CheckError(int err, const char* tag);          // returns true if err is a DrxSock error and also handles the error (prints it and closes the connection)
...
protected:
	void CheckSocketError(DrxSock::eDrxSockError sockErr, const char* description);
