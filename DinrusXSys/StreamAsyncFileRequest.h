...
struct SStreamEngineTempMemStats;
	...
	#include <plugin/z/lib/inftrees.h>
	#include <plugin/z/lib/inflate.h>
...
public:
	...
	{
		DrxInterlockedCompareExchange(reinterpret_cast<volatile LONG*>(&m_nError), nError, 0);
	}

	uint32         OpenFile(CDrxFile& file);

	uint32         ReadFile(CStreamingIOThread* pIOThread);
	uint32         ReadFileResume(CStreamingIOThread* pIOThread);
	uint32         ReadFileInPages(CStreamingIOThread* pIOThread, CDrxFile& file);
