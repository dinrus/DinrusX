...
#include <DinrusX/DinrusXSys/IMiniLog.h>
#include <DinrusX/DinrusXSys/ZipDir.h>
#include <DinrusX/DinrusXSys/MTSafeAllocator.h>
#include <DinrusX/CoreX/StlUtils.h>
#include <DinrusX/DinrusXSys/PakVars.h>
...
{
	CCachedFileData(class CDrxPak* pPak, ZipDir::Cache* pZip, unsigned int nArchiveFlags, ZipDir::FileEntry* pFileEntry, const char* szFilename);
	~CCachedFileData();
...
public:
	...
	CDrxPakFindData();
	explicit CDrxPakFindData(const char* szNameFilter);
	bool         empty() const;
	bool         Fetch(_finddata_t* pfd);
	virtual void Scan(CDrxPak* pPak, const char* szDir, bool bAllowUseFS = false);

	size_t       sizeofThis() const;
	void         GetMemoryUsage(IDrxSizer* pSizer) const;
protected:
	void         ScanFS(CDrxPak* pPak, const char* szDir);
	void         ScanZips(CDrxPak* pPak, const char* szDir);
...
public: // ---------------------------------------------------------------------------------------
	...
	virtual IDrxArchive* OpenArchive(const char* szPath, unsigned int nFlags = 0, IMemoryBlock* pData = 0) override;
	...
	virtual bool OpenPack(const char* pName, unsigned nFlags = 0, IMemoryBlock* pData = 0, DrxFixedStringT<IDrxPak::g_nMaxPath>* pFullPath = NULL) override;
	virtual bool OpenPack(const char* szBindRoot, const char* pName, unsigned nFlags = 0, IMemoryBlock* pData = 0, DrxFixedStringT<IDrxPak::g_nMaxPath>* pFullPath = NULL) override;
	// after this call, the file will be unlocked and closed, and its contents won't be used to search for files
	virtual bool ClosePack(const char* pName, unsigned nFlags = 0) override;
	virtual bool OpenPacks(const char* pWildcard, unsigned nFlags = 0, std::vector<DrxFixedStringT<IDrxPak::g_nMaxPath>>* pFullPaths = NULL) override;
	virtual bool OpenPacks(const char* szBindRoot, const char* pWildcard, unsigned nFlags = 0, std::vector<DrxFixedStringT<IDrxPak::g_nMaxPath>>* pFullPaths = NULL) override;

	bool         OpenPackCommon(const char* szBindRoot, const char* pName, unsigned int nPakFlags, IMemoryBlock* pData = 0);
	bool         OpenPacksCommon(const char* szDir, const char* pWildcardIn, char* cWork, int nPakFlags, std::vector<DrxFixedStringT<IDrxPak::g_nMaxPath>>* pFullPaths = NULL);
	...
	virtual IDrxPak::SignedFileSize GetFileSizeOnDisk(const char* filename) override;
	...
	IDrxArchive*       FindArchive(const char* szFullPath);
	const IDrxArchive* FindArchive(const char* szFullPath) const;
	...
	virtual int                          ComputeCachedPakCDR_CRC(const char* filename, bool useDrxFile /*=true*/, IMemoryBlock* pData /*=NULL*/) override;
