...
struct Cache
{
	...
	void Release()       { const int nRefs = DrxInterlockedDecrement(m_pRefCount); assert(nRefs >= 0); if (nRefs == 0) Delete(); }
	...
	ErrorEnum DecompressFile(FileEntry* pFileEntry, void* pCompressed, void* pUncompressed, DrxCriticalSection& csDecmopressLock);
