...
enum  : uint32 {_A_IN_DRXPAK = 0x80000000};
...
struct IDrxPak
{
	...
	virtual bool OpenPack(const char* pName, unsigned nFlags = FLAGS_PATH_REAL, IMemoryBlock* pData = 0, DrxFixedStringT<IDrxPak::g_nMaxPath>* pFullPath = 0) = 0;

	//! After this call, the pak file will be searched for files when they aren't on the OS file system.
	virtual bool OpenPack(const char* pBindingRoot, const char* pName, unsigned nFlags = FLAGS_PATH_REAL, IMemoryBlock* pData = 0, DrxFixedStringT<IDrxPak::g_nMaxPath>* pFullPath = 0) = 0;
	...
	virtual bool OpenPacks(const char* pWildcard, unsigned nFlags = FLAGS_PATH_REAL, std::vector<DrxFixedStringT<IDrxPak::g_nMaxPath>>* pFullPaths = NULL) = 0;

	//! Opens pack files by the path and wildcard.
	virtual bool OpenPacks(const char* pBindingRoot, const char* pWildcard, unsigned nFlags = FLAGS_PATH_REAL, std::vector<DrxFixedStringT<IDrxPak::g_nMaxPath>>* pFullPaths = NULL) = 0;
	...
	virtual IDrxPak::SignedFileSize GetFileSizeOnDisk(const char* filename) = 0;
	...
	//! \see EPakFlags
	virtual IDrxArchive* OpenArchive(const char* szPath, unsigned int nFlags = 0, IMemoryBlock* pData = 0) = 0;
	...
	virtual int  ComputeCachedPakCDR_CRC(const char* filename, bool useDrxFile = true, IMemoryBlock* pData = NULL) = 0;
	...
	{
		const char* szName; // pointer managed by IDrxPak
		bool        bIsFolder;
		size_t      size;
...
inline FILE* fxopen(const char* file, const char* mode, bool bGameRelativePath = false)
{
	...
	if (gEnv && gEnv->pDrxPak)
	{
		...
		char path[_MAX_PATH];
		const char* szAdjustedPath = gEnv->pDrxPak->AdjustFileName(file, path, nAdjustFlags);
...
{
public:
	CInMemoryFileLoader(IDrxPak* pDrxPak) : m_pPak(pDrxPak), m_pFile(0), m_pBuffer(0), m_pCursor(0), m_nFileSize(0) {}
	~CInMemoryFileLoader()
	{
		Close();
	}
