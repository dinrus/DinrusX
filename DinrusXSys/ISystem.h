...
#include <DinrusX/CoreX/Platform/platform.h> // Needed for LARGE_INTEGER (for consoles).
#include <DinrusX/CoreX/Assert/DrxAssert.h>

#include <DinrusX/DinrusXSys/IXml.h> // <> required for Interfuscator
#include <DinrusX/DinrusXSys/IValidator.h>         // <> required for Interfuscator
#include <DinrusX/DinrusXSys/ILog.h>     // <> required for Interfuscator
#include <DinrusX/CoreX/String/StringUtils.h>
#include <DinrusX/DinrusXSys/DrxVersion.h>
#include <DinrusX/CoreX/smartptr.h>
#include <DinrusX/CoreX/Memory/IMemory.h>          // <> required for Interfuscator
#include <DinrusX/DinrusXSys/ISystemScheduler.h> // <> required for Interfuscator

#include <DinrusX/DinrusXSys/FrameProfiler_Shared.h>
...
#include <DinrusX/DinrusXSys/IManualFrameStepController.h>
...
struct ISystem
{
	...
	virtual IDrxFactory* LoadModuleWithFactory(const char* szDllName, const DrxInterfaceID& moduleInterfaceId) = 0;
...
extern "C"
{
	DRXSYSTEM_API ISystem* CreateSystemInterface(SSystemInitParams& initParams, bool bManualEngineLoop);
}

//! Displays error message, logs it to console and file and error message box, tThen terminates execution.
void        DrxFatalError(const char*, ...) PRINTF_PARAMS(1, 2);
inline void DrxFatalError(const char* format, ...)
{
...
void DrxWarning(EValidatorModule, EValidatorSeverity, const char*, ...) PRINTF_PARAMS(3, 4);
inline void DrxWarning(EValidatorModule module, EValidatorSeverity severity, const char* format, ...)
{
...
{
	static DRX_FORCE_INLINE void AddCommand(const char* szCommand, ConsoleCommandFunc func, int flags = 0, const char* szHelp = nullptr, bool bIsManagedExternally = false)
	{
	...
	static DRX_FORCE_INLINE void AddCommand(const char* szName, const char* szScriptFunc, int flags = 0, const char* szHelp = nullptr)
	{
		DRX_ASSERT(gEnv && gEnv->pConsole);
	...
	static DRX_FORCE_INLINE ICVar* RegisterString(const char* szName, const char* szValue, int flags, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr)
	{
		DRX_ASSERT(gEnv && gEnv->pConsole);
	...
	static DRX_FORCE_INLINE ICVar* RegisterInt(const char* szName, int value, int flags, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr)
	{
		DRX_ASSERT(gEnv && gEnv->pConsole);
		if (gEnv && gEnv->pConsole)
		{
	...
	static DRX_FORCE_INLINE ICVar* RegisterInt64(const char* szName, int64 value, int flags, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr)
	{
		DRX_ASSERT(gEnv && gEnv->pConsole);
		if (gEnv && gEnv->pConsole)
		{
	...
	static DRX_FORCE_INLINE ICVar* RegisterFloat(const char* szName, float value, int flags, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr)
	{
		DRX_ASSERT(gEnv && gEnv->pConsole);
		if (gEnv && gEnv->pConsole)
		{
	...
	static DRX_FORCE_INLINE ICVar* Register(const char* szName, T* pSrc, U defaultValue, int flags = 0, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr, bool bAllowModify = true)
	{
...
private:
	...
	template<class T, class U>
	static DRX_FORCE_INLINE ICVar* RegisterImpl(enum_tag, const char* szName, T* pSrc, U defaultValue, int flags = 0, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr, bool bAllowModify = true)
	{
	...
	template<class T, class U>
	static DRX_FORCE_INLINE ICVar* RegisterImpl(non_enum_tag, const char* szName, T* pSrc, U defaultValue, int flags = 0, const char* szHelp = "", ConsoleVarFunc pChangeFunc = nullptr, bool bAllowModify = true)
	{
...
void        DrxLog(const char*, ...) PRINTF_PARAMS(1, 2);
inline void DrxLog(const char* format, ...)
{
...
void        DrxComment(const char*, ...) PRINTF_PARAMS(1, 2);
inline void DrxComment(const char* format, ...)
{
	// Fran: we need these guards for the testing framework to work
	if (gEnv && gEnv->pSystem && gEnv->pLog)
	{
...
void        DrxLogAlways(const char*, ...) PRINTF_PARAMS(1, 2);
inline void DrxLogAlways(const char* format, ...)
{
