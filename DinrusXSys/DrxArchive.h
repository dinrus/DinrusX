...
struct DrxArchiveSortByName
{
	...
	bool operator()(const char* left, const IDrxArchive* right) const
	{
		return stricmp(left, right->GetFullPath()) < 0;
	}
	bool operator()(const IDrxArchive* left, const char* right) const
	{
...
protected:
	...
	const char* AdjustPath(const char* szRelativePath, char szFullPathBuf[CDrxPak::g_nMaxPath])
	{
		if (!szRelativePath[0])
			return NULL;
		...
		if (szRelativePath[1] == ':' || (m_nFlags & FLAGS_ABSOLUTE_PATHS))
		{
			...
			const char* szFullPath = m_pPak->AdjustFileName(szRelativePath, szFullPathBuf, IDrxPak::FLAGS_PATH_REAL);
