...
public:
	...
	CachePtr New(const char* szFileName, IDrxPak::EInMemoryPakLocation eMemLocale); // throw (ErrorEnum);
	CachePtr New(const char* szFileName, IMemoryBlock* pData);
#if DRX_PLATFORM_ANDROID && defined(ANDROID_OBB)
	CachePtr New(const char* szFileName, IDrxPak::EInMemoryPakLocation eMemLocale, FILE* pPakFileRef, int nAssetOffset, int nAssetLength, FileEntry* pPakEntry);
	CachePtr New(const char* szFileName, IDrxPak::EInMemoryPakLocation eMemLocale, AAssetUpr* pAssetUpr);
#endif
