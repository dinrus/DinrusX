...
public:
	...
	bool TryGetJob()
	{
		...
		if (nCount > 0)
		{
			if (DrxInterlockedCompareExchange(alias_cast<volatile long*>(&m_nCounter), nCount - 1, nCount) == nCount)
				return true;
		}
		return false;
#else
...
{
public:
	CThreadBackEndWorkerThread(CThreadBackEnd* pThreadBackend, detail::CWaitForJobObject& rSemaphore, JobUpr::SJobQueue_ThreadBackEnd& rJobQueue, uint32 nId);
