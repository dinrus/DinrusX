...
bool BucketAllocator<TraitsT >::Refill(uint8 bucket)
{
	...
	#if CAPTURE_REPLAY_LOG
	{
		...
		do
		{
			...
			DrxGetIMemReplay()->MarkBucket(itemSize, alignment, (void*) sbBase, sbEnd - sbBase);
			sbBase = sbEnd;
		}
		...
		for (size_t segPageId = 0; segPageId != NumPages; ++segPageId)
		{
			...
			for (size_t sbId = 0; sbId < SmallBlocksPerPage; ++sbId)
			{
				...
				if (maximumFreeCount == sbInfo.freeItemCount)
				{
					...
					DrxGetIMemReplay()->UnMarkBucket(itemSize, (void*) sbStart);
	#endif
				}
			}
		}
	...
	if (segIdx == segCount)
	{
		if (segCount != MaxSegments)
		{
			...
			if (baseAddress)
			{
				...
				++id;
				DrxGetIMemReplay()->RegisterFixedAddressRange((void*)baseAddress, NumPages * PageLength, name);
	#endif
			}
			else
			{
