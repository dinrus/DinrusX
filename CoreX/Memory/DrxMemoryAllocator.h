...
inline void DrxInternalCreateCriticalSection(void* pCS)
{
	#if DRX_PLATFORM_LINUX || DRX_PLATFORM_ANDROID || DRX_PLATFORM_APPLE
...
public:
	...
	//#define ATOMIC_CAS(__dst, __val, __old_val) (DrxInterlockedCompareExchange((LONG volatile*)__dst, (LONG)__val, (LONG)__old_val) == (LONG)__old_val)/*(InterlockedCompareExchange64((LONG volatile*)__dst, (LONG)__val, (LONG)__old_val) == (LONG)__old_val)*/
	//#  ifndef ATOMIC_INCREMENT
	//#    define ATOMIC_INCREMENT(__x)           DrxInterlockedIncrement((int volatile*)__x)
	//#    define ATOMIC_DECREMENT(__x)           DrxInterlockedDecrement((int volatile*)__x)
	...
	static void Initialize()
	{
		...
		{
			m_bInitialized = true;
			for (int i = 0; i < DRX_ARRAY_COUNT(g_globalSections); ++i)
			{
...
public:
	...
	{
		return DrxInterlockedIncrement((int volatile*)x);
	}

	inline static LONG ATOMIC_DECREMENT(LONG volatile* x)
	{
		return DrxInterlockedDecrement((int volatile*)x);
	}
