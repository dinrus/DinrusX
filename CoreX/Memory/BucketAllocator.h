...
public:
	...
	size_t deallocate(void* ptr)
	{
		...
		if (this->IsInAddressRange(ptr))
		{
			...
			DrxInterlockedAdd(&m_consumed, -(int)sz);
	#endif
	...
	void   EnableExpandCleanups(bool enable)
	{
		...
		SegmentHot* pSegments = m_segmentsHot;
		for (int i = 0, c = m_numSegments; i != c; ++i)
			DrxGetIMemReplay()->BucketEnableCleanups(reinterpret_cast<void*>(pSegments[i].m_baseAddress), enable);
	#endif
	}
	...
	void ReplayRegisterAddressRange(const char* name)
	{
		SegmentHot* pSegments = m_segmentsHot;
		for (int i = 0, c = m_numSegments; i != c; ++i)
			DrxGetIMemReplay()->RegisterFixedAddressRange(reinterpret_cast<void*>(pSegments[i].m_baseAddress), NumPages * PageLength, name);
	}
	#endif
