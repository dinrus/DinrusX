...
{
public:
	explicit STraceHeapAllocatorAutoColor(uint32 color) { m_color = DrxGetIMemoryUpr()->TraceHeapGetColor(); DrxGetIMemoryUpr()->TraceHeapSetColor(color); }
...
DRXMEMORYMANAGER_API size_t DrxGetMemSize(void* p, size_t size);
...
DRXMEMORYMANAGER_API void   DrxGetIMemoryUprInterface(void** pIMemoryUpr);
...
#include <DinrusX/DinrusXSys/DrxMemReplay.h>
...
DRXMM_INLINE void* DrxModuleMalloc(size_t size) noexcept
{
...
DRXMM_INLINE void* DrxModuleRealloc(void* memblock, size_t size) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_DrxMalloc);
...
DRXMM_INLINE void DrxModuleFree(void* memblock) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_DrxMalloc);
	free(memblock);
...
DRXMM_INLINE void DrxModuleMemalignFree(void* memblock) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_DrxMalloc);
...
DRXMM_INLINE void* DrxModuleReallocAlign(void* memblock, size_t size, size_t alignment) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_DrxMalloc);
	#if defined(__GNUC__)
	...
	if ((size_t(ret) & (std::max<size_t>(alignment, 1) - 1)) != 0)       // MAX handles alignment == 0
	{
		...
		void* alignedAlloc = _aligned_malloc(size, alignment);
...
DRXMM_INLINE void* DrxModuleMemalign(size_t size, size_t alignment) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_DrxMalloc);
...
extern "C"
{
	...
	void* DrxModuleRealloc(void* memblock, size_t size) noexcept;
	...
	void* DrxModuleReallocAlign(void* memblock, size_t size, size_t alignment) noexcept;
	void  DrxModuleMemalignFree(void*) noexcept;
	...
	size_t DrxModuleMemSize(void* ptr, size_t alignment = 0) noexcept;
}

DRXMM_INLINE void* DrxModuleCRTMalloc(size_t s) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_CrtMalloc);
	void* ret = DrxModuleMalloc(s);
	MEMREPLAY_SCOPE_ALLOC(ret, s, 0);
	return ret;
}

DRXMM_INLINE void* DrxModuleCRTRealloc(void* p, size_t s) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_CrtMalloc);
	void* ret = DrxModuleRealloc(p, s);
	MEMREPLAY_SCOPE_REALLOC(p, ret, s, 0);
	return ret;
}

DRXMM_INLINE void DrxModuleCRTFree(void* p) noexcept
{
	MEMREPLAY_SCOPE(EMemReplayAllocClass::C_UserPointer, EMemReplayUserPointerClass::C_CrtMalloc);
	DrxModuleFree(p);
...
{
	void* pAlignedMemory = DrxModuleMemalign(sizeof(T), std::alignment_of<T>::value);
