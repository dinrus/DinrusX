...
struct InterfaceCast<IDrxUnknown>
{
	...
	static void* Op(T* p)
	{
		return const_cast<IDrxUnknown*>(static_cast<const IDrxUnknown*>(static_cast<const void*>(p)));
	}
};
}
...
{
	template<class T>
	static void* Op(T*, const DrxInterfaceID&)
	{
		return 0;
	}
};
...
{
	template<class T>
	static void* Op(T* p, const DrxInterfaceID& iid)
	{
...
#define DRXCOMPOSITE_ADD(member, membername)                                   \
  static_assert(DRX_ARRAY_COUNT(membername) > 1, "'membername' is too short"); \
  ...
  public:                                                                  \
    ...
    {                                                                      \
      return &s_factory;                                                   \
                                                                           ...
                                                                           \
  protected:                                                               \
    virtual void* QueryInterface(const DrxInterfaceID &iid) const override \
    {                                                                      \
...
#define _ENFORCE_DRXFACTORY_USAGE(implclassname, cname, cidHigh, cidLow) DRX_PP_ERROR("Deprecated macro: Use _ENFORCE_DRXFACTORY_USAGE_GUID instead. Please refer to the Migrating Guide from DRXENGINE 5.3 to DRXENGINE 5.4 for more details.")
...
#define DRXGENERATE_CLASS(implclassname, cname, cidHigh, cidLow) DRX_PP_ERROR("Deprecated macro: Use DRXGENERATE_CLASS_GUID instead. Please refer to the Migrating Guide from DRXENGINE 5.3 to DRXENGINE 5.4 for more details.")
...
#define DRXGENERATE_CLASS_FROM_INTERFACE(implclassname, interfaceName, cname, cidHigh, cidLow) DRX_PP_ERROR("Deprecated macro: Use DRXGENERATE_CLASS_FROM_INTERFACE_GUID instead. Please refer to the Migrating Guide from DRXENGINE 5.3 to DRXENGINE 5.4 for more details.")
...
#define DRXGENERATE_SINGLETONCLASS(implclassname, cname, cidHigh, cidLow) DRX_PP_ERROR("Deprecated macro: Use DRXGENERATE_SINGLETONCLASS_GUID instead. Please refer to the Migrating Guide from DRXENGINE 5.3 to DRXENGINE 5.4 for more details.")
