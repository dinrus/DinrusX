...
{
	return static_cast<const void*>(p0) == static_cast<const void*>(p1) || drxinterface_cast<const IDrxUnknown>(p0) == drxinterface_cast<const IDrxUnknown>(p1);
}
...
template<class Src>
std::shared_ptr<IDrxUnknown> drxcomposite_query(Src* p, const char* name, bool* pExposed = nullptr)
{
...
template<class Src>
std::shared_ptr<const IDrxUnknown> drxcomposite_query(const Src* p, const char* name, bool* pExposed = nullptr)
{
	void* pComposite = p ? p->QueryComposite(name) : nullptr;
	if (pExposed) *pExposed = pComposite != nullptr;
...
template<class Src>
std::shared_ptr<IDrxUnknown> drxcomposite_query(const std::shared_ptr<Src>& p, const char* name, bool* pExposed = nullptr)
{
	return drxcomposite_query(p.get(), name, pExposed);
}

template<class Src>
std::shared_ptr<const IDrxUnknown> drxcomposite_query(const std::shared_ptr<const Src>& p, const char* name, bool* pExposed = nullptr)
{
	return drxcomposite_query(p.get(), name, pExposed);
}
}
...
#define _BEFRIEND_DRXCOMPOSITE_QUERY()                                                                                                                   \
  template<class Src> friend std::shared_ptr<IDrxUnknown> CompositeQuerySemantics::drxcomposite_query(Src*, const char*, bool*);                         \
  template<class Src> friend std::shared_ptr<const IDrxUnknown> CompositeQuerySemantics::drxcomposite_query(const Src*, const char*, bool*);             \
  template<class Src> friend std::shared_ptr<IDrxUnknown> CompositeQuerySemantics::drxcomposite_query(const std::shared_ptr<Src> &, const char*, bool*); \
  template<class Src> friend std::shared_ptr<const IDrxUnknown> CompositeQuerySemantics::drxcomposite_query(const std::shared_ptr<const Src> &, const char*, bool*);
...
protected:
	...
	virtual void* QueryInterface(const DrxInterfaceID& iid) const = 0;
