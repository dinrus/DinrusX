...
void DrxLogAssert(const char*, const char*, unsigned int, bool*);
...
//! Use the platform's default assert.
	...
	template<typename T>
	inline T const& DrxVerify(T const& expr, const char* szMessage)
	{
...
inline void assertion_failed_msg(char const* const szExpr, char const* const szMsg, char const* const szFunction, char const* const szFile, long const line)
{
		...
		if (::Detail::DrxAssertHandler(assertData, assertCond, szMsg))
			DrxDebugBreak();
	#else
		DRX_ASSERT_TRACE(false, ("An assertion failed in boost: expr=%s, msg=%s, function=%s, file=%s, line=%d", szExpr, szMsg, szFunction, szFile, (int)line));
