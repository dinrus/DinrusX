...
enum DrxLockType
{
	DRXLOCK_FAST      = 1,  //!< A fast potentially (non-recursive) mutex.
	DRXLOCK_RECURSIVE = 2,  //!< A recursive mutex.
};
...
// Include architecture specific code.
	...
	#include <DinrusX/CoreX/Thread/DrxThread_win32.h>
#elif DRX_PLATFORM_POSIX
	#include <DinrusX/CoreX/Thread/DrxThread_posix.h>
#else
// Put other platform specific includes here!
	#include <DinrusX/CoreX/Thread/DrxThread_dummy.h>
#endif
...
public:
	...
	{
		SetDebugLocked(false);
		const int counter = DrxInterlockedDecrement(&m_readCount);     // release read
		assert(counter >= 0);
	...
	{
		SetDebugLocked(false);
		int counter = DrxInterlockedDecrement(&m_modifyCount);    // decrement write counter
		assert(counter >= 0);
...
inline  void N_ProducerSingleConsumerQueueBase::Push(void* pObj, volatile uint32& rProducerIndex, volatile uint32& rConsumerIndex, volatile uint32& rRunning, void* arrBuffer, uint32 nBufferSize, uint32 nObjectSize, volatile uint32* arrStates)
{
	...
	do
	{
		...
		if (DrxInterlockedCompareExchange(alias_cast<volatile LONG*>(&rProducerIndex), nProducerIndex + 1, nProducerIndex) == nProducerIndex)
			break;
	}
	while (true);
