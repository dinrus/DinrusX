...
#include <DinrusX/DinrusXSys/Pipe.h>
...
private:
	...
	DRX_ALIGN(16) volatile uint32 m_nProducerIndex;
	DRX_ALIGN(64) volatile uint32 m_nComsumerIndex;
};
...
{
	DrxModuleMemalignFree(m_arrBuffer);
	DrxModuleMemalignFree((void*)m_arrStates);
	m_nBufferSize = 0;
}
...
inline void N_ProducerSingleConsumerQueue<T >::Init(size_t nSize)
{
	...
	m_arrBuffer = alias_cast<T*>(DrxModuleMemalign(nSize * sizeof(T), 16));
	m_arrStates = alias_cast<uint32*>(DrxModuleMemalign(nSize * sizeof(uint32), 16));
...
inline void N_ProducerSingleConsumerQueue<T >::AddProducer()
{
	...
	if (m_nRunning == 0)
		__debugbreak();
#endif
	DrxInterlockedIncrement((volatile int*)&m_nProducerCount);
}
...
inline void N_ProducerSingleConsumerQueue<T >::RemoveProducer()
{
	...
	if (m_nRunning == 0)
		__debugbreak();
#endif
	if (DrxInterlockedDecrement((volatile int*)&m_nProducerCount) == 0)
		m_nRunning = 0;
}
...
inline void N_ProducerSingleConsumerQueue<T >::Push(const T& rObj)
{
	...
	assert(m_arrStates != NULL);
	assert(m_nBufferSize != 0);
	DrxMT::detail::N_ProducerSingleConsumerQueueBase::Push((void*)&rObj, m_nProducerIndex, m_nComsumerIndex, m_nRunning, m_arrBuffer, m_nBufferSize, sizeof(T), m_arrStates);
}
