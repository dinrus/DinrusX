	...
	: INumberArray<F, 9>
{
	...
	ILINE F                operator()(uint32 i, uint32 j) const   { DRX_MATH_ASSERT((i < 3) && (j < 3)); const F* const p_data = (const F*)(&m00);   return p_data[i * 3 + j]; }
	ILINE F&               operator()(uint32 i, uint32 j)         { DRX_MATH_ASSERT((i < 3) && (j < 3)); F* p_data = (F*)(&m00);   return p_data[i * 3 + j]; }
	ILINE void             SetRow(int i, const Vec3_tpl<F>& v)    { DRX_MATH_ASSERT(i < 3); F* p = (F*)(&m00);  p[0 + 3 * i] = v.x; p[1 + 3 * i] = v.y; p[2 + 3 * i] = v.z;   }
	ILINE Vec3_tpl<F>      GetRow(int i) const                    { DRX_MATH_ASSERT(i < 3); const F* const p = (const F*)(&m00);  return Vec3_tpl<F>(p[0 + 3 * i], p[1 + 3 * i], p[2 + 3 * i]); }
	ILINE void             SetColumn(int i, const Vec3_tpl<F>& v) { DRX_MATH_ASSERT(i < 3); F* p = (F*)(&m00);  p[i + 3 * 0] = v.x; p[i + 3 * 1] = v.y; p[i + 3 * 2] = v.z;   }
	ILINE Vec3_tpl<F>      GetColumn(int i) const                 { DRX_MATH_ASSERT(i < 3); const F* const p = (const F*)(&m00);  return Vec3_tpl<F>(p[i + 3 * 0], p[i + 3 * 1], p[i + 3 * 2]); }
