...
ILINE uint32                 pos_round(f32 f)             { DRX_MATH_ASSERT(f >= 0.0f); return uint32(f + 0.5f); }
...
ILINE uint8                  float_to_ufrac8(float f)     { uint i = pos_round(f * 255.f);  DRX_MATH_ASSERT(i < 256);       return uint8(i); }
ILINE int8                   float_to_ifrac8(float f)     { int i = int_round(f * 127.f);  DRX_MATH_ASSERT(abs(i) < 128);   return int8(i); }
...
ILINE int32 sgnnz(int32 i) { return drxmath::signnz(i); }
ILINE int32 sgn(int32 i)   { return drxmath::sign(i); }
