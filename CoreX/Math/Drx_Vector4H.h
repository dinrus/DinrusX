...
template<typename F> struct Vec4H
{
	...
	// Operators
	ILINE F&   operator[](int index)            { DRX_MATH_ASSERT(index >= 0 && index <= 3); return ((F*)this)[index]; }
	ILINE F    operator[](int index) const      { DRX_MATH_ASSERT(index >= 0 && index <= 3); return ((F*)this)[index]; }
