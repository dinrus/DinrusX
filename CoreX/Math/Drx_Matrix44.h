	...
	: INumberArray<F, 16>
{
	...
	ILINE F                  operator()(uint32 i, uint32 j) const    { DRX_MATH_ASSERT((i < 4) && (j < 4)); F* p_data = (F*)(&m00); return p_data[i * 4 + j]; }
	ILINE F&                 operator()(uint32 i, uint32 j)          { DRX_MATH_ASSERT((i < 4) && (j < 4)); F* p_data = (F*)(&m00); return p_data[i * 4 + j]; }

	ILINE void               SetRow(int i, const Vec3_tpl<F>& v)     { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[0 + 4 * i] = v.x; p[1 + 4 * i] = v.y; p[2 + 4 * i] = v.z; }
	ILINE void               SetRow4(int i, const Vec4_tpl<F>& v)    { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[0 + 4 * i] = v.x; p[1 + 4 * i] = v.y; p[2 + 4 * i] = v.z; p[3 + 4 * i] = v.w; }
	ILINE const Vec3_tpl<F>& GetRow(int i) const                     { DRX_MATH_ASSERT(i < 4); return *(const Vec3_tpl<F>*)(&m00 + 4 * i); }
	ILINE const Vec4_tpl<F>& GetRow4(int i) const                    { DRX_MATH_ASSERT(i < 4); return *(const Vec4_tpl<F>*)(&m00 + 4 * i); }

	ILINE void               SetColumn(int i, const Vec3_tpl<F>& v)  { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[i + 4 * 0] = v.x; p[i + 4 * 1] = v.y; p[i + 4 * 2] = v.z; }
	ILINE void               SetColumn4(int i, const Vec4_tpl<F>& v) { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); p[i + 4 * 0] = v.x; p[i + 4 * 1] = v.y; p[i + 4 * 2] = v.z; p[i + 4 * 3] = v.w; }
	ILINE Vec3_tpl<F>        GetColumn(int i) const                  { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); return Vec3_tpl<F>(p[i + 4 * 0], p[i + 4 * 1], p[i + 4 * 2]); }
	ILINE Vec4_tpl<F>        GetColumn4(int i) const                 { DRX_MATH_ASSERT(i < 4); F* p = (F*)(&m00); return Vec4_tpl<F>(p[i + 4 * 0], p[i + 4 * 1], p[i + 4 * 2], p[i + 4 * 3]); }
