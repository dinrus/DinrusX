...
#pragma warning(disable: 4244)
	...
	#define REG_COMPANY_NAME       L"Dinrus\\"
	#define REG_SETTING            L"Settings\\"
	#define REG_DRXTEK_SETTING_KEY REG_SOFTWARE REG_COMPANY_NAME REG_SETTING

// pseudo-variable that represents the DOS header of the module
EXTERN_C IMAGE_DOS_HEADER __ImageBase;

	#define INFOTEXT L"Please specify the directory of your DrxENGINE installation (RootPath):"
...
bool CEngineSettingsUpr::GetModuleSpecificStringEntryUtf16(const char* key, SettingsUprHelpers::CWCharBuffer wbuffer)
{
	...
	{
		CFixedString<wchar_t, 256> s = L"Software\\Dinrus\\Settings\\";
...
{
	RegKey key(L"Software\\Dinrus\\Settings\\DrxExport\\ProjectBuilds", false);
	if (key.pKey)
	{
		DWORD type;
...
bool CEngineSettingsUpr::StoreEngineSettingsToRegistry()
{
	...
	RegKey key1(L"Software\\Dinrus", true);
	if (!key1.pKey)
	{
		RegKey key0(L"Software", true);
		HKEY hKey1;
		RegCreateKeyA((HKEY)key0.pKey, "Dinrus", &hKey1);
		if (!hKey1)
			return false;
	}

	RegKey key2(L"Software\\Dinrus\\Settings", true);
	if (!key2.pKey)
	{
		RegKey key1(L"Software\\Dinrus", true);
		HKEY hKey2;
	...
	RegKey key(L"Software\\Dinrus\\Settings", true);
	if (!key.pKey)
	{
		bRet = false;
	}
...
bool CEngineSettingsUpr::StoreLicenseSettingsToRegistry()
{
	...
	RegKey key2(REG_DRXTEK_SETTING_KEY, true);
	if (!key2.pKey)
	{
		RegKey key1(REG_SOFTWARE REG_COMPANY_NAME, true);
		HKEY hKey2;
	...
	RegKey key(REG_DRXTEK_SETTING_KEY, true);
	if (!key.pKey)
	{
		bRet = false;
	}
...
void CEngineSettingsUpr::LoadEngineSettingsFromRegistry()
{
	...
	RegKey key(L"Software\\Dinrus\\Settings", false);
	if (key.pKey)
	{
		// Engine Specific
