...
struct SDrxLobbyPresenceConverter
{
	...
	uint32             m_numData;              //!< Number of SDrxLobbyUserData in array pointed to by m_pData. [in].
...
typedef void(*DrxLobbyPrivilegeCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, uint32 privilege, void* pArg);

//! \param taskID    Task ID allocated when the function was executed.
//! \param error     Error code - eCLE_Success if the function succeeded or an error that occurred while processing the function.
//! \param pArg      Pointer to application-specified data.
typedef void(*DrxLobbyOnlineStateCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg);
...
//! \param pArg        Pointer to application-specified data.
typedef void(*DrxLobbyCheckProfanityCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, const char* pString, bool isProfanity, void* pArg);
...
public:
	...
	virtual IDrxTCPServicePtr GetTCPService(const char* pService) = 0;
	...
	//! \return Pointer to IDrxTCPService for given server & port.
	virtual IDrxTCPServicePtr GetTCPService(const char* pServer, uint16 port, const char* pUrlPrefix) = 0;
	...
	virtual DrxUserID GetUserID(uint32 user) = 0;
	...
	virtual EDrxLobbyError GetUserPrivileges(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyPrivilegeCallback pCB, void* pCBArg) = 0;
	...
	//! \param pCbArg    Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError CheckProfanity(const char* const pString, DrxLobbyTaskID* pTaskID, DrxLobbyCheckProfanityCallback pCb, void* pCbArg) = 0;
	...
	virtual EDrxLobbyError GetSystemTime(uint32 user, SDrxSystemTime* pSystemTime) = 0;
...
//
// 'CSgs'		uint32	EDrxLobbyLoginGUIState constant to indicate whether other data requested is available
