...
typedef uint32 DrxSessionHandle;
...
public:
	...
	virtual uint16            GetChannelIDFromGameSessionHandle(DrxSessionHandle gh) = 0;
	...
	virtual void         LobbyAddrIDRecv(void(*cb)(void*, uint8* buffer, uint32 size, DRXSOCKET sock, TNetAddress& addr), void* cbData) {}                           // Need to implement if matchmaking service uses LobbyIdAddr's rather than SIPV4Addr
