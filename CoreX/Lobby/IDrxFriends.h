...
typedef void (* DrxFriendsCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg);
...
//! \param PArg         Pointer to application-specified data.
typedef void (* DrxFriendsGetFriendsListCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, SFriendInfo* pFriendInfo, uint32 numFriends, void* pArg);
...
struct IDrxFriends
{
	...
	virtual EDrxLobbyError FriendsGetFriendsList(uint32 user, uint32 start, uint32 num, DrxLobbyTaskID* pTaskID, DrxFriendsGetFriendsListCallback cb, void* pCbArg) = 0;
	...
	//! \param PCbArg      Pointer to application-specified data that is passed to the callback.
	//! \return	eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError FriendsSendGameInvite(uint32 user, DrxSessionHandle h, DrxUserID* pUserIDs, uint32 numUserIDs, DrxLobbyTaskID* pTaskID, DrxFriendsCallback cb, void* pCbArg) = 0;
	// </interfuscator:shuffle>
};
