...
typedef void (* DrxLobbyUICallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, void* pArg);

//! \param taskID       Task ID allocated when the function was executed.
//! \param error        Error code - eCLE_Success if the function succeeded or an error that occurred while processing the function.
//! \param pArg         Pointer to application-specified data.
typedef void (* DrxLobbyUIOnlineRetailStatusCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, SDrxLobbyUIOnlineRetailCounts* pCounts, void* pArg);
...
//! \param pArg          Pointer to application-specified data.
typedef void (* DrxLobbyUIGetConsumableOffersCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, uint32 nOffers, SDrxLobbyConsumableOfferData* pOffers, void* pArg);
...
//! \param pArg          Pointer to application-specified data.
typedef void (* DrxLobbyUIGetConsumableAssetsCallback)(DrxLobbyTaskID taskID, EDrxLobbyError error, uint32 nAssets, SDrxLobbyConsumableAssetData* pAssets, void* pArg);
...
struct IDrxLobbyUI
{
	...
	virtual EDrxLobbyError ShowGamerCard(uint32 user, DrxUserID userID, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg       Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowGameInvite(uint32 user, DrxSessionHandle h, DrxUserID* pUserIDs, uint32 numUserIDs, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg       Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowFriends(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg        Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowFriendRequest(uint32 user, DrxUserID userID, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \note Xbox Live only.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowParty(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	//! \note Xbox Live only.
	virtual EDrxLobbyError ShowCommunitySessions(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg    Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError SetRichPresence(uint32 user, SDrxLobbyUserData* pData, uint32 numData, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg       Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowOnlineRetailBrowser(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg      Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError CheckOnlineRetailStatus(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUIOnlineRetailStatusCallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg      Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowMessageList(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg   Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError GetConsumableOffers(uint32 user, DrxLobbyTaskID* pTaskID, const TStoreOfferID* pOfferIDs, uint32 offerIdCount, DrxLobbyUIGetConsumableOffersCallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg    Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ShowDownloadOffer(uint32 user, TStoreOfferID offerId, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg     Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError GetConsumableAssets(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUIGetConsumableAssetsCallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg     Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError ConsumeAsset(uint32 user, TStoreAssetID assetID, uint32 quantity, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \param pCbArg      Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError AddRecentPlayers(uint32 user, DrxUserID* pUserIDs, uint32 numUserIDs, const char* gameModeStr, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
	...
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError AddActivityFeed(uint32 user, DrxLobbyTaskID* pTaskID, DrxLobbyUICallback cb, void* pCbArg) = 0;
