...
typedef void (* DrxLobbyConfigurationCallback)(EDrxLobbyService service, SConfigurationParams* requestedParams, uint32 paramCount);
typedef void (* DrxLobbyCallback)(EDrxLobbyService service, EDrxLobbyError error, void* arg);
...
public:
	...
	virtual EDrxLobbyError Initialise(EDrxLobbyService service, EDrxLobbyServiceFeatures features, DrxLobbyConfigurationCallback cfgCb, DrxLobbyCallback cb, void* cbArg) = 0;
	...
	//! \param CbArg			Pointer to application-specified data that is passed to the callback.
	//! \return eCLE_Success if function successfully started or an error code if function failed to start.
	virtual EDrxLobbyError Terminate(EDrxLobbyService service, EDrxLobbyServiceFeatures features, DrxLobbyCallback cb, void* cbArg) = 0;
	...
	virtual void RegisterEventInterest(EDrxLobbySystemEvent eventOfInterest, DrxLobbyEventCallback cb, void* pUserArg) = 0;
	...
	virtual void UnregisterEventInterest(EDrxLobbySystemEvent eventOfInterest, DrxLobbyEventCallback cb, void* pUserArg) = 0;
	...
	virtual void SetDinrusXLoadHint(const char* levelName, const char* gameRules) = 0;

	//! \return Level name previously set by SetDinrusXLoadHint or "" if never set.
	virtual const char* GetDinrusXLevelNameHint() = 0;

	//! \return Game rules name previously set by SetDinrusXLoadHint or "" if never set.
	virtual const char* GetDinrusXRulesNameHint() = 0;
