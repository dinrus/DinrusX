...
#include <DinrusX/DinrusXSys/DrxUtils.h>
...
// Alignment|InitializerList support.
	...
	#if defined(_LIB)
// For static libraries, the module the library is linked into needs to set eDrxModule
// However, the linker will set detect_mismatch if the final module doesn't use memory manager
	#else
		#error Code configuration error: eDrxModule needs to be set for all modules using the DRXENGINE memory manager
	#endif
#else
...
#include <DinrusX/CoreX/Project/ProjectDefines.h>  // to get some defines available in every DinrusX project
#include <DinrusX/CoreX/Game/ExtensionDefines.h>
...
// Only include these in non RELEASE builds
	...
	#include <DinrusX/CoreX/Platform/Win32specific.h>
#endif

#if DRX_PLATFORM_WINDOWS && DRX_PLATFORM_64BIT
	#include <DinrusX/CoreX/Platform/Win64specific.h>
#endif

#if DRX_PLATFORM_LINUX && DRX_PLATFORM_64BIT
	#include <DinrusX/CoreX/Platform/Linux64Specific.h>
#endif

#if DRX_PLATFORM_LINUX && DRX_PLATFORM_32BIT
	#include <DinrusX/CoreX/Platform/Linux32Specific.h>
#endif

#if DRX_PLATFORM_ANDROID && DRX_PLATFORM_64BIT
	#include <DinrusX/CoreX/Platform/Android64Specific.h>
#endif

#if DRX_PLATFORM_ANDROID && DRX_PLATFORM_32BIT
	#include <DinrusX/CoreX/Platform/Android32Specific.h>
#endif

#if DRX_PLATFORM_DURANGO
	#include <DinrusX/CoreX/Platform/DurangoSpecific.h>
#endif

#if DRX_PLATFORM_ORBIS
	#include <DinrusX/CoreX/Platform/OrbisSpecific.h>
#endif

#if DRX_PLATFORM_MAC
	#include <DinrusX/CoreX/Platform/MacSpecific.h>
#endif

#if DRX_PLATFORM_IOS
	#include <DinrusX/CoreX/Platform/iOSSpecific.h>
#endif
...
bool			DrxInitializeEngine(struct SSystemInitParams& startupParams, bool bManualEngineLoop = false);
void            DrxDebugBreak();
void            DrxSleep(unsigned int dwMilliseconds);
void            DrxLowLatencySleep(unsigned int dwMilliseconds);
EQuestionResult DrxMessageBox(const char* lpText, const char* lpCaption, EMessageBox uType = eMB_Info);
...
void            DrxGetCurrentDirectory(unsigned int pathSize, char* szOutPath);
short           DrxGetAsyncKeyState(int vKey);
unsigned int    DrxGetFileAttributes(const char* lpFileName);
int             DrxGetWritableDirectory(unsigned int pathSize, char* szOutPath);

void            DrxGetExecutableFolder(unsigned int pathSize, char* szOutPath);
void            DrxFindEngineRootFolder(unsigned int pathSize, char* szOutPath);
...
// Wrapper code for non-windows builds.
	...
	#include <DinrusX/CoreX/Platform/Linux_Win32Wrapper.h>
#elif DRX_PLATFORM_ORBIS
	#include <DinrusX/CoreX/Platform/Orbis_Win32Wrapper.h>
#endif
...
#include <DinrusX/CoreX/TypeInfo_decl.h>

bool     DrxSetFileAttributes(const char* lpFileName, uint32 dwFileAttributes);
...
#include <functional>
#include <iterator>
#include <DinrusX/CoreX/Memory/STLAlignedAlloc.h>
