...
protected:
	...
	{
	public:
		virtual IFileFinder::EFileState FileExists(const char* path) { return gEnv->pDrxPak->IsFileExist(path) ? eFS_FileOrDirectory : eFS_NotExist; }
		virtual intptr_t                FindFirst(const char* filePattern, _finddata_t* fd)
		{
			return gEnv->pDrxPak->FindFirst(filePattern, fd, 0, true);
		}
		virtual int  FindClose(intptr_t handle)                 { return gEnv->pDrxPak->FindClose(handle); }
		virtual int  FindNext(intptr_t handle, _finddata_t* fd) { return gEnv->pDrxPak->FindNext(handle, fd); }
