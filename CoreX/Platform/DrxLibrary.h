...
// for compatibility with code written for windows
	...
	#else
		#define DrxSharedLibraryExtension ".so"
	#endif
...
static HMODULE DrxLoadLibrary(const char* libName, bool bLazy = false, bool bInModulePath = true)
{
	...
	const char* libPath = bInModulePath ? (DrxGetSharedLibraryStoragePath() ? DrxGetSharedLibraryStoragePath() : ".") : "";
#else
	const char* libPath = bInModulePath ? (GetModulePath() ? GetModulePath() : ".") : "";
#endif	

	const char* filePre = strncmp(libName, DrxSharedLibraryPrefix, DRX_ARRAY_COUNT(DrxSharedLibraryPrefix) - 1) != 0 ? DrxSharedLibraryPrefix : "";
	const char* fileExt = strcmp(libName + strlen(libName) - (DRX_ARRAY_COUNT(DrxSharedLibraryExtension) - 1), DrxSharedLibraryExtension) != 0 ? DrxSharedLibraryExtension : "";
...
{
public:
	CDrxLibrary(const char* szLibraryPath)
		: m_hModule(nullptr)
	{
