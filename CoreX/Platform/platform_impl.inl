...
#include <DinrusX/CoreX/Extension/RegFactoryNode.h>
...
// If not in static library.
	...
	#include <DinrusX/CoreX/Thread/DrxThreadImpl.h>
	#include <DinrusX/CoreX/Math/ISplineSerialization_impl.h>
...
// If we use cry memory manager this should be also included in every module.
		...
		#include <DinrusX/CoreX/Memory/DrxMemoryUpr_impl.h>
	#endif

	#include <DinrusX/CoreX/Assert/DrxAssert_impl.h>

//////////////////////////////////////////////////////////////////////////
bool DrxInitializeEngine(SSystemInitParams& startupParams, bool bManualEngineLoop)
{
	DrxFindRootFolderAndSetAsCurrentWorkingDirectory();

#if !defined(DRX_IS_MONOLITHIC_BUILD)
	CDrxLibrary systemLibrary(DrxLibraryDefName("DinrusSystem"));
	if (!systemLibrary.IsLoaded())
	{
		string errorStr = string().Format("Failed to load the " DrxLibraryDefName("DinrusSystem") " library!");
	...
	{
		string errorStr = string().Format(DrxLibraryDefName("DinrusSystem") " library was invalid, entry-point not found!");
		DrxMessageBox(errorStr.c_str(), "Engine initialization failed!");
...
#include <DinrusX/DinrusXSys/FrameProfiler_impl.h>

DRX_ALIGN(64) uint32 BoxSides[0x40 * 8] = {
	0, 0, 0, 0, 0, 0, 0, 0, //00
	0, 4, 6, 2, 0, 0, 0, 4, //01
	7, 5, 1, 3, 0, 0, 0, 4, //02
	0, 0, 0, 0, 0, 0, 0, 0, //03
...
#include <DinrusX/DinrusX3dEng/GeomRef.inl>
#include <DinrusX/CoreX/Math/GeomQuery.inl>
