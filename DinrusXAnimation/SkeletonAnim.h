...
#include<DinrusX/DinrusXAnimation/FacialModel.h>
#include <DinrusX/DinrusXAnimation/TransitionQueue.h>
#include <DinrusX/DinrusXAnimation/PoseModifier.h>
...
public:
	...
	uint32 BlendUpr(f32 deltatime, DynArray<CAnimation> &arrLayer, uint32 nLayer);
	void BlendUprDebug();
	uint32 BlendUprDebug(DynArray<CAnimation> &arrLayer, uint32 nLayer);
	void LayerBlendUpr(f32 fDeltaTime, uint32 nLayer);
	...
	// Interface
public:
	bool StartAnimation(const char* szAnimName0, const DrxCharAnimationParams &Params);
	bool StartAnimationById(int32 id, const DrxCharAnimationParams &Params);
...
private:
	uint32 AnimationToQueue(const ModelAnimationHeader * pAnim0, int a0, f32 btime, const DrxCharAnimationParams &AnimParams);
