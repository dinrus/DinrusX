...
struct ICharacterUpr
{
	...
	virtual bool DBA_PreLoad(const char* filepath, ICharacterUpr::EStreamingDBAPriority priority) = 0;
	virtual bool DBA_LockStatus(const char* filepath, uint32 status, ICharacterUpr::EStreamingDBAPriority priority) = 0;
...
struct IDefaultSkeleton
{
	...
	virtual DrxBonePhysics*      GetJointPhysInfo(uint32 jointIndex) = 0;
	...
	virtual uint32               GetTextureMemoryUsage2(IDrxSizer* pSizer = 0) const = 0;
	virtual uint32               GetMeshMemoryUsage(IDrxSizer* pSizer = 0) const = 0;
...
#include <DinrusX/DinrusXAnimation/IAnimationPoseModifier.h>                                                    // <> required for Interfuscator
...
struct ISkeletonAnim
{
	...
	virtual bool StartAnimation(const char* szAnimName0, const DrxCharAnimationParams& params) = 0;
	//! Starts playing back the specified animation in the layer specified in the provided parameters.
	//! \param id Unique animation identifier, useful to avoid looking up animation by name on every playback
	//! \param params Parameters that describe how the animation should be started, such as playback speed.
	virtual bool StartAnimationById(int32 id, const DrxCharAnimationParams& Params) = 0;
...
typedef ICharacterUpr* (* PFNCREATEDRXANIMATION)(ISystem* pSystem, const char* szInterfaceVersion);
...
DRXANIMATION_API ICharacterUpr* CreateCharUpr(ISystem* pSystem, const char* szInterfaceVersion = gAnimInterfaceVersion);
