...
#include <DinrusX/DinrusXAnimation/Memory.h>
...
struct SBatchUpdateValidator : pe_action_batch_parts_update::Validator
{
	...
	int          AddRef()  { return DrxInterlockedIncrement(&nRefCount); }
...
public:
	...
	void                     SetJointPhysInfo(uint32 iJoint, const DrxBonePhysics& pi, int nLod);
	const DrxBonePhysics&    GetJointPhysInfo(uint32 iJoint, int nLod) const;
